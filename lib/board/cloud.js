...
export async function loadAll() {
  const [p, md, de, pr] = await Promise.all([
  ...
  return {
    ...
    error: p.error || md.error || de.error, // prods table is optional
  };
}
...
export async function upsertProd(name, settings) {
  return supabase.from("prods").upsert(
    { name, colour: settings.colour || null, rate_card: settings.rateCard || null },
...
export async function updateProduction(id, rec) {
  ...
  let r = await supabase.from("productions").update(meta).eq("id", id);
