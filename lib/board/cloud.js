...
export function onAuthChange(cb) {
  if (!supabase) return;
  // the event name rides along as a second argument: the board needs to spot
  // PASSWORD_RECOVERY (arriving back from the "forgotten password" email)
  supabase.auth.onAuthStateChange((event, session) => cb(session || null, event));
}
...
// ---- forgotten password ----
// Supabase emails a one-time link back to this app. Opening it puts a
// short-lived "recovery" session in place — enough to call updateUser({password})
// and nothing else. The board watches for that (onAuthChange fires with a
// PASSWORD_RECOVERY event) and shows the "choose a new password" panel.
export const resetPasswordForEmail = (email) =>
  supabase.auth.resetPasswordForEmail(email, {
    redirectTo: window.location.origin + window.location.pathname,
  });
// A password reset exists because somebody else may be in the account. Ending
// every OTHER session is the whole point — without it the person who got in
// stays signed in on their own machine and the reset changes nothing for them.
export async function updatePassword(password) {
  const r = await supabase.auth.updateUser({ password });
  if (!r.error) {
    try { await supabase.auth.signOut({ scope: "others" }); } catch (e) {}
  }
  return r;
}
...
export async function loadAll() {
  ...
  const runAll = () => Promise.all([
    ...
    // ordered so a hydrate always merges these rows the same way round
    supabase.from("day_edits").select("*").order("production_id").order("kind"),
    ...
    supabase.from("user_blobs").select("*"),
  ]);
  let [p, md, de, pr, gl, ev, rc, ub] = await runAll();
  if ((p.error && AUTH_ERR.test(p.error.message || "")) || (de.error && AUTH_ERR.test(de.error.message || ""))) {
    await refreshSession().catch(() => {});
    [p, md, de, pr, gl, ev, rc, ub] = await runAll();
  }
  return {
    productions: p.data || [],
    manualDays: md.data || [],
    ...
    userBlobs: ub.data || [], // empty if migration-2026-08-05.sql hasn't run
    error: p.error || md.error || de.error, // prods/glossary/events/rateCards/userBlobs tables are optional
  };
}

// ---- account-level blobs (no production behind them) ----
// The calculators' scratch rows, the risk-assessment settings and the
// dashboard's stand-alone casting briefs belong to the PERSON, not to a
// production. One row per (owner, kind); see migration-2026-08-05.sql.
export async function upsertUserBlob(kind, data) {
  return authRetry(() => supabase.from("user_blobs").upsert(
    { kind, data, updated_at: new Date().toISOString() },
    { onConflict: "owner,kind" }
  ));
}
export async function deleteUserBlob(kind) {
  return supabase.from("user_blobs").delete().eq("kind", kind);
}

// ---- original schedule files (private Storage bucket) ----
// The uploaded PDFs/photos themselves, so a schedule's source document can be
// re-opened on any device. Bucket + policies: migration-2026-08-10.sql.
const SCHED_BUCKET = "schedule-files";
export async function uploadScheduleFile(path, blob, contentType) {
  return authRetry(() =>
    supabase.storage.from(SCHED_BUCKET).upload(path, blob, {
      upsert: true,
      contentType: contentType || "application/octet-stream",
    })
  );
}
export async function downloadScheduleFile(path) {
  return supabase.storage.from(SCHED_BUCKET).download(path);
}
export async function removeScheduleFiles(paths) {
  return supabase.storage.from(SCHED_BUCKET).remove(paths);
}
...
export async function insertProduction(rec) {
  ...
  // pdf_files (the stored-original manifest) arrives with
  // migration-2026-08-10.sql — kept alongside ai_model so a database without
  // the column still saves everything else
  const ai = { ai_model: rec.aiModel || null, doc_kind: rec.docKind || null, pdf_files: rec.pdfFiles || null };
  let { data, error } = await authRetry(() => supabase
    .from("productions")
    .insert({ ...base, ...meta, ...ai })
    .select("id")
    .single());
  if (error && /ai_model|doc_kind|pdf_files/i.test(error.message || "")) {
...
export async function updateProduction(id, rec) {
  ...
  const ai = { ai_model: rec.aiModel || null, doc_kind: rec.docKind || null, pdf_files: rec.pdfFiles || null }; // see insertProduction
  let r = await authRetry(() => supabase.from("productions").update({ ...meta, ...ai }).eq("id", id));
  if (r.error && /ai_model|doc_kind|pdf_files/i.test(r.error.message || ""))
...
// The blob rows (a whole production's per-scene work in ONE row) are the only
// place a second device can silently overwrite an hour of work: it uploads its
// whole stale subset over the row. So blob writes carry the stamp the tab last
// saw — if the account's row has moved on since, the write is REFUSED and the
// board asks for a reload instead of quietly winning.
export async function getDayEditStamp(productionId, key, kind) {
  let q = supabase.from("day_edits").select("updated_at").eq("key", key).eq("kind", kind);
  q = productionId ? q.eq("production_id", productionId) : q.is("production_id", null);
  const { data, error } = await q.maybeSingle();
  return { stamp: (data && data.updated_at) || null, error };
}
export async function upsertDayEditGuarded(productionId, key, kind, data, expected) {
  const cur = await getDayEditStamp(productionId, key, kind);
  // can't read the row → fall through to the plain write rather than block work
  if (!cur.error && cur.stamp && expected !== cur.stamp) return { stale: true, stamp: cur.stamp };
  const write = () => supabase.from("day_edits").upsert(
    { production_id: productionId, key, kind, data, updated_at: new Date().toISOString() },
    { onConflict: "owner,production_id,key,kind" }
  ).select("updated_at").single();
  let r = await authRetry(write);
  // a database that predates the updated_at column still saves everything else
  if (r.error && /updated_at/i.test(r.error.message || "")) {
    const p = await upsertDayEdit(productionId, key, kind, data);
    return { error: p.error, stamp: null };
  }
  return { error: r.error, stamp: r.data && r.data.updated_at };
}
