...
function renderFreeCalc(){
  ...
  <div class="fccol">
    ...
    <div class="tablecard fcpad"><div class="sl2">Rate card</div>
      <span class="seg" style="flex-wrap:wrap">${fcCardChoices().map(([k,label])=>`<button data-fccard="${esc(k)}" class="${(FC.card||'')===k?'on':''}">${esc(label)}</button>`).join('')}</span>
...
function renderStuntCalc(){
  ...
  <div class="fccol">
    ${calcKindSeg()}
    <div class="tablecard fcpad"><div class="sl2">Rate card</div>
      <span class="seg" style="flex-wrap:wrap">${cards.map(([k,l])=>`<button data-sccard="${esc(k)}" class="${(SC.card||'')===k?'on':''}">${esc(l)}</button>`).join('')}</span>
...
function renderDash(){
  ...
  $('#dashView').innerHTML=`
    ...
    <div class="dash-sub" style="margin:26px 0 8px">Calculator — rough budgets without opening a production</div>
