  ...
  <div class="box" style="max-width:560px">
    ...
    <div style="padding:16px">
      <div class="rates-grid" style="padding:0;grid-template-columns:repeat(2,1fr)">
        ...
        <div class="rfield"><label>Rate card</label><div class="inwrap"><select id="impRate" style="width:100%;border:none;background:var(--panel2);color:var(--ink);padding:9px 10px;font-family:var(--mono);font-size:13px;font-weight:600"></select></div></div>
      </div>
      <div style="display:flex;gap:10px;margin-top:16px;align-items:center">
...
function addSource(model,title,short,activate=true,opts={}){
  ...
  SOURCES.push({model,title,short:short||title,colour,kind:opts.kind,text:opts.text,unit:opts.unit,ns:opts.ns,cloudId:opts.cloudId,createdAt:opts.createdAt,prod:opts.prod,version:opts.version,schedDate:opts.schedDate,format:opts.format,rateCard:opts.rateCard||null});
...
function saveUserSources(){
  store.set('crowdos-sources',JSON.stringify(SOURCES.filter(s=>s.kind).map(s=>({kind:s.kind,title:s.title,short:s.short,unit:s.unit||'Main',text:s.text||null,prod:s.prod||null,version:s.version||null,schedDate:s.schedDate||null,colour:s.colour||null,format:s.format||null,rateCard:s.rateCard||null}))));
}
...
function parseWith(format,text){
  ...
  if(format==='oneliner')return parseSchedule(text);
...
function openEditMeta(i){
  ...
  fillImpRateSelect(s.rateCard&&s.rateCard.name||'');
...
$('#impGo').addEventListener('click',()=>{
  if(IMP_EDIT!=null){
    ...
    if(s){
      ...
      const rcName=$('#impRate').value;
      s.rateCard=rcName&&RATECARDS[rcName]?{name:rcName,vals:RATECARDS[rcName]}:null;
  ...
  for(const rec of res.productions){
    ...
    try{
      if(rec.kind==='pdf'&&rec.schedule_text){
        ...
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.schedule_text,unit:rec.unit||'Main',ns:'p:'+rec.title,cloudId:rec.id,colour:rec.colour||undefined,createdAt:rec.created_at,prod:rec.production,version:rec.version,schedDate:rec.sched_date,format:rec.format,rateCard:rec.rate_card});
      }else{
...
});
{
  ...
  setActive=function(i){
    ...
    if(s&&s.kind){
      ...
      // productions carry their own rate card — apply it when opened
      if(s.rateCard&&s.rateCard.vals)applyRateVals(s.rateCard.vals);
    }
  };
}
document.addEventListener('click',e=>{
  ...
  for(const rec of saved){
    try{
      if(rec.kind==='pdf'&&rec.text){
        ...
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.text,unit:rec.unit||'Main',ns:'p:'+rec.title,prod:rec.prod,version:rec.version,schedDate:rec.schedDate,colour:rec.colour||undefined,format:rec.format,rateCard:rec.rateCard});
