...
document.addEventListener('click',e=>{
  ...
  const add=e.target.closest('[data-rqadd]');
  if(add){
    const area=add.closest('.reqarea');const ed=add.closest('.reqedit');
    const tier=add.dataset.rqadd;
    const sec=add.parentElement;
    const i=sec.querySelectorAll('.reqrow').length;
    const row=document.createElement('div');row.className='reqrow';
    row.innerHTML=`<input data-rq="name" data-rt="${tier}" data-ri="${i}" value="" placeholder="Character / group"><input data-rq="count" data-rt="${tier}" data-ri="${i}" type="number" min="0" value="1"><button data-rqdel="${tier}" data-ri="${i}">✕</button>`;
    sec.insertBefore(row,add);row.querySelector('input').focus();
    return;
  }
  const rdel=e.target.closest('[data-rqdel]');
  if(rdel){const area=rdel.closest('.reqarea');rdel.closest('.reqrow').remove();commitReqEditor(area);return}
  const rclose=e.target.closest('[data-rqclose]');
  if(rclose){const area=rclose.closest('.reqarea');commitReqEditor(area);OPEN_REQ=null;area&&area.classList.add('hidden');return}
...
function saveSced(){store.set('crowdos-sced',JSON.stringify(SCED))}
...
function reqEditorHTML(nk){
  ...
  if(APPMODE==='crowd'){
    const feat=(e.feat||s.featured||[]).map(f=>({name:f.name,count:f.count}));
    const spact=(e.spact||s.spacts||[]).map(f=>({name:f.name,count:f.count}));
    const rows=(list,tier)=>list.map((f,i)=>`<div class="reqrow"><input data-rq="name" data-rt="${tier}" data-ri="${i}" value="${esc(f.name)}" placeholder="Character / group"><input data-rq="count" data-rt="${tier}" data-ri="${i}" type="number" min="0" value="${+f.count||0}"><button data-rqdel="${tier}" data-ri="${i}">✕</button></div>`).join('');
    return `<div class="reqedit" data-rk="${esc(nk)}">
      <div class="reqline"><label>SA</label><input data-rq="sa" type="number" min="0" value="${+((e.sa!=null?e.sa:s.sa))||0}"><span class="reqhint">background artists (crowd)</span></div>
      <div class="reqsec"><div class="reqseclabel">Featured</div>${rows(feat,'feat')}<button class="reqadd" data-rqadd="feat">+ Featured character</button></div>
      <div class="reqsec"><div class="reqseclabel">SPACT</div>${rows(spact,'spact')}<button class="reqadd" data-rqadd="spact">+ SPACT</button></div>
...
function commitReqEditor(area){
  ...
  if(APPMODE==='crowd'){
    ...
    const collect=tier=>{
      ...
      ed.querySelectorAll(`.reqrow`).forEach(row=>{
        const n=row.querySelector(`[data-rq="name"][data-rt="${tier}"]`);
        if(!n)return;
        const c=row.querySelector(`[data-rq="count"][data-rt="${tier}"]`);
        if(n.value.trim())out.push({name:n.value.trim(),count:Math.max(0,+c.value||0)});
      });
      return out;
    };
    e.feat=collect('feat'); e.spact=collect('spact');
    if(!e.sa&&!e.feat.length&&!e.spact.length)delete SCED[scedKey(nk)];else SCED[scedKey(nk)]=e;
  }else{
  ...
  saveSced();
  OPEN_REQ=nk; // keep this scene's editor open through the re-render
  refreshAll();
}
{
...
document.addEventListener('change',e=>{
  ...
  if(rq){const area=rq.closest('.reqarea');if(area)commitReqEditor(area);}
});
document.addEventListener('change',e=>{
