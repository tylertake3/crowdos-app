...
$('#impClose').addEventListener('click',()=>{PENDING_IMPORT=null;$('#impModal').classList.remove('open')});
$('#impModal').addEventListener('click',e=>{if(e.target.id==='impModal'){PENDING_IMPORT=null;$('#impModal').classList.remove('open')}});
...
function renderDash(){
  ...
  const card=({s,i})=>{const c=costsFor(s);return `
    <button class="dash-card" data-side="${i}">
      <div style="display:flex;align-items:baseline;gap:8px;justify-content:space-between"><h3>${esc(s.kind?(s.prod||s.title):s.title)}</h3><span class="dash-kind">${s.kind==='pdf'?'PDF schedule':s.kind?'Manual':'Sample'}</span></div>
  ...
  $('#dashView').innerHTML=`
    ...
    <div class="dash-sub">${who?esc(who)+' · ':''}${user.length||'no'} production${user.length===1?'':'s'} — open one, or start a new budget from a schedule PDF or from nothing.</div>
    <div class="dash-grid">
      ${user.map(card).join('')}
...
function showDash(){DASH=true;$('#boardView').classList.add('hidden');$('#dashView').classList.remove('hidden');renderDash();renderSidebar();window.scrollTo(0,0);}
...
});
{
  ...
  setActive=function(i){if(DASH)hideDash();origSetActive(i);renderSidebar();};
}
document.addEventListener('click',e=>{
