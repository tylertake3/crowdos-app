...
function cbCrowdNameCell(c,sc,i,cls,ph,isAdd){
  ...
  const chip=c&&!c.fromAbove
    ...
    : '';
  const above=c&&c.slot>=0?`<label class="cbabove"><input type="checkbox" data-cbabove="${addr}" ${c.explicitFromAbove?'checked':''}> FROM ABOVE</label>`:'';
  return `<td class="cbname ${cls} cbnamecell">${chip}${isAdd?cbAddBtnHTML(addr):''}<span class="cbedit cbnametext${ph?' cbph':''}" contenteditable="true" spellcheck="false" data-cbaddr="${addr}" data-cbf="name"${ph?` data-ph="${esc(ph)}"`:''} data-tip="Character or group name — leave blank for plain background">${esc(c?c.name:'')}</span>${above}${c&&!c.fromAbove?`<button class="cbdel" data-cbdel="${addr}" data-tip="Remove this line" aria-label="Remove line">✕</button>`:''}</td>`;
}
...
function cbCrowdComboCell(c,sc,i,cls,ph,isAdd){
  const addr=esc(cbdAddr(sc,i));
  // A carried line is read-only, but keeps its count visible so the recipient
  // can see exactly what is continuing from the scene above.
  if(c&&c.fromAbove)return `<td class="cbname ${cls} cbcombo">${esc((c.no!=null?c.no+' ':'')+c.name)}</td>`;
  const tier=c?String(c.tier||'SA'):'SA';
  const label=(CBD_TIERS.find(t=>t[0]===tier)||CBD_TIERS[0])[1];
  const chip=c&&!c.fromAbove
    ? `<button class="cbtier t-${tier.toLowerCase()}" data-cbtier="${addr}" data-tip="Click to change tier — SA, Featured or Spact">${esc(label)}</button>`
...
function cbdSceneHTML(sc,layout){
  ...
  for(let i=0;i<n;i++){
    ...
    const emptyName=first&&!sc.crowd.length?(sc.na?'N/A — no crowd':'Not yet assessed'):'';
