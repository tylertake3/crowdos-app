    ...
    return `<div class="daycard ${work?'has-stunts':''}${dayIsToday(d)?' today':''}" id="day-${d.id}" data-stunt="${work?1:0}">
      <div class="dayhead">
        <div class="dh-top">
          ...
          <button class="dloc-edit${d.locDoc!=null?' on':''}" data-locedit="${esc(d.id)}" data-tip="${d.locDoc!=null?'Real location set by hand — the schedule says “'+esc((d.locDoc||'(nothing)').slice(0,60))+'”. Click to change.':'Set the real shooting location (the schedule’s own text often only names the set, e.g. “INT APARTMENT”)'}">${icon('pin')}</button>
...
function saveSROWS(){store.set('crowdos-srows',JSON.stringify(SROWS))}
...
function saveDC(){store.set('crowdos-dancecalc',JSON.stringify(DC))}
let DROWS=[];try{DROWS=JSON.parse(store.get('crowdos-drows')||'[]')}catch(e){DROWS=[]}
function saveDROWS(){store.set('crowdos-drows',JSON.stringify(DROWS))}
function dcWeek(){
...
function briefChipsHTML(id,field,options,chosen){
  return `<div class="bf-chips">${options.map(o=>`<button class="tagchip${chosen.includes(o)?' on':''}" data-briefchip="${esc(id)}|${esc(field)}|${esc(o)}">${esc(o)}</button>`).join('')}</div>`;
}
...
function dayBriefsHTML(d,db){
  ...
  const rows=db.chars.map(ch=>{
    ...
    return `<div class="dbrow">
      ...
      ${st?`<span class="briefstatus ${esc(st)}" style="pointer-events:none">${BRIEF_STATUS[st]||st}</span>`:'<span class="briefstatus draft" style="pointer-events:none">No brief yet</span>'}
...
function refreshBriefBadges(){if(APPMODE==='crowd'&&MODEL){renderDays();renderCalendar();}}
...
function renderBriefs(){
  ...
  const rows=list.map(x=>{
    ...
    return `<tr class="briefrow" data-openbrief="${esc(x.id)}">
      ...
      <td class="datescol">${c?`<div class="daylist">${c.days.slice(0,8).map(x2=>`<button class="dchip" data-calpop="${esc(x2.d.id)}">${esc(chipDate(x2.d))}</button>`).join('')}${c.days.length>8?`<span class="dchip more">+${c.days.length-8}</span>`:''}</div>`
        :man.length?`<div class="daylist">${man.slice(0,8).map(iso=>`<span class="dchip" data-tip="Typed in by hand">${esc(fmtBriefDate(iso))}</span>`).join('')}${man.length>8?`<span class="dchip more">+${man.length-8}</span>`:''}</div>`:'—'}</td>
      <td>${statusPill(x)}${x.b.sent?` <button class="sentbadge" data-brieftogglesent="${esc(x.id)}" data-tip="Emailed ${esc(sentDateShort(x.b.sentAt))} — click to unmark">✓ Sent</button>`:''}</td>
  ...
  const unbriefedRows=unbriefed.map(c=>{
    ...
    return `<tr class="briefrow unbriefed" data-newbrieffor="${esc(c.name)}|${esc(c.tier)}" data-count="${c.max}">
      ...
      <td class="datescol"><div class="daylist">${c.days.slice(0,8).map(x2=>`<button class="dchip" data-calpop="${esc(x2.d.id)}">${esc(chipDate(x2.d))}</button>`).join('')}${c.days.length>8?`<span class="dchip more">+${c.days.length-8}</span>`:''}</div></td>
  ...
  ${(list.length||unbriefed.length)?`<div class="tscroll"><table><thead><tr><th class="briefselcell">${list.length?`<input type="checkbox" id="briefSelAll" aria-label="Select all briefs"${list.length&&list.every(x=>BRIEF_SEL.has(x.id))?' checked':''}>`:''}</th><th>Character</th><th>Tier</th><th class="num">Heads</th><th class="num">Days</th><th class="datescol">Dates</th><th>Status</th><th></th></tr></thead><tbody>${rows}${unbriefedRows}</tbody></table></div>`
    ...
    <div class="briefgrid">
      ...
      <div class="tablecard"><h3>Casting requirements</h3>
        <div class="briefform">
          ...
          <div class="cdinfo">Rate: ${gbp(rate)}/day + holiday ${holTxt}${b.tier==='Featured'?' + supplementary fees':''} — from the production's rate card</div>
          <div class="bf-block">Dates <span class="cdinfo">${c?'(from the schedule — add any extra dates by hand)':'(not on the schedule — pick the dates the agency needs)'}</span>
...
document.addEventListener('click',e=>{
  ...
  if(sset){
    ...
    else setStatus('Open a production, then the settings button (bottom of the sidebar) opens its settings.');
    return;
  }
  ...
  if(bda){
    ...
    const add=[];
    for(let dt=new Date(from+'T00:00:00'),end=new Date((to||from)+'T00:00:00');dt<=end&&add.length<120;dt.setDate(dt.getDate()+1))add.push(isoOf(dt));
...
document.addEventListener('click',e=>{
  ...
  if(!c)return;
  if(c.dataset.crumb==='dash'){PROD_HOME=null;showDash();}
...
function renderDash(){
  ...
  // wipe — it's the only copy, and the gallery path re-parents it into
  ...
  $('#dashView').innerHTML=`
    ...
    <div class="dash-head dash-calchead" data-calctoggle style="margin:30px 0 4px;cursor:pointer;user-select:none">Calculator <span class="chev"${store.get('crowdos-dashcalc-open')==='0'?'':' style="transform:rotate(90deg)"'}>▶</span><span class="dash-sub" style="display:block;font-weight:400">Rough budgets without opening a production</span></div>
    <div id="dashCalcSlot" class="${store.get('crowdos-dashcalc-open')==='0'?'hidden':''}"></div>`;
  // the shared calculator lives at the bottom of the dashboard
  {
    const vc=$('#viewCalc');
...
function showDash(){
  DASH=true;
  // Dashboard is the home screen: today bar, productions, calculator.
  // (The old single-production auto-jump skipped all of that — gone.)
  PROD_HOME=null;
...
document.addEventListener('click',e=>{
  const t=e.target.closest('[data-calctoggle]');
  if(!t)return;
  const slot=$('#dashCalcSlot');if(!slot)return;
  const nowHidden=slot.classList.toggle('hidden');
  store.set('crowdos-dashcalc-open',nowHidden?'0':'1');
  const chev=t.querySelector('.chev');if(chev)chev.style.transform=nowHidden?'':'rotate(90deg)';
});
...
$('#briefPhotoInput').addEventListener('change',async e=>{
  ...
  if(!added)return;
  b.updatedAt=new Date().toISOString();saveBriefs();renderBriefs();
