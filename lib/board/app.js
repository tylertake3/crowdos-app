...
const SHELL = `<input type="file" id="fileInput" accept="application/pdf" multiple style="display:none">
...
<div class="modal" id="addChooser">
  <div class="box" style="max-width:440px">
    ...
    <div style="padding:16px">
      <button class="chooser-opt" id="acUpload"><b>Upload a schedule PDF</b><span>A one-liner, a Full Fat, or both together — read automatically, reviewed before publish.</span></button>
...
function renderDays(){
  ...
  const pastHTML=past.length?`<details class="pastdrawer"${PAST_OPEN?' open':''}><summary>${past.length} past day${past.length===1?'':'s'} — archived, everything still opens</summary>${past.map(cardHTML).join('')}</details>`:'';
  $('#viewDays').innerHTML=banner+pastHTML+ahead.map(cardHTML).join('');
...
function unnamedSaDays(){
  ...
  for(const d of MODEL.days){
    ...
    for(const s of d.scenes)if(s.sa){peak=Math.max(peak,s.sa);scenes.push(s)}
...
function renderBriefs(){
  ...
  const anonRows=anon.map(x=>`<tr class="anonrow">
    <td class="mono"><button class="dchip" data-goto="${esc(x.d.id)}">D${x.d.num}</button></td>
    <td>${esc(chipDate(x.d))}</td>
    <td>${mapsLink(x.d.loc)}</td>
    <td class="num"><b>${x.peak}</b></td>
    <td><div class="daylist">${x.scenes.slice(0,6).map(s=>`<span class="dchip" data-goto="${esc(x.d.id)}" data-tip="${esc((s.slug||'')+' — '+(s.desc||'').slice(0,70))}">${esc(s.num)} · ${s.sa} SA</span>`).join('')}${x.scenes.length>6?`<span class="dchip more">+${x.scenes.length-6}</span>`:''}</div></td>
    <td><button class="dchip" data-goto="${esc(x.d.id)}">Name them ›</button></td>
  </tr>`).join('');
  host.innerHTML=`
  ...
  ${list.length?`<div class="tscroll"><table><thead><tr><th>Character</th><th>Tier</th><th class="num">Heads</th><th class="num">Days</th><th class="datescol">Dates</th><th>Status</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`
    :`<div class="note" style="text-align:center;padding:40px 16px">No briefs yet. ${unbriefed.length?'Generate one per character from the schedule, or add one by hand.':'Name some crowd characters on the day board first, then generate briefs from them.'}</div>`}
  </div>
...
async function aiParse(text,prod){
  if(aiBlocked(prod))throw new Error(AI_OFF_MSG);
  const tok=CLOUD.session&&CLOUD.session.access_token;
  const res=await fetch('/api/parse-schedule',{method:'POST',headers:{'Content-Type':'application/json',...(tok?{Authorization:'Bearer '+tok}:{})},body:JSON.stringify({text,glossary:glossaryFor(prod)})});
...
async function handlePDFs(files){
  try{
    ...
    m=prepModel(JSON.parse(JSON.stringify(aiModel||m)),'Main');
    ...
    PENDING_IMPORT={text,title,aiModel,mergeStats,isDetail:!spineDoc,docKind,questions,files:docs.map(d=>d.file.name).join(' + ')};
    IMP_EDIT=null;$('#impGo').textContent='Import schedule';$('#impTitle')&&($('#impTitle').textContent='Import schedule');
    $('#impSub').textContent=docs.map(d=>d.file.name).join(' + ');
    ...
    if(PENDING_IMPORT.isDetail)info=scenes+' scenes found — no shoot dates in this file';
...
const KIND_LABEL={oneliner:'One-liner',fullfat:'Full Fat',merged:'One-liner + Full Fat'};
...
$('#impAI').addEventListener('click',async()=>{
  ...
  if(!text){setStatus('Nothing to re-read — this schedule has no stored text.');return}
  ...
  try{
    ...
    const aiModel=(await aiParse(text,prodHint||null)).model;
...
$('#fileInput').addEventListener('change',e=>{if(e.target.files.length)handlePDFs(e.target.files);e.target.value='';});
...
function planRevisionCarry(prevSrc,newModel,diff,manualPlains){
  ...
  const seenDay=new Set(),labels={0:'day calculator',1:'stunt adjustments',2:'stunt hours',3:'stunt team'};
...
function updateAccountUI(){
  ...
  $('#btnAccount').textContent=s?(displayName(s)||'Account'):'Sign in';
...
$('#psSave').addEventListener('click',()=>{
  ...
  if(JSON.stringify(info)!==JSON.stringify(P.info||{}))
    {P.info=info;changed.push('production info');}
