...
// Only one thing differs: rate maths is imported from lib/engine (a tested
// faithful port — see tests/prototype-parity.test.ts).
// ============================================================================
/* eslint-disable */
import {
  PACT, OTINC, SP3,
  ...
  locationBand, bandFor, parseDayDate, weekKeyDate, dayPeakSA,
  parseAny, parseSchedule, parseExpanded, prepModel, mergeModels, mergeDetail, looksLikeSequenceTitle,
  diffRevisions, carriedDayRecords, carryCastMap, describeRevision, revisionChangeLines,
  sceneKey, dayUid,
  sceneSlotKey, slotIndexOn, planSlotMigration, carriedDaySuffix,
  money as engineMoney, round2 as engineRound2, sumMoney as engineSumMoney,
  UNKNOWN_LOCATION_BAND,
  dayCarryTargets, sceneCarryTargets, carryReviewReasons,
  compactRevisionDiff, storedDiffHeadline, storedDiffMoney,
  danceWeek, DANCE_2026,
  // On-costs: agency commission, contingency, named on-cost lines, meal
  // penalties, UK bank holidays and agency cancellation terms (oncosts.ts).
  MEAL_PENALTY_DEFAULTS, mealPenaltyPerHead,
  crowdProvenanceLines, crowdUpliftsInForce as engineCrowdUpliftsInForce,
  // Named roles with their own rate cards — "Stand-in", "Picture double"
  // (roles.ts). The app never re-derives a role's inherited figures; it asks
  // the engine what they would be.
  rolePactSettings as engineRolePactSettings,
  roleSpactSettings as engineRoleSpactSettings,
  rolePerHead as engineRolePerHead,
  roleFrameworkHours as engineRoleFrameworkHours,
  cutDayCancellations,
  bankHolidayName, bankHolidayDataMissing,
} from "../engine";
...
const TAKE3_WORDMARK=(cls)=>`<img class="t3wordmark${cls?' '+cls:''}" src="/brand/take3-black.png" alt="Laural" draggable="false">`;

// ---------- one product name ----------
// The app used to introduce itself four different ways — "Laural" on the gate,
// "Take 3 Agency" in the sidebar, "CrowdOS" in tooltips, print titles, export
// filenames and the Excel author field, "StuntOS" in the mode labels. A person
// who has just signed up cannot tell which of those is the thing they are
// using. There is one name, and it is the one the gate says.
const PRODUCT='Laural';
// The only support route the app has. Shown on the gate, in the account menu
// and wherever an error is something we would need to see the file to fix.
const SUPPORT_EMAIL='support@take3agency.com';
// The minimum password length the CLIENT enforces. It must match the minimum
// set on the Supabase project (docs/LAUNCH.md tells the owner to set 10): with
// the client on 6 and the server on 10, someone typing 8 passed every check in
// the browser, was rejected by the server, and was told "It needs to be at
// least 6 characters" — which they had already done.
const PW_MIN=10;
// The largest PDF the reader will take — the request-body limit with room for
// the JSON around it. Checked on the client so an oversized file is refused
// with a sentence, not with "AI read failed (413)".
const PDF_MAX_MB=18;
const supportHref=(subject,body)=>'mailto:'+SUPPORT_EMAIL+'?subject='+encodeURIComponent(subject||(PRODUCT+' help'))+(body?'&body='+encodeURIComponent(body):'');
...
<!-- D4: 3,006 focusable elements, 1,589 of them on the day board, and no way
     past them. Reaching D96 from the top of the page was about 1,500 tab
     presses. These are the first three stops in the document, and the day board
     itself carries a "Jump to day…" control (#dayJump) one tab further on. -->
<nav class="skiplinks" aria-label="Skip links">
  <a href="#viewDays" data-skip="main">Skip to the schedule</a>
  <a href="#dayJump" data-skip="jump">Jump to a shoot day</a>
  <a href="#search" data-skip="search">Skip to search</a>
</nav>
...
<!-- D1: THIS IS A MODAL, and it has to behave like one. It covers the whole
     app, but the app underneath stayed reachable: fifteen focusable controls
     sat outside it, so tabbing past "Need help?" walked straight into the
     board a signed-out user is not supposed to be able to touch. -->
<div id="gate" class="hidden" role="dialog" aria-modal="true" aria-labelledby="gateTitle">
  <div class="gate-card">
    <div class="gate-logo">${TAKE3_WORDMARK('big')}</div>
    <h1 class="gate-title" id="gateTitle">Welcome to Laural</h1>
    <div class="gate-sub" id="gateSub">Sign in to your account to continue</div>
    <div class="gate-fields hidden" id="auNameRow">
      ...
      <div class="gate-field"><label>What you do <b>*</b></label><select id="auRole">
        ...
        <option>Other</option>
      </select><span class="gate-hint">We use this to open the app on the right department for you. You can change it later.</span></div>
    </div>
    <div class="gate-fields" id="auMainFields">
      <div class="gate-field"><label>Email <b>*</b></label><input id="auEmail" type="email" autocomplete="email"></div>
      <div class="gate-field" id="auPassField"><label>Password <b>*</b></label><input id="auPass" type="password" autocomplete="current-password"><span class="gate-hint hidden" id="auPassHint"></span></div>
    </div>
    <!-- C12: the answer to "why didn't that work?" sits with the fields it is
         about, ABOVE the buttons — it used to land underneath "Create an
         account", three controls below the thing that failed. -->
    <div id="auStatus" class="gate-status" role="alert" aria-live="assertive"></div>
    <button class="gate-primary" id="auSignIn">Sign in</button>
    <button class="gate-primary hidden" id="auCreate">Create my account</button>
    <button class="gate-primary hidden" id="auSendReset">Email me a reset link</button>
    <div class="gate-or" id="auOr"><span></span>Or<span></span></div>
    ...
    <div class="gate-foot" id="auFootSignIn">Forgotten your password? <button id="auForgot">Reset it</button></div>
    <div class="gate-foot" id="auFootSwap">New here? <button id="auSignUp">Create an account</button></div>
    <div class="gate-foot hidden" id="auFootBack"><button id="auBackToSignIn">Back to sign in</button></div>
    <div class="gate-help">
      <a href="/privacy" target="_blank" rel="noopener">Privacy policy</a>
      <span aria-hidden="true">·</span>
      <a id="gateSupport" href="mailto:${SUPPORT_EMAIL}?subject=Laural%20help">Need help? Email us</a>
    </div>
  </div>
</div>

<div id="pwReset" class="hidden" role="dialog" aria-modal="true" aria-labelledby="pwResetTitle">
  <div class="gate-card">
    <div class="gate-logo">${TAKE3_WORDMARK('big')}</div>
    <h1 class="gate-title" id="pwResetTitle">Choose a new password</h1>
    <div class="gate-sub">You came here from the reset email. Pick a new password and you will be signed straight in.</div>
    <div class="gate-fields">
      <div class="gate-field"><label>New password <b>*</b></label><input id="pwNew" type="password" autocomplete="new-password"><span class="gate-hint">At least 10 characters.</span></div>
      <div class="gate-field"><label>Type it again <b>*</b></label><input id="pwNew2" type="password" autocomplete="new-password"></div>
    </div>
    <button class="gate-primary" id="pwSave">Save new password</button>
    <div class="gate-foot"><button id="pwCancel">Cancel — go back to sign in</button></div>
    <div id="pwStatus" class="gate-status" role="status"></div>
  </div>
</div>
...
<aside class="sidebar" id="sidebar">
  <div class="side-head">${TAKE3_WORDMARK('side')}<span class="side-head-name">${PRODUCT}<small id="sideWho">Not signed in</small></span><button class="side-head-close" id="sideClose" aria-label="Close menu">✕</button></div>
  ...
  <button class="side-item" id="sideRatesNav">Rate cards</button>
...
<div class="pagebar">
  ...
  <button type="button" class="colourpill" id="colourPill" aria-haspopup="menu" aria-expanded="false"></button>
  <button class="tb-btn" id="btnAccount" data-tip="Sync your productions across devices">Sign in</button>
  <button class="tb-btn" id="btnMode" data-tip="Switch between light and dark" aria-label="Switch between light and dark"><span aria-hidden="true">◐</span></button>
</div>
<div id="dashView" class="wrap hidden"></div>
<div id="boardView" class="wrap">
  <div class="summary" id="summary"></div>
  <!-- Where the money above comes from, and what it is not. Every figure on
       this board is an estimate off a named rate card; without that said out
       loud beside the hero total, a producer has no way to judge it. -->
  <div class="provbar hidden" id="provBar"></div>
  ...
  <div class="controls">
    <div class="tabsrow">
    <div class="tabs">
      <!-- Only the nine buttons that actually own a panel are tabs; Settings
           opens a dialog, so it sits OUTSIDE the tablist (a tablist may only
           contain tabs). .tabs-list is a flex row inside the flex .tabs, so
           nothing moves on screen. -->
      <div class="tabs-list" role="tablist" aria-label="Board views">
      <button class="on" data-view="days" role="tab" id="tab-days" aria-controls="viewDays" aria-selected="true">Day board</button>
      <button data-view="cal" role="tab" id="tab-cal" aria-controls="viewCal" aria-selected="false" tabindex="-1">Calendar</button>
      <button data-view="stunts" id="tabBreakdown" role="tab" aria-controls="viewStunts" aria-selected="false" tabindex="-1">Stunt cost breakdown</button>
      <button data-view="crowd" id="tabCrowd" role="tab" aria-controls="viewCrowd" aria-selected="false" tabindex="-1">Stunts by day</button>
      <button data-view="cbdoc" id="tabCbDoc" class="hidden" role="tab" aria-controls="viewCbdoc" aria-selected="false" tabindex="-1">Breakdown document</button>
      <button data-view="briefs" id="tabBriefs" class="hidden" role="tab" aria-controls="viewBriefs" aria-selected="false" tabindex="-1">Briefs</button>
      <button data-view="doods" id="tabDoods" class="hidden" role="tab" aria-controls="viewDoods" aria-selected="false" tabindex="-1">DOOD</button>
      <button data-view="calc" id="tabCalc" role="tab" aria-controls="viewCalc" aria-selected="false" tabindex="-1">Calculator</button>
      <button data-view="cast" role="tab" id="tab-cast" aria-controls="viewCast" aria-selected="false" tabindex="-1">Cast list</button>
      </div>
    ...
    <button class="tb-btn" id="btnRecheck" data-tip="Ask ${PRODUCT} to read this schedule again — e.g. if it missed the cast numbers">${icon('refresh')} Re-check</button>
    <div class="expbar" id="expBar">
      <!-- C10: a bare <details> announces itself as a disclosure triangle, not
           as a menu of two mutually exclusive downloads. -->
      <details class="expmenu" id="expMenu">
        <summary class="tb-btn" role="button" aria-haspopup="menu" aria-expanded="false">${icon('file')} Export</summary>
        <div class="expmenu-list" role="menu" aria-label="Export format">
          <button type="button" role="menuitem" data-exp="csv">CSV (.csv)</button>
          <button type="button" role="menuitem" data-exp="xlsx">Excel (.xlsx)</button>
        </div>
      </details>
      <button type="button" class="tb-btn" data-exp="pdf">Export as PDF</button>
    </div>
    <!-- C13: the crowd column speaks in chips — "SA 40", "Pedestrians ×20",
         "Pod Veh 10" — with no key anywhere. This is the key, next to the
         time-of-day one, and it only shows in Crowd mode. -->
    <details class="chipkey hidden" id="chipKey">
      <summary>What the crowd chips mean</summary>
      <div class="chipkey-list">
        <span><i class="code cr anon">SA 40</i>Supporting artists nobody has named yet — 40 of them. Name them and they get their own chip and their own casting brief.</span>
        <span><i class="code cr">Pedestrians ×20</i>A named group of supporting artists — 20 people.</span>
        <span><i class="code feat">Nurse ×2</i>Featured background: the SA rate plus a supplementary fee.</span>
        <span><i class="code spact">Driver ×1</i>SPACT — priced on the separate Take 3 SPACT card.</span>
        <span><i class="code veh">Pod Veh 10</i>Action vehicles. Counted, never costed as people. “Pod” means a pod-driven car.</span>
        <span><i class="code cr needsname">SA 40</i>A dashed edge means the group still has no usable name.</span>
      </div>
    </details>
    <div class="legend">
  ...
  <div id="viewDays" role="tabpanel" aria-labelledby="tab-days" tabindex="0"></div>
  <div id="viewCal" class="hidden" role="tabpanel" aria-labelledby="tab-cal" tabindex="0"></div>
  <div id="viewStunts" class="hidden" role="tabpanel" aria-labelledby="tabBreakdown" tabindex="0"></div>
  <div id="viewCrowd" class="hidden" role="tabpanel" aria-labelledby="tabCrowd" tabindex="0"></div>
  <div id="viewCbdoc" class="hidden" role="tabpanel" aria-labelledby="tabCbDoc" tabindex="0"></div>
  <div id="viewBriefs" class="hidden" role="tabpanel" aria-labelledby="tabBriefs" tabindex="0"></div>
  <div id="viewDoods" class="hidden" role="tabpanel" aria-labelledby="tabDoods" tabindex="0"></div>
  <div id="viewCalc" class="hidden" role="tabpanel" aria-labelledby="tabCalc" tabindex="0"></div>
  <div id="viewCast" class="hidden" role="tabpanel" aria-labelledby="tab-cast" tabindex="0"></div>
</div>
...
<div class="modal" id="cdayModal">
  <div class="box" style="max-width:860px">
    <div class="mhead"><h3 id="cdTitle"></h3><span class="sub" id="cdSub"></span><button class="x danger" id="cdReset" style="margin-left:auto">Reset to schedule</button><button class="x" id="cdClose" data-initfocus>Close</button></div>
    <div id="cdBody"></div>
  </div>
</div>
...
<!-- The app's own "are you sure?". The eight native window.confirm() dialogs
     this replaces were captioned "localhost:3000 says…" and looked, to anyone
     who did not build this, like the browser complaining rather than the app
     asking — worst of all during sign-in. -->
<div class="modal" id="confirmModal">
  <div class="box" style="max-width:460px">
    <div class="mhead"><h3 id="cfTitle">Are you sure?</h3><button class="x" id="cfClose">Close</button></div>
    <div style="padding:16px">
      <div id="cfBody" style="font-size:13px;color:var(--sub);line-height:1.65;white-space:pre-line"></div>
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:18px">
        <button class="tb-btn" id="cfNo">Cancel</button>
        <button class="tb-btn" id="cfYes" style="border-color:var(--hv-line);color:var(--hv)">Yes</button>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="box" style="max-width:420px">
    ...
    <div style="padding:16px">
      <div id="auSignedIn">
        ...
        <div class="gate-help" style="justify-content:flex-start;margin-top:14px">
          <a id="auSupport" href="${supportHref(PRODUCT+' help')}">Get help</a>
          <span aria-hidden="true">·</span>
          <a href="/privacy" target="_blank" rel="noopener">Privacy policy</a>
        </div>
      </div>
...
<div class="modal" id="impModal">
  <div class="box" style="max-width:560px">
    ...
    <div style="padding:16px">
      ...
      <div id="impWarn" class="imp-warn hidden" role="status"></div>
      <div style="display:flex;gap:10px;margin-top:16px;align-items:center">
        <button class="tb-btn primary-action" id="impGo" data-initfocus>Import schedule</button>
...
<div class="modal" id="addChooser">
  <div class="box" style="max-width:440px">
    ...
    <div style="padding:16px">
      <button class="chooser-opt" id="acUpload" data-initfocus><b>Upload a schedule</b><span>A PDF (one-liner, Full Fat, or both) — or photos of the pages, read by AI. You review everything before it is published.</span></button>
      ...
      <!-- Said BEFORE the upload, not as an error afterwards. These three are
           the whole of what goes wrong with a first upload. -->
      <div class="chooser-help">
        <b>What reads best</b>
        <ul>
          <li>A PDF exported from Movie Magic, Scenechronize, Yamdu or similar — one where you can select the text.</li>
          <li>Not a scan or a photocopy: if the pages are pictures there is nothing to read. Photograph the pages and upload those instead — that route works well.</li>
          <li>Not password-protected. Save a copy without the password first.</li>
          <li>Under ${PDF_MAX_MB} MB. Longer shoots import fine in parts — one unit or one block at a time.</li>
        </ul>
      </div>
    </div>
  </div>
...
<div class="modal revpage" id="revPage">
  <div class="rp-sheet">
    ...
    <div class="rp-foot">
      ...
      <button class="rp-pub" id="rpPublish" data-initfocus>Publish</button>
    </div>
  </div>
</div>

<!-- "What changed" — the artefact the crowd department circulates. Complete,
     never truncated, grouped by day, printable and exportable. -->
<div class="modal revpage" id="chgModal">
  <div class="rp-sheet chg-sheet">
    <div class="rp-head">
      <div>
        <div class="crumbs" id="chgCrumbs"></div>
        <h3>What changed</h3>
        <div class="rp-file" id="chgSub"></div>
      </div>
      <div class="grow"></div>
      <div class="expbar">
        <details class="expmenu">
          <summary class="tb-btn">Export</summary>
          <div class="expmenu-list">
            <button type="button" data-chgexp="csv">CSV (.csv)</button>
            <button type="button" data-chgexp="xlsx">Excel (.xlsx)</button>
          </div>
        </details>
        <button type="button" class="tb-btn" data-chgexp="pdf">Print / PDF</button>
      </div>
      <button class="x" id="chgClose">Close</button>
    </div>
    <div class="rp-body" id="chgReport"></div>
  </div>
</div>
...
// Revision-change markers (see "SEEING WHAT CHANGED" further down). Declared
// up here with the rest of the board state because the sticky-toolbar setup
// block runs at module load and asks whether the markers are on.
let CHG_SHOW=true, CHG_ONLY=false, CHG_DISMISS=null;
...
function exportProdTitle(){const s=SOURCES[ACTIVE]||{};return s.prod||s.title||PRODUCT}
...
function tablesFromView(sel){
  ...
  for(const card of src){
    ...
    // Every explanatory .note on the card — the travel-band explanation, the
    // rate-card year, the SPACT card terms — used to be dropped, because this
    // only ever read <table> elements. The spreadsheet someone forwards to a
    // producer was then the LEAST qualified thing the app makes. The notes ride
    // along under the rows they explain.
    const notes=[...card.querySelectorAll('.note')].map(cellText).filter(Boolean);
    if(notes.length&&out.length){
      const sh=out[out.length-1];
      sh.rows.push([]);
      for(const n of notes)sh.rows.push(['Note: '+n]);
    }
  }
  return out;
}
/**
 * A final sheet (CSV: a final section) saying where the money came from and
 * what it is not — the same words the board shows above the total. An export
 * leaves the app and gets read by someone who never saw the screen, so it has
 * to carry its own qualification.
 */
function provenanceSheet(){
  const rows=[['About these figures'],[]];
  const s=SOURCES[ACTIVE]||{};
  rows.push(['Production',s.prod||s.title||'']);
  rows.push(['Schedule',s.title||'']);
  rows.push(['Department',APPMODE==='crowd'?'Crowd':'Stunts']);
  rows.push(['Exported from',PRODUCT]);
  rows.push(['Exported on',new Date().toLocaleString('en-GB',{dateStyle:'full',timeStyle:'short'})]);
  rows.push([]);
  try{for(const l of moneyProvenanceLines())rows.push([l])}catch(e){}
  rows.push([]);
  rows.push(['Questions about these numbers: '+SUPPORT_EMAIL]);
  return {name:'About these figures',rows};
}
...
function doExport(view,fmt){
  if(view==='cbdoc'&&fmt==='xlsx'){
    ...
    exportCbdXlsx(exportFileBase(view)).then(
      ...
      err=>setStatus('We couldn’t build that Excel file. Try again — and if it keeps happening, export it as CSV instead and email '+SUPPORT_EMAIL+'.'));
    return;
  }
  const cfg=EXPORT_VIEWS[view];if(!cfg){setStatus('This page can’t be exported.');return}
  // A crash while building the rows used to be swallowed: the export fell back
  // to a one-cell "No data on this page yet" workbook and still reported
  // success, so a broken export was indistinguishable from an empty page. Say
  // what happened and download nothing — the same way the crowd breakdown does.
  let sheets=[];
  try{sheets=cfg.build()||[]}
  catch(err){
    console.error(err);
    setStatus('We couldn’t build the '+exportLabel(view)+' export, so nothing was downloaded. Try again — and email '+SUPPORT_EMAIL+' if it keeps happening.');
    return;
  }
  if(!sheets.length||!sheets.some(sh=>sh.rows&&sh.rows.length)){
    setStatus('There’s nothing on this page to export yet.');
    return;
  }
  // the provenance goes with every export, always — it is the only thing in
  // the file that says these are estimates and which rate card produced them
  try{sheets=[...sheets,provenanceSheet()]}catch(e){}
...
// The one print path, for every page.
//
// `node` is a CLONE of the real thing, dropped into a clean iframe carrying the
// app's own stylesheets. It paginates as ordinary in-flow content, so it prints
// byte-for-byte like the screen however long it is. This replaced the old
// generic approach (hide the page with `visibility`, pull the view to 0,0 with
// `position:absolute`) which took the content out of flow — pagination then
// stopped tracking it and the first pages of anything long came out blank. A
// 60-day board is every bit as long as the crowd breakdown that first needed
// this, so both go through here now.
function printNodeInFrame(node,opts){
  opts=opts||{};
  ...
  if(!doc){frame.remove();setStatus('Couldn’t open the print view — try again.');return}
  const sheets=[...document.querySelectorAll('link[rel="stylesheet"]')].map(l=>l.href);
  doc.open();
  doc.write(`<!doctype html><html><head><meta charset="utf-8">
    <title>${esc(opts.title||PRODUCT)}</title>
    ${sheets.map(h=>`<link rel="stylesheet" href="${esc(h)}">`).join('')}
    <style>
      @page{size:${opts.page||'A4 portrait'};margin:${opts.margin||'10mm'}}
      ...
      /* keep every fill the browser would otherwise drop */
      *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
      .tscroll,.doodscroll,.rp-tblwrap{overflow:visible!important;max-height:none!important}
      ${opts.css||''}
    </style></head><body class="light ${esc(opts.bodyClass||'')}"></body></html>`);
  doc.close();
  ...
  // wait for the copied stylesheets, or the page prints unstyled
  let left=sheets.length;
  if(!left){setTimeout(go,60);return}
  let fired=false;
...
// Screen-only furniture that must never reach paper, whichever page it is on.
function stripScreenOnly(node){
  node.querySelectorAll('.cbtier,.cbdel,.cbabove,.cbaddbtn,.noprint,.expbar,.colgrip,.colmenu-btn,#tipbox').forEach(n=>n.remove());
  node.querySelectorAll('[contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));
  // collapsed drawers print open — the paper copy is the whole record
...
function exportCbdPDF(){
  ...
  stripScreenOnly(node);
  cbdStripAddRows(node);
  printNodeInFrame(node,{
    title:exportFileBase('cbdoc'),
    page:'A3 landscape',margin:'8mm',bodyClass:'printing-cbdoc',
    css:`
...
// PDF that looks exactly like the page — the day board, calendar, cost
// breakdown, DOODs and cast list all take the same route as the breakdown.
const PDF_PAGE={
  days:{page:'A4 portrait'},
  cal:{page:'A3 landscape',margin:'8mm'},
  stunts:{page:'A4 landscape'},
  crowd:{page:'A4 landscape'},
  doods:{page:'A3 landscape',margin:'8mm'},
  cast:{page:'A4 portrait'},
};
function exportViewPDF(view){
  if(view==='cbdoc'){exportCbdPDF();return}
  const cfg=EXPORT_VIEWS[view];if(!cfg){setStatus('This page can’t be exported.');return}
  const host=$(cfg.sel);
  if(!host||!host.children.length){setStatus('There’s nothing on this page to export yet.');return}
  const node=/** @type {any} */(host.cloneNode(true));
  stripScreenOnly(node);
  // the printed page carries the same qualification the screen does: which
  // rate card, which year, and that every figure is an estimate
  try{
    const prov=document.createElement('div');
    prov.className='note';
    prov.style.marginTop='14px';
    prov.innerHTML=moneyProvenanceLines().map(l=>`<div>${esc(l)}</div>`).join('');
    node.appendChild(prov);
  }catch(e){}
  const pg=PDF_PAGE[view]||{};
  printNodeInFrame(node,{
    title:exportFileBase(view),
    page:pg.page,margin:pg.margin,bodyClass:'printing-page',
    css:`
      .tablecard,.daycard{break-inside:avoid;page-break-inside:avoid}
      .doodtable thead th,.doodtable .dood-name{position:static}
      table thead{display:table-header-group}
      .strip,.cal-cell,tr{break-inside:avoid;page-break-inside:avoid}`,
  });
}
/**
 * "It is working." An export takes several seconds on a long schedule and used
 * to give no sign of life at all, so the only feedback the user got was the
 * browser's download shelf — if they were still looking.
 */
function exportBusy(on,fmt){
  const bar=$('#expBar');if(!bar)return;
  bar.classList.toggle('busy',!!on);
  bar.querySelectorAll('button,summary').forEach(b=>{
    /** @type {any} */(b).setAttribute('aria-busy',on?'true':'false');
  });
  const pdf=/** @type {any} */(bar.querySelector('[data-exp="pdf"]'));
  if(pdf)pdf.disabled=!!on;
  if(on)setStatus((fmt==='pdf'?'Building the PDF':fmt==='xlsx'?'Building the Excel file':'Building the CSV')+'… on a long schedule this takes a few seconds.',{spinner:true});
}
...
function syncToolRow(){
  ...
  // the "Changes since …" strip belongs to the board and the calendar too
  if(typeof renderChangeBar==='function')renderChangeBar();
}

// Every interpolation into HTML goes through this. The single quote and the
// backtick are in the set because the board now builds thousands of attributes
// from template literals: the day it acquires one single-quoted attribute — or
// one that ends up inside a nested template — an unescaped ' or ` closes it and
// the rest of the value becomes markup.
function esc(s){return String(s??'').replace(/[&<>"'`]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;','`':'&#96;'}[c]))}
// A user-supplied image for a CSS url(). Two separate jobs, both needed:
//  · it must be a data: image and nothing else — a remote URL here would phone
//    a third party from every board that opens the production, and `javascript:`
//    / `url(...)` tricks belong nowhere near an inline style attribute;
//  · what survives that is still going into an HTML attribute, so it is escaped
//    like anything else.
function safeImageUrl(v){
  const s=String(v??'').trim();
  if(!/^data:image\/(png|jpe?g|gif|webp|avif);base64,[A-Za-z0-9+/=\s]*$/i.test(s))return '';
  return s.replace(/\s+/g,'');
}
/** The same value, ready to drop into `style="background-image:url('…')"`. */
function coverCssUrl(v){const u=safeImageUrl(v);return u?`url('${esc(u)}')`:''}
...
/**
 * ONE settled figure, printed in whole pounds.
 *
 * `gbp(Math.round(x))` was scattered through the views, and where `x` was a
 * float the view had re-derived itself (heads × rate × holiday) rather than the
 * amount the engine actually settled, the printed figure disagreed with the
 * engine's own total by pounds. Everything that prints a single total now goes
 * through here, on a number that came OUT of the engine — see lib/engine/money.
 * Columns of figures use footPounds() instead, which makes the column foot to
 * the total printed beside it.
 */
const gbpWhole = n => gbp(Math.round(engineRound2(Number(n)||0)));
/**
 * A column of pounds-and-pence figures, rounded to whole pounds in a way that
 * still ADDS UP to the whole-pound total printed beside it.
 *
 * Rounding sixteen week rows independently and rounding the grand total
 * separately leaves the printed column disagreeing with the printed total by a
 * few pounds — which is the "your columns don't add up" complaint the engine's
 * penny-level work just fixed, reappearing one order of magnitude larger. So:
 * round every row, then hand the leftover pounds, one at a time, to the rows
 * that lost the most in their own rounding. No row moves by more than £1, and
 * the column foots exactly.
 */
function footPounds(vals,total){
  const nums=vals.map(v=>Number(v)||0);
  const out=nums.map(v=>Math.round(v));
  let diff=Math.round(Number(total)||0)-out.reduce((a,x)=>a+x,0);
  if(!diff||!out.length)return out;
  const step=diff>0?1:-1;
  const order=nums.map((v,i)=>({i,rem:v-Math.round(v)}))
    .sort((a,b)=>step>0?b.rem-a.rem:a.rem-b.rem);
  for(let k=0;diff!==0&&k<order.length*8;k++){out[order[k%order.length].i]+=step;diff-=step}
  return out;
}

// ---------- safe storage (falls back to memory) ----------
// MEM is the fallback copy. The trap it used to fall into: when localStorage
// is FULL, setItem throws but getItem keeps working and keeps returning the
// OLD value — so every read-back after a failed write silently served the
// pre-edit data and the user's work vanished later with no error at all.
// Once any write has failed, MEM becomes the authoritative overlay: reads
// prefer it, and successful writes keep it in step.
const MEM={};
let MEM_AUTHORITATIVE=false;
let STORAGE_FULL_WARNED=false;
function warnStorageFull(){
  if(STORAGE_FULL_WARNED)return;
  STORAGE_FULL_WARNED=true;
  // setStatus needs the page markup, and this can fire during boot
  setTimeout(()=>{try{setStatus('This browser has run out of room to save your work on this device. Your latest changes are safe in this tab only — don’t close it. Sign in so everything saves to your account, and close any other tabs of the app.',{sticky:true})}catch(e){}},0);
}
const store={
  get(k){
    if(MEM_AUTHORITATIVE&&Object.prototype.hasOwnProperty.call(MEM,k))return MEM[k];
    try{return window.localStorage.getItem(k)}catch(e){return MEM[k]??null}
  },
  set(k,v){
    try{
      window.localStorage.setItem(k,v);
      if(MEM_AUTHORITATIVE)MEM[k]=v; // keep the overlay in step with a write that did land
    }catch(e){
      MEM[k]=v;MEM_AUTHORITATIVE=true;warnStorageFull();
    }
  }
};
// ---- account-level sync ----
...
// kinds whose cloud write has FAILED and never since succeeded — sign-out
// keeps these on the device rather than wiping the only copy
const CLOUD_FAIL=new Set();
let CLOUD_WARNED=false;
// ---- another tab, or another device, has moved on ----
// Nothing here merges: a write sends the WHOLE of a production's subset. So a
// second tab (or an iPad whose copy is an hour old) could upload its stale
// copy over an afternoon's work with no warning. Once this tab knows it is
// behind it stops writing to the account and says so, plainly, until reload.
let TAB_STALE=false;
function markTabStale(src){
  if(TAB_STALE)return;
  TAB_STALE=true;
  const what=(src&&(src.prod||src.title))?'“'+(src.prod||src.title)+'”':'this production';
  setStatus('Someone has changed '+what+' somewhere else — another tab, another computer, or your iPad — since this page was opened. Nothing more from this tab is being saved to your account, so it can’t overwrite that newer work. Reload this page to pick up the latest version.',{sticky:true});
}
// the same thing seen from the local side: another tab on THIS computer wrote
// to storage, so what is on screen here is already out of date
try{
  window.addEventListener('storage',e=>{
    if(TAB_STALE||!e||!e.key||!/^(crowdos|stuntos)-/.test(e.key))return;
    if(e.newValue===e.oldValue)return;
    if(e.key==='crowdos-signout-kept')return;
    markTabStale(null);
  });
}catch(e){}
function cloudSyncUser(kind,data){
  if(!CLOUD||!CLOUD.session||!cloud.upsertUserBlob||TAB_STALE)return;
...
// What each account-level blob is CALLED, in the words the user knows it by.
// The toast used to name the internal storage key ("your raedits") and a SQL
// file in our repo — neither of which means anything to an AD.
const BLOB_LABEL={
  ra:'your risk-assessment settings',
  raedits:'your risk-assessment notes',
  freecalc:'your calculator',
  stuntcalc:'your stunt calculator',
  dancecalc:'your dancer calculator',
  fcrows:'your calculator rows',
  srows:'your stunt calculator rows',
  drows:'your dancer calculator rows',
  dbriefs:'your casting briefs',
};
function noteCloudFail(what,err){
  CLOUD_FAIL.add(what);CLOUD_OK.delete(what);
  if(CLOUD_WARNED)return;CLOUD_WARNED=true;
  setStatus(friendlyCloudError(err,BLOB_LABEL[what]||'your work'));
}
...
// A tooltip a screen reader can actually reach: role="tooltip" plus a live
// aria-describedby link from whichever element is currently showing it, cleared
// again on hide so nothing points at stale text.
tipbox.setAttribute('role','tooltip');
document.body.appendChild(tipbox);
let TIP_EL=null;
let TIP_DESCRIBED=null;
function describeByTip(el){
  if(TIP_DESCRIBED&&TIP_DESCRIBED!==el){
    if(TIP_DESCRIBED.dataset.tipDesc==='1'){TIP_DESCRIBED.removeAttribute('aria-describedby');delete TIP_DESCRIBED.dataset.tipDesc;}
    TIP_DESCRIBED=null;
  }
  // never clobber a describedby the markup set itself
  if(el&&!el.getAttribute('aria-describedby')&&!el.getAttribute('aria-label')){
    el.setAttribute('aria-describedby','tipbox');
    el.dataset.tipDesc='1';
    TIP_DESCRIBED=el;
  }
}
function hideTip(){
...
document.addEventListener('mouseover',e=>{
  ...
  else if(TIP_EL){TIP_EL=null;hideTip()}
});
document.addEventListener('focusin',e=>{const el=e.target.closest&&e.target.closest('[data-tip]');if(el){TIP_EL=el;positionTip(el)}});
document.addEventListener('focusout',()=>{TIP_EL=null;hideTip()});
window.addEventListener('scroll',()=>{TIP_EL=null;hideTip()},true);
...
document.addEventListener('click',e=>{
  ...
  if(el&&!el.closest('button,a,input,select,label,[role=button]')){
    ...
    TIP_TIMER=setTimeout(()=>{if(TIP_EL===el){TIP_EL=null;hideTip()}},4000);
  }else if(TIP_EL){TIP_EL=null;hideTip()}
});

// ============================================================================
// DIALOGS — one place, every modal.
//
// The app has ~20 overlays, all of them built the same way: a `.modal` (or
// `.raOverlay`) div that becomes visible when the class `open` is added. They
// were plain divs: no role, no label, no focus trap, no focus return, and the
// board underneath stayed reachable by Tab, so a screen-reader user tabbed
// straight out of the day calculator into the schedule behind it.
//
// `classList.add('open')` opens a dialog and closeModal() shuts it. Because the
// class is the switch, a MutationObserver does the accessibility work — which
// means the dozens of existing
// `.classList.add('open')` call sites are routed through the same code without
// a single one of them changing behaviour. Nothing here closes a dialog that
// did not already close itself the same way: Escape and backdrop-click are
// opt-in per dialog (see MODAL_OPTS), and both fire the dialog's own Close
// button so its existing side effects (recalculate, tear down the PDF viewer,
// clear the context) still run exactly as they did.
// ============================================================================
const MODAL_SEL='.modal,.raOverlay';
// Per-dialog wiring. `label` names it; `close` is the control Escape and a
// backdrop click press (its own handler does the closing, so no behaviour
// moves); `esc`/`backdrop` are false where an existing listener already covers
// it, or where dismissing by accident would throw work away.
const MODAL_OPTS={
  costModal:{label:'cmTitle'},
  calModal:{label:'calTitle'},
  cdayModal:{label:'cdTitle'},
  splitModal:{label:'splitTitle'},
  cbSetupModal:{label:'Breakdown settings'},
  authModal:{label:'Account'},
  confirmModal:{label:'cfTitle',close:'cfClose',esc:true,backdrop:true},
  dayModal:{label:'Add shoot day'},
  // #smClose is Close WITHOUT saving (#smSave is a separate button), so Escape
  // here threw away a scene renumber or a cast edit
  sceneModal:{label:'smTitle',close:'smClose'},
  impModal:{label:'Import schedule'},           // Escape would bin a pending import
  prodSettings:{label:'Production settings',close:'psClose',esc:true},
  rateAdminModal:{label:'Rate cards',close:'rcaClose',esc:true,backdrop:true},
  addChooser:{label:'Add schedule',close:'acClose',esc:true},
  // #bkClose is labelled Cancel — Escape discarded a whole multi-date selection
  bulkModal:{label:'Add shoot days',close:'bkClose'},
  revPage:{label:'Review schedule'},            // Cancel only — never on Escape
  origModal:{label:'origTitle',close:'origClose',esc:true,backdrop:true},
  // #pmClose is Cancel; a backdrop click (including the one that ends a
  // drag-select of text inside the box) threw away a half-typed production
  prodModal:{label:'pmTitle',close:'pmClose'},
  stuntDayModal:{label:'sdmTitle',close:'sdmClose',esc:true},
  chgModal:{label:'What changed'},
  raOverlay:{label:'Risk assessment',close:'raClose',esc:true},
};
const MODAL_STACK=[];
// ---------- the app's own confirmation dialog ----------
// Returns a promise for true/false. Falls back to window.confirm only if the
// markup somehow isn't there, so no caller can ever be left waiting forever.
let CF_RESOLVE=null;
function askConfirm(opts){
  opts=opts||{};
  const el=document.getElementById('confirmModal');
  if(!el)return Promise.resolve(window.confirm(opts.body||opts.title||'Are you sure?'));
  document.getElementById('cfTitle').textContent=opts.title||'Are you sure?';
  document.getElementById('cfBody').textContent=opts.body||'';
  const yes=/** @type {any} */(document.getElementById('cfYes'));
  const no=/** @type {any} */(document.getElementById('cfNo'));
  yes.textContent=opts.yes||'Yes';
  no.textContent=opts.no||'Cancel';
  // a destructive answer must not be the calm-looking one
  yes.classList.toggle('danger',!!opts.danger);
  el.classList.add('open');
  // Cancel takes focus, so Enter on a dialog nobody read does nothing
  setTimeout(()=>no.focus({preventScroll:true}),0);
  return new Promise(res=>{
    CF_RESOLVE=v=>{CF_RESOLVE=null;el.classList.remove('open');res(v)};
  });
}
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(!t.closest)return;
  if(t.closest('#cfYes')){if(CF_RESOLVE)CF_RESOLVE(true);return}
  if(t.closest('#cfNo')||t.closest('#cfClose')){if(CF_RESOLVE)CF_RESOLVE(false);return}
  if(t.id==='confirmModal'){if(CF_RESOLVE)CF_RESOLVE(false);}
});
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'&&CF_RESOLVE)CF_RESOLVE(false);
});
/** One wording for "delete this production", used everywhere it is offered. */
function confirmDeleteProduction(name){
  return askConfirm({
    title:'Delete “'+name+'”?',
    body:'Every schedule under this production goes with it — all its shoot days, crowd numbers, day-calculator settings and casting briefs. This cannot be undone, on any of your devices.',
    yes:'Delete the production',no:'Keep it',danger:true,
  });
}
function modalIsOpen(el){return !!el&&el.classList.contains('open')}
/** Give a dialog its role, its accessible name and (once) a focusable shell. */
function labelModal(el){
  const opt=MODAL_OPTS[el.id]||{};
  el.setAttribute('role','dialog');
  el.setAttribute('aria-modal','true');
  if(!el.getAttribute('aria-label')&&!el.getAttribute('aria-labelledby')){
    const byId=opt.label&&document.getElementById(opt.label);
    if(byId){
      if(!byId.id)byId.id=el.id+'-title';
      el.setAttribute('aria-labelledby',byId.id);
    }else{
      const h=el.querySelector('h3,h1,h2');
      if(h){if(!h.id)h.id=el.id+'-title';el.setAttribute('aria-labelledby',h.id);}
      else if(opt.label)el.setAttribute('aria-label',opt.label);
    }
  }
  const box=el.querySelector('.box,.rp-sheet,.orig-sheet')||el;
...
function modalFocusables(el){
  return /** @type {any[]} */([...el.querySelectorAll(FOCUSABLE)])
    .filter(n=>!n.closest('.hidden')&&n.offsetParent!==null);
}
/** Everything behind the dialogs stops being reachable while one is up. */
function setBackgroundInert(on){
  const bg=/** @type {any} */(document.querySelector('.layout'));
  if(!bg)return;
  if(on){try{bg.inert=true}catch(e){}bg.setAttribute('aria-hidden','true');}
  else{try{bg.inert=false}catch(e){}bg.removeAttribute('aria-hidden');}
}
/**
 * A selector that will still find the control a dialog was opened FROM after
 * the board behind it has been re-rendered.
 *
 * The day calculator recalculates the whole board as it closes, which destroys
 * the pencil that opened it — so `returnTo` pointed at a detached node, the
 * "is it still connected?" test failed, and focus was dumped on the page
 * instead of coming back to where the keyboard user actually was.
 */
function stableTriggerSel(el){
  if(!el||!el.getAttribute)return '';
  if(el.id)return '#'+CSS.escape(el.id);
  for(const a of ['data-costday','data-cdopen','data-stuntday','data-split','data-goto','data-delday','data-openrev','data-side'])
    if(el.hasAttribute(a))return `[${a}="${CSS.escape(el.getAttribute(a))}"]`;
  return '';
}
function modalActivate(el){
  if(MODAL_STACK.some(m=>m.el===el))return;
  const box=labelModal(el);
  MODAL_STACK.push({el,box,returnTo:document.activeElement,returnSel:stableTriggerSel(document.activeElement)});
  setBackgroundInert(true);
  // Only take focus if the dialog has not already placed it itself — several
  // of these focus a specific field on open and that must win.
  if(!el.contains(document.activeElement)){
    // [data-initfocus] names the control that should be under the keyboard on
    // open. Without it focus lands on whatever happens to come FIRST in the
    // markup — which on the day calculator is "Reset to schedule", so Enter on
    // a dialog nobody has read wiped the day. Never let a destructive or a
    // cancelling action be the default.
    const want=/** @type {any} */(el.querySelector('[data-initfocus]'));
    const f=modalFocusables(el);
    ((want&&f.includes(want)?want:f[0])||box).focus({preventScroll:true});
  }
}
function modalDeactivate(el){
  const i=MODAL_STACK.findIndex(m=>m.el===el);
  if(i<0)return;
  const [rec]=MODAL_STACK.splice(i,1);
  if(!MODAL_STACK.length)setBackgroundInert(false);
  // Focus goes back where it came from. Where the trigger no longer exists —
  // most board buttons are re-rendered by the recalculation the dialog kicks
  // off — it lands on the open page instead, so the keyboard carries on from
  // the board rather than from the top of the document.
  // Only the dialog that is actually on top hands focus back. Restoring from a
  // dialog buried in the stack pushed focus onto the `.layout` behind it, which
  // is inert while anything is open — the browser then drops it on <body> and
  // the keyboard is stranded outside the dialog still on screen.
  if(MODAL_STACK.length){
    ...
    if(!top.el.contains(document.activeElement)){
      ...
      try{(f[0]||top.box).focus({preventScroll:true})}catch(e){}
    }
    return;
  }
  let back=/** @type {any} */(rec.returnTo);
  // The trigger is routinely re-rendered by the recalculation the dialog kicks
  // off on close (the day calculator rebuilds the whole board). Look the same
  // control up again rather than giving up and dropping focus on the page.
  if((!back||!back.isConnected)&&rec.returnSel){
    try{back=/** @type {any} */(document.querySelector(rec.returnSel))}catch(e){}
  }
  if(back&&back.isConnected&&typeof back.focus==='function'&&!back.closest('.hidden')){
    try{back.focus({preventScroll:true});return}catch(e){}
  }
  const panel=/** @type {any} */([...document.querySelectorAll('[role="tabpanel"]')].find(p=>!p.classList.contains('hidden')));
  if(panel)try{panel.focus({preventScroll:true})}catch(e){}
}
function closeModal(el){
  el=typeof el==='string'?document.getElementById(el.replace(/^#/,'')):el;
  if(!el||!modalIsOpen(el))return;
  // Press the dialog's own Close control where it has one, so whatever that
  // button does (recalculate the day, tear down the PDF viewer, clear the
  // editing context) still happens.
  const opt=MODAL_OPTS[el.id]||{};
  const btn=/** @type {any} */(opt.close&&document.getElementById(opt.close));
  // A close button that is missing, disabled or hidden cannot do the closing —
  // returning here left the dialog on screen with no way out at all.
  if(btn&&!btn.disabled&&btn.offsetParent!==null){btn.click();if(!modalIsOpen(el))return}
  el.classList.remove('open');
}
// Watch the dialog roots themselves (not the whole document) so a board-wide
// filter pass doesn't push thousands of irrelevant mutations through here.
{
  const mo=new MutationObserver(muts=>{
    for(const m of muts){
      const el=/** @type {any} */(m.target);
      if(modalIsOpen(el))modalActivate(el);else modalDeactivate(el);
    }
  });
  document.querySelectorAll(MODAL_SEL).forEach(el=>mo.observe(el,{attributes:true,attributeFilter:['class']}));
}
// Focus trap — Tab and Shift+Tab cycle inside the topmost dialog.
document.addEventListener('keydown',e=>{
  if(e.key!=='Tab'||!MODAL_STACK.length)return;
  const {el,box}=MODAL_STACK[MODAL_STACK.length-1];
  const f=modalFocusables(el);
  if(!f.length){e.preventDefault();box.focus({preventScroll:true});return}
  const first=f[0],last=f[f.length-1];
  const cur=/** @type {any} */(document.activeElement);
  if(!el.contains(cur)){e.preventDefault();(e.shiftKey?last:first).focus();return}
  if(e.shiftKey&&cur===first){e.preventDefault();last.focus()}
  else if(!e.shiftKey&&cur===last){e.preventDefault();first.focus()}
},true);
// Escape — only for the dialogs that opted in above (the rest already have
// their own handler, or must not be dismissed by accident).
document.addEventListener('keydown',e=>{
  if(e.key!=='Escape'||!MODAL_STACK.length)return;
  const el=MODAL_STACK[MODAL_STACK.length-1].el;
  if(!(MODAL_OPTS[el.id]||{}).esc)return;
  e.preventDefault();
  closeModal(el);
});
// Backdrop click — same rule, and it has to be a click that BEGAN on the
// backdrop. Drag-selecting text inside the box and releasing over the dark area
// fires `click` on the dialog root, which dismissed the dialog and binned
// whatever had been typed into it.
let MODAL_DOWN_ON=null,MODAL_UP_ON=null;
const modalRootOf=el=>(el&&el.matches&&el.matches(MODAL_SEL))?el:null;
document.addEventListener('mousedown',e=>{MODAL_DOWN_ON=modalRootOf(e.target);MODAL_UP_ON=null},true);
document.addEventListener('mouseup',e=>{MODAL_UP_ON=modalRootOf(e.target)},true);
/** True when the press BOTH started and finished on the dialog root itself.
 *  `click` alone is not enough: it fires on the nearest common ancestor of the
 *  press and the release, so drag-selecting text inside the box and letting go
 *  over the dark area produced a click on the dialog root and dismissed it —
 *  taking a half-typed production with it. Every backdrop-dismiss on the page
 *  goes through this, including the hand-wired ones on the import dialog and
 *  the review page. */
function isBackdropDismiss(e,id){
  const t=/** @type {any} */(e.target);
  if(!t||(id?t.id!==id:!(t.matches&&t.matches(MODAL_SEL))))return false;
  return MODAL_DOWN_ON===t&&MODAL_UP_ON===t;
}
document.addEventListener('click',e=>{
  const el=/** @type {any} */(e.target);
  if(!isBackdropDismiss(e))return;
  if(!(MODAL_OPTS[el.id]||{}).backdrop)return;
  closeModal(el);
});

// ---------- state that is conveyed by colour alone ----------
// Segmented controls, the production-settings switches and the Stunt/Crowd mode
// pills all say "selected" with a class and nothing else. This walks them after
// anything that could have changed one and states it in ARIA too. It runs off a
// captured click (so it lands after the handler and any re-render it caused)
// and after every full refresh.
function syncToggleA11y(){
  document.querySelectorAll('.seg').forEach(seg=>{
    if(!seg.getAttribute('role'))seg.setAttribute('role','group');
    seg.querySelectorAll(':scope > button').forEach(b=>b.setAttribute('aria-pressed',b.classList.contains('on')?'true':'false'));
  });
  document.querySelectorAll('.srcbar button,.calctl button[data-calview],.qscope button').forEach(b=>{
    b.setAttribute('aria-pressed',b.classList.contains('on')?'true':'false');
  });
  document.querySelectorAll('.ps-tgl').forEach(t=>{
...
/**
 * D2 — 56 of 66 inputs had no programmatic label.
 *
 * Every settings and rate field in this app is written the same way:
 *
 *     <div class="rfield"><label>Early call travel</label>
 *       <div class="inwrap"><span>£</span><input id="cET"></div></div>
 *
 * which LOOKS labelled and is not: the <label> has no `for`, the input is not
 * inside it, and a screen reader announces "edit, blank". Rather than hand-edit
 * 78 markup sites (and lose the next one someone adds), the association is made
 * structurally: a label with no `for`, that does not already wrap its control,
 * is tied to the first form control in the same field box. Anything still
 * nameless falls back to its own placeholder.
 *
 * Idempotent, and it stops finding work as soon as everything is wired.
 */
let LBL_SEQ=0;
function wireFieldLabels(root){
  ...
  scope.querySelectorAll('label:not([for])').forEach(l=>{
    ...
    if(!box)return;
    const ctl=/** @type {any} */(box.querySelector('input:not([type=hidden]),select,textarea'));
    if(!ctl||l.contains(ctl))return;
    if(ctl.getAttribute('aria-label')||ctl.getAttribute('aria-labelledby'))return;
    if(!ctl.id)ctl.id='fld-'+(++LBL_SEQ);
    l.setAttribute('for',ctl.id);
  });
  scope.querySelectorAll('input:not([type=hidden]):not([aria-label]):not([aria-labelledby]),select:not([aria-label]):not([aria-labelledby]),textarea:not([aria-label]):not([aria-labelledby])').forEach(el=>{
    const c=/** @type {any} */(el);
    if(c.labels&&c.labels.length)return;
    if(c.closest('label'))return;
    const ph=c.getAttribute('placeholder')||c.getAttribute('title');
    if(ph)c.setAttribute('aria-label',ph);
  });
}
let A11Y_SYNC=null;
function scheduleA11ySync(){
  if(A11Y_SYNC)return;
  A11Y_SYNC=requestAnimationFrame(()=>{A11Y_SYNC=null;syncToggleA11y()});
}
document.addEventListener('click',scheduleA11ySync,true);
...
function saveNote(key,val){
  ...
  (NOTES_LEGACY=null,store.set('stuntos-notes',JSON.stringify(NOTES)));
}
// Notes written before scene work was keyed on the SCENE ended in the scene's
// ARRAY POSITION ("…|Main|12|45||3"), and older ones than that were not
// namespaced either. Both shapes are still on people's devices, so a lookup
// that only tries the current shape makes years of notes silently disappear.
// Index the position-keyed leftovers once, lowest position first, and rebuild
// it whenever NOTES changes.
let NOTES_LEGACY=null;
function legacyNoteIndex(){
  if(NOTES_LEGACY)return NOTES_LEGACY;
  NOTES_LEGACY=new Map();
  for(const k of Object.keys(NOTES)){
    const seg=k.split('|');
    if(seg.length!==5||!/^\d+$/.test(seg[4]))continue;
    const base=seg.slice(0,4).join('|');
    const cur=NOTES_LEGACY.get(base);
    if(cur===undefined||+seg[4]<+cur.split('|')[4])NOTES_LEGACY.set(base,k);
  }
  return NOTES_LEGACY;
}
let RASET={};
...
// THE PAPER COLOUR IDENTIFIES THE REVISION. IT IS NOT THE APP'S COLOUR.
//
// This used to drive --hv/--hv2 — the whole highlight set — so publishing the
// Pink revision repainted the hero total, the mode switch, every button, every
// day border and every row tint hot pink. Beyond looking broken, it collided
// head-on with the change highlighting: the legend says a pink edge means "new
// or moved here", while every edge on the board was pink.
//
// So the paper colour now lands on ONE small identifying element — the revision
// pill in the page bar — through its own --paper custom property. The change
// colours are fixed tokens (--chg-new / --chg-changed / --chg-down, see
// globals.css) that no schedule colour and no brand colour can ever reach, so
// the legend always decodes what is actually on the board.
/** The revision pill's label. The trailing " schedule" is a separate span so a
 *  narrow screen can drop the word and keep the control. */
function setPillLabel(name){
  const pill=$('#colourPill');
  if(!pill)return;
  pill.innerHTML=name?`<span class="cp-lab">${esc(name)}</span><span class="cp-sfx"> schedule</span> ▾`:'';
}
function applyTheme(colour){
  const [hv]=THEMES[colour]||THEMES.white;
  const pill=$('#colourPill');
  const named=colour&&colour!=='white';
  pill.style.setProperty('--paper',named?hv:'transparent');
  setPillLabel(named?colour:'');
  pill.style.display=named?'':'none';
}
...
// The app's own accent, used whenever a production has not chosen a brand
// colour of its own. Deliberately NOT the schedule's paper colour — see
// applyTheme.
function resetHighlight(){const [hv,hv2]=THEMES.white;setHighlight(hv,hv2);}
...
function refreshBrandingUI(name){
  ...
  if(prev){prev.style.backgroundImage=coverCssUrl(cov);prev.classList.toggle('empty',!cov);prev.innerHTML=cov?'':'<span>No cover yet</span>';}
  ...
  if(bprev){bprev.style.backgroundImage=coverCssUrl(ban);bprev.classList.toggle('empty',!ban);bprev.innerHTML=ban?'':'<span>No banner yet</span>';}
  ...
  if(shown){
    ...
    if(banner)banner.style.backgroundImage=coverCssUrl(shown);
...
// ---------- dates ----------
// Week captions read the Monday of the week in LOCAL time. `new Date("2026-07-06")`
// is read as UTC midnight, which renders the Sunday before for anyone west of
// Greenwich — weekKeyDate builds the local date instead.
function fmtWeek(k){const d=weekKeyDate(k);if(!d)return 'w/c '+String(k||'');return 'w/c '+d.toLocaleDateString('en-GB',{day:'numeric',month:'short'})}
...
function chipDate(d){
  // callers can reach here with nothing at all (a schedule where no day has a
  // date yet) — a missing day is a dash, never a crash
  if(!d)return '—';
...
// A day's own place in the day-keyed stores. Normally unit+number — but an
// already-shot day whose NUMBER the new schedule has given to a different day
// is stitched in beside that live day, and unit+number alone made the two share
// one calculator, one travel band and one "needs checking" flag. Such a day
// carries a suffix naming the revision it was shot in, which is exactly the
// identity dayCarryTargets carries its work to, so the two always agree.
function cdayPlain(d){
  const base=((d&&d.unit)||'Main')+'|'+(d&&d.num);
  return (d&&d.collided)?base+'-'+carriedDaySuffix(d,''):base;
}
function cdayKey(d){return (NS?NS+'|':'')+cdayPlain(d)}
...
// Every keystroke in the day calculator used to stringify the whole of CDAY,
// deep-clone it and stringify it twice more per changed key. On a 120-day
// production that is a visible stutter, so the write settles a moment after
// the typing stops instead — and is flushed the instant the tab goes away.
let CDAY_T=null;
function persistCDAY(){
  clearTimeout(CDAY_T);
  CDAY_T=setTimeout(()=>{CDAY_T=null;writeCDAY()},400);
}
function writeCDAY(){store.set('stuntos-cday',JSON.stringify(CDAY));cloudSyncMap('cday')}
function flushCDAY(){if(CDAY_T){clearTimeout(CDAY_T);CDAY_T=null;writeCDAY()}}
// stop anything still queued from writing after a sign-out has cleared up
function cancelPendingSaves(){
  clearTimeout(CDAY_T);CDAY_T=null;
  try{for(const k of Object.keys(BLOB_T)){clearTimeout(BLOB_T[k]);BLOB_T[k]=null}}catch(e){}
  try{for(const k of Object.keys(USER_BLOB_T))clearTimeout(USER_BLOB_T[k])}catch(e){}
}
...
function seedCday(d){
  ...
  // the tier a role group falls back to, read off the scene it is named in, so
  // deleting the role later prices it the way it would have been priced anyway
  const fallbackTierOf=n=>{
    for(const s of d.scenes){
      if((s.spacts||[]).some(f=>f&&f.name===n))return 'SPACT';
      if((s.featured||[]).some(f=>f&&f.name===n))return 'Featured';
    }
    return 'SA';
  };
  if(pd&&(pd.saChars||pd.feats||pd.spacts||pd.roles)){
    ...
    // groups engaged on a named role keep that role when the day is opened —
    // otherwise pricing a day would silently move a stand-in back onto the SA
    // rate and quietly change the budget
    for(const t of Object.values(pd.roles||{}))
      for(const [n,c] of Object.entries(t.groups||{}))
        chars.push({name:n,count:c,tier:fallbackTierOf(n),roleId:t.roleId,scene:sc(n),sup:supOf(n)});
  }else{
    ...
    const roleOf=n=>{for(const s of d.scenes)for(const f of [...(s.saChars||[]),...(s.featured||[]),...(s.spacts||[])])if(f&&f.name===n&&f.roleId)return f.roleId;return ''};
    const withRole=(n,o)=>{const rid=roleOf(n);return rid?{...o,roleId:rid}:o};
    for(const [n,c] of Object.entries(grp('saChars')))chars.push(withRole(n,{name:n,count:c,tier:'SA',scene:sc(n),sup:supOf(n)}));
    for(const [n,c] of Object.entries(grp('featured')))chars.push(withRole(n,{name:n,count:c,tier:'Featured',scene:sc(n),sup:supOf(n)}));
    for(const [n,c] of Object.entries(grp('spacts')))chars.push(withRole(n,{name:n,count:c,tier:'SPACT',scene:sc(n),sup:supOf(n)}));
  }
  ...
  // Night iff the set times cross midnight (1930–0430). Same single test the
  // board warning uses — a CWN label on daytime hours (0700–1922) is NOT a night.
  const night=dayIsNight(d);
  return {shift:night?'Night':'Day',fw:(d.type||'').toUpperCase().includes('CWD')?'cwd':'std',ph:false,
    call,wrap,travel:bandFor(d.loc,/** @type {any} */({bands:activeBands(),unknownBand:activeUnknownBand()})).band,chars,
...
/** The band an unlisted location is charged at, for one production. The engine
 *  defaults to B (the safer, higher guess); a production can ask for the older
 *  Cat A behaviour in Production Settings → Locations. */
function prodUnknownBand(prodName){
  const p=prodName&&PRODS[prodName];
  return (p&&p.unknownBand==='A')?'A':UNKNOWN_LOCATION_BAND;
}
function activeUnknownBand(){
  const s=SOURCES[ACTIVE];
  return s&&s.kind?prodUnknownBand(s.prod||s.title):UNKNOWN_LOCATION_BAND;
}
/** @returns {Record<string, 'A'|'B'>|undefined} */
...
//
// The rate cards price what the ARTIST is paid; that is not what the production
// pays. A line-producer review found the headline number 15–20% light without
// agency commission, employment on-costs and contingency. See
// lib/engine/oncosts.ts and the on-costs section of RATE-ENGINE-NOTES.md.
//
// EVERY setting here defaults to zero / off, so no existing production's totals
// move by a penny until the user opts in. They are stored per PRODUCTION
// (PRODS[name].oncosts), because they are commercial terms for the production,
// not properties of one schedule revision.
// ---------------------------------------------------------------------------
/** The stored on-cost settings for a production, normalised into the exact
 *  shape the engine expects. Percentages are PERCENT numbers (17.5 = 17.5%). */
function prodOncosts(prodName){
  const o=(prodName&&PRODS[prodName]&&PRODS[prodName].oncosts)||{};
  const lines=(Array.isArray(o.additional)?o.additional:[])
    .map(l=>({label:String((l&&l.label)||'').trim(),pct:+((l&&l.pct))||0}))
    .filter(l=>l.label||l.pct);
  return {
    uplift:{
      agencyPct:Math.max(0,+o.agencyPct||0),
      contingencyPct:Math.max(0,+o.contingencyPct||0),
      additional:lines,
    },
    autoPublicHolidays:!!o.autoPublicHolidays,
    cancellation:{
      noticeDays:Math.max(0,Math.round(+o.cancelNoticeDays||0)),
      pct:Math.max(0,+o.cancelPct||0),
    },
  };
}
function activeOncosts(){
  const s=SOURCES[ACTIVE];
  return prodOncosts(s&&s.kind?(s.prod||s.title):null);
}
// ---------------------------------------------------------------------------
// NAMED ROLES WITH THEIR OWN RATES — "Stand-in", "Picture double", "Driver".
//
// A stand-in is not an SA with a fee bolted on: they are engaged on their own
// negotiated day rate with their own overtime and travel. So a production can
// name a role, give it its own rates, and put crowd groups on it. See
// lib/engine/roles.ts.
//
// They live on the PRODUCTION (PRODS[name].roles), exactly like the on-costs
// and the rate cards, because they are that production's deal. Until somebody
// creates one, `roles` is absent from the engine settings and every number and
// every screen is exactly what it was.
//
// The optional money fields are how a role INHERITS: leave the night rate
// blank and the role uses the agreement's own night:day relationship off its
// own day rate. Every input on the role form shows the figure it would use.
// ---------------------------------------------------------------------------
/** Money fields a role may state; anything left blank inherits (roles.ts). */
const ROLE_MONEY_FIELDS=['night','phDay','phNight','hol','otDay','otNight','otPhDay','otPhNight','earlyTravel','travelA','travelB'];
/**
 * One production's roles, sanitised into the exact shape the engine expects.
 * This is user data coming back out of storage and it renders, so everything
 * is re-checked here: ids and labels are trimmed and length-capped, duplicate
 * ids are dropped (rows point at ids), money is coerced to a finite
 * non-negative number, and anything unrecognised is thrown away.
 */
function prodRoles(prodName){
  const p=prodName&&PRODS[prodName];
  ...
  for(const r of raw){
    ...
    const base=r.base==='spact'?'spact':'sa';
    const day=Math.max(0,+r.day||0);
    const role={id,label:String(r.label||'').trim().slice(0,60)||'Role',base,day};
    for(const k of ROLE_MONEY_FIELDS){
      const v=r[k];
      if(v==null||v==='')continue;
      const n=+v;
      if(!Number.isFinite(n)||n<0)continue;
      // holiday on an SA-shaped role is a FRACTION of the day rate, so a value
      // that could only have been meant as a percentage is not silently
      // charged as 1207% — it is ignored and the card's own figure inherits.
      if(k==='hol'&&base==='sa'&&n>1)continue;
      role[k]=n;
    }
    const note=String(r.note||'').trim().slice(0,200);
    if(note)role.note=note;
    seen.add(id);out.push(role);
  }
  return out;
}
/** The roles of the production the open schedule belongs to. */
function activeRoles(){
  const s=SOURCES[ACTIVE];
  return s&&s.kind?prodRoles(s.prod||s.title):[];
}
function roleById(id){return activeRoles().find(r=>r.id===id)||null}
/** What a role is called on screen. */
function roleLabel(r){return (r&&String(r.label||'').trim())||(r&&r.id)||'Role'}
/** "the SA working day (9 hours)" — how we say a base in plain English. */
const ROLE_BASE_LABEL={sa:'SA day — 9 hours (7 continuous)',spact:'SPACT day — 10 hours incl. lunch (8 continuous)'};
const ROLE_BASE_SHORT={sa:'SA day',spact:'SPACT day'};
/** Fresh, collision-proof id for a new role. */
function newRoleId(existing){
  let n=1,id;
  const taken=new Set((existing||[]).map(r=>r.id));
  do{id='role'+n++}while(taken.has(id));
  return id;
}
/**
 * Every figure a role would use, stated or inherited — this is what the role
 * form shows beside each blank box so the user can see what they are
 * overriding. Routed through the engine's own inheritance, never re-derived.
 */
function roleResolved(role){
  const cards=/** @type {any} */(crowdSettingsFromDOM());
  const r=/** @type {any} */(role);
  try{
    return /** @type {any} */(role.base==='spact'
      ?{...engineRoleSpactSettings(r,cards),day:engineRoleSpactSettings(r,cards).basic}
      :{...engineRolePactSettings(r,cards),day:engineRolePactSettings(r,cards).sa});
  }catch(e){return /** @type {any} */({day:+role.day||0})}
}
/** TRUE when anything on top of the artist fees is switched on. */
function upliftsOn(){return engineCrowdUpliftsInForce(/** @type {any} */(crowdSettingsFromDOM()))}
/** One number off the active rate card, by schema key, for one talent type.
 *  Falls back to the card defaults so a card saved before a field existed
 *  behaves exactly as it did. */
function rcNum(dept,key,fallback){
  const id=crowdFieldId(dept,key);
  const v=(ACTIVE_RATES&&ACTIVE_RATES[id]!=null&&ACTIVE_RATES[id]!=='')?ACTIVE_RATES[id]:RC_DEFAULTS[id];
  const n=+v;
  return Number.isFinite(n)?n:(fallback||0);
}
/** The meal-penalty table off the active card — day, night and public holiday
 *  for each of the three penalties an agreement can charge. */
function rcMeals(dept){
  const rate=(k,label)=>({label,
    day:rcNum(dept,k+'Day'),night:rcNum(dept,k+'Night'),
    phDay:rcNum(dept,k+'PhDay'),phNight:rcNum(dept,k+'PhNight')});
  return {
    short:rate('mealShort','Short lunch (meal break under 1 hour)'),
    late:rate('mealLate','Late lunch (no break within 6 hours of call)'),
    supper:rate('mealSupper','Short supper (no 2nd break within 13 hours of call)'),
  };
}
/**
 * The engine settings for the production on screen.
 *
 * Named "…FromDOM" because the day rate and holiday still come off the hidden
 * calculator inputs a user can type into. Everything else now comes straight
 * off the resolved rate card, so a card that states its own night rate, public
 * holiday rate, overtime, framework hours or meal penalties is actually paid
 * on them — before this, a user could type next year's card and still be
 * charged the frozen 2026 numbers on exactly the days that cost the most.
 */
function crowdSettingsFromDOM(){
  const R=crowdRates();
  const oc=activeOncosts();
  const roles=activeRoles();
  return {
    pact:{
      sa:R.sa,hol:R.hol,
      otDay:gOTd(),otNight:gOTn(),earlyTravel:gETsa(),travelA:gTA(),travelB:gTB(),
      night:rcNum('sa','night'),
      phDay:rcNum('sa','phDay'),phNight:rcNum('sa','phNight'),
      otPhDay:rcNum('sa','otPhDay'),otPhNight:rcNum('sa','otPhNight'),
      fwStd:rcNum('sa','fwStd'),fwCwd:rcNum('sa','fwCwd'),
    },
    spact:{
      basic:R.spact,night:gSpNight(),hol:gSpHol(),earlyTravel:gSpET(),
      // SPACT keeps its own overtime and travel money — the two agreements
      // print the same figures today, but they are separate agreements and a
      // change to one must never move the other
      otDay:rcNum('spact','otDay'),otNight:rcNum('spact','otNight'),
      travelA:rcNum('spact','travelA'),travelB:rcNum('spact','travelB'),
      phDay:rcNum('spact','phDay'),phNight:rcNum('spact','phNight'),
      otPhDay:rcNum('spact','otPhDay'),otPhNight:rcNum('spact','otPhNight'),
      fwStd:rcNum('spact','fwStd'),fwCwd:rcNum('spact','fwCwd'),
    },
    // meal penalties are SA-side money on the card and are charged per head on
    // whichever day they are ticked
    meals:rcMeals('sa'),
    bands:activeBands(),
    baseDay:activeBaseDay(),
    unknownBand:activeUnknownBand(),
    uplift:oc.uplift,
    autoPublicHolidays:oc.autoPublicHolidays,
    cancellation:oc.cancellation,
    // absent entirely until a role exists, so a production with none is
    // byte-identical to the app before roles were built
    ...(roles.length?{roles}:{}),
  };
}
function cdPerHead(c,tier,ch){return engineCdPerHead(ch?engineCdRowConfig(c,ch):c,tier,crowdSettingsFromDOM())}
/**
 * What ONE person on a day-calculator row costs — through the row's named role
 * if it has one and that role still exists, otherwise through its tier. A row
 * whose role has been deleted falls back to its tier here exactly as it does in
 * the engine, so the screen and the budget always agree.
 */
function cdRowPerHead(c,ch){
  const s=/** @type {any} */(crowdSettingsFromDOM());
  const role=(ch&&ch.roleId)?roleById(ch.roleId):null;
  if(role)return engineRolePerHead(engineCdRowConfig(c,ch),/** @type {any} */(role),s);
  return engineCdPerHead(ch?engineCdRowConfig(c,ch):c,ch?ch.tier:'SA',s);
}
...
function computeCrowdCosts(){
  ...
  CROWD={R,hp,...engineComputeCrowdCosts(MODEL,cd,/** @type {any} */(crowdSettingsFromDOM()))};
  trackHeroTotal(CROWD.grand);
...
function renderSummary(){
  if(APPMODE==='crowd'){
    ...
    // person-days on named roles — a tile only once a role is being used
    const roleDays=days.reduce((a,d)=>a+(CROWD.perDay[d.id]?.roleHeads||0),0);
    $('#summary').innerHTML=`
      <div class="stat hero costable"><div class="n">${gbpWhole(CROWD.grand)}</div><div class="l">Total crowd cost <span class="est">estimate</span></div>${heroSplitHTML()}</div>
      ...
      <div class="stat costable"><div class="n">${spactDays}</div><div class="l">Spact days</div></div>
      ${roleDays?`<div class="stat costable"><div class="n">${roleDays.toLocaleString()}</div><div class="l">Named-role days</div></div>`:''}`;
    renderHeroMove();
    renderProvBar();
    return;
  }
  ...
  $('#summary').innerHTML=`
    <div class="stat hero costable"><div class="n">${gbpWhole(COST.grand)}</div><div class="l">Total stunt cost <span class="est">estimate</span></div></div>
    ...
    <div class="stat costable"><div class="n">${stuntDays?gbpWhole(COST.grand/stuntDays):'—'}</div><div class="l">Avg cost / stunt day</div></div>`;
  renderHeroMove();
  renderProvBar();
}
/**
 * THE HERO TOTAL, BROKEN OUT.
 *
 * "£1,204,880" answers nothing on its own. A producer needs to know how much of
 * it is the artists, how much is the agency's commission and how much is the
 * contingency, because those are three different conversations. With every
 * on-cost still at zero — the default — there is nothing to split, so the hero
 * stays exactly as it was.
 */
function heroSplitHTML(){
  if(!CROWD)return '';
  const u=CROWD.upliftGrand||{agency:0,additional:0,contingency:0,total:0};
  const meals=+CROWD.mealGrand||0;
  if(!u.total&&!meals)return '';
  const named=(activeOncosts().uplift.additional||[]).filter(l=>l.pct);
  const bits=[`<span><b>${gbpWhole(engineRound2(CROWD.artistGrand)-meals)}</b> artists</span>`];
  if(meals)bits.push(`<span><b>${gbpWhole(meals)}</b> meal penalties</span>`);
  if(u.agency)bits.push(`<span><b>${gbpWhole(u.agency)}</b> agency</span>`);
  if(u.additional)bits.push(`<span><b>${gbpWhole(u.additional)}</b> ${esc(named.length===1?named[0].label:'other on-costs')}</span>`);
  if(u.contingency)bits.push(`<span><b>${gbpWhole(u.contingency)}</b> contingency</span>`);
  return `<div class="hero-split" aria-label="What the total is made of">${bits.join('')}</div>`;
}
// ---- C2: the hero total must never just BECOME a different number ----------
// Applying the night fix moved it by £97,159 (16%) in silence. Any recompute
// that moves the headline now says so, in words, until it is read.
let HERO_PREV={key:'',grand:NaN};
let HERO_MOVE=null;   // {from,to,why}
let HERO_WHY='';      // set by whatever is about to move it
function noteHeroMove(why){HERO_WHY=why||''}
function heroKey(){return srcUid(SOURCES[ACTIVE])+'|crowd'}
/** Called straight after a recompute, with the figure that is now on screen. */
function trackHeroTotal(grand){
  const key=heroKey();
  const g=engineRound2(Number(grand)||0);
  if(HERO_PREV.key===key&&Number.isFinite(HERO_PREV.grand)&&Math.round(HERO_PREV.grand)!==Math.round(g))
    HERO_MOVE={from:HERO_PREV.grand,to:g,why:HERO_WHY};
  HERO_PREV={key,grand:g};
  HERO_WHY='';
}
function renderHeroMove(){
  const host=$('#summary');
  if(!host)return;
  const old=host.querySelector('.hero-move');if(old)old.remove();
  const m=HERO_MOVE;
  if(!m)return;
  const delta=engineRound2(m.to-m.from);
  if(!Math.round(delta))return;
  const pct=m.from?Math.abs(delta/m.from)*100:0;
  const up=delta>0;
  const el=document.createElement('div');
  el.className='hero-move '+(up?'up':'down');
  el.innerHTML=`<span>${icon(up?'warn':'refresh')} This total went <b>${up?'up':'down'} ${esc(gbpWhole(Math.abs(delta)))}</b>${pct>=0.5?` (${pct.toFixed(pct<10?1:0)}%)`:''}, from ${esc(gbpWhole(m.from))}${m.why?' — '+esc(m.why):''}.</span><button type="button" class="chg-btn" data-heromoveok>Got it</button>`;
  host.appendChild(el);
}
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(t.closest&&t.closest('[data-heromoveok]')){HERO_MOVE=null;renderHeroMove();}
});

// ============================================================================
// Where the money comes from — provenance, and the two things that quietly
// change it.
//
// The hero figure used to sit on the page with nothing beside it: no rate card
// named, no year, no word that it is an estimate. The strings that DID say all
// that were computed into #ratesBar / #crowdRatesBar, which are deliberately
// hidden (rates are edited in Production Settings), so nobody ever read them.
// They are said here instead — on the board, in the breakdown, and in every
// export.
// ============================================================================
/** "SA: PACT/FAA 2026 · SPACT: Take 3 SPACT 2026" for the current mode. */
function rateCardLine(){
  const s=SOURCES[ACTIVE]||{};
  const p=(s.prod&&PRODS[s.prod])||null;
  try{
    return deptsForMode().map(d=>{
      const card=prodCardFor(p,d.kind);
      return d.label+' '+((card&&card.name)||d.defaults.replace(' (defaults)',''));
    }).join(' · ');
  }catch(e){return ''}
}
/**
 * Does this day actually run into the night? A night shoot is one whose set
 * times cross midnight (call and wrap on different sides of 00:00) — e.g.
 * 1930–0430. The SCHEDULE'S type label (CWN/SWN/"night") is only a fallback for
 * when the hours are missing or unreadable: a day tagged CWN but scheduled
 * 0700–1922 is a daytime day and must NOT be priced at night rates.
 * `dayIsNight` and seedCday MUST agree — both call this one test.
 */
function dayIsNight(d){
  const m=/(\d{1,2})[:.]?(\d{2})\s*[-–]\s*(\d{1,2})[:.]?(\d{2})/.exec((d&&d.hours)||'');
  // Hours present and readable: they are the authority. Night iff wrap lands on
  // the day AFTER the call — i.e. the shoot passes midnight.
  if(m)return (+m[3]*60+ +m[4])<(+m[1]*60+ +m[2]);
  // No usable hours — the only signal left is what the schedule called the day.
  return /CWN|SWN|night/i.test((d&&d.type)||'');
}
/**
 * Costed crowd days the SCHEDULE calls a night but which are still being priced
 * off the flat day rate, because nobody has opened and saved their day
 * calculator yet. The night BDR is materially higher than the day BDR, so each
 * of these is under-budgeted per head per day, before any overtime.
 */
function nightDaysPricedAsDay(){
  if(!MODEL||!CROWD||!CROWD.perDay)return [];
  return MODEL.days.filter(d=>{
    const c=CROWD.perDay[d.id];
    if(!c)return false;            // no crowd that day — no money at stake
    if(c.edited)return false;      // already priced on its own saved config
    return dayIsNight(d);
  });
}
/**
 * Is this day's travel band a guess? Judged from the LOCATION, never from the
 * engine's `known` flag: an edited day hardcodes known:true even though the
 * band it carries was seeded from exactly the same guess, which quietly erased
 * the "check this" flag the moment anyone touched the day.
 */
function bandGuessFor(d){
  try{return bandFor(d&&d.loc,/** @type {any} */({bands:activeBands(),unknownBand:activeUnknownBand()})).known===false}
  catch(e){return false}
}
/** Costed days whose travel band is a guess, because the location isn't in the gazetteer. */
function guessedBandDays(){
  if(!MODEL||!CROWD||!CROWD.perDay)return [];
  // `known` is only reported by the engine on UNEDITED days; an edited day
  // hardcodes known:true even though its band was seeded from the same guess.
  // So the guess is judged from the location itself, always.
  return MODEL.days.filter(d=>CROWD.perDay[d.id]&&bandGuessFor(d));
}
/** True when nothing on this board has had its real hours priced yet. */
function noHoursPricedYet(){
  if(!MODEL||!CROWD||!CROWD.perDay)return false;
  const costed=MODEL.days.filter(d=>CROWD.perDay[d.id]);
  return costed.length>0&&!costed.some(d=>CROWD.perDay[d.id].edited)&&!activeBaseDay();
}
/**
 * The plain-English provenance of every figure on the page, as lines.
 * Shown on the board and carried into CSV, Excel and print, so the spreadsheet
 * someone forwards to a producer is qualified the same way the screen is.
 */
function moneyProvenanceLines(opts){
  const out=[];
  const cards=rateCardLine();
  out.push('These are estimates for budgeting, not a quote and not a payroll figure. Actual cost depends on the hours worked on the day, who is booked and what the agency charges.');
  if(cards)out.push('Rates used: '+cards+'. Change them in Production Settings → Rate cards.');
  if(APPMODE==='crowd'){
    // Everything that sits ON TOP of the rate cards — agency commission, the
    // production's own named on-cost lines, contingency, whether bank holidays
    // are flagged automatically, and the cancellation terms. Written by the
    // engine (crowdProvenanceLines) so the screen, every export footer and the
    // printed breakdown all say exactly the same thing.
    try{out.push(...crowdProvenanceLines(/** @type {any} */(crowdSettingsFromDOM())))}catch(e){}
    out.push('Supporting artists are costed as the day rate plus '+((CROWD&&CROWD.R?CROWD.R.hol:0.1207)*100).toFixed(2)+'% holiday. Overtime and early-call payments are charged at holiday-inclusive rates ('+gbp(gOTd())+' per 30 minutes day, '+gbp(gOTn())+' per 30 minutes night and early call). Travel is added per head from each day’s location: Cat A '+gbp(gTA())+', Cat B '+gbp(gTB())+'.');
    if(noHoursPricedYet())
      out.push('No day has been priced off its own scheduled hours yet, so no overtime or early-call money is in these totals. Anything longer than a standard day will cost more than shown. Open a day’s calculator, or set a budget assumption in Production Settings → Rate cards.');
    // The night-shoot problem has its own headline block on the board with the
    // one-press fix in it. Repeating the same sentence in the list below it was
    // simply the same warning twice — so it is only added for the EXPORTS,
    // which carry no such block.
    const nights=nightDaysPricedAsDay();
    if(nights.length&&!(opts&&opts.skipNights))
      out.push(nights.length+' day'+(nights.length===1?' is':'s are')+' scheduled as a night but still priced at the DAY rate — the night basic is roughly £55 more per head per day, before overtime. Use “Price nights off their hours” on the board to correct them.');
    const guessed=guessedBandDays();
    if(guessed.length)
      out.push(guessed.length+' day'+(guessed.length===1?'’s location is':'s’ locations are')+' not in our list of places, so the travel band is a guess. We use the dearer band (Cat B) so a budget can only come down when the place is identified, never up.');
  }
  return out;
}
/** Paint the provenance strip and its one-click night fix under the hero figures. */
function renderProvBar(){
  const bar=$('#provBar');if(!bar)return;
  if(!MODEL||(APPMODE==='crowd'?!CROWD:!COST)){bar.classList.add('hidden');bar.innerHTML='';return}
  // A night still on day rates that the user has chosen to set aside this
  // session no longer raises the loud headline — but it is NOT resolved, so the
  // warning is demoted into the "How these numbers are worked out" list below
  // (skipNights is false when nothing is left to headline) rather than lost.
  const nights=(APPMODE==='crowd'?nightDaysPricedAsDay():[]).filter(d=>!PROV_NIGHTS_DISMISSED.has(d.id));
  const lines=moneyProvenanceLines({skipNights:!!nights.length});
  const warn=nights.length
    ? `<div class="prov-warn">
        <div class="prov-warn-t">${icon('warn')} ${nights.length} night shoot${nights.length===1?'' :'s'} ${nights.length===1?'is':'are'} being costed at day rates</div>
        <div class="prov-warn-b">The schedule marks ${nights.length===1?'this day':'these days'} as a night, but until a day is priced off its own hours the app charges the standard day rate. The night basic is about £55 more per head per day, and none of the night overtime is in the total either — so the crowd budget for ${nights.length===1?'that day':'those days'} is understated. One press prices ${nights.length===1?'it':'them'} off the call and wrap already in the schedule.</div>
        <div class="prov-warn-a"><button class="tb-btn" id="provFixNights">Price ${nights.length===1?'this night':'these '+nights.length+' nights'} off their hours</button><button class="tb-btn" id="provShowNights" aria-expanded="${PROV_NIGHTS_OPEN?'true':'false'}">${PROV_NIGHTS_OPEN?'Hide the list':'Show me which days'}</button><button class="tb-btn prov-warn-dismiss" id="provDismissNights">Dismiss</button></div>
        ${PROV_NIGHTS_OPEN?`<ul class="prov-daylist">${nights.map(d=>`<li><button type="button" class="dchip" data-goto="${esc(d.id)}">D${d.num}</button> <span>${esc(chipDate(d))}${d.hours?' · '+esc(d.hours):''}${d.loc?' · '+esc(d.loc):''}</span></li>`).join('')}</ul>`:''}
      </div>`
    : '';
  // C1: SIX PARAGRAPHS OF GREY IS NOT A DISCLOSURE, IT IS WALLPAPER.
  // 180px of body copy on desktop and a full screen on a phone, directly under
  // the number it qualifies, so it was scrolled past every time. One line that
  // opens when it is wanted, and stays open for the rest of the session.
  bar.innerHTML=warn+`<details class="prov-more"${PROV_OPEN?' open':''}>
    <summary><span class="prov-sum-t">How these numbers are worked out</span><span class="prov-sum-n">${lines.length} note${lines.length===1?'':'s'}</span></summary>
    <div class="prov-lines">${lines.map(l=>`<div>${esc(l)}</div>`).join('')}</div>
  </details>`;
  bar.classList.remove('hidden');
}
let PROV_OPEN=false;
let PROV_NIGHTS_OPEN=false;
// Night days the user has set aside this session. Keyed by day id so that a
// newly added or newly un-priced night still raises the headline — dismissing
// only silences the days that were actually on screen when it was pressed.
let PROV_NIGHTS_DISMISSED=new Set();
document.addEventListener('toggle',e=>{
  const t=/** @type {any} */(e.target);
  if(t&&t.classList&&t.classList.contains('prov-more'))PROV_OPEN=t.open;
},true);
/** Seed and SAVE a real day config for every night still on day rates. */
function priceNightDaysFromHours(){
  const days=nightDaysPricedAsDay();
  if(!days.length)return;
  noteHeroMove(days.length+' night shoot'+(days.length===1?'':'s')+' now priced off '+(days.length===1?'its':'their')+' own hours');
  const before=JSON.stringify(CDAY);
  const keys=[];
  for(const d of days){const k=cdayKey(d);CDAY[k]=seedCday(d);keys.push(k);}
  saveCDAY(keys); // saving is what promotes them from "seeded" to real
  registerUndo(()=>{
    const back=JSON.parse(before);
    for(const k of Object.keys(CDAY))delete CDAY[k];
    Object.assign(CDAY,back);persistCDAY();refreshAll();
  },'pricing '+days.length+' night'+(days.length===1?'':'s')+' off their hours');
  refreshAll();
  setStatus(days.length+' night shoot'+(days.length===1?' is':'s are')+' now priced off the call and wrap in the schedule — night basic, night overtime and early calls included. Check each day’s calculator if the hours in the schedule aren’t the hours you expect to work.',{undo:crowdUndo,timeout:11000});
}
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(t.closest&&t.closest('#provFixNights')){priceNightDaysFromHours();return}
  // C4: the label says "Show me which days". It used to scroll to the first one
  // and raise a toast — which is not a list, and gave no way to see the other
  // eleven. It is a list now, on the strip, with each day a button.
  if(t.closest&&t.closest('#provShowNights')){
    const nights=nightDaysPricedAsDay();
    if(!nights.length){setStatus('Every night shoot is already priced off its own hours.');return}
    PROV_NIGHTS_OPEN=!PROV_NIGHTS_OPEN;
    renderProvBar();
    return;
  }
  // Set the headline aside without pricing the nights. The days are NOT
  // resolved — the warning moves down into the "How these numbers are worked
  // out" list — so this is offered with an undo and reverses cleanly.
  if(t.closest&&t.closest('#provDismissNights')){
    const nights=nightDaysPricedAsDay().filter(d=>!PROV_NIGHTS_DISMISSED.has(d.id));
    if(!nights.length){renderProvBar();return}
    for(const d of nights)PROV_NIGHTS_DISMISSED.add(d.id);
    PROV_NIGHTS_OPEN=false;
    renderProvBar();
    setStatus('Night-rate warning set aside for '+nights.length+' day'+(nights.length===1?'':'s')+'. The figures are unchanged — those days are still on the day rate, noted under “How these numbers are worked out”.',{
      undo:()=>{for(const d of nights)PROV_NIGHTS_DISMISSED.delete(d.id);renderProvBar();},
      timeout:9000
    });
    return;
  }
...
// ---------- day board ----------
// Cast / extra chips are LABELS, not controls — their only extra content is the
// tooltip. They used to carry tabindex="0" with no role and no action, which
// put several hundred dead tab stops on a long board. The tooltip text now
// rides along as screen-reader-only text instead, so the information is there
// without the tab stop.
const srOnly=t=>t?`<span class="sr-only"> — ${esc(t)}</span>`:'';
function codeChip(c){const n=personName(c.code);return `<span class="code ${codeClass(c)}" data-tip="${esc(n)}">${esc(c.code)}${n&&n!==c.code?srOnly(n):''}</span>`}
function extraChip(x){const t=`Additional stunt performer${x.count>1?'s ×'+x.count:''}`;return `<span class="code xt" data-tip="${esc(t)}">${esc(x.name)}${x.count>1?' ×'+x.count:''}${srOnly(t)}</span>`}
// Scene notes are filed against the scene (the same slot key SCED uses, so a
// re-parse that shifts the day's scene order can't re-attach a note to the
// wrong scene); a whole-day note keeps the |||DAY form and follows the day.
function noteKey(d,s,idx){
  if(s==null)return (NS?NS+'|':'')+[d.unit||'Main',d.num,'','','DAY'].join('|');
  return (NS?NS+'|':'')+sceneNK(d,s,idx);
}
...
function getNote(nk){
  ...
  if(!NS)return '';
  const plain=nk.slice(NS.length+1);
  if(NOTES[plain]!=null)return NOTES[plain];
  // Fall back to the pre-slot-key shape as well. Only the plain first
  // occurrence claims a position-keyed note — a day that genuinely lists the
  // same scene twice is not something the old keys could tell apart, and
  // guessing there would put one scene's note on another.
  const seg=plain.split('|');
  if(seg.length!==5||seg[4]!=='#0')return '';
  const k=legacyNoteIndex().get(seg.slice(0,4).join('|'));
  return (k&&NOTES[k])||'';
}
...
function stripHTML(d,s,idx,scMap){
  ...
  // These read-only chips (view / stunt mode) are labels, not controls — no
  // tabindex, and the truncated name is spelled out for screen readers.
  const namedChips=(list,cls,tip)=>{
    const shown=list.slice(0,3).map(f=>{
      const short=f.name.length>24?f.name.slice(0,22)+'…':f.name;
      return `<span class="code ${cls}${crowdNeedsName(f.name)?' needsname':''} named${APPMODE==='crowd'?' click':''}" data-tip="${esc(tip)}">${esc(short)}${f.count>1?' ×'+f.count:''}${short!==f.name?srOnly(f.name):''}</span>`;
    }).join('');
    const more=list.length>3?`<span class="code ${cls} named" data-tip="${esc(tip)}">+${list.length-3}${srOnly(list.slice(3).map(f=>f.name+(f.count>1?' ×'+f.count:'')).join(', '))}</span>`:'';
    return shown+more;
  };
  ...
  const crChip=(g,cls,tip)=>{
    ...
    // a group on a named role says so on the chip — "Commuters · Stand-in"
    const gRole=g.roleId?roleById(g.roleId):null;
    const roleSuffix=g.roleId?' · '+(gRole?roleLabel(gRole):'role missing'):'';
    const label=isAnon?`SA ${g.count}`
      :`${esc((g.name||(g.tier==='SPACT'?'SPACT':'SA')).length>24?(g.name||'').slice(0,22)+'…':(g.name||(g.tier==='SPACT'?'SPACT':'SA')))}${g.count>1?' ×'+g.count:''}${esc(roleSuffix)}`;
    // In crowd mode this chip IS a control (click to edit, right-click / menu
    // key for move-copy-delete, drag to another scene) — so it says so:
    // role="button" makes Enter and Space work through the generic activation
    // listener, and the label spells out what pressing it does.
    const plain=isAnon?`SA ${g.count}`:`${g.name||(g.tier==='SPACT'?'SPACT':'SA')}${g.count>1?' ×'+g.count:''}${roleSuffix}`;
    const aria=`${plain}${fromAbove?' (from above)':''} — edit this scene's crowd. Menu key for move, copy or delete.`;
    return `<span class="code ${cls}${needsName?' needsname':''}${fromAbove?' fromabove':''} named click crgrp" draggable="true" role="button" aria-label="${esc(aria)}" data-crgrp="1" data-gtier="${g.tier}" data-gfeat="${g.featured?1:0}" data-gname="${esc(g.name||'')}" data-gcount="${g.count}" data-gabove="${fromAbove?1:0}" data-grole="${esc(g.roleId||'')}" data-tip="${esc(tip)}${APPMODE==='crowd'?' · right-click or drag to move, copy or delete':''}" tabindex="0">${label}${fromAbove?' (FROM ABOVE)':''}</span>`;
  };
  let crowdChips;
  if(APPMODE==='crowd'){
    const parts=[];
    ...
    crowdChips=[
      s.sa?`<span class="code cr anon" data-tip="SA ×${s.sa}">SA ${s.sa}${srOnly('supporting artists')}</span>`:'',
  ...
  // what this revision did to this scene, if anything (see "SEEING WHAT
  // CHANGED") — a rail plus a worded chip, never colour on its own
  const chg=chgOn()&&scMap?chgSceneHTML(scMap.get(String(sceneKey(s)||'').toLowerCase())):{cls:'',chips:''};
  return `<div class="strip ${todClass(s)} ${sceneHasReq(s)?'stunt-row':''}${chg.cls}" data-stunt="${sceneHasReq(s)?1:0}" data-dayid="${esc(d.id)}" data-sceneidx="${idx}">
    <div class="rail"></div>
    ...
    <div class="ccol reqcell${APPMODE==='stunt'?' editable':''}"${APPMODE==='stunt'?` data-reqedit="${esc(snk)}" data-reqmode="stunt" role="button" tabindex="0" aria-label="Stunt performers on scene ${esc(s.num)} — press to edit" data-tip="Click to add stunt performers to this scene"`:''}><div class="codes">${(stunts.length||(s.extras||[]).length)?stunts.map(codeChip).join('')+(s.extras||[]).map(extraChip).join(''):`<span class="dash">${APPMODE==='stunt'?'＋':'—'}</span>`}</div></div>
    <div class="ccol reqcell${APPMODE==='crowd'?' editable':''}"${APPMODE==='crowd'?` data-reqedit="${esc(snk)}" data-reqmode="crowd" role="button" tabindex="0" aria-label="Crowd on scene ${esc(s.num)} — press to edit" data-tip="Click to add crowd to this scene"`:''}><div class="codes">${crowdChips||`<span class="dash">${APPMODE==='crowd'?'＋':'—'}</span>`}</div></div>
...
function dayHeadCrowd(d){
  ...
  // Named roles are bodies on the day like any other — leaving them out of this
  // line under-counts the day the moment a role is used.
  for(const t of Object.values(cd.roles||{}))bits.push(`<b>${t.heads}</b> on ${esc(t.label)}`);
  const chips=[
    ...
    ...Object.values(cd.roles||{}).flatMap(t=>Object.entries(t.groups||{}).map(([n,c])=>`<span class="person sd" data-tip="${esc(t.label)} — ${esc(gbp(t.perHead))} a head today">${esc(n||t.label)}${c>1?' <b>×'+c+'</b>':''}</span>`))
  ];
...
function dayLocHTML(d){
  ...
  return `<span class="dlocs">`+blocks.map(b=>{
    ...
    const editBtn=`<button class="dloc-edit${hasReal?' on':''}" data-locedit="${esc(d.id)}" data-locblk="${esc(scene)}" aria-label="Set the real filming location for ${esc(scene||('D'+d.num))}" data-tip="${hasReal?('Real filming location set: “'+esc(real)+'”. Click to change.'):'Set where this day is actually filmed — the schedule only names the scene’s location, so the real place is TBC'}">${icon('pin')}</button>`;
...
function renderDays(){
  ...
  const cardHTML=d=>{
    ...
    // revision markers: one rail tone + worded chips, and the day is tagged so
    // "changed days only" can filter on it without a second pass
    const chgC=chgOn()?revDiffForDay(d):null, chgR=chgOn()?dayReviewFor(d):null;
    const chgTone=chgOn()?chgDayTone(chgC,d):'';
    const scMap=chgOn()?chgSceneMap(d):null;
    return `<div class="daycard ${work?'has-stunts':''}${dayIsToday(d)?' today':''}${chgTone?' chg chg-'+chgTone:''}${chgR?' chg-needscheck':''}" id="day-${d.id}" data-stunt="${work?1:0}" data-chg="${(chgC||chgR)?1:0}">
      <div class="dayhead">
        <div class="dh-top">
          <span class="ddate" data-tip="${esc(f.tip)}">${f.big}${srOnly(f.tip)}</span>
          ...
          ${APPMODE==='crowd'&&(cd&&cd.sa||peak)?(ct=>`<button class="dpill sa click" data-splitcrowd="${esc(d.id)}" aria-label="${esc((cd&&cd.sa?cd.sa:peak)+' supporting artists — '+ct)}" data-tip="${esc(ct)}">SA ${cd&&cd.sa?cd.sa:peak}</button>`)(crowdCharTip(d)):''}
          ${APPMODE==='crowd'&&cd&&!cd.edited&&dayIsNight(d)?`<button class="dpill nightwarn" data-cdnight="${esc(d.id)}" data-tip="The schedule calls this a night, but its crowd is still costed at the DAY rate — the night basic is about £55 more per head, and no night overtime is counted. Open the day calculator (or use “Price nights off their hours” at the top of the board) to cost it properly.">${icon('warn')} night, day rates</button>`:''}
          ...
          ${APPMODE==='crowd'&&cd?`<button class="dh-cost costable" data-costday="${esc(d.id)}" data-tip="Open the day calculator">${gbpWhole(cd.cost)}<small>${cd.edited?icon('pencil')+' ':''}Crowd cost</small></button>`:''}
      ...
      ${d.scenes.map((s,i)=>s._ch?'':stripHTML(d,s,i,scMap)).join('')}
      ${chgGoneRowsHTML(d)}
  ...
  loadTodayWeather((tDay||next||{}).loc,$('#wxSlot'),(SOURCES[ACTIVE]&&(SOURCES[ACTIVE].prod||SOURCES[ACTIVE].title))||null);
  renderChangeBar();
  applyFilters();
}
document.addEventListener('toggle',e=>{
  const t=/** @type {any} */(e.target);
  if(t&&t.classList&&t.classList.contains('expmenu')){
    const sum=t.querySelector('summary');
...
async function loadTodayWeather(loc,slot,prod){
  if(!slot)return;
  // A production can switch off everything leaving this device (Production
  // Settings → General). The forecast lookup sends the day's LOCATION to
  // open-meteo.com, which is exactly what that switch says will not happen —
  // so it is off here too, and said out loud rather than quietly missing.
  if(aiBlocked(prod)){
    slot.innerHTML=`<span class="wx-off">Weather is off for this production — looking it up would send the day's location out of the app.</span>`;
    return;
  }
  // the forecast is fetched with timezone=auto (the shoot's local day), so the
  // cache has to be keyed on the LOCAL date too — toISOString() is UTC and
  // served yesterday's or tomorrow's forecast either side of midnight
  const n=new Date();
...
/**
 * What this day's public-holiday state actually is, and WHO decided it.
 *
 * The engine reports `ph = {applied, auto, user, name}`. An engine guess from
 * the bank-holiday table must never be shown as though a human ticked it — so
 * an auto-flagged day says so, names the holiday, and says the table is England
 * & Wales. Whatever the user does with the tick wins from then on (phSet).
 */
function cdPhNoteHTML(d,c){
  const entry=CROWD&&CROWD.perDay&&CROWD.perDay[d.id];
  const ph=entry&&entry.ph;
  if(c.phSet===true)
    return `<div class="cdinfo cdphnote">You set this day's public-holiday tick yourself, so it stays as you left it — the automatic bank-holiday flag will never change it.</div>`;
  if(ph&&ph.auto)
    return `<div class="cdinfo cdphnote auto">${icon('warn')} <b>We flagged this automatically</b> — ${esc(ph.name||'a UK bank holiday')} (England &amp; Wales). Nobody has confirmed it. Scotland and Northern Ireland differ. Tick or untick above and your answer wins from then on.</div>`;
  if(bankHolidayDataMissing(d))
    return `<div class="cdinfo cdphnote">We hold UK bank-holiday dates for 2026–2028 only, so we cannot say whether this date is one. Set it by hand if it is.</div>`;
  const oc=activeOncosts();
  if(!oc.autoPublicHolidays&&bankHolidayName(d._date||null))
    return `<div class="cdinfo cdphnote">This date is ${esc(bankHolidayName(d._date||null))} in England &amp; Wales. Automatic bank-holiday flagging is off for this production — switch it on in Production settings → On-costs, or tick it here.</div>`;
  return '';
}
/**
 * A group is asking to be paid as a role that no longer exists.
 *
 * The engine never drops it and never costs it at nothing: it falls back to the
 * group's tier and reports the missing id. That is a safe number but it is not
 * the number the user asked for, so we say so plainly, and offer the two ways
 * out — point the group at a role that does exist, or put it back on its tier.
 */
function missingRoleNoticeHTML(ids,where){
  const list=(ids||[]).filter(Boolean);
  if(!list.length)return '';
  const live=activeRoles();
  const names=list.map(id=>`“${esc(id)}”`).join(', ');
  return `<div class="note rolegone" role="alert">${icon('warn')}
    <span>${list.length===1?'A crowd group is asking to be paid as a role that has been deleted':'Crowd groups are asking to be paid as roles that have been deleted'} (${names}). ${list.length===1?'It is':'They are'} being costed on ${list.length===1?'its':'their'} own tier instead — so the total is safe, but it is not what you asked for. Put ${list.length===1?'it':'them'} on a role that exists, or clear the role.</span>
    <span class="rolegone-acts">${live.map(r=>`<button type="button" class="chg-btn" data-rolefix="${esc(r.id)}" data-rolefixwhere="${esc(where||'')}">Use ${esc(roleLabel(r))}</button>`).join('')}<button type="button" class="chg-btn" data-rolefix="" data-rolefixwhere="${esc(where||'')}">Clear the role</button></span>
  </div>`;
}
/**
 * Point every group and every day-calculator row that names a role which no
 * longer exists at `toRoleId` — or, with a blank id, put them back on their own
 * tier. Both stores are walked, because a group can be named on the schedule
 * (SCED) and again on a priced day (CDAY).
 */
function repointMissingRoles(toRoleId){
  const live=new Set(activeRoles().map(r=>r.id));
  const to=(toRoleId||'').trim();
  let n=0;
  const fix=row=>{
    if(!row||!row.roleId||live.has(row.roleId))return;
    if(to)row.roleId=to;else delete row.roleId;
    n++;
  };
  for(const cfg of Object.values(CDAY))if(cfg&&Array.isArray(cfg.chars))cfg.chars.forEach(fix);
  for(const e of Object.values(SCED))if(e&&Array.isArray(e.chars))e.chars.forEach(fix);
  if(!n)return 0;
  saveCDAY();saveSced();applySced(MODEL);refreshAll();
  return n;
}
document.addEventListener('click',e=>{
  const b=e.target.closest&&e.target.closest('[data-rolefix]');
  if(!b)return;
  const to=b.dataset.rolefix||'';
  const n=repointMissingRoles(to);
  const r=to?roleById(to):null;
  setStatus(n?(n+' crowd '+(n===1?'group is':'groups are')+(r?' now paid as '+roleLabel(r)+'.':' back on their own rate.')):'Nothing to change.');
  if(CD_CTX&&CD_MOUNT&&document.body.contains(CD_MOUNT))renderCdModal();
});
function renderCdModal(){
  ...
  // a row on a live named role is not an SA — it is that role, so it must not
  // be counted against the schedule's SA peak
  const saAlloc=c.chars.filter(x=>x.tier==='SA'&&!(x.roleId&&roleById(x.roleId))).reduce((a,x)=>a+(+x.count||0),0);
  ...
  // The production's named roles are choices in the same picker as SA,
  // Featured and SPACT — that is what they are to whoever is filling this in.
  const cdRoles=activeRoles();
  const rowHTML=(ch,i)=>{
    const ph=cdRowPerHead(c,ch);
    const hasOverride=!!(ch.call||ch.wrap);
    const rowCfg=engineCdRowConfig(c,ch), effCall=rowCfg.call, effWrap=rowCfg.wrap;
    // Every field carries its own label: the .charboxhead row is the only thing
    // naming these eight controls, and it is hidden below 820px — on a phone
    // that left an unlabelled grid of inputs.
    const who=ch.name?esc(ch.name):('character '+(i+1));
    return `<div class="charbox" data-ri="${i}" role="group" aria-label="${who}">
      <input data-cdchar="name" data-i="${i}" value="${esc(ch.name)}" aria-label="Character name" placeholder="Character — e.g. Hotel guests">
      <input data-cdchar="scene" data-i="${i}" value="${esc(ch.scene||'')}" aria-label="Scenes for ${who}" placeholder="Sc" style="font-family:var(--mono);font-size:11px" data-tip="Scene(s) this character belongs to">
      <input class="cnt2" data-cdchar="count" data-i="${i}" type="number" min="0" aria-label="How many ${who}" value="${ch.count}">
      <select data-cdrow="tier" data-i="${i}" aria-label="${cdRoles.length?'Paid as':'Tier'} for ${who}"><option value="SA"${!ch.roleId&&ch.tier==='SA'?' selected':''}>SA</option><option value="Featured"${!ch.roleId&&ch.tier==='Featured'?' selected':''}>Featured</option><option value="SPACT"${!ch.roleId&&ch.tier==='SPACT'?' selected':''}>SPACT</option>${cdRoles.map(r=>`<option value="role:${esc(r.id)}"${ch.roleId===r.id?' selected':''}>${esc(roleLabel(r))}</option>`).join('')}${ch.roleId&&!cdRoles.some(r=>r.id===ch.roleId)?`<option value="role:${esc(ch.roleId)}" selected>${esc(ch.roleId)} — role deleted</option>`:''}</select>
      <select data-cdsup="${i}" aria-label="Supplementary fee for ${who}" data-tip="Supplementary fee per head — Featured SA = SA rate + fees">
      ...
      <span class="num mono"><span class="sr-only">Per head </span>${gbp(ph.per+(+ch.sup||0))}</span>
      <span class="num money cdsub" data-i="${i}"><span class="sr-only">Subtotal </span>${gbp((ph.per+(+ch.sup||0))*(+ch.count||0))}</span>
      <button class="del" data-cddel="${i}" aria-label="Remove ${who}">✕</button>
      <button class="charbox-time ${hasOverride?'on':''}" data-cdchartoggle="${i}" aria-label="Call and wrap times for ${who}" data-tip="Override this character's call/wrap time" aria-expanded="${hasOverride?'true':'false'}">${icon('clock')}${hasOverride?` ${esc(effCall)}–${esc(effWrap)}`:''}</button>
  ...
  CD_MOUNT.innerHTML=`
  ${missingRoleNoticeHTML(dc.missingRoles,'cd')}
  ...
  <div class="cdsec"><div class="sl2">1b · Meal penalties</div>
    <div class="cdrow">
      ${(['short','late']).map(k=>{
        const rate=MEAL_PENALTY_DEFAULTS[k];
        const per=c.shift==='Night'?rate.night:rate.day;
        return `<label class="chk2"><input type="checkbox" data-cdmeal="${k}" ${(c.meals&&c.meals[k])?'checked':''}> ${esc(rate.label)} — ${gbp(per)} a head</label>`;
      }).join('')}
    </div>
    <div class="cdrow"><span class="cdinfo">${(()=>{
      const mp=mealPenaltyPerHead(c.meals,c.shift==='Night'?'Night':'Day',MEAL_PENALTY_DEFAULTS);
      const heads=c.chars.reduce((a,x)=>a+(+x.count||0),0);
      return mp.per
        ?`Paid per head: ${gbp(mp.per)} × ${heads} = <b>${gbp(engineMoney(mp.per,heads))}</b> on this day.`
        :'Paid per head, so they are real money on a big day — a late lunch on a 200-head day is about £4,676. Tick one only when the day actually incurred it.';
    })()}</span></div>
  </div>
  ...
  <div class="cdsec"><div class="sl2">3 · Travel</div>
    <div class="cdrow">
      ...
      <span class="cdinfo">${(()=>{const lb=bandFor(d.loc,{bands:activeBands(),unknownBand:activeUnknownBand()});return lb.known?`auto: “${esc(d.loc)}” → Cat ${lb.band}`:`“${esc(d.loc)}” isn’t in our list of places — charged at Cat ${esc(activeUnknownBand())} (${esc(gbp(activeUnknownBand()==='B'?gTB():gTA()))} a head), the safer guess. Override it here, or change the default in Production Settings → Locations.`})()}</span>
    </div>
  </div>
  <div class="cdsec"><div class="sl2">4 · Characters</div>
    <div id="cdChars"><div class="charboxwrap">
    <div class="charbox charboxhead"><span>Character</span><span>Scene</span><span class="num">Count</span><span>${cdRoles.length?'Paid as':'Tier'}</span><span class="num">Supp £</span><span class="num">Per head</span><span class="num">Subtotal</span><span></span><span></span></div>
  ...
  ${(()=>{
    ...
    return `<div class="note cdgapnote">The board costs this day at <b>${gbpWhole(boardCost)}</b> — ${bd?`your budget assumption (${bd.fw==='cwd'?'CWD':'Standard Day'}${bd.otHours?` + ${bd.otHours}h OT`:''})`:'the flat day rate, because this production has no budget assumption set'}. Priced on its own scheduled hours (${esc(c.call)}–${esc(c.wrap)}) it comes to <b>${gbpWhole(here)}</b>. Change anything here and ${gbpWhole(here)} becomes this day's cost; set a production-wide assumption in Production settings → Rate cards to apply the same thinking to every day.</div>`;
  })()}
  <div class="cdtotal">
...
function renderSplitModal(){
  ...
  const rowHTML=(r,i)=>`<div class="reqrow splitrow" data-ri="${i}">
    ...
    <input data-sq="name" value="${esc(r.name||'')}" placeholder="Character or group" data-tip="What to call this group — e.g. Hotel guests, Market traders">
...
/**
 * THE ON-COSTS, ONCE, AT THE TOP OF THE BREAKDOWN.
 *
 * The tier tables below price what the ARTISTS are paid — `saCost`, `featCost`,
 * `spactCost` and `supCost` are artist-only subtotals and no longer sum to the
 * day's `cost`. That is exactly the point: the difference is the agency's
 * commission, the production's own on-costs, its contingency and any meal
 * penalties, and a producer needs them on the page as their own lines rather
 * than buried inside a per-day figure. With every on-cost at zero — the default
 * — nothing is shown, because there is nothing to show.
 */
function upliftSummaryHTML(){
  if(!CROWD)return '';
  const u=CROWD.upliftGrand||{agency:0,additional:0,contingency:0,total:0};
  const meals=+CROWD.mealGrand||0;
  if(!u.total&&!meals)return '';
  const oc=activeOncosts();
  const named=(oc.uplift.additional||[]).filter(l=>l.pct);
  const artists=engineRound2(engineRound2(CROWD.artistGrand)-meals);
  const rows=[[`Artists (rate cards, supplementary fees)`,'',artists]];
  if(meals)rows.push(['Meal penalties','per head, on the days that incurred them',meals]);
  if(u.agency)rows.push([`Agency commission`,oc.uplift.agencyPct+'% of the artist cost',u.agency]);
  for(const l of named){
    const amt=engineRound2(engineRound2(CROWD.artistGrand)*(l.pct/100));
    rows.push([l.label,l.pct+'% of the artist cost',amt]);
  }
  if(u.contingency)rows.push(['Contingency',oc.uplift.contingencyPct+'% of the subtotal (artist cost plus commission and on-costs)',u.contingency]);
  const foot=footPounds(rows.map(r=>r[2]),CROWD.grand);
  return `<div class="tablecard"><h3>What the crowd budget is made of<span class="sum costable">${gbpWhole(CROWD.grand)}</span></h3>
  <div class="tscroll"><table><thead><tr><th>Line</th><th>How it is worked out</th><th class="num">Amount</th></tr></thead><tbody>
  ${rows.map((r,i)=>`<tr><td class="rowlabel">${esc(r[0])}</td><td style="color:var(--sub)">${esc(r[1])}</td><td class="num money">${gbp(foot[i])}</td></tr>`).join('')}
  <tr class="total"><td>Total crowd cost</td><td></td><td class="num money">${gbpWhole(CROWD.grand)}</td></tr>
  </tbody></table></div>
  <div class="note">The tables below price what the <b>artists</b> are paid. Their totals are artist money only — the lines above are added once, here, so nothing is counted twice. Change any of them in Production settings → On-costs.</div></div>`;
}
function renderCrowdBreakdown(){
  const R=CROWD.R,hp=CROWD.hp;
  // a "Role-days" column only exists once a role is actually being used
  const anyRoleDays=CROWD.weeks.some(w=>w.roleDays>0);
  // The overtime / early-call / supplementary-fee breakdown used to exist ONLY
  // in the mouse tooltip, so a keyboard or screen-reader user landed on a bare
  // number. The same words now ride in the cell as screen-reader-only text —
  // the figure still reads as a figure on screen, and the explanation is there
  // for anyone who cannot hover. (The strings are already escaped.)
  const sr=t=>t?`<span class="sr-only"> — ${t}</span>`:'';
  const saRows=MODEL.days.filter(d=>CROWD.perDay[d.id]&&(CROWD.perDay[d.id].sa||CROWD.perDay[d.id].featPD));
  // the Total column has to add up to the figure in this card's heading
  const saSum=engineSumMoney(...saRows.map(d=>engineSumMoney(CROWD.perDay[d.id].saCost,CROWD.perDay[d.id].featCost)));
  const saTot=footPounds(saRows.map(d=>engineSumMoney(CROWD.perDay[d.id].saCost,CROWD.perDay[d.id].featCost)),saSum);
  let html=missingRoleNoticeHTML(Object.keys((CROWD&&CROWD.missingRoles)||{}),'costs')+upliftSummaryHTML()+`<div class="tablecard"><h3>Supporting artists &amp; featured background<span class="cnt">${saRows.length} days</span><span class="sum costable">${gbpWhole(saSum)}</span></h3>
  <div class="bandkey" aria-label="What the travel band chips mean">
    <span><i class="bandchip">A</i> Cat A — TfL Zones 1–3, ${esc(gbp(gTA()))} a head</span>
    <span><i class="bandchip b">B</i> Cat B — major studios / beyond Zone 3, ${esc(gbp(gTB()))} a head</span>
    <span><i class="bandchip b guess">B?</i> A dashed edge means we do not recognise the location, so the band is a guess. We charge the dearer band, so identifying the place can only bring the budget down.</span>
  </div>
  <div class="tscroll"><table><caption class="sr-only">Supporting artists and featured background, day by day. The Total column is the ARTISTS' money only — agency commission, on-costs and contingency are added once, at the foot of this page.</caption><thead><tr><th>Day</th>${MODEL.multiUnit?'<th>Unit</th>':''}<th>Date</th><th>Location</th><th class="num">SA</th><th class="num">Featured</th><th class="num">Day rates</th><th class="num">Holiday</th><th class="num">Overtime</th><th class="num">Early call</th><th class="num">Fees</th><th class="num">Artist cost</th></tr></thead><tbody>
  ${saRows.map((d,saI)=>{const c=CROWD.perDay[d.id];const k=c.saComp;
    ...
    const R2=crowdRates();
    // Every component printed here comes from the engine ALREADY settled to the
    // penny (one artist's figure, then × the headcount — see lib/engine/money).
    // Multiplying a raw per-head rate by a headcount at print time, next to a
    // Total that was quantised properly, is how the four columns beside the
    // total stopped adding up to it. Featured heads are added the same way.
    // "—" in the Overtime / Early call columns used to mean two completely
    // different things: this day works standard hours and owes no overtime, or
    // nobody has priced this day off its hours yet so overtime has not been
    // WORKED OUT. A producer reads the first. Say which it is.
    const notPriced=!c.edited&&!activeBaseDay();
    const blank=t=>notPriced
      ?`<span class="notcalc" data-tip="${esc(t)}" tabindex="0">not costed${sr(t)}</span>`
      :'—';
    // The travel band is a guess whenever the location isn't in our list —
    // including on a day that has since been edited, where the engine reports
    // known:true even though the band it carries came from the same guess.
    const bandGuess=bandGuessFor(d);
    const rates=engineSumMoney(k.rates,engineMoney(R2.feat,c.featPD));
    const hol=engineSumMoney(k.hol,engineMoney(R2.feat*R2.hol,c.featPD));
    ...
    const ot=engineSumMoney(k.ot||0,engineMoney(k.otPer||0,c.featPD));
    const early=engineSumMoney(k.early||0,engineMoney(k.earlyPer||0,c.featPD));
    return `<tr class="cdopen" data-cdopen="${esc(d.id)}"><td class="mono">${c.edited?`<span style="color:var(--note)" data-tip="Edited in the day calculator">${icon('pencil')}</span> `:''}<button class="dchip ${d.unit==='2nd'?'u2':''}" data-goto="${esc(d.id)}">D${d.num}</button></td>${MODEL.multiUnit?`<td>${d.unit==='2nd'?'2nd':'Main'}</td>`:''}<td>${esc(chipDate(d))}</td><td><a class="loclink" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.loc)}" target="_blank" rel="noopener" data-tip="Open in Google Maps">${esc(d.loc)}</a> ${(tb=>`<span class="bandchip ${c.travel&&c.travel.band==='B'?'b':''}${bandGuess?' guess':''}" data-tip="${tb}">${c.travel?c.travel.band:'—'}${bandGuess?'?':''}${sr(tb)}</span>`)('Travel '+(c.travel?('Cat '+c.travel.band+' · '+gbp(c.travel.amt)+'/head'+(bandGuess?' · we don’t recognise this location, so the band is a guess — we use the dearer one, Cat B. Check it.':' · read automatically from the location')):'—'))}</td><td class="num"><b>${c.sa||'—'}</b></td><td class="num ${c.featPD?'':'dim'}" ${featTip?`data-tip="${featTip}" tabindex="0"`:''}>${c.featPD||'—'}${sr(featTip)}</td><td class="num">${gbpWhole(rates)}</td><td class="num">${gbpWhole(hol)}</td><td class="num ${ot?'':'dim'}" ${otTip?`data-tip="${otTip}" tabindex="0"`:''}>${ot?gbpWhole(ot):blank('Overtime hasn’t been worked out for this day — it is still costed at a flat day rate. Open the day calculator, or set a budget assumption in Production Settings → Rate cards.')}${sr(otTip)}</td><td class="num ${early?'':'dim'}" ${eaTip?`data-tip="${eaTip}" tabindex="0"`:''}>${early?gbpWhole(early):blank('Early-call payments haven’t been worked out for this day — it is still costed at a flat day rate. Open the day calculator to price it off its real call time.')}${sr(eaTip)}</td><td class="num ${fees?'':'dim'}" ${feeTip?`data-tip="${feeTip}" tabindex="0"`:''}>${fees?gbpWhole(fees):'—'}${sr(feeTip)}</td><td class="num money">${gbp(saTot[saI])}</td></tr>`}).join('')}
  </tbody></table></div></div>`;
  {const rows=Object.values(CROWD.spactPeople).sort((a,b)=>b.heads-a.heads||a.code.localeCompare(b.code));
  if(rows.length){
    const sub=engineSumMoney(...rows.map(p=>engineMoney(engineRound2(R.spact+gSpHol()),p.heads)));
    html+=`<div class="tablecard"><h3>Spacts — Take 3 2026 rate card<span class="cnt">${rows.length}</span><span class="sum costable">${gbpWhole(sub)}</span></h3>
    ...
    ${rows.map(p=>`<tr><td class="rowlabel">${esc(p.code)}</td><td class="num">${gbp(engineMoney(R.spact,p.heads))}</td><td class="num"><b>${p.heads}</b></td><td class="num">${gbp(engineMoney(R.spact,p.heads))}</td><td class="num">${gbp(engineMoney(gSpHol(),p.heads))}</td><td class="num money">${gbp(engineMoney(engineRound2(R.spact+gSpHol()),p.heads))}</td><td class="datescol"><div class="daylist">${dateChips(p)}</div></td></tr>`).join('')}
    </tbody></table></div>
  ...
  // ---- Named roles. Their money is ARTIST money — it is already inside the
  // artist total above, broken out here rather than added to it, and it is
  // deliberately NOT folded into the SA line: a stand-in on £180 is a stand-in
  // on £180 and a budget has to be able to see that. ----
  {
    const rg=Object.values((CROWD&&CROWD.roleGrand)||{});
    if(rg.length){
      const sub=engineSumMoney(...rg.map(t=>t.cost));
      const foot=footPounds(rg.map(t=>t.cost),sub);
      html+=`<div class="tablecard"><h3>Named roles<span class="cnt">${rg.length}</span><span class="sum costable">${gbpWhole(sub)}</span></h3>
      <div class="tscroll"><table><caption class="sr-only">The production's named roles — a stand-in, a picture double — each on its own rate card. This money is part of the artists' cost above, shown here on its own.</caption><thead><tr><th>Role</th><th class="num">Day rate</th><th>Working day it follows</th><th class="num">Days</th><th class="num">Most on any day</th><th class="num">Person-days</th><th class="num">Fees</th><th class="num">Artist cost</th></tr></thead><tbody>
      ${rg.map((t,i)=>{
        const r=roleById(t.roleId);
        return `<tr><td class="rowlabel">${esc(t.label)}</td><td class="num mono">${r?gbp(r.day):'—'}</td><td style="color:var(--sub)">${esc(ROLE_BASE_SHORT[t.base]||'SA day')} — ${engineRoleFrameworkHours('std',t.base)}h standard, ${engineRoleFrameworkHours('cwd',t.base)}h continuous</td><td class="num">${t.days}</td><td class="num">${t.maxPerDay}</td><td class="num"><b>${t.heads.toLocaleString()}</b></td><td class="num ${t.sup?'':'dim'}">${t.sup?gbp(t.sup):'—'}</td><td class="num money">${gbp(foot[i])}</td></tr>`;
      }).join('')}
      <tr class="total"><td>Total</td><td colspan="5"></td><td class="num">${gbp(engineSumMoney(...rg.map(t=>t.sup)))}</td><td class="num money">${gbpWhole(sub)}</td></tr>
      </tbody></table></div>
      <div class="note">Each role is paid on its own day rate, with its own overtime and travel; anything you left blank on the role follows the SA or SPACT card it is based on. This is part of the artists' money above — it is not an extra line on the budget.</div></div>`;
    }
  }
  ...
  if(tRows.length){
    html+=`<div class="tablecard"><h3>Travel allowance<span class="cnt">${tRows.length} days</span><span class="sum costable">${gbpWhole(tSum)}</span></h3>
    ...
    ${(()=>{const tTot=footPounds(tRows.map(d=>CROWD.perDay[d.id].travel.total),tSum);return tRows.map((d,ti)=>{const t=CROWD.perDay[d.id].travel;const heads=Math.round(t.total/t.amt);return `<tr class="cdopen" data-cdopen="${esc(d.id)}"><td class="mono">D${d.num}</td><td>${mapsLink(d.loc)}</td><td><span class="bandchip ${t.band==='B'?'b':''}${bandGuessFor(d)?' guess':''}">${t.band}</span> ${bandGuessFor(d)?'<span style="color:var(--note);font-size:10px">a guess — check</span>':''}</td><td class="num">${heads}</td><td class="num">${gbp(t.amt)}</td><td class="num money">${gbp(tTot[ti])}</td></tr>`}).join('')})()}
    <tr class="total"><td>Total</td><td colspan="4"></td><td class="num money">${gbpWhole(tSum)}</td></tr>
    </tbody></table></div>
    <div class="note">Travel band is read automatically from each day’s location — Cat A (TfL Zones 1–3) ${gbp(gTA())}/head, Cat B (major studios / beyond Zone 3) ${gbp(gTB())}/head. A place that isn't in our list is charged at the safer of the two bands — Cat B — and flagged, so a budget can only come down when it is identified, never up. Open the day calculator to override a single day, or Production Settings → Locations to change the default for the whole production.</div></div>`;
  }
  html+=`<div class="tablecard"><h3>Cost by production week<span class="cnt">${CROWD.weeks.length} weeks</span><span class="sum costable">${gbpWhole(CROWD.grand)}</span></h3>
  <div class="tscroll"><table><thead><tr><th>Week</th><th class="num">Crowd days</th><th class="num">SA-days</th><th class="num">Featured-days</th><th class="num">Spact-days</th>${anyRoleDays?'<th class="num">Role-days</th>':''}<th class="num">Week total</th></tr></thead><tbody>
  ${(()=>{const wk=footPounds(CROWD.weeks.map(w=>w.cost),CROWD.grand);return CROWD.weeks.map((w,wi)=>`<tr><td class="mono">${esc(fmtWeek(w.key))}</td><td class="num">${w.days}</td><td class="num">${w.saDays.toLocaleString()}</td><td class="num">${w.featDays}</td><td class="num">${w.spactDays}</td>${anyRoleDays?`<td class="num">${(w.roleDays||0).toLocaleString()}</td>`:''}<td class="num money">${gbp(wk[wi])}</td></tr>`).join('')})()}
  <tr class="total"><td>Total</td><td class="num">${CROWD.weeks.reduce((a,w)=>a+w.days,0)}</td><td class="num">${CROWD.weeks.reduce((a,w)=>a+w.saDays,0).toLocaleString()}</td><td class="num">${CROWD.weeks.reduce((a,w)=>a+w.featDays,0)}</td><td class="num">${CROWD.weeks.reduce((a,w)=>a+w.spactDays,0)}</td>${anyRoleDays?`<td class="num">${CROWD.weeks.reduce((a,w)=>a+(w.roleDays||0),0).toLocaleString()}</td>`:''}<td class="num money">${gbpWhole(CROWD.grand)}</td></tr>
  </tbody></table></div>
...
// A group on a live custom role is paid on THAT role's card, so the document
// asks the engine for the role's own per-head figure for the day — the same
// figure the cost page prints. The projection passes a roleId only when the
// role still exists; a deleted one arrives here as undefined and falls back to
// the group's tier, which is what the engine costs it at too.
function cbdPerHead(dayId,tier,roleId){
  const e=CROWD&&CROWD.perDay?CROWD.perDay[dayId]:null;
  if(!e)return 0;
  if(roleId){
    const t=e.roles?e.roles[roleId]:null;
    // travelPer is the day's travel allowance the engine reports BESIDE the
    // role's per-head figure on an un-priced day (it charges it once at day
    // level there). The tier figures below always carry travel, and this
    // document prints the two side by side, so it goes back on here — else a
    // stand-in reads cheaper than an SA by exactly the travel allowance.
    if(t)return engineSumMoney(t.perHead||0,t.travelPer||0);
  }
  if(!e.perHeadBy)return 0;
...
// What a custom role is called, for the projection. Returning nothing means the
// role has been deleted: the projection then prices that line on its own tier
// and reports the id on doc.missingRoles, so the document says so rather than
// printing a wrong number quietly.
function cbdRoleLabel(id){const r=roleById(id);return r?roleLabel(r):undefined}
function cbdDoc(){
  cbdDefaults();
...
function cbdEditCell(l,sc,i,field,cls,isAdd){
  // a carried line is a pointer to other people — editing it here would
  // silently fork the group, so it stays read-only
  if(l&&l.fromAbove){
...
function cbCrowdNameCell(c,sc,i,cls,ph,isAdd){
  ...
  // A line on a named role says the ROLE, not the tier underneath it — that is
  // who is being booked. With no roles defined this chip is exactly what it was.
  const cbRole=c&&c.roleId?roleById(c.roleId):null;
  const label=cbRole?roleLabel(cbRole).toUpperCase().slice(0,10):(CBD_TIERS.find(t=>t[0]===tier)||CBD_TIERS[0])[1];
  const chip=c&&!c.fromAbove
    ? `<button class="cbtier t-${cbRole?'role':tier.toLowerCase()}" data-cbtier="${addr}" data-tip="${activeRoles().length?'Click to change how these people are paid':'Click to change tier — SA, Featured or Spact'}">${esc(label)}</button>`
    : '';
  const above=cbAboveToggle(c,addr);
  // A carried line's name is written BY the document ("SA (FROM ABOVE)") —
  // it points at people booked elsewhere, so typing over it here would fork the
  // group away from the one it is pointing at. It reads, it doesn't type.
  const nameHtml=c&&c.fromAbove
    ? `<span class="cbnametext">${esc(c.name)}</span>`
    : `<span class="cbedit cbnametext${ph?' cbph':''}" contenteditable="true" spellcheck="false" data-cbaddr="${addr}" data-cbf="name"${ph?` data-ph="${esc(ph)}"`:''} data-tip="Character or group name — leave blank for plain background">${esc(c?c.name:'')}</span>`;
  return `<td class="cbname ${cls} cbnamecell">${chip}${isAdd?cbAddBtnHTML(addr):''}${nameHtml}${above}${c&&!c.fromAbove?`<button class="cbdel" data-cbdel="${addr}" data-tip="Remove this line" aria-label="Remove line">✕</button>`:''}</td>`;
}
...
function cbCrowdComboCell(c,sc,i,cls,ph,isAdd){
  const addr=esc(cbdAddr(sc,i));
  // A carried line is read-only, and keeps the figure this scene needs — it is
  // labelled as the same people ("48 SA (FROM ABOVE)"), which is what stops the
  // day counting them twice.
  if(c&&c.fromAbove)return `<td class="cbname ${cls} cbcombo">${esc((c.no!=null?c.no+' ':'')+c.name)}${cbAboveToggle(c,addr)}</td>`;
  const tier=c?String(c.tier||'SA'):'SA';
  // A line on a named role says the ROLE, not the tier underneath it — that is
  // who is being booked. With no roles defined this chip is exactly what it was.
  const cbRole=c&&c.roleId?roleById(c.roleId):null;
...
// The two STUNTS/OTHER cells — read-only, driven from the schedule.
function cbOtherNoCell(o){
  if(!o)return `<td class="cbno"></td>`;
  const c=['cbrow-'+String(o.tier||'SA').toLowerCase(),o.fromAbove?'fromabove':'',o.tbc?'tbc':''].filter(Boolean).join(' ');
  return `<td class="cbno ${c}">${esc(o.no||'')}</td>`;
}
function cbOtherNameCell(o){
  if(!o)return `<td class="cbname"></td>`;
  const c=['cbrow-'+String(o.tier||'SA').toLowerCase(),o.fromAbove?'fromabove':'',o.tbc?'tbc':''].filter(Boolean).join(' ');
  return `<td class="cbname ${c}">${esc(o.name)}</td>`;
}
...
function cbFeeCell(l,sc,i){
  ...
  return `<td class="cbfee">
      ...
      ${sup?`<span class="cbfeetot" data-tip="${heads} × ${gbp(sup)}">${gbpWhole(heads*sup)}</span>`:''}
    </td>`;
}
function cbCostCell(l){
  if(!l||l.fromAbove)return `<td class="cbcost"></td>`;
  return `<td class="cbcost">${l.cost?gbpWhole(l.cost):'—'}</td>`;
}
  ...
  for(let i=0;i<n;i++){
    ...
    for(const def of layout){
      switch(def.role){
        ...
        case 'crowdNo': out+=wc(cbCrowdNoCell(c,sc,i,ccls,isAdd),`${i}|no`); break;
        case 'crowdName': out+=wc(cbCrowdNameCell(c,sc,i,ccls,emptyName,isAdd),`${i}|name`); break;
        case 'crowdCombo': out+=wc(cbCrowdComboCell(c,sc,i,ccls,emptyName,isAdd),`${i}|combo`); break;
        case 'crowdNotes': out+=wc(cbCrowdNoteCell(c,sc,i,ccls),`${i}|note`); break;
        case 'otherNo': out+=wc(cbOtherNoCell(o),`${i}|ono`); break;
        case 'otherName': out+=wc(cbOtherNameCell(o),`${i}|oname`); break;
        case 'fees': out+=wc(cbFeeCell(c,sc,i),`${i}|fee`); break;
        case 'cost': out+=wc(cbCostCell(c),`${i}|cost`); break;
      }
      col++;
    }
    out+='</tr>';
...
function renderCbDoc(){
  ...
  const bar=`<div class="cbdbar noprint">
    ...
    <!-- Revealed by the column-sort handler once the table has been re-sorted.
         Without it, sorting the breakdown was a one-way door: the sort state
         lives only on the DOM, so a re-render is the way back. -->
    <button class="tb-btn" id="cbdReset" hidden data-tip="Put the breakdown back in the schedule's own order">${icon('refresh')} Reset order</button>
    ${cbHlBarHTML()}
    <span class="grow"></span>
    ${doc.costs?`<span class="cbdstat money" data-tip="Whole-breakdown cost, from the crowd cost engine">${gbpWhole(t.cost)} total</span>`:''}
  ...
  // Day totals foot into their week total, and the week totals foot into the
  // grand total, so the document adds up DOWN the page as well as across it —
  // an AD reading a printed breakdown adds the week rows in their head.
  const footed=cbdFootedCosts(doc.rows);
  let body='';
  for(let ri=0;ri<doc.rows.length;ri++){
    ...
    // everything left is a total row — day, week or the breakdown's own
    const t=/** @type {import('../engine/breakdown-doc').CbTotalRow} */(r);
    const cls=t.kind==='grandTotal'?'cbgrand':t.kind==='weekTotal'?'cbwktotal':'cbtotal';
    const rk=`t|${t.kind}|${esc(t.label)}`;
    let row=`<tr class="${cls}">`;
    if(leadCount>0)row+=`<td colspan="${leadCount}" data-hlk="${rk}|label" class="cbtlabel">${esc(t.label)}</td>`;
    for(let idx=leadCount;idx<layout.length;idx++){
      const def=layout[idx];
      const dc=` data-col="${idx}" data-hlk="${rk}|${idx}"`;
      if(def.role==='crowdNo'||def.role==='crowdCombo')row+=`<td${dc} class="cbno"><b>${t.no}</b></td>`;
      else if(def.role==='otherNo')row+=`<td${dc} class="cbno"><b>${t.otherNo}</b></td>`;
      else if(def.role==='fees')row+=`<td${dc} class="cbfee">${t.fees?gbpWhole(t.fees):''}</td>`;
      else if(def.role==='cost')row+=`<td${dc} class="cbcost"><b>${t.cost?gbp(footed.has(ri)?footed.get(ri):Math.round(t.cost)):'—'}</b></td>`;
      else if(idx===otherLabelIdx&&otherIdx>=0&&t.otherLabel&&!def.count&&!def.money)row+=`<td${dc} class="cbtlabel">${esc(t.otherLabel)}</td>`;
      else row+=`<td${dc}></td>`;
    }
    body+=row+'</tr>';
  }
  ...
  host.innerHTML=bar+missingRoleNoticeHTML(doc.missingRoles,'cbd')+`<div class="cbdoc" style="${cbStyle}">
...
/**
 * Whole-pound figures for the document's total rows that agree with each other.
 * Same reasoning as footPounds: each total was rounded on its own, so a page of
 * week rows did not add up to the grand total printed under them.
 * Returns row index → pounds; rows it has nothing to say about are left alone.
 */
function cbdFootedCosts(rows){
  const out=new Map();
  const gi=rows.findIndex(r=>r.kind==='grandTotal');
  const wi=[];rows.forEach((r,i)=>{if(r.kind==='weekTotal')wi.push(i)});
  const dayIdxIn=(from,to)=>{const a=[];for(let i=from;i<to;i++)if(rows[i].kind==='dayTotal')a.push(i);return a};
  const end=gi>=0?gi:rows.length;
  const grand=gi>=0
    ?Math.round(+rows[gi].cost||0)
    :Math.round((wi.length?wi:dayIdxIn(0,end)).reduce((a,i)=>a+(+rows[i].cost||0),0));
  if(gi>=0)out.set(gi,grand);
  const bucket=(from,to,target)=>{
    const di=dayIdxIn(from,to);
    if(!di.length)return;
    footPounds(di.map(i=>+rows[i].cost||0),target).forEach((v,k)=>out.set(di[k],v));
  };
  if(wi.length){
    const wk=footPounds(wi.map(i=>+rows[i].cost||0),grand);
    wi.forEach((i,n)=>out.set(i,wk[n]));
    let from=0;
    wi.forEach((i,n)=>{bucket(from,i,wk[n]);from=i+1});
    bucket(from,end,grand-wk.reduce((a,x)=>a+x,0));
  }else bucket(0,end,grand);
  return out;
}
...
// The supplementary fees a crowd line can carry come off the ACTIVE RATE CARD,
// not off a frozen list — a production on a different agreement gets that
// agreement's categories and money. A fee already set on a line is kept even
// if the card doesn't offer it, so switching cards never silently rewrites a
// number somebody typed.
const SUP_KEYS=[
  {k:'hair',key:'supA',label:'Change of clothing, hair cut / shaving'},
  {k:'clothing',key:'supAOwn',label:'Providing own clothing'},
  {k:'unused',key:'supAUnused',label:'Unused change of clothes, per set'},
  {k:'sports',key:'supB',label:'Doubling, special clothing, sports equipment, wet weather'},
  {k:'scans',key:'supC',label:'Firearms, swimming, driving, scans, minimal dialogue'},
  {k:'uniform',key:'supD',label:'Creative reaction, uniforms, specialised driving, provision of car'},
  {k:'lookalike',key:'supE',label:'Lookalike doubling, stand-in or dialogue (10+ words)'},
  {k:'intimacy',key:'intimacy',label:'Sexual intimacy'},
  {k:'nudity1',key:'nudityTopless',label:'Nudity (topless) — not featured'},
  {k:'nudity2',key:'nudityToplessFeat',label:'Nudity (topless) — featured'},
  {k:'nudity3',key:'nudityFull',label:'Full frontal nudity'},
];
/**
 * The supplementary fees on offer right now.
 *
 * The SA card leads, because it is the crowd baseline. Where it prices a
 * category at zero — the FAA card prints no intimacy fees, for instance — the
 * SPACT card answers instead, so a production running both talent types can
 * still pick the fee its SPACT card actually states. A category neither card
 * prices is left off, so the picker only ever offers real money.
 */
function supsNow(){
  return SUP_KEYS
    .map(s=>({k:s.k,label:s.label,amt:rcNum('sa',s.key)||rcNum('spact',s.key)}))
    .filter(s=>s.amt>0);
}
/** The meal penalties the day calculator can tick — also the card's. */
function mealsNow(){
  const m=rcMeals('sa');
  return {
    short:{label:'Short lunch (meal break under 1 hour on SWD)',day:m.short.day,night:m.short.night},
    late:{label:'Late lunch (no break within 6 hours of call)',day:m.late.day,night:m.late.night},
    supper:{label:'Short supper (no 2nd break within 13 hours of call)',day:m.supper.day,night:m.supper.night},
  };
}
// Rebuilt by applyRateVals() whenever the active card changes, so a picker
// opened after a card switch offers that card's fees, not the last one's.
// Seeded with the 2026 baseline because this runs before the rate-card section
// of the file has been evaluated.
let SUPS=[
  {k:'hair',label:'Change of clothing, hair cut / shaving',amt:23},
  {k:'clothing',label:'Providing own clothing',amt:23},
  {k:'scans',label:'Firearms, swimming, driving, scans, minimal dialogue',amt:30.51},
  {k:'uniform',label:'Creative reaction, uniforms, specialised driving, provision of car',amt:37.22},
  {k:'lookalike',label:'Lookalike doubling, stand-in or dialogue (10+ words)',amt:61.63},
];
let MEAL={
  short:{label:'Short lunch (meal break under 1 hour on SWD)',day:23.38,night:35.08},
  late:{label:'Late lunch (no break within 6 hours of call)',day:23.38,night:35.08},
  supper:{label:'Short supper (no 2nd break within 13 hours of call)',day:23.38,night:35.08},
};
function refreshCardLists(){SUPS=supsNow();MEAL=mealsNow();}
/** A fee the user already picked, even if the current card no longer offers
 *  it — never silently drop money somebody typed. */
function supAmt(k){const s=SUPS.find(x=>x.k===k);return s?s.amt:0}
...
function fcSettings(){
  ...
  // by schema key, for everything the card carries beyond the original seven
  const k=(dept,key,cur)=>n(crowdFieldId(dept,key),cur);
  return {...s,baseDay:undefined,
    pact:{...s.pact,sa:n('cSA',s.pact.sa),hol:v.cHol!=null&&v.cHol!==''?(+v.cHol)/100:s.pact.hol,otDay:n('cOTday',s.pact.otDay),otNight:n('cOTnight',s.pact.otNight),earlyTravel:n('cET',s.pact.earlyTravel),travelA:n('cTravelA',s.pact.travelA),travelB:n('cTravelB',s.pact.travelB),
      night:k('sa','night',s.pact.night),phDay:k('sa','phDay',s.pact.phDay),phNight:k('sa','phNight',s.pact.phNight),
      otPhDay:k('sa','otPhDay',s.pact.otPhDay),otPhNight:k('sa','otPhNight',s.pact.otPhNight),
      fwStd:k('sa','fwStd',s.pact.fwStd),fwCwd:k('sa','fwCwd',s.pact.fwCwd)},
    spact:{...s.spact,basic:n('cSpact',s.spact.basic),night:n('cSpactNight',s.spact.night),hol:n('cSpactHol',s.spact.hol),earlyTravel:n('cSpactET',s.spact.earlyTravel),
      otDay:k('spact','otDay',s.spact.otDay),otNight:k('spact','otNight',s.spact.otNight),
      travelA:k('spact','travelA',s.spact.travelA),travelB:k('spact','travelB',s.spact.travelB),
      phDay:k('spact','phDay',s.spact.phDay),phNight:k('spact','phNight',s.spact.phNight),
      otPhDay:k('spact','otPhDay',s.spact.otPhDay),otPhNight:k('spact','otPhNight',s.spact.otPhNight),
      fwStd:k('spact','fwStd',s.spact.fwStd),fwCwd:k('spact','fwCwd',s.spact.fwCwd)},
  };
}
...
function renderStuntCalc(){
  ...
  const roster=SROWS.map((r,i)=>{
    ...
    return `<tr>
      ...
      <td class="num money">${gbpWhole(per*(+r.count||0))}</td>
  ...
  <div class="fccol">
    ...
    <div class="tablecard fc-roster"><h3>Rough day budget<span class="cnt">${rosterHeads} heads</span><span class="sum costable">${gbpWhole(rosterTotal)}</span></h3>
...
function renderDanceCalc(){
  ...
  const stepper=(k,label,note,val,min,max)=>`<div class="cdrow" style="margin-bottom:8px"><span class="cdfield" style="flex:1"><label>${label}</label><span class="cdinfo">${note}</span></span>
    <span class="seg"><button data-dstep="${k}" data-d="-1" ${val<=min?'disabled':''} aria-label="One fewer">−</button><output class="segval" aria-live="polite" aria-label="${esc(label)}">${val}</output><button data-dstep="${k}" data-d="1" ${val>=max?'disabled':''} aria-label="One more">+</button></span></div>`;
  const d0=c.perDay[0];
  const roster=DROWS.map((r,i)=>`<tr>
      ...
      <td class="num money">${gbpWhole(c.gross*(+r.count||0))}</td>
  ...
  <div class="fccol">
    <div class="tablecard fcpad" id="fcOut">
      ...
      <div class="fcgross">
        <div class="sl2" style="color:var(--hv)">Estimated gross</div>
        <div class="grossline"><input type="number" min="1" data-dcheads value="${DC.heads}"> <span>×</span> <input type="text" data-dcname placeholder="Character or group" data-tip="e.g. Featured dancers" style="flex:1 1 210px;min-width:150px"> <span>@</span> <span class="mono">${gbp(c.gross)}</span> <span>=</span> <b class="costable">${gbp(c.gross*(DC.heads||1))}</b> <button id="dcToRoster" class="tb-btn" style="margin-left:10px;padding:6px 12px;font-size:11.5px;border-color:var(--hv-line);color:var(--hv)">+ Add to rough budget</button></div>
      </div>
    </div>
    <div class="tablecard fc-roster"><h3>Rough week budget<span class="cnt">${rosterHeads} heads</span><span class="sum costable">${gbpWhole(rosterTotal)}</span></h3>
...
function fcPerTotal(){
  ...
  return p.per+FC.sups.reduce((a,k)=>a+supAmt(k),0)
    +Object.keys(FC.meals).filter(k=>FC.meals[k]&&MEAL[k]).reduce((a,k)=>a+(FC.shift==='Night'?MEAL[k].night:MEAL[k].day),0);
}
function renderFcOut(){
  ...
  $('#fcOut').innerHTML=`<div class="sl2">Breakdown — per artiste</div>
    ...
    ${row('Supplementary fees',FC.sups.length?FC.sups.map(k=>{const s=SUPS.find(x=>x.k===k);return s?s.label:k}).join(' · '):'none selected',FC.sups.reduce((a,k)=>a+supAmt(k),0))}
    ${row('Meal break penalties',Object.keys(FC.meals).filter(k=>FC.meals[k]&&MEAL[k]).length?Object.keys(FC.meals).filter(k=>FC.meals[k]&&MEAL[k]).map(k=>MEAL[k].label.split(' (')[0]).join(' · ')+(FC.shift==='Night'?' · night rate':''):'none',Object.keys(FC.meals).filter(k=>FC.meals[k]&&MEAL[k]).reduce((a,k)=>a+(FC.shift==='Night'?MEAL[k].night:MEAL[k].day),0))}
    ${row('Gross per artiste','',p.per+FC.sups.reduce((a,k)=>a+supAmt(k),0)+Object.keys(FC.meals).filter(k=>FC.meals[k]&&MEAL[k]).reduce((a,k)=>a+(FC.shift==='Night'?MEAL[k].night:MEAL[k].day),0),true)}
    <div class="fcgross">
      <div class="sl2" style="color:var(--hv)">Estimated gross</div>
      <div class="grossline"><input type="number" min="1" data-fcheads value="${FC.heads}"> <span>×</span> <input type="text" data-fcname placeholder="Character or group" data-tip="e.g. Nurses" style="flex:1 1 210px;min-width:150px"> <span>@</span> <span class="mono">${gbp(fcPerTotal())}</span> <span>=</span> <b class="costable">${gbp(fcPerTotal()*(FC.heads||1))}</b></div>
    </div>`;
}
// ---------- cast list ----------
function renderCast(){
...
function renderCalendar(){
  ...
  // C14: "Totals only" sat OUTSIDE the Condensed/Expanded pill as unstyled
  // loose text, reading like a third, broken option. It is what Condensed
  // means, so it is said inside the option itself.
  const ctl=`<div class="calctl"><span class="seg"><button data-calview="cond" class="${CALVIEW==='cond'?'on':''}">Condensed <small>totals only</small></button><button data-calview="exp" class="${CALVIEW==='exp'?'on':''}">Expanded <small>characters per day</small></button></span></div>`;
  $('#viewCal').innerHTML=ctl+months.map(m=>{
    ...
    for(let dd=1;dd<=days;dd++){
      ...
      const inner=list.map(d=>{
        ...
        if(APPMODE==='crowd'){
          if(cd){monthCost+=cd.cost;monthStunt++;
            if(CALVIEW==='exp'){
              const key=cdayKey(d);
              const chars=CDAY[key]?CDAY[key].chars.map(x=>({name:x.name,count:+x.count||0,tier:x.tier,roleId:x.roleId||''}))
                :[...(cd.sa?[{name:"SA's",count:cd.sa,tier:'SA',roleId:''}]:[]),
                  ...Object.entries(cd.feats).map(([n,x])=>({name:n,count:x,tier:'Featured',roleId:''})),
                  ...Object.entries(cd.spacts).map(([n,x])=>({name:n,count:x,tier:'SPACT',roleId:''})),
                  ...Object.values(cd.roles||{}).flatMap(t=>Object.entries(t.groups||{}).map(([n,x])=>({name:n||t.label,count:x,tier:'SA',roleId:t.roleId})))];
              // a role head is neither an SA nor a SPACT — it gets its own tally
              const roleTot=chars.filter(x=>x.roleId&&roleById(x.roleId)).reduce((a,x)=>a+x.count,0);
              const saTot=chars.filter(x=>x.tier!=='SPACT'&&!(x.roleId&&roleById(x.roleId))).reduce((a,x)=>a+x.count,0);
              const spTot=chars.filter(x=>x.tier==='SPACT'&&!(x.roleId&&roleById(x.roleId))).reduce((a,x)=>a+x.count,0);
              const MAXC=6;
              ...
              workBits=`<div class="ccost costable">${gbpWhole(cd.cost)}</div><div class="cchars">${lines}<div class="crow ctot"><b>Total ${saTot}</b> SA's${spTot?` · <b>${spTot}</b> SPACT`:''}${roleTot?` · <b>${roleTot}</b> on roles`:''}</div></div>${briefBit}`;
            }else{
              // role heads are part of the day's body count — without them
              // this line reads short the moment a role is used
              const tc=[cd.sa?cd.sa+' SA':'',cd.featPD?cd.featPD+' feat':'',cd.spactPD?cd.spactPD+' spact':'',
                ...Object.values(cd.roles||{}).map(t=>t.heads+' on '+t.label)].filter(Boolean).join(' · ');
              workBits=`<div class="ccost costable">${gbpWhole(cd.cost)}</div><div class="cteam">${tc}</div>${briefBit}`;
            }}
        }else{
      ...
      // A shoot day is the calendar's one interactive thing, so it is a real
      // button to the keyboard as well as the mouse: role + tabindex make it
      // focusable and the generic Enter/Space listener opens the same popup.
      const cellLbl=new Date(y,mo,dd).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})
        +' — '+list.map(d=>'D'+d.num+(d.loc?' at '+d.loc:'')).join('; ')+'. Press to open the day.';
      cells+=`<div class="cal-cell shoot ${anyStunt?'stunt':''}${isToday?' todaycell':''}" role="button" tabindex="0" aria-label="${esc(cellLbl)}" data-calpop="${esc(ids)}" data-ids="${esc(ids)}"><span class="dnumtxt">${dd}</span>${isToday?'<span class="todaytag">Today</span>':''}${inner}</div>`;
    }
    const trail=(lead+days)%7;
    if(trail)for(let i=trail;i<7;i++)cells+='<div class="cal-cell off"></div>';
    const wl=APPMODE==='crowd'?'crowd':'stunt';
    return `<div class="cal-month"><h3>${MONFULL[mo]} ${y}${monthStunt?`<span class="cnt" style="font-size:10.5px;background:var(--panel2);border:1px solid var(--line2);color:var(--sub);border-radius:20px;padding:2px 10px;font-family:var(--body);letter-spacing:0">${monthStunt} ${wl} day${monthStunt>1?'s':''}</span>`:''}${monthCost?`<span class="sum costable">${gbpWhole(monthCost)}</span>`:''}</h3>
  ...
  $('#calBody').innerHTML=list.map(d=>{
    ...
    return `<div class="cald">
      ...
      <div class="actions">
        ...
        ${APPMODE==='crowd'&&CROWD.perDay[d.id]?`<button data-costday="${esc(d.id)}" class="costable">Cost breakdown — ${gbpWhole(CROWD.perDay[d.id].cost)}</button>`:''}
...
function crowdCharData(){
  // A group on a live named role goes into that role's OWN bucket ("role:xyz")
  // rather than the tier it sits under — otherwise stand-ins would be listed
  // and priced as SAs. With no roles defined there are exactly three buckets,
  // as there always were.
  const roleKeys=activeRoles().map(r=>'role:'+r.id);
  const blank=()=>{const o={SA:{},Featured:{},SPACT:{}};for(const k of roleKeys)o[k]={};return o};
  const tiers=blank();
  const bucketOf=(f,tier)=>{
    const rid=(f&&f.roleId||'').trim();
    return (rid&&roleById(rid))?'role:'+rid:tier;
  };
  for(const d of MODEL.days){
    const peak=blank();
    let anon=0;
    const put=(f,tier)=>{const b=peak[bucketOf(f,tier)];b[f.name||'']=Math.max(b[f.name||'']||0,f.count)};
    for(const s of d.scenes){
      if(s.sa)anon=Math.max(anon,s.sa);
      for(const f of s.saChars||[])put(f,'SA');
      for(const f of s.featured||[])put(f,'Featured');
      for(const f of s.spacts||[])put(f,'SPACT');
    }
    if(anon)peak.SA['']=Math.max(peak.SA['']||0,anon); // anonymous SA joins the unnamed bucket
    for(const tier of Object.keys(tiers))for(const [name,cnt] of Object.entries(peak[tier])){
      if(!cnt)continue;
      const roleHere=tier.startsWith('role:')?roleById(tier.slice(5)):null;
      const key=name||(roleHere?roleLabel(roleHere)+' (unnamed)':tier==='SA'?'SA (unnamed)':tier+' (unnamed)');
...
function crowdByCharHTML(){
  ...
  const card=(tier,label,rate,holLabel)=>{
    ...
    return `<div class="tablecard"><h3>${label}<span class="cnt">${rows.length} character${rows.length===1?'':'s'}</span><span class="sum costable">${gbpWhole(sub)}</span></h3>
    ...
    ${rows.map(p=>{const dr=p.heads*rate;const hol=tier==='SPACT'?p.heads*gSpHol():dr*R.hol;return `<tr><td class="rowlabel">${esc(p.code)}</td><td class="num">${p.dayCounts.size}</td><td class="num">${p.max}</td><td class="num"><b>${p.heads}</b></td><td class="num">${gbpWhole(dr)}</td><td class="num">${gbpWhole(hol)}</td><td class="num money">${gbpWhole(dr+hol)}</td><td class="datescol"><div class="daylist">${dateChips(p)}</div></td></tr>`}).join('')}
    </tbody></table></div></div>`;
  };
  // one card per named role, on the role's own day rate and its own holiday
  // convention (a percentage on an SA-shaped role, a flat amount on a SPACT one)
  const roleCards=activeRoles().map(r=>{
    const res=roleResolved(r);
    const flat=r.base==='spact';
    const holLabel=flat?'Holiday (in lieu)':'Holiday';
    const rows=Object.values(tiers['role:'+r.id]||{}).sort((a,b)=>b.heads-a.heads||a.code.localeCompare(b.code));
    if(!rows.length)return '';
    const holOf=heads=>flat?heads*(+res.hol||0):heads*r.day*(+res.hol||0);
    const sub=rows.reduce((a,p)=>a+p.heads*r.day+holOf(p.heads),0);
    return `<div class="tablecard"><h3>${esc(roleLabel(r))} — own rate card<span class="cnt">${rows.length} group${rows.length===1?'':'s'}</span><span class="sum costable">${gbpWhole(sub)}</span></h3>
    <div class="tscroll"><table><thead><tr><th>Character / group</th><th class="num">Days</th><th class="num">Max heads</th><th class="num">Person-days</th><th class="num">Day rates</th><th class="num">${holLabel}</th><th class="num">Total</th><th class="datescol">Dates</th></tr></thead><tbody>
    ${rows.map(p=>{const dr=p.heads*r.day,hol=holOf(p.heads);return `<tr><td class="rowlabel">${esc(p.code)}</td><td class="num">${p.dayCounts.size}</td><td class="num">${p.max}</td><td class="num"><b>${p.heads}</b></td><td class="num">${gbpWhole(dr)}</td><td class="num">${gbpWhole(hol)}</td><td class="num money">${gbpWhole(dr+hol)}</td><td class="datescol"><div class="daylist">${dateChips(p)}</div></td></tr>`}).join('')}
    </tbody></table></div>
    <div class="note">${esc(roleLabel(r))} is paid ${gbp(r.day)} a day on the ${esc(ROLE_BASE_SHORT[r.base].toLowerCase())} — ${engineRoleFrameworkHours('std',r.base)} hours standard, ${engineRoleFrameworkHours('cwd',r.base)} continuous. Overtime, early calls and travel are priced on the by-shoot-day view.</div></div>`;
  }).join('');
  const html=card('SA','Supporting artists',R.sa,'Holiday')+card('Featured','Featured background',R.feat,'Holiday')+card('SPACT','Spacts — Take 3 2026',R.spact,'Holiday (in lieu)')+roleCards;
...
function renderDoods(){
  ...
  const meta={
    crowd:{dayFn:sceneHasCrowd,nameCol:'Character / group',unit:'group',title:'Day out of days — crowd',
      ...
      note:'Built live from the schedule — each cell is how many of that character or group the day needs, so a revision can never leave the grid stale. Click a group with more than one head to expand its individual slots, and put a name against each slot as you book it. Day headers jump to the day board.'},
  ...
  if(KIND==='crowd'){
    const tiers=crowdCharData();
    // buckets are the three tiers plus one per named role, so a stand-in gets
    // its own rows and its own badge on the grid rather than reading as an SA
    for(const tier of Object.keys(tiers)){
      const r=tier.startsWith('role:')?roleById(tier.slice(5)):null;
      for(const p of Object.values(tiers[tier]))
        groups.push({...p,tier,label:p.code,badge:r?roleLabel(r):(tier==='SA'?'':tier),single:false});
    }
  ...
  const head=`<thead><tr><th class="dood-name"><span class="dood-nametext">${meta.nameCol}</span></th><th class="num">Days</th><th class="num">P-days</th>
  ...
  const body=groups.map(g=>{
    ...
    let html=`<tr class="dood-row${open?' open':''}" ${expandable?`data-doodtoggle="${esc(key)}"`:''}>
      <td class="dood-name">${expandable?`<span class="dood-chev">${open?'▾':'▸'}</span>`:'<span class="dood-chev"></span>'}<span class="dood-nametext" title="${esc(g.label||g.code)}">${esc(g.label||g.code)}${g.badge?` <span class="ctype">${esc(g.badge)}</span>`:''}</span></td>
    ...
    if(open&&expandable){
      ...
      for(let i=1;i<=n;i++){
        ...
        html+=`<tr class="dood-slot"><td class="dood-name"><span class="dood-nametext">${esc(g.code)} ${i}<span class="dood-un">unassigned</span></span></td><td class="num">${slotDays}</td><td class="num"></td>
          ${days.map(d=>{const c=g.dayCounts.get(d.id)||0;return `<td class="dood-cell${c>=i?' w':''}">${c>=i?'W':''}</td>`}).join('')}</tr>`;
      }
      if(g.max>n)html+=`<tr class="dood-slot"><td class="dood-name dood-more"><span class="dood-nametext">… ${g.max-n} more slots</span></td><td class="num"></td><td class="num"></td>${days.map(()=>'<td class="dood-cell"></td>').join('')}</tr>`;
    }
    return html;
  }).join('');
  const colTotals=days.map(d=>groups.reduce((a,g)=>a+(g.dayCounts.get(d.id)||0),0));
  const foot=`<tr class="dood-total"><td class="dood-name"><span class="dood-nametext">Heads on the day</span></td><td class="num">${days.length}</td><td class="num">${groups.reduce((a,g)=>a+g.heads,0)}</td>${colTotals.map(c=>`<td class="dood-cell num">${c||''}</td>`).join('')}</tr>`;
...
let BRIEFS={};
// bumped whenever the briefs change, so briefFor's index knows to rebuild
let BRIEFS_VER=0;
...
// briefFor is asked once per crowd character, twice per refresh, and used to
// walk the whole of BRIEFS (allocating a fresh array each time) to answer.
// Index it once per set of briefs instead — the map is thrown away the moment
// anything about the briefs changes.
let BRIEF_IDX=null;
function briefIndex(){
  const ns=briefNs();
  const n=Object.keys(BRIEFS).length;
  if(BRIEF_IDX&&BRIEF_IDX.ns===ns&&BRIEF_IDX.ver===BRIEFS_VER&&BRIEF_IDX.n===n)return BRIEF_IDX.map;
  const map=new Map();
  for(const x of briefsForNs()){
    const k=(x.b.character||'').trim().toLowerCase()+'|'+(x.b.tier||'SA');
    if(!map.has(k))map.set(k,x);
  }
  BRIEF_IDX={ns,ver:BRIEFS_VER,n,map};
  return map;
}
function briefFor(character,tier){
  const want=(character||'').trim().toLowerCase();
  return briefIndex().get(want+'|'+(tier||'SA'));
}
...
function renderBriefs(){
  ...
  // C13: a "new" badge on EVERY row says nothing — it only means anything
  // beside rows that are not new. When they all are, the heading already says
  // so and the badges come off.
  const allNew=unbriefed.length>0&&list.length===0;
  const unbriefedRows=unbriefed.map(c=>{
    ...
    return `<tr class="briefrow unbriefed" data-newbrieffor="${esc(c.name)}|${esc(c.tier)}" data-count="${c.max}">
      ...
      <td class="rowlabel">${esc(c.name)}${allNew?'':`<span class="newtag" data-tip="Named on the day board — click the row to start its brief">new</span>`}</td>
      <td>${esc(c.tier)}</td>
...
function briefEditorHTML(id){
  ...
  const contLine=c&&cont?(cont.gaps
    ...
    :(cont.runs[0]&&cont.runs[0].length
      ...
      // no day on this schedule has a date yet, so there is no run to describe
      :'Dates not set on this schedule yet')):'';
...
document.addEventListener('focusout',e=>{
  const cell=e.target.closest?e.target.closest('[data-cbaddr]'):null;
  if(!cell)return;
  const was=cell.dataset.cbwas??'';
  const now=(cell.textContent||'').replace(/\s+/g,' ').trim();
  if(now===was)return;
  cbdCommit(cell.dataset.cbaddr,cell.dataset.cbf,now);
});
...
document.addEventListener('click',e=>{
  ...
  if(del){
    ...
    const rows=cbdRowsFor(d,idx);
    if(line<rows.length){
      // removing a requirement line changes the day's crowd everywhere — make
      // it undoable from the same SCED snapshot the crowd chips use
      const snap=JSON.stringify(SCED);
      const gone=rows[line];
      rows.splice(line,1);
      cbdWrite(dayId,idx,rows);
      const what=(gone&&gone.name&&gone.name.trim())
        ?'“'+gone.name.trim()+'”'+(gone.count?' ×'+gone.count:'')
        :'a crowd line of '+((gone&&gone.count)||0);
      registerCrowdUndo(snap,'removed '+what);
      setStatus('Crowd line removed — day board, costs and DOODs follow.',{undo:crowdUndo});
    }
    return;
  }
  ...
  if(tier){
    ...
    // Once the production has named roles there are more than three answers,
    // so the chip opens a list instead of cycling. With none, it cycles exactly
    // as it always has.
    const cbRoles=activeRoles();
    if(cbRoles.length){
      const rect=tier.getBoundingClientRect();
      const set=fn=>{const rs=cbdRowsFor(d,idx);const row=rs[line];if(!row)return;fn(row);cbdWrite(dayId,idx,rs);};
      openCtxMenu(rect.left,rect.bottom+4,[
        {label:'SA'+(!r.roleId&&r.tier!=='SPACT'&&!r.featured?'  ✓':''),onClick:()=>set(row=>{row.roleId='';row.tier='SA';row.featured=false;})},
        {label:'Featured'+(!r.roleId&&r.featured?'  ✓':''),onClick:()=>set(row=>{row.roleId='';row.tier='SA';row.featured=true;if(!(row.name||'').trim())row.name='Featured';})},
        {label:'Spact'+(!r.roleId&&r.tier==='SPACT'?'  ✓':''),onClick:()=>set(row=>{row.roleId='';row.tier='SPACT';row.featured=false;})},
        {sep:true},
        ...cbRoles.map(role=>({label:roleLabel(role)+' — '+gbp(role.day)+'/day'+(r.roleId===role.id?'  ✓':''),onClick:()=>set(row=>{
          row.roleId=role.id;
          // A role group is always a NAMED group: plain background has nowhere
          // to carry a role (it is stored as a bare head count), and an unnamed
          // one could not be told apart from the crowd it is standing in for.
          if(!(row.name||'').trim())row.name=roleLabel(role);
        })})),
      ]);
      return;
    }
...
function raPDF(){
  if(!RA_CTX){
    // globals.css gates the "hide everything but the assessment" print rule on
    // body:has(.raOverlay.open) OR body.printing-ra. :has() is not universal,
    // so set the class flag as well — without it, a browser without :has()
    // prints the whole board behind the overlay.
    document.body.classList.add('printing-ra');
    const clean=()=>{document.body.classList.remove('printing-ra');window.removeEventListener('afterprint',clean)};
...
function applyFilters(){
  ...
  // "Changed days only" rides on the SAME machinery as the stunt filter and
  // the search — one pass, one empty state, one past-days drawer rule.
  const ch=CHG_ONLY&&chgOn();
  const changed=ch?chgDayIds():null;
  let shown=0;
  for(const d of MODEL.days){
    const card=document.getElementById('day-'+d.id);
    if(!card)continue;
    let show=(!st||card.dataset.stunt==='1')&&(!ch||changed.has(d.id));
    let anyHit=false;
  ...
  document.querySelectorAll('#viewDays .breakline').forEach(b=>b.style.display=(st||q||ch)?'none':'');
  // Past days are folded into a closed drawer, so a match in one used to stay
  // invisible — the search looked like it had found nothing. Open the drawer
  // when it holds a match and say how many are inside.
  const drawer=/** @type {any} */(document.querySelector('#viewDays details.pastdrawer'));
  if(drawer){
    const sum=/** @type {any} */(drawer.querySelector('summary'));
    const inPast=[...drawer.querySelectorAll('.daycard')].filter(c=>/** @type {any} */(c).style.display!=='none').length;
    if(sum){
      if(sum.dataset.base==null)sum.dataset.base=sum.textContent||'';
      sum.textContent=((st||q||ch)&&inPast)
        ?inPast+' past day'+(inPast===1?'':'s')+' match — archived, everything still opens'
        :sum.dataset.base;
    }
    if((st||q||ch)&&inPast&&!drawer.open){drawer.open=true;PAST_OPEN=true;}
    drawer.style.display=((st||q||ch)&&!inPast)?'none':'';
  }
  // A blank board is indistinguishable from a crash — always say what happened.
  const host=$('#viewDays');
  let none=document.getElementById('noDayMatch');
  if(host&&(st||q||ch)&&!shown){
    if(!none){
      none=document.createElement('div');
      none.id='noDayMatch';
      none.className='empty-first';
      host.appendChild(none);
    }
    none.innerHTML=q
      ?`<h2>No days match “${esc($('#search').value.trim())}”</h2><p>Nothing on this schedule matches that. Try a day number like <b>D12</b>, a date, a location, a scene number or a cast name.</p>`
      :ch
      ?`<h2>Nothing changed on these days</h2><p>No shoot day differs from ${esc((activeRevDiff()||{}).prevLabel||'the last schedule')}${st?' among the stunt days':''}. Turn “Changed days only” off to see the whole board again.</p>`
      :`<h2>No days with stunts</h2><p>Nothing on this schedule is marked as a stunt day. Turn the stunts filter off to see every day again.</p>`;
...
// ---------- tab strip: Left / Right / Home / End ----------
// The board's nine pages are a real tablist, so they answer to the arrow keys
// the way every other tab strip does. Hidden tabs (Briefs, Doods, Crowd
// breakdown before they apply) are skipped.
document.addEventListener('keydown',e=>{
  const et=/** @type {any} */(e.target);
  const tab=et&&et.closest&&et.closest('.tabs-list [role="tab"]');
  if(!tab)return;
  const tabs=/** @type {any[]} */([...document.querySelectorAll('.tabs-list [role="tab"]')]).filter(b=>!b.classList.contains('hidden'));
...
// ---------- generic keyboard activation ----------
// A <div role="button"> does NOT synthesise a click from Enter/Space the way a
// real <button> does, so every custom control built that way (editable
// crowd/stunt cells, crowd chips, calendar days) took focus and then did
// nothing. One listener gives them all the keyboard behaviour their role
// promises. Real buttons/links/fields are left alone.
document.addEventListener('keydown',e=>{
  ...
  if(/^(BUTTON|A|INPUT|TEXTAREA|SELECT|SUMMARY|OPTION)$/.test(t.tagName))return;
  if(t.getAttribute('role')!=='button')return;
  if(t.getAttribute('aria-disabled')==='true')return;
  e.preventDefault();          // Space must not scroll the board
  t.click();
});
...
document.addEventListener('click',e=>{
  ...
  const sset=/** @type {HTMLElement} */(e.target).closest('#sideSettings');
  if(sset){
    const s=SOURCES[ACTIVE];
  ...
  if(t){
    // roving tabindex + aria-selected + which panel shows all live in
    // applyBoardTab, so a click and a reload-restore light up identically.
    applyBoardTab(t.dataset.view);
    return;
  }
  ...
  if(exb){
    ...
    if(dd)dd.open=false;
    // C10: NOTHING happened on screen for several seconds on a 96-day PDF — no
    // spinner, no message, nothing — so people pressed it again. Say it has
    // started before the work begins, and let the browser paint that before the
    // synchronous build blocks the main thread.
    exportBusy(true,fmt);
    setTimeout(()=>{
      try{if(fmt==='pdf')exportViewPDF(CUR_VIEW);else doExport(CUR_VIEW,fmt)}
      finally{exportBusy(false,fmt)}
    },40);
    return;
  }
  ...
  if(bpd){
    ...
    const bk=briefKey(bid), b=BRIEFS[bk];
    if(b&&b.photos&&b.photos[+i]!==undefined){
      // an uploaded reference photo cannot be got back once it is gone, so the
      // removal is undoable in place
      const before=b.photos.slice();
      b.photos.splice(+i,1);b.updatedAt=new Date().toISOString();saveBriefs();
      registerUndo(()=>{const t=BRIEFS[bk];if(t){t.photos=before;t.updatedAt=new Date().toISOString();saveBriefs();renderBriefs();}},
        'removed a reference photo');
      setStatus('Reference photo removed.',{undo:crowdUndo});
    }
    renderBriefs();return;
  }
  const bdel=e.target.closest('[data-delbrief]');
  if(bdel){
    // A brief holds hand-typed sizing, attributes, fitting notes and uploaded
    // reference photos, and the ✕ is a small glyph — so the delete is undoable
    // and says so, rather than being instant and final.
    const bid=bdel.dataset.delbrief, bk=briefKey(bid);
    const gone=BRIEFS[bk], wasSel=BRIEF_SEL.has(bid);
    if(!gone)return;
    const name=(gone.character||'').trim()||'this brief';
    delete BRIEFS[bk];BRIEF_SEL.delete(bid);
    if(BRIEF_OPEN===bid)BRIEF_OPEN=null;
    saveBriefs();refreshBriefBadges();renderBriefs();
    registerUndo(()=>{BRIEFS[bk]=gone;if(wasSel)BRIEF_SEL.add(bid);saveBriefs();refreshBriefBadges();renderBriefs();},
      'deleted the “'+name+'” brief');
    setStatus('Deleted the “'+name+'” brief — sizing, notes and any reference photos went with it.',{undo:crowdUndo});
    return;
  }
  ...
  if(cg){
    ...
    $('#fltStunt').checked=false;$('#search').value='';CHG_ONLY=false;renderChangeBar();applyFilters();
  ...
  const nw=/** @type {any} */(e.target).closest('[data-cdnight]');
  if(nw){openCrowdDay(nw.dataset.cdnight);return}
...
document.addEventListener('change',e=>{
  ...
  // "Paid as" on a day-calculator row: one of the three tiers, or one of the
  // production's named roles. Picking a role keeps the tier underneath it —
  // that is what prices the row if the role is ever deleted.
  if(e.target.matches('[data-cdrow="tier"]')){
    const d=COST.dayById[CD_CTX], c=CDAY[cdayKey(d)];
    const ch=c.chars[+e.target.dataset.i];
    if(ch){
      const v=e.target.value||'SA';
      if(v.startsWith('role:'))ch.roleId=v.slice(5);
      else{delete ch.roleId;ch.tier=v;}
      saveCDAY();renderCdModal();cdRecalcApp();
    }
    return;
  }
  ...
  if(e.target.matches('[data-cdph]')){
    const d=COST.dayById[CD_CTX], c=CDAY[cdayKey(d)];
    c.ph=e.target.checked;
    // The USER has now stated it, either way — and a user's own answer always
    // beats the bank-holiday table, including "no, this is not a holiday for
    // us". Without this flag, unticking an auto-flagged day did nothing at all.
    c.phSet=true;
    saveCDAY();computeCrowdCosts();renderCdModal();return;
  }
  const meal=/** @type {any} */(e.target).dataset&&/** @type {any} */(e.target).dataset.cdmeal;
  if(meal){
    const d=COST.dayById[CD_CTX], c=CDAY[cdayKey(d)];
    c.meals=Object.assign({},c.meals);
    if(/** @type {any} */(e.target).checked)c.meals[meal]=true;else delete c.meals[meal];
    if(!Object.keys(c.meals).length)delete c.meals;
    saveCDAY();computeCrowdCosts();renderCdModal();return;
  }
});
...
function gotoDay(id){
  document.querySelector('.tabs button[data-view="days"]').click();
  $('#fltStunt').checked=false;CHG_ONLY=false;renderChangeBar();applyFilters();
...
// Skip links move focus as well as the scroll position — an anchor alone moves
// the viewport and leaves the keyboard exactly where it was.
document.addEventListener('click',e=>{
  const a=/** @type {any} */(e.target).closest&&/** @type {any} */(e.target).closest('.skiplinks a');
  if(!a)return;
  e.preventDefault();
  const t=/** @type {any} */(document.querySelector(a.getAttribute('href')));
  if(!t)return;
...
['rPerf','rHol','rIns','rInsDays','rUse','rCoord','rSDRate','rSDDays','rSDOn','cSA','cSpact','cHol','cOTday','cOTnight','cET','cTravelA','cTravelB','cSpactNight','cSpactHol','cSpactET'].forEach(id=>{
  const run=()=>{if(!MODEL)return;computeCosts();computeCrowdCosts();renderSummary();renderDays();renderStunts();renderCrowd();if(!$('#viewDoods').classList.contains('hidden'))renderDoods();renderCalendar();renderCast();renderFreeCalc()};
  // Both 'input' and 'change' fire on these, and each pass rebuilds tens of
  // thousands of nodes — so typing a rate rebuilt the whole board per digit.
  // Settle first, then render once.
  let t=null;
  const fn=()=>{clearTimeout(t);t=setTimeout(()=>{t=null;run()},150)};
  // (renderFreeCalc branches to the stunt calculator in StuntOS mode)
  $('#'+id).addEventListener('input',fn);
  $('#'+id).addEventListener('change',fn);
});

// ---------- app mode (Stunt / Crowd) ----------
// The role picked at sign-up is not decoration: it chooses which department
// the app opens on the first time, so a stunt coordinator isn't shown a crowd
// board and a crowd AD isn't shown a stunt one. Once the person has used the
// Stunt/Crowd toggle themselves, their choice is stored and always wins.
function roleAppMode(session){
  const r=(session&&session.user&&session.user.user_metadata&&session.user.user_metadata.role)||'';
  return /stunt/i.test(r)?'stunt':r?'crowd':'';
}
/** Apply the role default once, and only while the user has never chosen. */
function applyRoleAppMode(session){
  if(store.get('stuntos-appmode'))return;      // their own choice wins
  const m=roleAppMode(session);
  if(m&&m!==APPMODE){setAppMode(m);store.set('stuntos-appmode','');}
}
function setAppMode(m){
  ...
  // Three tabs all began "Crowd…" — "Crowd cost breakdown", "Crowd" and
  // "Crowd breakdown" — and the third is not a breakdown at all, it is the
  // document generator for the deliverable. Named for what each one IS.
  $('#tabBreakdown').textContent=m==='crowd'?'Cost breakdown':'Stunt cost breakdown';
  $('#fltLabel').textContent=m==='crowd'?'Crowd days only':'Stunt days only';
  $('#tabCrowd').textContent=m==='crowd'?'Crowd by day':'Stunts by day';
  // Briefs (casting briefs) and Doods (day-out-of-days) are CrowdOS concerns
  {const ck=$('#chipKey');if(ck){ck.classList.toggle('hidden',m!=='crowd');if(m!=='crowd')/** @type {any} */(ck).open=false;}}
...
function renderSrcBar(){
  // The top schedule strip was superseded by the sidebar. #srcBar no longer
  // exists in the markup, so this was building a list of buttons for nothing
  // on every schedule open. Kept as a genuine no-op so its callers (setActive,
  // addSource) don't need to change; the sidebar does the real work.
}
// Clamp a wanted schedule index onto what actually exists. Returns -1 when
// there is nothing to show at all (a brand-new account, or the last schedule
// just deleted) — callers land on the dashboard instead.
function clampActive(i){
  if(!SOURCES.length)return -1;
  let n=Number(i);
  if(!Number.isInteger(n)||n<0)n=0;
  if(n>SOURCES.length-1)n=SOURCES.length-1;
  return SOURCES[n]&&SOURCES[n].model?n:-1;
}
function setActive(i){
  // Never open a schedule that isn't there. A brand-new account has NO
  // schedules, and asking for one used to throw part-way through sign-in and
  // leave the board blank behind a stuck "Syncing your productions…".
  i=clampActive(i);
  if(i<0){ACTIVE=0;showDash();return}
  ACTIVE=i;MODEL=SOURCES[i].model;NS=SOURCES[i].ns||'';
  CHG_SHOW=true;CHG_ONLY=false; // each schedule is read fresh
  applyTheme(SOURCES[i].colour);
  // a production brand colour overrides the schedule-colour highlight
  {const pr=SOURCES[i].prod&&PRODS[SOURCES[i].prod];applyProdBranding(pr||null);}
  renderSrcBar();
...
function syncBoardPdfBtn(){
  const btn=/** @type {any} */($('#btnBoardPdf'));if(!btn)return;
  const has=sourceHasFiles(SOURCES[ACTIVE]);
  // It was rendered at half opacity — which reads as disabled — while staying
  // enabled, focusable and doing nothing at all when pressed. Either it works
  // or it is properly disabled. There is no original to open, so: disabled.
  btn.classList.toggle('muted',!has);
  btn.disabled=!has;
  if(has)btn.removeAttribute('aria-disabled');else btn.setAttribute('aria-disabled','true');
  btn.dataset.tip=has?'Open the original schedule PDF alongside the board to cross-check':'There is no original PDF stored for this schedule, so there is nothing to open. Upload one and it will be kept with the schedule.';
}
// Every schedule carries its own id. Titles come from the uploaded filename,
// so two schedules routinely share one (the same file imported for Main and
// 2nd unit, or a re-imported revision) — anything stored per schedule has to
// hang off this instead, or one schedule's work lands on another's.
let UID_SEQ=0;
function newSourceUid(cloudId){
  if(cloudId)return 'c:'+cloudId; // the account's own id when there is one
  return 's:'+Date.now().toString(36)+':'+(UID_SEQ++).toString(36)+':'+Math.random().toString(36).slice(2,8);
}
// the key anything stored per schedule should use, with the old title-keyed
// format still readable so nothing is lost on upgrade
function srcUid(s){return (s&&s.uid)||(s&&s.cloudId&&'c:'+s.cloudId)||(s&&s.title)||''}
// The namespace EVERY per-day and per-scene store keys off. It has to be the
// schedule's own id, not its title: Movie Magic exports every revision as
// "Shooting Schedule.pdf", so a title-keyed namespace put the Pink revision's
// day configs on top of the Blue one's — and deleting either revision wiped
// the survivor's whole breakdown. See migrateNsKeys for the one-off upgrade.
function nsFor(uid,kind){return (kind==='manual'?'m:':'p:')+uid}
// what the same schedule's namespace looked like before that change
function legacyNsOf(rec){return (rec&&rec.kind==='manual'?'m:':'p:')+((rec&&rec.title)||'')}
// every store that files work under a schedule's namespace
function nsStores(){
  return [
    [CDAY,()=>store.set('stuntos-cday',JSON.stringify(CDAY))],
    [ADJ,()=>store.set('stuntos-adj',JSON.stringify(ADJ))],
    [SCED,()=>store.set('crowdos-sced',JSON.stringify(SCED))],
    [NOTES,()=>{NOTES_LEGACY=null;store.set('stuntos-notes',JSON.stringify(NOTES))}],
    [STUNTCFG,()=>store.set('crowdos-stuntcfg',JSON.stringify(STUNTCFG))],
    [STUNTDAY,()=>store.set('crowdos-stuntday',JSON.stringify(STUNTDAY))],
    [BRIEFS,()=>store.set('crowdos-briefs',JSON.stringify(BRIEFS))],
    [DAYLOC,()=>store.set('crowdos-dayloc',JSON.stringify(DAYLOC))],
  ];
}
/** One-off upgrade of work saved under the old title-keyed namespace onto the
 *  schedule's own id. Safe to run as often as it likes: once a schedule's work
 *  has moved, nothing is left under the old prefix. Where two loaded schedules
 *  genuinely shared a title they were also SHARING this work on screen, so each
 *  gets its own copy — nobody loses a breakdown, and from here on the two can
 *  never overwrite or delete each other's. */
function migrateNsKeys(pairs){
  const byOld=new Map();
  for(const [oldNs,newNs] of pairs||[]){
    if(!oldNs||!newNs||oldNs===newNs)continue;
    if(!byOld.has(oldNs))byOld.set(oldNs,[]);
    const list=byOld.get(oldNs);
    if(!list.includes(newNs))list.push(newNs);
  }
  if(!byOld.size)return 0;
  let moved=0;
  for(const [map,save] of nsStores()){
    let dirty=false;
    for(const k of Object.keys(map)){
      for(const [oldNs,targets] of byOld){
        if(k.indexOf(oldNs+'|')!==0)continue;
        const rest=k.slice(oldNs.length+1);
        for(const ns of targets){const nk=ns+'|'+rest;if(map[nk]===undefined)map[nk]=map[k]}
        delete map[k];moved++;dirty=true;
        break;
      }
    }
    if(dirty)save();
  }
  // the stored original documents are filed under the same namespace
  let idxDirty=false;
  for(const [oldNs,targets] of byOld){
    if(!FILEIDX.has(oldNs))continue;
    FILEIDX.delete(oldNs);
    for(const ns of targets)FILEIDX.add(ns);
    idxDirty=true;
    idbGet(oldNs)
      .then(rec=>rec?Promise.all(targets.map(ns=>idbPut(ns,rec))).then(()=>idbDel(oldNs)):null)
      .catch(()=>{});
  }
  if(idxDirty)saveFileIdx();
  return moved;
}
/** A blob row's keys carry whatever namespace the device that wrote them used,
 *  and older devices used the schedule's title. The row already names the
 *  schedule it belongs to, so re-file its keys onto that schedule's own
 *  namespace as they come in — nothing can land on the wrong schedule. */
function adoptBlobKeys(data,src){
  const ns=(src&&src.ns)||'';
  if(!ns)return data||{};
  const out={};
  const legacy=legacyNsOf(src);
  for(const [k,v] of Object.entries(data||{})){
    let key=k;
    if(k.indexOf(ns+'|')===0){/* already ours */}
    else if(legacy&&k.indexOf(legacy+'|')===0)key=ns+'|'+k.slice(legacy.length+1);
    else{
      const seg=String(k).split('|');
      if(seg.length>1&&/^[pm]:/.test(seg[0]))key=ns+'|'+seg.slice(1).join('|');
    }
...
function addSource(model,title,short,activate=true,opts={}){
  ...
  const uid=opts.uid||newSourceUid(opts.cloudId);
  // an explicit namespace (the sample schedule) is honoured — unless another
  // loaded schedule already holds it, which is exactly how one schedule's work
  // used to land on another's
  let ns=opts.ns||nsFor(uid,opts.kind);
  if(SOURCES.some(s=>s.ns===ns))ns=nsFor(uid,opts.kind);
  SOURCES.push({model,title,short:short||title,colour,uid,ns,kind:opts.kind,text:opts.text,unit:opts.unit,cloudId:opts.cloudId,createdAt:opts.createdAt,prod:opts.prod,version:opts.version,schedDate:opts.schedDate,format:opts.format,rateCard:opts.rateCard||null,current:!!opts.current,aiModel:opts.aiModel||null,docKind:opts.docKind||null,pdfFiles:opts.pdfFiles||null});
...
function setStatus(msg,opts){
  ...
  // `undo` is the common case; `action:{label,fn}` is the same button offering
  // something else entirely (e.g. "Email it to us" on a schedule we can't read)
  const act=(msg&&opts.action&&typeof opts.action.fn==='function')?opts.action:null;
  STATUS_UNDO_FN=act?act.fn:((msg&&typeof opts.undo==='function')?opts.undo:null);
  const ub=$('#statusUndo');
  if(ub){ub.textContent=act?act.label:'↺ Undo';ub.classList.toggle('hidden',!STATUS_UNDO_FN);}
  if(!bar)return;
  bar.classList.remove('fadeout');
  // a spinner for the several-second jobs (building a 96-day PDF) so the bar
  // reads as "working", not as a message that has already finished
  bar.classList.toggle('working',!!(msg&&opts.spinner));
  bar.classList.toggle('hidden',!msg);
  if(msg&&!opts.sticky&&!opts.spinner){
    const ms=opts.timeout||(STATUS_UNDO_FN?9000:5500);
    STATUS_TIMER=setTimeout(dismissStatus,ms);
  }
}
/**
 * Turn a save/sync failure into something a user can act on. What matters to
 * them is only ever: is my work safe, and is there anything I should do?
 * The service's own message is kept out of the sentence — nobody reading this
 * app can do anything with "PGRST204" or "Failed to fetch".
 */
function friendlyCloudError(err,what){
  const raw=((err&&err.message)||String(err||'')).toLowerCase();
  const thing=what||'that change';
  if(/does not exist|schema cache|relation|column/.test(raw))
    return 'Couldn’t save '+thing+' to your account — your account isn’t set up for this yet. It is safe on this device. Email '+SUPPORT_EMAIL+' and we’ll switch it on.';
  if(/failed to fetch|network|load failed|offline|timeout|timed out|aborted/.test(raw))
    return 'Couldn’t reach your account just now, so '+thing+' is saved on this device only. It will sync as soon as you’re back online.';
  if(/jwt|token|expired|session/.test(raw))
    return 'Your sign-in has expired, so '+thing+' hasn’t reached your account yet. Refresh the page and sign in again — nothing is lost.';
  if(/row-level security|permission|not authoriz|forbidden|policy/.test(raw))
    return 'Your account isn’t allowed to save '+thing+'. It is safe on this device. Email '+SUPPORT_EMAIL+' and we’ll look at it.';
  if(/duplicate|conflict/.test(raw))
    return 'Something else changed '+thing+' at the same time. Refresh the page to pick up the latest version.';
  if(/payload|too large|quota|storage/.test(raw))
    return 'That’s too big to save to your account. It is still on this device — email '+SUPPORT_EMAIL+' if you need it synced.';
  return 'Couldn’t save '+thing+' to your account. It is still saved on this device. Try again in a moment, and email '+SUPPORT_EMAIL+' if it keeps happening.';
}
/** A file the browser wouldn't read (photos, covers, avatars). */
function friendlyFileError(err){
  const raw=((err&&err.message)||String(err||'')).toLowerCase();
  if(/too large|size|quota/.test(raw))return 'That image is too big — try one under about 5 MB, or take a smaller photo.';
  if(/type|format|decode|not an image/.test(raw))return 'We couldn’t read that image. JPEG, PNG and HEIC photos all work.';
  return 'We couldn’t use that image. Try a different one — JPEG or PNG works best.';
}
...
// Deleting a casting brief, a crowd-breakdown line, a brief photo or a day on
// the review page is just as destructive and just as easy to do by accident, so
// they all file an undo here too rather than growing a second mechanism: each
// records how to put its own work back, and the same Undo button in the status
// bar takes the top of the stack.
const CROWD_UNDO=[];
function registerUndo(restore,label){
  CROWD_UNDO.push({restore,label});
  if(CROWD_UNDO.length>50)CROWD_UNDO.shift();
}
function registerCrowdUndo(snapshot,label){
  registerUndo(()=>{SCED=JSON.parse(snapshot);saveSced();refreshAll();},label);
}
function crowdUndo(){
  const u=CROWD_UNDO.pop();
  if(!u)return false;
  try{u.restore()}catch(_){return false;}
...
function updateCrumbs(){
  ...
  if(DASH){
    ...
    else if(DASH_PAGE==='rates')seg.push('<span class="sep">›</span><span class="cur">Rate cards</span>');
...
// ---------- keeping the AD's place on the board ----------
// Every commit — a crowd edit, a split, an undo — goes through refreshAll(),
// which re-renders the whole day board by replacing #viewDays' innerHTML. That
// collapses the container to nothing for an instant, the browser clamps the
// scroll position to zero, and on a 60-day board the AD is thrown back to the
// top after every single edit. So: note which day card the user is looking at
// and exactly where on screen its top edge sits, then put it back there after
// the re-render. Anchoring to the CARD (not to a raw scrollY) is what makes it
// hold when the re-render changes heights above the fold — a day gaining a
// warning pill, the past-days drawer, the weather strip arriving late.
function captureBoardAnchor(){
  const host=document.getElementById('viewDays');
  if(!host||host.classList.contains('hidden')||!host.offsetParent)return null;
  const y=window.scrollY||window.pageYOffset||0;
  for(const card of host.querySelectorAll('.daycard')){
    const r=card.getBoundingClientRect();
    if(r.bottom>0)return {id:card.id,top:r.top,y};   // first card still on screen
  }
  return {id:null,top:0,y};
}
function restoreBoardAnchor(a){
  if(!a)return;
  // `behavior:'instant'` matters: globals.css sets html{scroll-behavior:smooth},
  // so a plain scrollTo would ANIMATE the restore and the board would visibly
  // glide after every keystroke-sized edit. Putting it back is not a journey.
  const to=y=>window.scrollTo({top:Math.max(0,y),left:0,behavior:'instant'});
  const put=()=>{
    if(a.id){
      const el=document.getElementById(a.id);
      if(el){to((window.scrollY||window.pageYOffset||0)+el.getBoundingClientRect().top-a.top);return}
    }
    to(a.y);
  };
  put();
  // images, the weather strip and the past-days drawer settle a frame later
  requestAnimationFrame(put);
}
function refreshAll(){
  const anchor=captureBoardAnchor();
  computeCosts();computeCrowdCosts();renderSummary();renderDays();renderStunts();renderCrowd();if(!$('#viewDoods').classList.contains('hidden'))renderDoods();renderCalendar();renderCast();renderFreeCalc();if(!$('#viewCbdoc').classList.contains('hidden'))renderCbDoc();
  if($('#viewBriefs')&&!$('#viewBriefs').classList.contains('hidden'))renderBriefs();
...
function saveUserSources(){
  store.set('crowdos-sources',JSON.stringify(SOURCES.filter(s=>s.kind).map(s=>({uid:s.uid||null,kind:s.kind,title:s.title,short:s.short,unit:s.unit||'Main',text:s.text||null,prod:s.prod||null,version:s.version||null,schedDate:s.schedDate||null,colour:s.colour||null,format:s.format||null,rateCard:s.rateCard||null,current:!!s.current,createdAt:s.createdAt||null,aiModel:s.aiModel||null,docKind:s.docKind||null,ns:s.ns||null,cloudId:s.cloudId||null,pdfFiles:s.pdfFiles||null,revDiff:storedRevDiffFor(s)}))));
}
// ONE copy of the stored diff per source in localStorage. `aiModel` already
// carries `_revDiff`, so writing a separate `revDiff` field beside it wrote the
// whole (deliberately uncapped) account of the revision twice, on top of each
// source's full text — hundreds of kilobytes doubled on a big reshuffle, which
// is what was pushing people into the storage-full banner. Only schedules with
// no aiModel (regex-parsed ones) need the standalone field.
function storedRevDiffFor(s){
  if(s&&s.aiModel&&typeof s.aiModel==='object'&&s.aiModel._revDiff)return null;
  return (s&&s.revDiff)||null;
}
// Hand-added shoot days used to be filed under the schedule's TITLE, which
// comes from the uploaded filename and is routinely duplicated — so one
// schedule's hand-built days overwrote another's and turned up on the wrong
// schedule. They are filed under the schedule's own id now. Entries still
// under an old title key are left alone unless the schedule they belong to is
// loaded (in which case its days have already been read back and re-filed),
// so upgrading loses nothing.
function saveManualDays(){
  let map={};
  try{map=JSON.parse(store.get('crowdos-manualdays')||'{}')}catch(e){map={}}
  const loaded=new Set();
  for(const s of SOURCES){loaded.add(srcUid(s));if(s.title)loaded.add(s.title)}
  // rebuild everything filed the new way; keep old title-filed entries only
  // while their schedule isn't loaded (once it is, its days have been read
  // back and are about to be re-filed under its own id)
  for(const k of Object.keys(map))if(loaded.has(k)||/^[sc]:/.test(k))delete map[k];
  for(const s of SOURCES){
    const md=s.model.days.filter(d=>d.manual);
    if(md.length)map[srcUid(s)]=md.map(d=>({num:d.num,date:d.date,loc:d.loc,hours:d.hours,type:d.type,unit:d.unit,scenes:(d.scenes||[]).map(sc=>({num:sc.num,part:sc.part,ie:sc.ie,tod:sc.tod,scriptDay:sc.scriptDay,pages:sc.pages,slug:sc.slug,desc:sc.desc}))}));
  }
...
function restoreManualDays(s){
  ...
  // new id-based key first, then the old title key for anything saved before
  const recs=map[srcUid(s)]||map[s.title]||[];
...
// ---------- what can go wrong with a PDF, said in advance ----------
// The three failures a first upload actually hits are a scan with no text in
// it, a password-protected file, and a file too big for the reader. Each used
// to surface as a raw pdf.js or HTTP string ("No password given", "AI read
// failed (413)"). Each is now named, and each says what to do instead.
class ScannedPdfError extends Error{constructor(){super('scanned');this.name='ScannedPdfError'}}
class LockedPdfError extends Error{constructor(){super('locked');this.name='LockedPdfError'}}
async function pdfToText(buf){
  ...
  const task=pdfjsLib.getDocument({data:buf,cMapUrl:'/pdfjs/cmaps/',cMapPacked:true,standardFontDataUrl:'/pdfjs/standard_fonts/'});
  // Without an onPassword handler pdf.js rejects with its own bare string
  // ("No password given"), which is what the user was being shown. Refusing
  // the prompt here turns it into a rejection we can name properly. (We do not
  // ask for the password: a PDF that needs one on a shared production is
  // exactly the kind of thing not to start collecting.)
  try{task.onPassword=()=>{throw new LockedPdfError()}}catch(e){}
  let doc;
  try{doc=await task.promise}
  catch(err){
    const nm=String((err&&err.name)||'')+' '+String((err&&err.message)||'');
    if(err instanceof LockedPdfError||/password|passwordexception|encrypted|\blocked\b/i.test(nm))throw new LockedPdfError();
    throw err;
  }
  const out=[];
  ...
  const text=out.join('\n');
  // A scan (or a photographed PDF) has pages but no text layer at all. There
  // is nothing to parse and nothing worth sending to the reader — but the
  // photo route DOES work on exactly this document, so say so rather than
  // reporting an empty read.
...
/**
 * What went wrong reading a schedule, said so the person holding the PDF knows
 * what to do next. The reader used to surface "AI read failed (504)" — which
 * told an AD nothing, least of all that the answer is "try a smaller file".
 */
function readErrorFor(status,serverMsg){
  if(status===413)return 'that file is too big for the reader. Split it — one unit or one block at a time — and import the parts separately.';
  if(status===504||status===408)return 'the schedule took too long to read. It is usually a very long document: try importing one unit or one block at a time.';
  if(status===429)return 'the reader is busy. Wait a minute and try again.';
  if(status===401)return 'your sign-in has expired. Refresh the page, sign in again and re-import.';
  // 403 is NOT an expired sign-in — it is this production's own "no AI" switch,
  // enforced on the server as well as in the browser (see the RESPONSE CONTRACT
  // at the top of app/api/parse-schedule/route.ts). Telling the user to sign in
  // again sends them off to fix something that is not broken, and the schedule
  // text still never leaves the app.
  if(status===403)return 'AI reading is switched off for this production, so the schedule was not sent anywhere. Turn it on in Production Settings → General if you want it read by AI, or import it with the built-in parser instead.';
  if(status>=500)return 'the reader had a problem at our end. Try again in a minute — and email '+SUPPORT_EMAIL+' if it keeps happening.';
  // the route's own message is written for users, so it is preferred when present
  if(serverMsg)return String(serverMsg);
  return 'the reader could not make sense of it. Email it to '+SUPPORT_EMAIL+' and we will look at it.';
}
...
async function aiParse(text,prod,images,feedback){
  ...
  // NAME THE PRODUCTION. The privacy policy promises that a production marked
  // "no AI" never has its schedule sent to Anthropic, and that promise cannot
  // rest on a boolean in the browser alone. The route re-reads prods.no_ai with
  // the caller's own JWT and refuses with 403 before any schedule text leaves
  // the server — but only if we tell it which production this is. The name must
  // match prods.name exactly as stored, which is what `prod` is.
  const res=await fetch('/api/parse-schedule',{method:'POST',headers:{'Content-Type':'application/json',...(tok?{Authorization:'Bearer '+tok}:{})},body:JSON.stringify({text,glossary:glossaryFor(prod),...(prod?{prodName:String(prod)}:{}),...(images&&images.length?{images}:{}),...(feedback?{feedback}:{})})});
  let data={};try{data=await res.json()}catch(e){}
  if(!res.ok||!data.model)throw new Error(readErrorFor(res.status,data.error));
  return {model:data.model,questions:data.questions||[],read:readReport(data)};
}
/**
 * What the reader actually managed to read.
 *
 * A 200 does NOT mean the whole schedule came back. A big schedule can run out
 * of the route's time budget part-way through, and rather than throw away the
 * days it DID read the route returns them with the shortfall flagged (see the
 * RESPONSE CONTRACT at the top of app/api/parse-schedule/route.ts). Dropping
 * those fields on the floor is how a producer ends up budgeting a hundred-day
 * shoot off thirty days without ever being told — worse than the read failing
 * outright. Returns null when the read was complete.
 */
function readReport(data){
  const partial=data.status==='partial'||!!data.partial;
  const truncated=!!data.truncated;
  if(!partial&&!truncated)return null;
  const days=+data.readDays||0, got=+data.chunksRead||0, all=+data.totalChunks||0;
  const lines=[];
  if(partial){
    const stated=String(data.partialMessage||'').trim();
    lines.push(stated||('Only part of this schedule could be read'+
      (days?' — '+days+' shoot day'+(days===1?'':'s')+' came back':'')+
      (got&&all?' ('+got+' of '+all+' sections)':'')+
      '. The rest of the schedule is missing.'));
  }
  if(truncated)
    lines.push('At least one section was too long for the reader to finish, so some scenes inside the days it did read may be missing as well.');
  return {partial,truncated,days,chunksRead:got,totalChunks:all,lines};
}
/** The same thing as one plain sentence, for the places with no room for a
 *  panel (the status bar after a re-read). */
function readReportLine(rep){return rep?rep.lines.join(' '):''}
...
async function handleImages(files){
  try{
    ...
    openImportConfirm({m,text:'',title,aiModel:r.model,mergeStats:null,isDetail:!m.days.some(d=>d.date),docKind:'photo',questions:r.questions,reads:[r.read],filesLabel:[...files].map(f=>f.name).join(' + '),images});
...
let AI_TIMER=null;
// The overlay is appended to <body>, outside the dialog system, so nothing was
// making the page behind it unreachable: for the 60–120 seconds of an AI read
// the whole board (and any dialog the read was started from) was still
// tabbable and still clickable underneath the dim. Hold everything behind it
// inert for as long as it is up, and hand it the focus so the keyboard has
// somewhere legitimate to be.
let AI_INERT=[];
function setAiInert(on){
  if(on){
    const layout=document.querySelector('.layout');
    AI_INERT=/** @type {any[]} */([layout,...document.querySelectorAll(MODAL_SEL)]
      .filter(el=>el&&(el===layout||el.classList.contains('open'))));
    for(const el of AI_INERT){try{el.inert=true}catch(e){}}
  }else{
    for(const el of AI_INERT){
      // the modal system owns .layout's inertness while a dialog is open
      if(el.classList.contains('layout')&&MODAL_STACK.length)continue;
...
function aiBusy(on,msg){
  ...
  if(on){
    if(!ov){
      ...
      // a blocking progress screen, not a dialog — announce it, don't trap in it
...
// Offer the route that would actually work on this document: photograph the
// pages and let the reader look at them. That path already exists and handles
// scans fine — the user was simply never told about it.
function offerPhotoRoute(name){
  setStatus('“'+(name||'That PDF')+'” is a scan — there is no text inside it, only a picture of the page, so there is nothing to read off it. Photograph or screenshot the pages instead and upload those: the reader looks at images and it works well. Use + Add schedule → Upload a schedule and pick the photos.',{sticky:true});
}
async function handlePDFs(files){
  try{
    ...
    // Checked here, before anything is read: a file over the reader's limit
    // used to sail through extraction and come back as "AI read failed (413)".
    const tooBig=[...files].filter(f=>f.size>PDF_MAX_MB*1024*1024);
    if(tooBig.length){
      setStatus(tooBig.map(f=>'“'+f.name+'”').join(' and ')+' '+(tooBig.length===1?'is':'are')+' too big to read ('+tooBig.map(f=>Math.round(f.size/1048576)+' MB').join(', ')+'; the limit is '+PDF_MAX_MB+' MB). Split the schedule — one unit, or one block of the shoot at a time — and import the parts one after another. They will sit together under the same production.',{sticky:true});
      return;
    }
    const docs=[];
    for(const f of [...files]){
      ...
      let text;
      try{text=await pdfToText(buf)}
      catch(err){
        if(err instanceof ScannedPdfError){offerPhotoRoute(f.name);return}
        if(err instanceof LockedPdfError){
          setStatus('“'+f.name+'” is password-protected, so we can’t open it. Open it in a PDF reader with the password, save or print it as a new PDF without one, and upload that.',{sticky:true});
          return;
        }
        throw err;
      }
    ...
    // A schedule long enough that the reader has to work through it in chunks
    // can come back part-read. Warn BEFORE the read rather than presenting a
    // clipped document as a complete one.
    {
      const chars=docs.reduce((a,d)=>a+d.text.length,0);
      if(chars>420000)setStatus('That is a very long schedule ('+Math.round(chars/1000)+'k characters). The reader may not get all the way through it in one go — check the shoot-day count on the review screen against the schedule before you publish, and import it in parts if days are missing.',{timeout:12000});
    }
    ...
    const questions=[],qSeen=new Set(),reads=[];
    ...
    if(spineDoc){
      ...
      if(!m.days.length){
        ...
        try{const r=await aiParse(spineDoc.text,CURPROD);aiModel=r.model;addQs(r.questions);reads.push(r.read);}
    ...
    if(detailDoc){
      ...
      try{const r=await aiParse(detailDoc.text,CURPROD);detailModel=r.model;addQs(r.questions);reads.push(r.read);}
      catch(err){aiBusy(false);setStatus('Couldn’t read the scene detail — '+err.message);return}
      aiBusy(false);
    }
    ...
    openImportConfirm({m:prepModel(JSON.parse(JSON.stringify(aiModel||m)),'Main'),text,title,aiModel,mergeStats,isDetail:!spineDoc,docKind,questions,reads,filesLabel:docs.map(d=>d.file.name).join(' + '),pdfFiles:docs.map(d=>d.file)});
  }catch(err){
    console.error(err);
    if(err instanceof ScannedPdfError){offerPhotoRoute('');return}
    if(err instanceof LockedPdfError){setStatus('That PDF is password-protected, so we can’t open it. Save a copy without the password and upload that.',{sticky:true});return}
    setStatus('We couldn’t open that PDF — '+((err&&err.message)||'the file may be damaged')+'. Try re-saving it from whatever produced it, or photograph the pages and upload those instead.');
  }
}
...
function openImportConfirm({m,text,title,aiModel,mergeStats,isDetail,docKind,questions,reads,filesLabel,images,pdfFiles}){
  if(!m.days.length&&!isDetail){
    // There is no "send it to us" mechanism in the app, so the old copy asked
    // for something impossible. This is the mechanism: a pre-addressed email
    // naming the file and what we found, which the user attaches the PDF to.
    const diag='File: '+(filesLabel||title||'(unnamed)')+'\nFirst lines we read:\n'+String(text||'').split('\n').slice(0,12).join('\n');
    setStatus('We couldn’t find any shoot days in that schedule — its layout isn’t one we recognise yet. Send it to us and we’ll teach the reader its format. In the meantime you can build the days by hand.',{sticky:true,action:{
      label:'Email it to us',
      fn:()=>{try{window.open(supportHref(PRODUCT+' — a schedule layout you don’t read yet','Please attach the PDF to this email and we will teach the reader its layout.\n\n'+diag),'_blank')}catch(e){}},
    }});
    return;
  }
  const guess=guessImportMeta(text||'',title);
  PENDING_IMPORT={text:text||'',title,aiModel,mergeStats,isDetail,docKind,questions,reads:reads||null,files:filesLabel,images:images||null,pdfFiles:pdfFiles||null};
  ...
  showReadWarning(reads,m.days.length);
  updateMergeRow();
  $('#impModal').classList.add('open');
  setStatus('');
}
/**
 * The panel that says the schedule on screen is NOT all of it.
 *
 * It sits directly under the "N shoot days · N scenes found" line, because
 * that line is the thing it contradicts: on a partial read those days are
 * whatever the reader got to before it ran out of time, and importing them as
 * though they were the whole schedule is how a budget ends up short by two
 * thirds without anybody noticing.
 */
function showReadWarning(reads,dayCount){
  const host=$('#impWarn');
  if(!host)return;
  const list=(reads||[]).filter(Boolean);
  if(!list.length){host.className='imp-warn hidden';host.innerHTML='';return}
  const partial=list.some(r=>r.partial);
  const lines=[...new Set(list.flatMap(r=>r.lines))];
  host.className='imp-warn'+(partial?'':' imp-warn-soft');
  host.innerHTML=`<div class="imp-warn-hd">${icon('warn')} <b>${partial?'This is only part of the schedule':'Some scenes may be missing'}</b></div>`+
    lines.map(l=>`<p>${esc(l)}</p>`).join('')+
    (partial?`<p>You can still import ${dayCount===1?'this day':'these '+dayCount+' days'} — but treat the day count and every total that comes from it as incomplete until the rest is read. Importing the file again, or splitting it into two smaller files, usually gets the rest.</p>`:'');
}
...
function openEditMeta(i){
  ...
  showReadWarning(null,s.model.days.length);
...
$('#impGo').addEventListener('click',()=>{
  if(IMP_EDIT!=null){
    ...
    if(s){
      ...
      if(CLOUD.session&&s.cloudId)cloud.updateProduction(s.cloudId,s).then(r=>{
        if(r.error)setStatus(friendlyCloudError(r.error,'that change'));
      });
  ...
  if(tgt){
    ...
    let next=prepModel(JSON.parse(JSON.stringify(r.model)),s.unit||'Main');
    // a merge replaces the model the board is showing, so it diffs and carries
    // exactly like a new revision does
    const mergeCtx={mode:'merge',prod:s.prod||s.title,unit:s.unit||'Main',version:revLabel(s),docKind:'merged',files:P.files||P.title,questions:P.questions||[],text:P.text,rawModel:r.model,viewer:{pdfs:P.pdfFiles,images:P.images}};
    const mergeCarry=planRevisionChange({prevSrc:s,raw:r.model,model:next,unit:s.unit||'Main',prod:s.prod||s.title,ctx:mergeCtx});
    next=mergeCarry.model;
    openReview({
      ...
      onAccept(){
        ...
        restoreManualDays(s);
        const res=commitRevisionChange(mergeCarry,s);
        if(res.n)refreshAll();
        saveUserSources();
        ...
        if(CLOUD.session&&s.cloudId)cloud.updateProduction(s.cloudId,s).then(x=>{if(x.error)setStatus(friendlyCloudError(x.error,'that change'))});
        ...
        const lead='Full Fat detail merged — '+r.stats.matched+' of '+r.stats.spineScenes+' scenes enriched';
        if(res.collisions.length||res.n)reportCarryResult(res,mergeCarry.stitched,lead);
        else setStatus(lead+'.');
      },
    });
    PENDING_IMPORT=null;
  ...
  // The previous revision does NOT have to be an upload: a hand-built one is a
  // normal way to prep, and skipping the diff there meant no carry-over at all
  // and a review page claiming there was nothing to compare against.
  const revs=!isNew&&unitsOf(prod).get(unit);
  const prevCur=revs&&revs.length?currentRev(revs):null;
  const prevSrc=prevCur&&prevCur.s&&(prevCur.s.model&&(prevCur.s.model.days||[]).length)?prevCur.s:null;
  let carry=null;
  if(prevSrc){
    carry=planRevisionChange({prevSrc,raw,model:m,unit,prod,ctx,carryCast:true});
    m=carry.model;
  }
  const commit=()=>{
    ...
    // …and the account of what changed rides on `raw` too, so it survives a
    // reload and reaches every device — store `raw` whenever there is one
    const storeAi=(P.aiModel||ctx.edited||ctx.stitched||ctx.castCarried||(carry&&carry.stored))?raw:null;
    if(addSource(m,P.title,P.title.slice(0,18),true,{kind:'pdf',text:P.text,unit,prod,version:ctx.version,schedDate,colour,format,rateCard,aiModel:storeAi,docKind:P.docKind||null})){
      ...
      // onto the new revision so restoreManualDays picks them up
      if(prevSrc&&prevSrc!==src){
        try{
          const mdMap=JSON.parse(store.get('crowdos-manualdays')||'{}');
          const from=(mdMap[srcUid(prevSrc)]||mdMap[prevSrc.title]||[]);
          if(from.length&&!(mdMap[srcUid(src)]||[]).length){
            mdMap[srcUid(src)]=JSON.parse(JSON.stringify(from));
            store.set('crowdos-manualdays',JSON.stringify(mdMap));
          }
        }catch(e){}
      }
      restoreManualDays(src);
      const res=commitRevisionChange(carry,src);
      const carriedN=res.n;
...
function syncRecheckBtn(){
  ...
  btn.dataset.tip=ok?'Ask '+PRODUCT+' to read this schedule again — e.g. if it missed the cast numbers'
...
function openRecheckPop(){
  ...
  p.innerHTML=`
    <div class="rc-h">Ask ${PRODUCT} to re-check this schedule</div>
...
async function runRecheck(note){
  ...
  try{
    ...
    aiBusy(false);
    let nextModel=prepModel(JSON.parse(JSON.stringify(r.model)),unit);
    if(!nextModel.days.length){setStatus('The re-read found no shoot days — try rewording your note, or re-import the file.');return}
    // A re-read REPLACES the schedule on the board, so a partial one would
    // quietly delete the days it never got to. Say so before the review page.
    if(r.read)setStatus('Careful — the re-read did not get through the whole schedule. '+readReportLine(r.read)+' Publishing it would replace what is on the board with only the days listed on the review page.',{sticky:true});
    ...
    // a re-read replaces the model the board is showing, so it diffs and
    // carries exactly like a new revision does — including stitching back the
    // already-shot days, which the original document no longer contains
    const rcCtx={mode:'recheck',prod,unit,version:revLabel(s),docKind:s.docKind||null,
      files:(s.prod||s.title)+' — re-check',questions:r.questions||[],text,rawModel:r.model,viewer};
    const rcCarry=planRevisionChange({prevSrc:s,raw:r.model,model:nextModel,unit,prod,ctx:rcCtx});
    nextModel=rcCarry.model;
    openReview({
      prev:s,nextModel,mergeStats:null,
      ctx:rcCtx,
      onAccept(){
        // the review page can sit open for minutes; in that time the schedule
        // can be deleted, or rebuilt from the account on another device's
        // change. Writing to the old copy would look like it worked and save
        // nowhere at all, so say plainly that nothing was changed.
        const idx=SOURCES.indexOf(s);
        if(idx<0){
          setStatus('This schedule was removed or replaced while the re-check was open, so nothing has been changed. Open the schedule again and run the re-check once more.',{sticky:true});
          return;
        }
        s.aiModel=r.model;
        s.model=modelFrom(s,unit);
        restoreManualDays(s);
        const res=commitRevisionChange(rcCarry,s);
        saveUserSources();
        if(CLOUD.session&&s.cloudId)cloud.updateProduction(s.cloudId,s).then(x=>{if(x.error)setStatus(friendlyCloudError(x.error,'that change'))});
        if(!DASH&&ACTIVE===idx)setActive(idx);else{renderSidebar();if(DASH)renderDash();}
        logProdEvent(s.prod||s.title,'schedule','Schedule re-checked with AI — '+nextModel.days.length+' days · note: '+note.slice(0,120)+(res.n?' · '+res.n+' edits carried over':'')+(res.collisions.length?' · '+res.collisions.length+' could not be moved':''));
        if(res.collisions.length||res.n)reportCarryResult(res,rcCarry.stitched,'Schedule re-checked and updated');
        else setStatus('Schedule re-checked and updated.');
      },
    });
...
// wipe every cached original on this device — used when signing out, so the
// next person on a shared production-office computer can't open the previous
// production's call sheets and schedules. Signed-in users keep their originals
// in their own private account storage, so nothing is actually lost.
function idbClearAll(){return idbOpen().then(db=>new Promise((res,rej)=>{const tx=db.transaction(IDB_STORE,'readwrite');tx.objectStore(IDB_STORE).clear();tx.oncomplete=()=>res();tx.onerror=()=>rej(tx.error);}))}
...
async function addSourceFiles(src,files){
  ...
  // The manifest is built from what the schedule ALREADY lists, so the names
  // of files uploaded earlier survive — a second device reads this list, and
  // rebuilding it from scratch dropped everything before this batch.
  const known=src.pdfFiles||[];
  src.pdfFiles=merged.map((f,i)=>(known[i]&&known[i].name===f.name)?known[i]:{name:f.name,type:f.type});
  renderSidebar();syncBoardPdfBtn();
  await uploadSourceFilesToCloud(src,files);
}
// Originals still to reach account storage. A failed upload used to be
// swallowed while `pdfFiles` was set anyway, so the button lit up and the user
// was told everything was saved — with the confidential PDFs living in this
// browser alone, and wiped at the next sign-out.
const FILE_SAVE_PENDING=new Map(); // schedule uid → {src,files}
function retryFileUploads(){
  for(const [uid,p] of [...FILE_SAVE_PENDING]){
    FILE_SAVE_PENDING.delete(uid);
    if(!p.src||!SOURCES.includes(p.src)||!p.src.cloudId){SYNC_FAIL.files.delete(uid);continue}
    uploadSourceFilesToCloud(p.src,p.files);
  }
...
async function uploadSourceFilesToCloud(src,files){
  ...
  const owner=(CLOUD.session.user&&CLOUD.session.user.id)||'u';
  const man=(src.pdfFiles||[]).slice();
  const base=man.length;
  const failed=[];
  for(let i=0;i<files.length;i++){
    const f=files[i],path=owner+'/'+src.cloudId+'/'+(base+i)+'_'+safeName(f.name);
    let err=null;
    try{const r=await cloud.uploadScheduleFile(path,f.blob,f.type);err=r&&r.error}
    catch(e){err=e}
    // keep the file's NAME in the manifest either way; only a landed upload
    // gets a path, so a retry knows exactly what is still outstanding
    const at=man.findIndex(m=>m&&m.name===f.name&&!m.path);
    const rec=err?{name:f.name,type:f.type}:{name:f.name,type:f.type,path};
    if(at>=0)man[at]=rec;else man.push(rec);
    if(err){console.error('cloud original upload failed',err);failed.push(f)}
  }
  src.pdfFiles=man;
  let saveErr=null;
  try{const r=await cloud.updateProduction(src.cloudId,src);saveErr=r&&r.error}
  catch(e){saveErr=e}
  if(saveErr)console.error('manifest save failed',saveErr);
  const uid=srcUid(src);
  const outstanding=failed.length?failed:(saveErr?files:[]);
  if(outstanding.length){
    FILE_SAVE_PENDING.set(uid,{src,files:outstanding});
    SYNC_FAIL.files.add(uid);
    scheduleSyncRetry('files');
  }else{
    FILE_SAVE_PENDING.delete(uid);SYNC_FAIL.files.delete(uid);SYNC_RETRY.files.delay=0;
  }
  refreshUnsavedWarning();
}
...
function scGroupsOf(s){
  ...
  if((s.sa||0)>0)out.push({tier:'SA',featured:false,name:'',count:s.sa,note:'',sup:0,fromAbove:false,roleId:''});
  for(const g of s.saChars||[])if((g.count||0)>0)out.push({tier:'SA',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),roleId:g.roleId||''});
  for(const g of s.featured||[])if((g.count||0)>0)out.push({tier:'SA',featured:true,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),roleId:g.roleId||''});
  for(const g of s.spacts||[])if((g.count||0)>0)out.push({tier:'SPACT',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),roleId:g.roleId||''});
  return out;
}
// the editable char-rows for a scene, in the shape SCED / the inline editor use
function sceneCrowdRows(s){
  return scGroupsOf(s).map(g=>({name:g.name,count:g.count,tier:g.tier,featured:g.featured,note:g.note||'',sup:+g.sup||0,fromAbove:!!g.fromAbove,roleId:g.roleId||''}));
}
...
function writeSceneCrowd(d,idx,rows){
  ...
  e.chars=rows.filter(r=>(+r.count||0)>0)
    .map(r=>{
      ...
      // the named role this group is engaged on ("Stand-in"), if any — it is
      // the group's deal, so it survives a recompute and travels with the
      // group when it is moved to another scene
      if((r.roleId||'').trim())o.roleId=String(r.roleId).trim();
      return o;
    });
...
// Same people = same tier, same Featured flag, same name AND the same named
// role: "Commuters" on the Stand-in role and "Commuters" on the SA rate are two
// different bookings paid two different amounts, so they must never merge.
function sameGroup(r,g){return r.tier===g.tier&&(!!r.featured)===(!!g.featured)&&(r.name||'')===(g.name||'')&&(r.roleId||'')===(g.roleId||'');}
...
function mergeSameInto(rows,g){
  ...
  if(j>=0){
    const r=rows[j];
    r.count=Math.max(r.count,g.count);
    // the group's supplementary fee and continuity note travel WITH it — if
    // the scene it lands in has none of its own, carry them across rather than
    // dropping them (a lost supplementary is money missing from the budget)
    if(!(r.note||'').trim()&&(g.note||'').trim())r.note=String(g.note).trim();
    if(!(+r.sup>0)&&+g.sup>0)r.sup=+g.sup;
    if(g.fromAbove)r.fromAbove=true;
  }else{
    const o={name:g.name,count:g.count,tier:g.tier,featured:!!g.featured};
    if((g.note||'').trim())o.note=String(g.note).trim();
    if(+g.sup>0)o.sup=+g.sup;
    if(g.fromAbove)o.fromAbove=true;
    if((g.roleId||'').trim())o.roleId=String(g.roleId).trim();
    rows.push(o);
  }
}
...
function askSamePeople(group,sceneLabel){
  return new Promise(resolve=>{
    ...
    wrap.innerHTML=`<div class="samenew" role="dialog" aria-modal="true" aria-labelledby="samenewH">
      <div class="samenew-h" id="samenewH">Add <b>${esc(crowdGroupLabel(group))}</b> to scene ${esc(sceneLabel)}</div>
...
/**
 * Put a crowd group on a named role (or back on its tier). The role is the
 * group's deal, so it is written on the group in the scene and follows it
 * everywhere — day board, costs, breakdown, DOODs.
 */
function setCrowdGroupRole(d,idx,g,roleId){
  const rows=sceneCrowdRows(d.scenes[idx]);
  const i=findRowIdx(rows,g);
  if(i<0)return false;
  if((rows[i].roleId||'')===(roleId||''))return false;
  const snap=JSON.stringify(SCED);
  rows[i].roleId=roleId||'';
  // A role group is always a NAMED group: plain background is stored as a bare
  // head count with nowhere to carry a role, and an unnamed one could not be
  // told apart from the crowd it is standing in for.
  if(roleId&&!(rows[i].name||'').trim()){
    const r=roleById(roleId);
    if(r)rows[i].name=roleLabel(r);
  }
  registerCrowdUndo(snap,crowdGroupLabel(g)+(roleId?' put on a role':' put back on its rate'));
  writeSceneCrowd(d,idx,rows);
  saveSced();applySced(MODEL);refreshAll();
  return true;
}
function openCrowdGroupMenu(x,y,ctx){
  ...
  const items=[];
  // Only offered once the production has named a role — with none defined this
  // menu is exactly what it always was.
  const roles=activeRoles();
  if(roles.length&&!g.fromAbove){
    items.push({label:'Paid as…',icon:'pencil',onClick:()=>{
      openCtxMenu(x,y,[
        {label:(g.tier==='SPACT'?'SPACT':g.featured?'Featured':'SA')+(g.roleId?'':'  ✓'),onClick:()=>{
          if(setCrowdGroupRole(d,idx,g,''))setStatus(crowdGroupLabel(g)+' is back on the '+(g.tier==='SPACT'?'SPACT':g.featured?'Featured':'SA')+' rate.',{undo:crowdUndo});
        }},
        ...roles.map(r=>({label:roleLabel(r)+' — '+gbp(r.day)+'/day'+(g.roleId===r.id?'  ✓':''),onClick:()=>{
          if(setCrowdGroupRole(d,idx,g,r.id))setStatus(crowdGroupLabel(g)+' is now paid as '+roleLabel(r)+' — '+gbp(r.day)+'/day.',{undo:crowdUndo});
        }})),
      ]);
    }});
    items.push({sep:true});
...
function rpChangesHtml(prev,nextModel,mergeStats,carry){
  let html='';
  // NOTHING here is truncated any more. A revision that renumbers 30 days used
  // to show 8 and silently hide 22. The first few are shown flat; the rest sit
  // one click away in a disclosure that says exactly how many are inside.
  const SHOWN=6;
  const cap=(arr,f,what)=>{
    if(!arr.length)return '';
    const head=arr.slice(0,SHOWN).map(f).join('');
    if(arr.length<=SHOWN)return head;
    return head+`<details class="revmore"><summary>Show the other ${arr.length-SHOWN} ${esc(what||'entries')}</summary>${arr.slice(SHOWN).map(f).join('')}</details>`;
  };
  // A long list of bare scene numbers reads better as one sentence than as
  // rows — but it still shows every single one.
  const nameList=(xs,what)=>{
    if(!xs.length)return '';
    if(xs.length<=12)return `<div class="revline none">${esc(what)}: ${esc(xs.join(', '))}</div>`;
    return `<div class="revline none">${esc(what)}: ${esc(xs.slice(0,12).join(', '))}</div>`
      +`<details class="revmore"><summary>Show the other ${xs.length-12}</summary><div class="revline none">${esc(xs.slice(12).join(', '))}</div></details>`;
  };
  ...
  if(carry){
    ...
    if(D.shotDays.length||D.collisions.length){
      html+=`<div class="rev-sec info"><h4>Already shot — kept<span>${D.shotDays.length}</span></h4>
        ...
        ${cap(fresh,dayLine,'days shot since then')}
    ...
    html+=`<div class="rev-sec ok"><h4>New shoot days<span>${D.addedDays.length}</span></h4>${D.addedDays.length?cap(D.addedDays,dayLine,'new days'):'<div class="revline none">No new shoot days.</div>'}</div>`;
    html+=`<div class="rev-sec bad"><h4>Days cut<span>${D.cutDays.length}</span></h4>${D.cutDays.length?cap(D.cutDays,dayLine,'dropped days'):'<div class="revline none">No upcoming days dropped.</div>'}</div>`;
    html+=`<div class="rev-sec warn"><h4>Days renumbered / moved<span>${reshaped.length+D.supersededDays.length}</span></h4>${reshaped.length?cap(reshaped,shapeLine,'renumbered or moved days'):'<div class="revline none">No days moved.</div>'}
    ...
    html+=`<div class="rev-sec warn"><h4>Scene changes<span>${D.scenes.moved.length+D.scenes.added.length+D.scenes.cut.length}</span></h4>
      ...
      ${cap(D.scenes.moved,sceneLine,'scenes that moved day')}
      ${nameList(D.scenes.added.map(s=>s.key),'Added')}
      ${nameList(D.scenes.cut.map(s=>s.key),'Cut')}
    </div>`;
    const carried=plan.dayMoves.length+plan.sceneMoves.length;
    const strandLine=x=>`<div class="revline"><b>${esc(x.scene||x.from)}</b> ${esc(x.label)} — ${esc(x.scene?'scene not in the new schedule':(x.reason||'day dissolved'))}; stays on ${esc(revLabel(prev))}</div>`;
    html+=`<div class="rev-sec ${plan.dayStranded.length+plan.sceneStranded.length?'warn':'ok'}"><h4>Your work<span>${carried}</span></h4>
      ...
      ${cap([...plan.sceneStranded,...plan.dayStranded],strandLine,'pieces of work that stay put')}
      ${(()=>{
        // A day calculator carries its frozen crowd numbers, its call and wrap
        // (worked out from the OLD day's hours) and its travel band. Where the
        // day has changed underneath them, say so before anyone publishes and
        // reads those figures as confirmed.
        const need=Object.keys(plan.review||{}).length;
        if(!need)return '';
        return `<div class="revline none">${need} day${need===1?'':'s'} will keep settings you saved against the old schedule (crowd numbers, call and wrap, travel band) even though the day itself has changed — each one will be marked for checking after you publish.</div>`;
      })()}
    </div>`;
    // THE FULL ACCOUNT. Everything above is a summary; this is the complete,
    // uncapped, day-by-day list — the same words the board's "What changed"
    // report shows after publishing, so the two never disagree. It can be
    // copied straight into an email, which is what the crowd department
    // actually does with it.
    {
      const full=describeRevision(D);
      const nLines=full.reduce((a,g)=>a+g.lines.length,0);
      html+=`<div class="rev-sec"><h4>Every change, in words<span>${nLines}</span></h4>
        <div class="revline">Grouped by day, in schedule order. Nothing here is shortened — this is the whole account.</div>
        <details class="revmore"><summary>Read all ${nLines} change${nLines===1?'':'s'}</summary>
          <div class="chg-report">${full.map(g=>`<section class="chg-grp"><h4><span>${esc(g.label)}</span></h4><ul>${g.lines.map(l=>`<li>${esc(l)}</li>`).join('')}</ul></section>`).join('')}</div>
        </details>
        <div style="margin-top:9px"><button type="button" class="chg-btn" data-copychanges>Copy the whole list as text</button></div>
      </div>`;
    }
    ...
    html+=`<div class="rp-sec-title">Changes vs ${esc(revLabel(prev))} (current revision)</div>`;
    html+=`<div class="rev-sec ok"><h4>Additions<span>${adds.length}</span></h4>${adds.length?cap(adds,dayLine,'new days'):'<div class="revline none">No new shoot days.</div>'}</div>`;
    html+=`<div class="rev-sec bad"><h4>Deletions<span>${dels.length}</span></h4>${dels.length?cap(dels,dayLine,'dropped days'):'<div class="revline none">No dropped shoot days.</div>'}</div>`;
    html+=`<div class="rev-sec warn"><h4>Date shifts<span>${shifts.length}</span></h4>${shifts.length?cap(shifts,shiftLine,'date shifts'):'<div class="revline none">No days moved.</div>'}</div>`;
  }else{
    // Only reached when there genuinely is no earlier schedule to compare
    // against — a hand-built previous revision now diffs like any other.
    html+=`<div class="rp-sec-title">Changes</div><div class="rev-sec"><div class="revline none">${prev?'The schedule this replaces has no shoot days in it yet, so there is nothing to compare against.':'This is the first schedule for this unit, so there is nothing to compare it against yet.'}</div></div>`;
  }
  if(mergeStats){
    const ms=mergeStats;
    html+=`<div class="rev-sec info"><h4>Full Fat detail</h4>
      ...
      ${nameList(ms.unmatchedSpine,"No detail found for these scenes — the one-liner's counts were kept")}
      ${nameList(ms.unmatchedDetail,'Detail scenes not on any shoot day')}
    </div>`;
  }
  const mode=modeWord();
  ...
  html+=`<div class="rev-sec"><h4>${mode==='crowd'?'Crowd':'Stunt'} budget</h4>
    ${prev?`<div class="revrow"><span>Current total</span><b>${gbpWhole(before)}</b></div>`:''}
    <div class="revrow"><span>${prev?'New proposed':'Projected total'}</span><b>${gbpWhole(after)}</b></div>
...
// "Copy the whole list as text" — revisionChangeLines is the flat, ordered,
// uncapped sentence list; it pastes straight into an email or a call sheet.
document.addEventListener('click',e=>{
  const b=/** @type {any} */(e.target).closest&&/** @type {any} */(e.target).closest('[data-copychanges]');
  if(!b)return;
  const D=REV_CTX&&REV_CTX.carry&&REV_CTX.carry.diff;
  if(!D){setStatus('There is no earlier schedule to compare this one against.');return}
  const txt=revisionChangeLines(D).join('\n');
  const done=()=>setStatus('The full list of changes is on your clipboard — paste it into an email or a call sheet.');
  if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(txt).then(done,()=>setStatus('Couldn’t reach the clipboard — select the list and copy it by hand.'));
  else setStatus('Couldn’t reach the clipboard — select the list and copy it by hand.');
});
...
function openReview({prev,nextModel,mergeStats,onAccept,ctx}){
  ...
  // day matches the reviewer has REJECTED — nothing carries onto these days
  const REPOINT_OFF=new Set();
  // the pairings as the engine found them, so "Undo" really does put one back
  const ORIG_MATCHES=((ctx.carry&&ctx.carry.diff&&ctx.carry.diff.matches)||[]).slice();
    ...
    aiParse(ctx.text,ctx.prod).then(r=>{
      ...
      crossSlot.innerHTML=diffs.length
        ?`<div class="rev-sec warn"><h4>AI cross-check<span>${diffs.length}</span></h4>
            ...
            ${diffs.slice(0,6).join('')}${diffs.length>6?`<details class="revmore"><summary>Show the other ${diffs.length-6} days</summary>${diffs.slice(6).join('')}</details>`:''}
  ...
  // ---- what the revision did to each row -------------------------------
  // An AD reviewing 45 rows cannot see which ones changed. Each row now says
  // so in its own words, and a weak match — the thing that silently drags a
  // whole day calculator onto the wrong day — can be rejected or re-pointed
  // BEFORE anything is published.
  const plainOf=d=>(d.unit||'Main')+'|'+d.num;
  const matchOf=d=>{
    const D=ctx.carry&&ctx.carry.diff;
    if(!D)return null;
    return D.matches.find(m=>(m.newDay.id&&d.id&&m.newDay.id===d.id)||plainOf(m.newDay)===plainOf(d))||null;
  };
  const isNewDay=d=>{
    const D=ctx.carry&&ctx.carry.diff;
    return !!D&&!matchOf(d)&&D.addedDays.some(x=>plainOf(x)===plainOf(d));
  };
  const changeWords=m=>{
    const out=[];
    if(m.renumbered)out.push(`Was D${m.oldDay.num}`);
    if(m.daysShifted)out.push(`${Math.abs(m.daysShifted)} day${Math.abs(m.daysShifted)===1?'':'s'} ${m.daysShifted>0?'later':'earlier'}`);
    else if(m.dateMoved)out.push('Date changed');
    if(m.locChanged)out.push('New location');
    if(m.typeChanged)out.push('Day type changed');
    if(m.hoursChanged)out.push('Hours changed');
    if(m.scenesGained.length)out.push(`+${m.scenesGained.length} scene${m.scenesGained.length===1?'':'s'}`);
    if(m.scenesLost.length)out.push(`−${m.scenesLost.length} scene${m.scenesLost.length===1?'':'s'}`);
    return out;
  };
  function matchCell(d){
    if(!(ctx.carry&&ctx.carry.diff))return '<td class="rp-match"><span class="rp-mnone">—</span></td>';
    if(REPOINT_OFF.has(plainOf(d)))
      return `<td class="rp-match rejected"><span class="rp-mtag gone">Treated as a new day</span>
        <div class="rp-mwhy">Nothing will be carried onto it.</div>
        <button type="button" class="chg-btn" data-rpundo="${esc(plainOf(d))}">Undo</button></td>`;
    const m=matchOf(d);
    if(!m)return isNewDay(d)
      ?'<td class="rp-match"><span class="rp-mtag new">New day</span></td>'
      :'<td class="rp-match"><span class="rp-mnone">Not compared</span></td>';
    const pct=Math.round((m.overlap||0)*100);
    const low=m.matchedBy!=='scenes'||pct<50;
    const words=changeWords(m);
    const conf=m.matchedBy!=='scenes'
      ?`Matched by ${esc(m.matchedBy)} — this day lists no scenes`
      :`${pct}% of the scenes are the same`;
    const prevDays=(ctx.carry.prevSrc&&ctx.carry.prevSrc.model.days)||[];
    return `<td class="rp-match${low?' low':''}">
      ${words.length?words.map(w=>`<span class="rp-mtag">${esc(w)}</span>`).join(''):`<span class="rp-mtag same">Unchanged</span>`}
      <div class="rp-mwhy">${low?icon('warn')+' ':''}Carried from D${esc(String(m.oldDay.num))} · ${conf}</div>
      ${low?`<div class="rp-mfix">
        <button type="button" class="chg-btn" data-rpreject="${esc(plainOf(d))}" data-tip="Don't carry anything from the old day onto this one">Not the same day</button>
        <label class="rp-mrepoint">Point at
          <select data-rprepoint="${esc(plainOf(d))}" aria-label="Choose which old day this one continues">
            <option value="">D${esc(String(m.oldDay.num))} (current)</option>
            ${prevDays.filter(x=>!x.manual).map(x=>`<option value="${esc(plainOf(x))}">D${esc(String(x.num))}${x.date?' — '+esc(String(x.date).slice(0,18)):''}</option>`).join('')}
          </select></label>
      </div>`:''}
    </td>`;
  }
  function renderTable(){
    const rows=nextModel.days.map(d=>{
      ...
      const m=matchOf(d);
      const touched=!!(ctx.carry&&ctx.carry.diff)&&!REPOINT_OFF.has(plainOf(d))&&(isNewDay(d)||(m&&changeWords(m).length>0));
      return `<tr data-dnum="${d.num}" class="${touched?'rp-changed':''}"><td class="dchip">D${d.num}</td>
        ...
        ${matchCell(d)}
        <td><button class="rp-delday" data-deldaynum="${d.num}" aria-label="Remove D${d.num} from this import" title="Delete this day — it won't be published">✕</button></td></tr>${crowdDrawer(d)}`;
    }).join('');
    const follow=ctx.viewer&&ctx.viewer.pdfs&&ctx.viewer.pdfs.length;
    const nChanged=(ctx.carry&&ctx.carry.diff)?nextModel.days.filter(d=>{const m=matchOf(d);return !REPOINT_OFF.has(plainOf(d))&&(isNewDay(d)||(m&&changeWords(m).length>0))}).length:0;
    $('#rpTable').innerHTML=`<div class="rp-sec-title">Day by day<span class="cnt">${nextModel.days.length} days${nChanged?` · ${nChanged} changed`:''} · ${follow?'click a row to find it in the original · ':''}click a date, location or crowd number to edit · ✕ removes a day</span></div>
      ${(ctx.carry&&ctx.carry.diff)?`<div class="rp-tblkey">Rows with an amber edge and a word in the last column changed since ${esc(revLabel(prev))}. A partial match is flagged there too — reject it or point it at the right day before you publish.</div>`:''}
      <div class="rp-tblwrap"><table class="rp-tbl${follow?' followable':''}"><thead><tr><th>Day</th><th>Date</th><th>Location</th><th class="num">Crowd</th><th class="num">Cast</th><th class="num">Scenes</th><th>Vs ${esc((prev&&revLabel(prev))||'previous')}</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }
  ...
  $('#rpTable').onchange=e=>{
    if(token!==REV_OPEN)return;
    const rp=/** @type {any} */(e.target).closest('[data-rprepoint]');
    if(rp){
      const v=rp.value;
      if(v)repointMatch(rp.dataset.rprepoint,v);
      return;
    }
  ...
  // Rebuild the carry plan, the money and the change list from the matches
  // actually in force, then repaint everything that reads them.
  function afterMatchChange(){
    ctx.edited=true;
    rebuildCarryPlan(ctx.carry);
    renderChanges();renderTable();
  }
  /** Drop a day match: the new day is then treated as a day of its own and no
   *  saved day settings follow onto it. */
  function rejectMatch(plain){
    const D=ctx.carry&&ctx.carry.diff;if(!D)return;
    const i=D.matches.findIndex(m=>(m.newDay.unit||'Main')+'|'+m.newDay.num===plain);
    if(i<0)return;
    REPOINT_OFF.add(plain);
    D.matches.splice(i,1);
    afterMatchChange();
  }
  /**
   * Point a new day at a DIFFERENT old day. The pairing fields are worked out
   * the same way the engine works them out (see buildDayMatch in
   * lib/engine/revise.ts) — this is the only place the board has to, because
   * the engine only ever pairs days automatically.
   */
  function repointMatch(plain,oldPlain){
    const D=ctx.carry&&ctx.carry.diff;if(!D)return;
    const prevDays=(ctx.carry.prevSrc&&ctx.carry.prevSrc.model.days)||[];
    const oldDay=prevDays.find(x=>(x.unit||'Main')+'|'+x.num===oldPlain);
    const newDay=nextModel.days.find(x=>(x.unit||'Main')+'|'+x.num===plain)
      ||(D.matches.find(m=>(m.newDay.unit||'Main')+'|'+m.newDay.num===plain)||{}).newDay;
    if(!oldDay||!newDay)return;
    const nrm=x=>String(x||'').trim().replace(/\s+/g,' ');
    const loose=x=>nrm(x).toLowerCase().replace(/[^a-z0-9]+/g,'');
    const change=(a,b)=>loose(a)===loose(b)?null:{before:nrm(a),after:nrm(b)};
    const keysOf=d=>new Set((d.scenes||[]).map(sceneKey).filter(Boolean));
    const ka=keysOf(oldDay),kb=keysOf(newDay);
    let inter=0;for(const k of ka)if(kb.has(k))inter++;
    const overlap=(ka.size+kb.size-inter)?inter/(ka.size+kb.size-inter):0;
    const sameDay=(a,b)=>a._date&&b._date?a._date.toDateString()===b._date.toDateString():loose(a.date)===loose(b.date);
    const m={
      oldDay,newDay,overlap,
      renumbered:oldDay.num!==newDay.num,
      dateMoved:!sameDay(oldDay,newDay),
      oldUid:dayUid(oldDay),newUid:dayUid(newDay),
      matchedBy:'scenes',
      daysShifted:(oldDay._date&&newDay._date)?Math.round((newDay._date-oldDay._date)/86400000):null,
      locChanged:change(oldDay.loc,newDay.loc),
      typeChanged:change(oldDay.type,newDay.type),
      hoursChanged:change(oldDay.hours,newDay.hours),
      scenesGained:[...kb].filter(k=>!ka.has(k)),
      scenesLost:[...ka].filter(k=>!kb.has(k)),
    };
    // one old day can only continue into one new day, and vice versa
    for(let i=D.matches.length-1;i>=0;i--){
      const x=D.matches[i];
      if((x.newDay.unit||'Main')+'|'+x.newDay.num===plain||(x.oldDay.unit||'Main')+'|'+x.oldDay.num===oldPlain)D.matches.splice(i,1);
    }
    D.matches.push(m);
    REPOINT_OFF.delete(plain);
    afterMatchChange();
  }
  $('#rpTable').onclick=e=>{
    if(token!==REV_OPEN)return;
    const rj=/** @type {any} */(e.target).closest('[data-rpreject]');
    if(rj){rejectMatch(rj.dataset.rpreject);return}
    const un=/** @type {any} */(e.target).closest('[data-rpundo]');
    if(un){
      // put the day back where it was: re-derive the original pairing
      REPOINT_OFF.delete(un.dataset.rpundo);
      const d=nextModel.days.find(x=>(x.unit||'Main')+'|'+x.num===un.dataset.rpundo);
      const src=ORIG_MATCHES.find(m=>(m.newDay.unit||'Main')+'|'+m.newDay.num===un.dataset.rpundo);
      if(src&&d&&ctx.carry&&ctx.carry.diff)ctx.carry.diff.matches.push(src);
      afterMatchChange();
      return;
    }
    ...
    if(del){
      ...
      // hold the exact arrays back so the day (and its place in the running
      // order) can be put straight back — a mis-click here silently drops a
      // whole shoot day out of the import
      const beforeNext=nextModel.days.slice();
      const beforeRaw=rawModel?rawModel.days.slice():null;
      nextModel.days=nextModel.days.filter(x=>x.num!==num);
      if(rawModel)rawModel.days=rawModel.days.filter(x=>x.num!==num);
      ctx.edited=true;
      renderStats();renderChanges();renderTable();
      registerUndo(()=>{
        if(token!==REV_OPEN)return; // the review was closed — nothing to put back into
        nextModel.days=beforeNext;
        if(rawModel&&beforeRaw)rawModel.days=beforeRaw;
        renderStats();renderChanges();renderTable();
      },'removed D'+num+' from this import');
      setStatus('D'+num+' removed — it won’t be published. Cancel discards every edit.',{undo:crowdUndo});
      return;
    }
...
$('#revPage').addEventListener('click',e=>{if(isBackdropDismiss(e,'revPage'))discardReview()});
...
$('#impAI').addEventListener('click',async()=>{
  const btn=$('#impAI');
  // An AI read can take minutes. In that time the dialog can be closed (which
  // clears IMP_EDIT) and another schedule opened (which repoints it), so the
  // index is NOT safe to re-read afterwards — it would write schedule A's
  // reading onto schedule B. Hold on to the schedule itself, and to the exact
  // pending import, from the moment the button is pressed.
  const target=IMP_EDIT!=null?SOURCES[IMP_EDIT]:null;
  const pending=IMP_EDIT==null?PENDING_IMPORT:null;
  const text=target?target.text:(pending&&pending.text);
  const images=(pending&&pending.images)||null; // photographed imports re-read from their pages
  ...
  try{
    const prodHint=target?target.prod:resolveImpProd();
    const reread=await aiParse(text||'',prodHint||null,images);
    const aiModel=reread.model;
    ...
    if(target){
      ...
      if(idx<0){
        setStatus('The AI finished reading, but that schedule has since been removed or replaced, so nothing has been changed. Open the schedule again and re-read it.',{sticky:true});
        return;
      }
      target.aiModel=aiModel;
      target.model=modelFrom(target,target.unit||'Main');
      saveUserSources();
      // This re-read replaces the whole cost model. A refused save used to
      // vanish into an empty .catch() and the app still said "AI read 82
      // shoot days" — so say what actually happened, as every other save does.
      if(CLOUD.session&&target.cloudId){
        let saveErr=null;
        try{const r=await cloud.updateProduction(target.cloudId,target);saveErr=r&&r.error}
        catch(e){saveErr=e}
        if(saveErr){
          if(!DASH&&ACTIVE===idx)setActive(idx);else{renderSidebar();if(DASH)renderDash();}
          setStatus(friendlyCloudError(saveErr,'the new AI reading')+' It is on this computer only for now — keep this tab open.',{sticky:true});
          return;
        }
      }
      if(!DASH&&ACTIVE===idx)setActive(idx);else{renderSidebar();if(DASH)renderDash();}
    }else if(pending){
      if(PENDING_IMPORT!==pending){
        setStatus('The AI finished reading, but a different import has been started since, so nothing has been changed. Start the import again to use the AI reading.',{sticky:true});
        return;
      }
      pending.aiModel=aiModel;
    }
    $('#impInfo').textContent=preview.days.length+' shoot days · '+scenes+' scenes found · read by AI';
    showReadWarning([reread.read],preview.days.length);
    if(pending)pending.reads=[reread.read];
    setStatus(reread.read
      ?'AI read '+preview.days.length+' shoot days — but not the whole schedule. '+readReportLine(reread.read)
      :'AI read '+preview.days.length+' shoot days.',reread.read?{sticky:true}:undefined);
  }catch(err){setStatus('Couldn’t re-read that schedule — '+((err&&err.message)||'something went wrong')+(/[.!?]$/.test((err&&err.message)||'')?'':'.'))}
...
$('#impModal').addEventListener('click',e=>{if(isBackdropDismiss(e,'impModal'))closeImp()});
$('#fileInput').addEventListener('change',e=>{if(e.target.files.length)handleUploads(e.target.files);e.target.value='';});

// ---------- drag and drop ----------
// The empty state has always said "Drop in a shooting schedule". Until now
// there was nothing to drop onto — dragging a PDF over the window did what the
// browser does by default and opened it, losing whatever you were doing. Now
// the whole window is the target.
{
  let depth=0;
  const veil=document.createElement('div');
  veil.id='dropVeil';veil.className='hidden';
  veil.innerHTML=`<div class="dropveil-card">${icon('file')}<b>Drop the schedule here</b><span>A PDF of the shooting schedule — one-liner, Full Fat, or both. Photos of the pages work too.</span></div>`;
  ...
  window.addEventListener('drop',e=>{
    ...
    if(files&&files.length)handleUploads(files);
  });
}
...
$('#pmDelete').addEventListener('click',async()=>{
  ...
  if(!await confirmDeleteProduction(name))return;
...
function deleteProduction(name){
  ...
  ACTIVE=Math.max(0,clampActive(ACTIVE));CURPROD=null;
  showDash();
}
...
function showEmptyProd(name){
  ...
  // A brand-new production has no numbers. The headline figures and the
  // provenance strip belong to whatever schedule was open before, so leaving
  // them up puts another production's total under this production's name.
  $('#summary').innerHTML='';
  {const pb=$('#provBar');if(pb){pb.innerHTML='';pb.classList.add('hidden');}}
  // Every view gets the SAME message, not just the day board. The tab strip
  // stays on screen here, so clicking Calendar or Cost breakdown on a brand-new
  // production used to land on a completely blank page with nothing on it and
  // no way back — which reads as the app having crashed.
  const acts=`<div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:18px">
      <button class="tb-btn" data-epimport style="border-color:var(--hv-line);color:var(--hv);padding:11px 16px">Import a schedule (PDF or photos)</button>
      <button class="tb-btn" data-epmanual style="padding:11px 16px">Add shoot days by hand</button>
    </div>`;
  const card=(what)=>`<div class="tablecard" style="text-align:center;padding:40px 20px">
    <div class="dash-head" style="font-size:22px">${esc(name)}</div>
    <div style="color:var(--sub);font-size:12.5px;margin:6px auto 0;max-width:460px;line-height:1.6">${what}</div>
    ${acts}</div>`;
  const EMPTY_FOR={
    viewDays:'No schedules yet. Import the shooting schedule and every shoot day, scene, location and crowd number is read off it for you — or build the days by hand if the schedule hasn’t landed.',
    viewCal:'The calendar fills in once this production has a schedule — it lays the shoot days out month by month.',
    viewStunts:'There is nothing to cost yet. Add a schedule and the whole shoot is costed day by day and week by week.',
    viewCrowd:'The day-by-day crowd list is built from a schedule. Add one and it appears here.',
    viewCast:'The cast list is read off the schedule. Add one and every cast code and character turns up here.',
    viewDoods:'Day-out-of-days is built from the schedule’s scenes. Add a schedule and the grid builds itself.',
    viewBriefs:'Casting briefs hang off a production’s shoot days. Add a schedule first — or use Casting briefs in the menu to write one without a production.',
    viewCbdoc:'The crowd breakdown document is built from the schedule’s scenes. Add a schedule and it appears here.',
  };
  for(const [id,msg] of Object.entries(EMPTY_FOR)){
    const el=$('#'+id);
    if(el)el.innerHTML=card(msg);
  }
  // the calculator works with no schedule at all, so it is left exactly as it is
  // land on the day board; the tab strip switches between the panels as normal,
  // and each one now has something to say
  Object.keys(EMPTY_FOR).forEach(id=>$('#'+id)&&$('#'+id).classList.add('hidden'));
  $('#viewCalc')&&$('#viewCalc').classList.add('hidden');
  $('#viewDays').classList.remove('hidden');
  document.querySelectorAll('.tabs button[data-view]').forEach(b=>{
    const on=/** @type {any} */(b).dataset.view==='days';
...
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(!t.closest)return;
  if(t.closest('[data-epimport]')){$('#fileInput').click();return}
  if(t.closest('[data-epmanual]')){
...
function renameSceneKeyRefs(d,idx,oldNum,oldPart,newNum,newPart){
  ...
  if(NOTES[oldNK]!==undefined){NOTES[newNK]=NOTES[oldNK];delete NOTES[oldNK];NOTES_LEGACY=null;}
}
...
function persistDayScenes(d){
  ...
  if(d.manual){
    saveManualDays();
    if(CLOUD.session&&src&&src.cloudId)cloud.upsertManualDay(src.cloudId,d).then(r=>{if(r&&r.error)setStatus(friendlyCloudError(r.error,'that change'))});
  }else if(src){
...
$('#smSave').addEventListener('click',()=>{
  ...
  saveSced();(NOTES_LEGACY=null,store.set('stuntos-notes',JSON.stringify(NOTES)));
  persistDayScenes(d);
...
async function deleteManualDayById(id){
  const d=MODEL.days.find(x=>x.id===id);if(!d)return;
  if(!await askConfirm({
    title:'Remove '+d.id+'?',
    body:'This takes '+d.id+' ('+d.date+') off the schedule. Anything you set in its day calculator is kept, so if you add the day back it comes back with it.',
    yes:'Remove the day',danger:true}))return;
  MODEL.days=MODEL.days.filter(x=>x!==d);
  saveManualDays();
  const src=SOURCES[ACTIVE];
  if(CLOUD.session&&src&&src.cloudId)cloud.deleteManualDay(src.cloudId,d.unit,d.num).then(r=>{if(r&&r.error)setStatus(friendlyCloudError(r.error,'that deletion'))});
  refreshAll();
}
...
function createManualRevision(name,unit){
  ...
  addSource({days:[],castMap:{},notes:[]},title,name.slice(0,16),true,{kind:'manual',allowEmpty:true,colour:(PRODS[name]&&PRODS[name].colour)||'white',prod:name,unit});
  const src=SOURCES[SOURCES.length-1];
  src.createdAt=stamp.toISOString();src.sessionNew=true;
  saveUserSources();
  // the same hardened path every upload uses: a failed save is warned about,
  // retried, and held back from the sign-out wipe. A bare insert here meant a
  // hand-built 40-day revision could be wiped at sign-out without a word.
  saveSourceToCloud(src);
...
$('#bkCreate').addEventListener('click',()=>{
  ...
  for(const row of rows){
    ...
    if(CLOUD.session&&src&&src.cloudId)cloud.upsertManualDay(src.cloudId,d).then(r=>{if(r.error)setStatus(friendlyCloudError(r.error,'that change'))});
  }
  sortDays(MODEL);
...
const CARRY_DAY_STORES=[
  // persistCDAY, not saveCDAY: carrying a config across is not the user
  // editing a day, so it must not clear the `seeded` flag on whatever day the
  // calculator happens to be open on
  ()=>[CDAY,persistCDAY],
  ()=>[ADJ,saveAdj],
...
function planRevisionCarry(prevSrc,newModel,diff,manualPlains,prevLabel){
  const prevNs=prevSrc.ns||'';
  const pre=prevNs?prevNs+'|':'';
  // old 'unit|num' → new 'unit|num': matched days move (best scene overlap
  // first, so when two old days want the same new day the closer match owns it
  // and the other is REPORTED, never quietly dropped), shot days carry
  // verbatim, hand-added days keep their numbers (restoreManualDays re-adds
  // them to the new revision unchanged)
  // `fromRev` is not optional in practice: a collided already-shot day is
  // carried to `Main|12-<revision it was shot in>`, and the stitched record is
  // filed under that same suffix. Leave it out and every collided day's
  // calculator, stunt hours and travel band land on a key nothing reads.
  const targets=dayCarryTargets(diff,manualPlains,{fromRev:prevLabel||''});
  const dayTo=targets.map;
  const contested=new Map();
  for(const c of targets.collisions)contested.set(c.from,c);
  const manualSet=new Set(manualPlains||[]);
  const plan={dayMoves:[],dayStranded:[],sceneMoves:[],sceneStranded:[],prevNs,
    prevLabel:prevLabel||'',review:{},dayContested:targets.collisions};
  // A day matched on changed content still carries its frozen crowd numbers,
  // its call/wrap (worked out from the OLD day's hours) and its travel band.
  // Record, in plain English, what those numbers were based on that has moved.
  for(const m of diff.matches){
    const r=carryReviewReasons(m);
    if(r.needsReview)plan.review[(m.newDay.unit||'Main')+'|'+m.newDay.num]={from:plan.prevLabel,reasons:r.reasons};
  }
  const plainNum=p=>String(p||'').split('|')[1]||'';
  ...
  CARRY_DAY_STORES.forEach((get,si)=>{
    ...
    for(const k of Object.keys(map)){
      ...
      const clash=contested.get(plain);
      const item={store:si,label:labels[si],from:plain,to:to||null,key:k,
        reason:to?null:clash?('D'+plainNum(clash.heldBy)+' is a closer match for D'+plainNum(clash.to)+' in the new schedule'):'day dissolved'};
      if(to)plan.dayMoves.push(item);
      else if(!seenDay.has(plain+'|'+si)){plan.dayStranded.push(item)}
      seenDay.add(plain+'|'+si);
    }
  });
  // Where a scene's work can land, best first: a live, still-to-shoot day
  // beats a carried already-shot day. Binding to the historical day left the
  // upcoming instance blank on a scene that was in fact fully broken down.
  const targetCache=new Map();
  const sceneTargets=key=>{
    if(!targetCache.has(key))targetCache.set(key,sceneCarryTargets(newModel,key)||[]);
  ...
  for(const [map,label] of sceneStores){
    for(const k of Object.keys(map)){
      ...
      const all=sceneTargets(carrySceneKey(scene,part));
      ...
      const item={label,scene:scene+(part?' pt'+part:''),from:'D'+num,
        ...
        newPlain:hit?sceneSlotKey(hit.day,hit.idx):manualCarry?plain:null,
        // every other place this scene shoots, in case the best one is taken
        alts:all.slice(1).map(t=>sceneSlotKey(t.day,t.idx)),
        altDays:all.slice(1).map(t=>'D'+t.day.num),
        map};
...
function applyRevisionCarry(plan,newNs){
  ...
  // Two pieces of work wanting one place is a decision, not a detail: the
  // first writer used to win and the loser vanished with nothing said.
  const collisions=[];
  const same=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
  const stamp=new Date().toISOString();
  for(const item of plan.dayMoves){
    const [map]=CARRY_DAY_STORES[item.store]();
    const nk=npre+item.to;
    if(map[nk]===undefined){map[nk]=JSON.parse(JSON.stringify(map[item.key]));n++}
    else if(!same(map[nk],map[item.key]))collisions.push({kind:'day',label:item.label,from:'D'+String(item.from).split('|')[1],to:'D'+String(item.to).split('|')[1]});
    touched.add(item.store);
  }
  for(const item of plan.sceneMoves){
    let nk=npre+item.newPlain;
    if(item.map[nk]!==undefined&&!same(item.map[nk],item.map[item.key])){
      // that slot already holds different work — put this on the next day the
      // same scene shoots rather than picking a winner in silence
      const free=(item.alts||[]).map(p=>npre+p).find(p=>item.map[p]===undefined);
      if(free)nk=free;
      else{
        collisions.push({kind:'scene',label:item.label,scene:item.scene,from:item.from,to:item.to});
  ...
  // A day config carried onto a day whose scenes, hours, type or location have
  // moved is no longer something anyone checked — mark it, with the reasons,
  // right next to the config the cost engine reads.
  for(const [toPlain,rev] of Object.entries(plan.review||{})){
    const [cmap]=CARRY_DAY_STORES[0]();
    const c=cmap[npre+toPlain];
    if(!c||typeof c!=='object'||c.seeded)continue;
    c.review={from:rev.from||'',at:stamp,reasons:(rev.reasons||[]).slice()};
    touched.add(0);
  }
  for(const si of touched)if(typeof si==='number')CARRY_DAY_STORES[si]()[1]();
  if(touched.has('sced'))saveSced();
  if(touched.has('notes')){(NOTES_LEGACY=null,store.set('stuntos-notes',JSON.stringify(NOTES)));cloudSyncBlob('notes',NOTES)}
...
// ---------- one pipeline for every model replacement ----------
// A new revision, a re-read of the same document with AI, and a Full Fat
// detail merge all throw away the model the board is showing. All three must
// therefore diff against it, keep the days that have already been shot, and
// carry the user's work forward — otherwise the work silently stops matching
// the schedule it was made against. Mutates `raw` (shot days stitched back in,
// cast names carried) and hands back the model to publish.
function planRevisionChange(o){
  const prevSrc=o.prevSrc,unit=o.unit||'Main',raw=o.raw,ctx=o.ctx||{};
  let m=o.model;
  const out={diff:null,plan:null,stored:null,stitched:0,model:m,prevLabel:'',
    castListMissing:false,castCount:0,castCarried:false,
    // kept so the review page can reject / re-point a day match and rebuild
    prevSrc:null,newDoc:null,manualPlains:[],oldMoney:null,newMoney:null,moneyUnknown:false};
  if(!prevSrc||!prevSrc.model||!(prevSrc.model.days||[]).length)return out;
  out.prevLabel=revLabel(prevSrc)||'previous revision';
  if(o.carryCast){
    ...
    const prevCast={};
    for(const [code,rec] of Object.entries(collectCast(o.prod||prevSrc.prod||prevSrc.title)))if(rec&&rec.character)prevCast[code]=rec.character;
    const ownCastCount=Object.keys(raw.castMap||{}).length;
    const carriedCast=carryCastMap(prevCast,raw.castMap||{});
    out.castCarried=Object.keys(carriedCast).length>ownCastCount;
    out.castListMissing=ownCastCount===0&&Object.keys(carriedCast).length>0;
    out.castCount=Object.keys(carriedCast).length;
  ...
  // The diff holds references to THESE two models' day records, so the money
  // per day has to be costed against them — not against the stitched model
  // built below (which is a fresh deep copy).
  const newDoc=m;
  const recs=carriedDayRecords(diff,out.prevLabel);
  if(recs.length){
    // Match on the RECORD'S OWN ID, not on unit+number. An already-shot day is
    // classed as a collision precisely BECAUSE the new schedule reuses its
    // number, so a unit+number test excluded every single collision — the shot
    // day, and its spend, dropped off the timeline, which is the exact loss
    // carriedDayRecords' suffixed ids (`M12` → `M12-Blue`) exist to prevent.
    const have=new Set((m.days||[]).map(d=>d.id).filter(Boolean));
    const add=recs.filter(d=>!have.has(d.id));
    if(add.length){
      raw.days=[...add,...raw.days];
      const m2=prepModel(JSON.parse(JSON.stringify(raw)),unit);
      m2._raw=m._raw;
      m=m2;
    }
    out.stitched=add.length;
  }
  out.model=m;
  out.diff=diff;
  const manualPlains=prevSrc.model.days.filter(d=>d.manual).map(d=>(d.unit||'Main')+'|'+d.num);
  // MONEY. Both sides of the revision are costed through the SAME engine the
  // board costs with, so "day dropped" can say what dropping it saves and a
  // cut scene carrying 200 supporting artists never reads like one carrying
  // none. When the engine can't be run (no rate fields on screen yet) the
  // money is left unknown and said so, rather than quietly shown as £0.
  const prodName=o.prod||prevSrc.prod||prevSrc.title;
  out.prevSrc=prevSrc;out.newDoc=newDoc;out.manualPlains=manualPlains;
  // The saved day configs are filed against the OLD schedule's day numbers.
  // Costing the NEW model with them unchanged put every carried config on the
  // wrong day the moment a block renumbered — which is precisely the reshuffle
  // this whole feature exists to explain. Move them through the same carry map
  // the work itself follows before costing the new side.
  const cfgMove=dayCarryTargets(diff,manualPlains,{fromRev:out.prevLabel}).map;
  const oldCosts=perDayCrowdCosts(prevDoc,prevSrc.ns,prodName);
  out.oldMoney=oldCosts?(d=>{const r=d&&d.id?oldCosts.perDay[d.id]:null;return r?Math.round(r.cost||0):0}):null;
  out.newMoney=perDayCrowdMoney(newDoc,prevSrc.ns,prodName,cfgMove);
  out.moneyUnknown=!out.oldMoney||!out.newMoney;
  // WHAT CUTTING A DAY ACTUALLY SAVES.
  // The diff reports a dropped day as a clean negative — "you saved £38,000".
  // Inside the agency's notice window the production still owes most of it, so
  // that clean negative is a confidently wrong number in the one place this
  // product is unique. Price the cut days off the OLD revision's costs (they do
  // not exist in the new one, so only the old costing CAN price them) and carry
  // the result with the stored account of what changed. With no cancellation
  // terms set — the default — `charge` is 0 and `saved` is the full cost,
  // exactly what the diff said before.
  out.cancel=null;
  if(oldCosts&&diff.cutDays.length){
    try{
      const sum=cutDayCancellations(diff.cutDays,/** @type {any} */(oldCosts),
        /** @type {any} */(crowdSettingsFromDOM()),new Date());
      if(sum&&sum.inForce&&sum.charge)
        out.cancel={fullCost:sum.fullCost,charge:sum.charge,saved:sum.saved,
          noticeDays:activeOncosts().cancellation.noticeDays,pct:activeOncosts().cancellation.pct,
          days:sum.days.filter(x=>x.withinNotice).map(x=>({num:x.dayNum,unit:x.unit,charge:x.charge,daysNotice:x.daysNotice})),
          undated:sum.days.filter(x=>x.daysNotice===null).length};
    }catch(e){out.cancel=null}
  }
  rebuildCarryPlan(out);
  ctx.castCarried=out.castCarried;ctx.castListMissing=out.castListMissing;
  ctx.carry=out;ctx.stitched=out.stitched>0;
  return out;
}
/**
 * Per-day crowd cost for ANY model, through the very engine the board costs
 * with (engineComputeCrowdCosts — same call `costsFor` makes). No new costing
 * maths lives here.
 *
 * Returns `day → whole pounds`, or NULL when the engine can't be run — the
 * caller then says "couldn't be worked out" instead of printing £0, because a
 * silent zero is indistinguishable from a day that genuinely costs nothing.
 */
function perDayCrowdCosts(model,ns,prodName,cfgMove){
  if(!model||!(model.days||[]).length)return null;
  try{
    // COST A COPY. applyDayLocs writes d.loc / d.locDoc in place, and the
    // previous revision's day records are the very objects that revision's
    // source still holds — costing the old side used to rewrite the old
    // schedule's locations on the way past. Copying the day records (the
    // scenes underneath are only read) is enough and is cheap.
    const doc={...model,days:model.days.map(d=>({...d}))};
    applyDayLocs(doc,ns||'');
    const strip=map=>ns
      ?Object.fromEntries(Object.entries(map).filter(([k])=>k.startsWith(ns+'|')).map(([k,v])=>[k.slice(ns.length+1),v]))
      :Object.fromEntries(Object.entries(map).filter(([k])=>!keyParts(k).ns));
    let cfg=strip(CDAY);
    if(cfgMove){
      // old 'unit|num' → new 'unit|num'. A config whose day dissolved is left
      // out rather than allowed to land on whatever day now has its number.
      const moved={};
      for(const [k,v] of Object.entries(cfg)){const to=cfgMove.get(k);if(to)moved[to]=v;}
      cfg=moved;
    }
    const res=engineComputeCrowdCosts(doc,cfg,
      /** @type {any} */({...crowdSettingsFromDOM(),baseDay:prodName?prodBaseDay(prodName):undefined}));
    const by=(res&&res.perDay)||{};
    if(!Object.keys(by).length)return null;
    return res;
  }catch(e){return null}
}
/** The same, as `day → whole pounds` (what the diff stores). */
function perDayCrowdMoney(model,ns,prodName,cfgMove){
  const res=perDayCrowdCosts(model,ns,prodName,cfgMove);
  if(!res)return null;
  const by=res.perDay||{};
  return d=>{const r=d&&d.id?by[d.id]:null;return r?Math.round(r.cost||0):0};
}
/**
 * (Re)build the carry plan and the stored account from `carry.diff`.
 * Called once at import, and again whenever the reviewer rejects or re-points
 * a day match — so the plan, the money and the change list always describe the
 * matches actually in force.
 */
function rebuildCarryPlan(carry){
  if(!carry||!carry.diff||!carry.prevSrc)return carry;
  carry.plan=planRevisionCarry(carry.prevSrc,carry.model,carry.diff,carry.manualPlains,carry.prevLabel);
  carry.stored=compactRevisionDiff(carry.diff,{prevLabel:carry.prevLabel,
    ...(carry.oldMoney?{oldDayMoney:carry.oldMoney}:{}),
    ...(carry.newMoney?{newDayMoney:carry.newMoney}:{})});
  if(carry.moneyUnknown)Object.assign(carry.stored,{moneyUnknown:true});
  if(carry.cancel)Object.assign(carry.stored,{cancel:carry.cancel});
  return carry;
}
// Apply a planned change on publish: move the work, then keep the account of
// what changed with the schedule it describes.
function commitRevisionChange(carry,src){
  if(!carry||(!carry.plan&&!carry.stored))return {n:0,collisions:[]};
  const res=carry.plan?applyRevisionCarry(carry.plan,src.ns):{n:0,collisions:[]};
  if(carry.stored){
    setSrcRevDiff(src,carry.stored);
    // THE FLAGSHIP FEATURE HAS TO APPEAR ON PUBLISH, NOT ON RELOAD.
    // The board is rendered when the new revision is activated (addSource →
    // setActive), which happens BEFORE this line — so at render time
    // activeRevDiff() was still null and #chgBar came out empty: no strip, no
    // legend, no "What changed", no changed-days filter, until a hard refresh.
    // The account of what changed now exists, so paint it straight away.
    if(SOURCES[ACTIVE]===src&&!DASH){
      CHG_SHOW=true;CHG_ONLY=false;
      renderDays();renderCalendar();
    }
  }
  return res;
}
// The account of what changed rides with the model (src.aiModel), which is
// what reaches both localStorage and the account — no new table, and it comes
// back on any device that opens the schedule.
// ---------- reading a stored diff back ----------
// A stored diff is written by this app, but by the time it is read again it is
// UNTRUSTED INPUT: it round-trips through localStorage (where a half-finished
// write leaves a truncated object, and an older build left a bare `true`) and
// through the account's `ai_model` jsonb (which anyone with write access to the
// row can change). Two separate failures follow from reading it raw.
//
//  1. THE BOARD GOES BLANK. renderChangeBar calls storedDiffHeadline on every
//     single board render, and chgReportHTML calls storedDiffMoney. Both walk
//     `sd.counts` / `sd.days` without checking, so any truthy value that is not
//     the expected shape threw a TypeError, the exception escaped renderDays,
//     and the whole board rendered empty.
//  2. NUMBERS BECOME MARKUP. Every figure here is interpolated into innerHTML.
//     `"<img src=x onerror=…>".toLocaleString()` returns the string untouched,
//     and `||` / `+` on a non-empty string are no-ops, so a number that is
//     really a string is script on the page of everyone who opens "What
//     changed". Strings were already escaped; numbers were not.
//
// So: validate the shape once on read, coerce every numeric field to a number
// and every string field to a string, and hand back null for anything else.
const sdNum=n=>{const v=+n;return Number.isFinite(v)?v:0};
const sdStr=v=>v==null?'':String(v);
const sdStrs=v=>Array.isArray(v)?v.map(sdStr):[];
const sdPair=v=>(v&&typeof v==='object')?{before:sdNum(v.before),after:sdNum(v.after)}:null;
function sdScene(c){
  const kind=c&&(c.kind==='added'||c.kind==='movedIn')?c.kind:'changed';
  const out={key:sdStr(c&&c.key),kind,label:sdStr(c&&c.label)};
  if(c&&c.fromDay!=null)out.fromDay=sdStr(c.fromDay);
  if(c&&c.sameDay)out.sameDay=true;
  if(c&&c.newKey)out.newKey=sdStr(c.newKey);
  const crowd=sdPair(c&&c.crowd);if(crowd)out.crowd=crowd;
  const stunt=sdPair(c&&c.stunt);if(stunt)out.stunt=stunt;
  if(c&&c.castAdded)out.castAdded=sdStrs(c.castAdded);
  if(c&&c.castDropped)out.castDropped=sdStrs(c.castDropped);
  if(c&&c.slugChanged)out.slugChanged=true;
  return out;
}
function sdDay(d){
  const kind=d&&(d.kind==='new'||d.kind==='shot')?d.kind:'changed';
  const out={id:sdStr(d&&d.id),uid:sdStr(d&&d.uid),num:sdStr(d&&d.num),unit:sdStr((d&&d.unit)||'Main'),
    kind,labels:sdStrs(d&&d.labels),money:sdNum(d&&d.money),
    scenes:Array.isArray(d&&d.scenes)?d.scenes.map(sdScene):[]};
  // these three are "was X" chips: absent and empty mean different things
  for(const k of ['locBefore','typeBefore','hoursBefore'])if(d&&d[k]!=null)out[k]=sdStr(d[k]);
  if(d&&d.lost)out.lost=sdStrs(d.lost);
  if(d&&d.gained)out.gained=sdStrs(d.gained);
  return out;
}
function sanitizeStoredDiff(sd){
  if(!sd||typeof sd!=='object'||sd.v!==1)return null;
  if(!Array.isArray(sd.days)||!sd.counts||typeof sd.counts!=='object')return null;
  const c=sd.counts;
  return {
    v:1,
    prevLabel:sdStr(sd.prevLabel),
    at:sdStr(sd.at),
    moneyUnknown:!!sd.moneyUnknown,
    // Agency cancellation terms in force when this revision was published, and
    // what they charge on the days it dropped. Untrusted like everything else
    // here — every number is coerced.
    cancel:(sd.cancel&&typeof sd.cancel==='object')?{
      fullCost:sdNum(sd.cancel.fullCost),charge:sdNum(sd.cancel.charge),saved:sdNum(sd.cancel.saved),
      noticeDays:sdNum(sd.cancel.noticeDays),pct:sdNum(sd.cancel.pct),
      undated:sdNum(sd.cancel.undated),
      days:(Array.isArray(sd.cancel.days)?sd.cancel.days:[]).map(x=>({
        num:sdStr(x&&x.num),unit:sdStr((x&&x.unit)||'Main'),
        charge:sdNum(x&&x.charge),daysNotice:sdNum(x&&x.daysNotice)})),
    }:null,
    days:sd.days.map(sdDay),
    cutDays:(Array.isArray(sd.cutDays)?sd.cutDays:[]).map(d=>({
      num:sdStr(d&&d.num),unit:sdStr((d&&d.unit)||'Main'),date:sdStr(d&&d.date),
      loc:sdStr(d&&d.loc),scenes:sdStrs(d&&d.scenes),money:sdNum(d&&d.money)})),
    cutScenes:(Array.isArray(sd.cutScenes)?sd.cutScenes:[]).map(x=>({
      key:sdStr(x&&x.key),fromDay:sdStr(x&&x.fromDay),crowd:sdNum(x&&x.crowd)})),
    groups:(Array.isArray(sd.groups)?sd.groups:[]).map(g=>({
      label:sdStr(g&&g.label),
      ...(g&&g.dayId?{dayId:sdStr(g.dayId)}:{}),
      ...(g&&g.dayUid?{dayUid:sdStr(g.dayUid)}:{}),
      lines:sdStrs(g&&g.lines),
      ...(g&&g.money!=null?{money:sdNum(g.money)}:{})})),
    counts:{
      daysNew:sdNum(c.daysNew),daysChanged:sdNum(c.daysChanged),daysCut:sdNum(c.daysCut),
      daysShot:sdNum(c.daysShot),scenesAdded:sdNum(c.scenesAdded),scenesMoved:sdNum(c.scenesMoved),
      scenesCut:sdNum(c.scenesCut),scenesChanged:sdNum(c.scenesChanged),
      crowdBefore:sdNum(c.crowdBefore),crowdAfter:sdNum(c.crowdAfter)},
    ...(sd.money&&typeof sd.money==='object'?{money:{before:sdNum(sd.money.before),after:sdNum(sd.money.after)}}:{}),
  };
}
// One cleaned copy per stored object — everything downstream compares diffs by
// identity (REVDIFF_CACHE), and renderChangeBar runs on every board render.
const SD_CLEAN=new WeakMap();
function srcRevDiff(s){
  if(!s)return null;
  const raw=s.revDiff||(s.aiModel&&s.aiModel._revDiff)||null;
  if(!raw||typeof raw!=='object')return null;
  if(!SD_CLEAN.has(raw)){
    let clean=null;
    try{clean=sanitizeStoredDiff(raw)}catch(e){clean=null}
    SD_CLEAN.set(raw,clean);
  }
  return SD_CLEAN.get(raw);
}
function setSrcRevDiff(s,sd){
  if(!s||!sd)return;
  s.revDiff=sd;
  if(s.aiModel&&typeof s.aiModel==='object')s.aiModel._revDiff=sd;
}
// ---------- "can this day still be trusted?" ----------
// The stored diff for the schedule on screen, indexed by day id.
let REVDIFF_CACHE={sd:null,byDay:null,cut:null};
function activeRevDiff(){return srcRevDiff(SOURCES[ACTIVE])}
function revDiffIndex(){
  const sd=activeRevDiff();
  if(REVDIFF_CACHE.sd!==sd){
    // Index on the day RECORD's own uid first and on its id second. `id` is
    // only unit+number, and a stitched model can hold two records that answer
    // to the same number — the live D12 and the already-shot D12 kept from the
    // previous revision — so an id-only index gives one day the other's
    // changes and the other's money.
    const byDay=new Map(),cut=new Map();
    const put=(m,k,v)=>{if(k&&!m.has(k))m.set(k,v)};
    for(const d of (sd&&sd.days)||[]){
      put(byDay,'u|'+d.uid,d);put(byDay,'i|'+d.id,d);
      if(d.lost&&d.lost.length){put(cut,'u|'+d.uid,d.lost);put(cut,'i|'+d.id,d.lost)}
    }
    REVDIFF_CACHE={sd,byDay,cut};
  }
  return REVDIFF_CACHE;
}
const revDiffLook=(m,d)=>(d&&((d._uid&&m.get('u|'+d._uid))||(d.id&&m.get('i|'+d.id))))||null;
/** What the stored diff says about one day of the current schedule, or null. */
function revDiffForDay(d){return revDiffLook(revDiffIndex().byDay,d)}
/** Scene numbers that were on this day in the previous schedule and have gone. */
function revDiffCutFromDay(d){return revDiffLook(revDiffIndex().cut,d)||[]}
/**
 * Whole-revision money movement in pounds (0 when it couldn't be worked out).
 *
 * A cut day is stored as its full negative cost. Where cancellation terms are
 * in force and the day was cut inside the notice window, that money is NOT
 * saved — the production still owes it — so the charge is added back and the
 * strip reports what is genuinely saved. `cancelCharge()` says the rest out
 * loud beside it, because a number that quietly went from −£38,000 to £0 would
 * be no better than the wrong number it replaced.
 */
function revDiffMoney(){
  const sd=/** @type {any} */(activeRevDiff());
  if(!sd)return 0;
  return storedDiffMoney(sd)+((sd.cancel&&sd.cancel.charge)||0);
}
/** The cancellation charge riding on this revision's cut days, or null. */
function cancelCharge(){
  const sd=/** @type {any} */(activeRevDiff());
  return (sd&&sd.cancel&&sd.cancel.charge)?sd.cancel:null;
}
/**
 * Does this day's saved config still describe the day, and if not, why?
 * Returns {from,at,reasons[]} or null. The reasons are finished sentences —
 * print them as they are.
 */
function dayReviewFor(d){
  if(!d)return null;
  const c=CDAY[cdayKey(d)];
  if(c&&c.review&&(c.review.reasons||[]).length)
    return {from:c.review.from||'',at:c.review.at||'',reasons:c.review.reasons.slice()};
  return null;
}
/** How many days on this schedule are carrying settings that need checking. */
function daysNeedingReview(){
  return (MODEL.days||[]).filter(d=>dayReviewFor(d)).length;
}
/** "These numbers are right" — keeps the config, clears the flag. */
function confirmDayConfig(d){
  const key=cdayKey(d),c=CDAY[key];
  if(!c||!c.review)return false;
  delete c.review;
  saveCDAY(key); // confirming IS an edit: the day now costs on its own numbers
  return true;
}
/** "Start again from the new schedule" — rebuilds the config from the day as
 *  it now stands, which also clears the flag. */
function reseedDayConfig(d){
  const key=cdayKey(d);
  delete CDAY[key];
  CDAY[key]=seedCday(d);
  persistCDAY();
...
// SEEING WHAT CHANGED
//
// A new schedule lands every few days. The board's job is to make the
// difference visible without turning into a Christmas tree, so there is
// exactly ONE visual grammar here and it is used everywhere:
//
//   · a 3px LEFT RAIL on the thing that changed  — amber (--warn) for changed,
//     blue (--hv) for new, red (--danger) for gone, and always paired with…
//   · a small WORD chip saying what changed ("Was D12", "Crowd 20 → 200").
//     Never colour on its own: an AD reads this on an iPad in daylight.
//   · one calm STRIP above the board with the totals and the money, which
//     turns the whole treatment off, filters to changed days, opens the full
//     list, and can be dismissed for good once the revision has been read.
// ===========================================================================
// CHG_SHOW / CHG_ONLY / CHG_DISMISS are declared with the board state at the
// top of the file. The dismissal list is read from storage on first use.
function chgSeen(){
  if(!CHG_DISMISS){try{CHG_DISMISS=JSON.parse(store.get('crowdos-chgseen')||'[]')||[]}catch(e){CHG_DISMISS=[]}}
  return CHG_DISMISS;
}
function saveChgDismiss(){store.set('crowdos-chgseen',JSON.stringify(chgSeen().slice(-200)))}
/** Identity of "this revision's changes, for this person" — so dismissing the
 *  strip on one revision never hides the next one. */
function chgStamp(){
  const sd=activeRevDiff();if(!sd)return '';
  const who=(CLOUD.session&&CLOUD.session.user&&CLOUD.session.user.id)||'local';
  return who+'|'+srcUid(SOURCES[ACTIVE])+'|'+(sd.at||'')+'|'+(sd.prevLabel||'');
}
function chgDismissed(){const k=chgStamp();return !!k&&chgSeen().includes(k)}
function chgDismiss(){const k=chgStamp();if(k&&!chgSeen().includes(k)){chgSeen().push(k);saveChgDismiss()}}
function chgUndismiss(){const k=chgStamp();const i=chgSeen().indexOf(k);if(i>=0){chgSeen().splice(i,1);saveChgDismiss()}}
/** Are change markers being painted right now? */
function chgOn(){return CHG_SHOW&&!!activeRevDiff()}
/** True when the money in the stored diff is real rather than an unknown 0. */
function chgMoneyKnown(){const sd=/** @type {any} */(activeRevDiff());return !!sd&&!sd.moneyUnknown}
/** Signed money, worded. "" when it could not be worked out. */
function chgMoneyText(n){
  if(!Number.isFinite(n)||!n)return '';
  return (n>0?'+':'−')+gbp(Math.abs(Math.round(n)));
}
/** Every day id the stored diff has something to say about (changed, new, or
 *  a day that lost a scene) — the set the "changed days only" filter keeps. */
function chgDayIds(){
  const sd=activeRevDiff(),out=new Set();
  for(const d of (sd&&sd.days)||[])if(d.id)out.add(d.id);
  for(const d of (MODEL&&MODEL.days)||[])if(dayReviewFor(d))out.add(d.id);
  return out;
}
/** How many days the strip is offering to take you to. */
function chgDayCount(){return chgDayIds().size}
/**
 * WHY "CHANGED DAYS ONLY" IS EMPTY — said in words, not by a silent 0.
 *
 * The strip can honestly read "4 dropped days · 12 scenes cut" while the filter
 * count is 0, because a DROPPED day is not on this board: the new schedule does
 * not contain it, so there is no day card for the filter to keep. Turning the
 * filter on then produced "Nothing changed on these days", which flatly
 * contradicts the headline. The control is disabled at zero and this sentence
 * says which of the two situations it is.
 */
function emptyFilterWhy(sd){
  const c=(sd&&sd.counts)||{};
  const cut=+c.daysCut||0, sc=+c.scenesCut||0;
  if(cut||sc){
    const bits=[];
    if(cut)bits.push(cut+' dropped day'+(cut===1?'':'s'));
    if(sc)bits.push(sc+' cut scene'+(sc===1?'':'s'));
    return 'Nothing on this board to filter to: '+bits.join(' and ')+' are not in this schedule, so there is no day card to show. Open “What changed” to read them in full.';
  }
  return 'Nothing to filter to — no day on this schedule changed.';
}

// ---- the day card ---------------------------------------------------------
/** Which rail a day earns: new day, day that lost work, or ordinary change. */
function chgDayTone(c,d){
  if(!c)return dayReviewFor(d)?'check':'';
  if(c.kind==='new')return 'new';
  if((c.lost&&c.lost.length)||(c.money||0)<0)return 'down';
  return 'changed';
}
/** The chips + sentence that ride under a changed day's header. */
function chgDayHTML(d){
  if(!chgOn())return '';
  const c=revDiffForDay(d),rv=dayReviewFor(d);
  if(!c&&!rv)return '';
  const bits=[];
  if(c){
    for(const l of c.labels||[])bits.push(`<span class="chg-chip">${esc(l)}</span>`);
    if(c.locBefore!=null)bits.push(`<span class="chg-chip was">Was ${esc(c.locBefore||'not stated')}</span>`);
    if(c.typeBefore!=null)bits.push(`<span class="chg-chip was">Was ${esc(c.typeBefore||'not stated')}</span>`);
    if(c.hoursBefore!=null)bits.push(`<span class="chg-chip was">Was ${esc(c.hoursBefore||'not stated')}</span>`);
    const mt=chgMoneyKnown()?chgMoneyText(c.money):'';
    if(mt)bits.push(`<span class="chg-chip money ${c.money>0?'up':'down'}">${esc(mt)} crowd cost</span>`);
  }
  const lost=revDiffCutFromDay(d);
  const head=bits.length?`<div class="chg-row">${icon('refresh')}<span class="chg-lead">Changed since ${esc((activeRevDiff()||{}).prevLabel||'the last schedule')}</span>${bits.join('')}</div>`:'';
  const lostLine=lost.length
    ?`<div class="chg-row gone">${icon('warn')}<span class="chg-lead">No longer on this day</span><span class="chg-chip gone">Scene${lost.length===1?'':'s'} ${esc(lost.join(', '))} moved off or cut</span></div>`
    :'';
  // A day whose SAVED settings were confirmed against a different day is being
  // priced on numbers nobody has checked. Say it in full, and offer the two
  // real answers.
  const check=rv?`<div class="chg-check" role="group" aria-label="Settings need checking on day ${esc(String(d.num))}">
      <div class="chg-check-hd">${icon('warn')} <b>Check this day's saved settings</b></div>
      <div class="chg-check-why">Its crowd numbers, call and wrap times and travel band were confirmed against <b>${esc(rv.from||'an earlier schedule')}</b>, and the day has changed since:</div>
      <ul class="chg-check-list">${rv.reasons.map(r=>`<li>${esc(r)}</li>`).join('')}</ul>
      <div class="chg-check-act">
        <button type="button" class="chg-btn primary" data-chgreseed="${esc(d.id)}">Use the new schedule's details</button>
        <button type="button" class="chg-btn" data-chgkeep="${esc(d.id)}">Keep mine — they're right</button>
      </div>
    </div>`:'';
  return head+lostLine+check;
}
// ---- the scene row --------------------------------------------------------
/** Scene-key → what the revision did to that scene on this day. */
function chgSceneMap(d){
  const c=revDiffForDay(d),m=new Map();
  for(const s of (c&&c.scenes)||[])if(s.key)m.set(String(s.key).toLowerCase(),s);
  return m;
}
/** The rail class + chips for one scene row. */
function chgSceneHTML(sc){
  if(!sc)return {cls:'',chips:''};
  const chips=[];
  // A scene that "moved in" from the day it was already on has not moved at
  // all — its DAY slipped and it went with it. Saying "Moved here from D12" on
  // D12 reads as a scene that was re-scheduled, which is a different piece of
  // news entirely, so the engine now flags it and the wording follows.
  const movedDays=sc.kind==='movedIn'&&!sc.sameDay;
  const kind=(sc.kind==='added'||movedDays)?'new':'changed';
  if(sc.kind==='added')chips.push(`<span class="chg-chip">Added here</span>`);
  else if(movedDays)chips.push(`<span class="chg-chip">${sc.fromDay!=null?`Moved here from D${esc(String(sc.fromDay))}`:'Moved here'}</span>`);
  else if(sc.kind==='movedIn')chips.push(`<span class="chg-chip">This day moved — the scene stayed on it</span>`);
  // The single most valuable number on the page: 20 supporting artists
  // becoming 200 used to be completely invisible.
  if(sc.crowd)chips.push(`<span class="chg-chip crowd ${sc.crowd.after>sc.crowd.before?'up':'down'}">Crowd ${Number(sc.crowd.before)||0} → ${Number(sc.crowd.after)||0}</span>`);
  if(sc.stunt)chips.push(`<span class="chg-chip">Stunts ${Number(sc.stunt.before)||0} → ${Number(sc.stunt.after)||0}</span>`);
  if(sc.castAdded&&sc.castAdded.length)chips.push(`<span class="chg-chip">Cast added ${esc(sc.castAdded.join(', '))}</span>`);
  if(sc.castDropped&&sc.castDropped.length)chips.push(`<span class="chg-chip gone">Cast dropped ${esc(sc.castDropped.join(', '))}</span>`);
  if(sc.newKey)chips.push(`<span class="chg-chip">Renumbered from ${esc(sc.key)}</span>`);
  if(!chips.length&&sc.slugChanged)chips.push(`<span class="chg-chip">Set changed</span>`);
  if(!chips.length)return {cls:'',chips:''};
  return {cls:' chg chg-'+(sc.crowd&&sc.crowd.after<sc.crowd.before?'down':kind),
    chips:`<div class="chg-scene">${chips.join('')}</div>`};
}
/** Ghost rows for scenes that were on this day last time and have gone. */
function chgGoneRowsHTML(d){
  if(!chgOn())return '';
  const lost=revDiffCutFromDay(d);
  if(!lost.length)return '';
  return `<div class="strip chg chg-gone" data-chggone="1">
    <div class="rail"></div>
    <div class="scn">${esc(lost.join(', '))}</div>
    <div class="ie">—</div>
    <div class="body"><div class="slug">Cut from this day</div><div class="desc">${lost.length===1?'This scene was':'These scenes were'} on D${esc(String(d.num))} in ${esc((activeRevDiff()||{}).prevLabel||'the last schedule')} and ${lost.length===1?'is':'are'} not any more.</div></div>
  </div>`;
}
// ---- the "Changes since …" strip -----------------------------------------
/** One calm row above the board: what moved, what it costs, and the switches
 *  for seeing it. Dismissible per person per revision so it stops nagging. */
function renderChangeBar(){
  const host=document.getElementById('chgBar');
  if(!host)return;
  const sd=activeRevDiff();
  const onBoard=CUR_VIEW==='days'||CUR_VIEW==='cal';
  if(!sd||!MODEL||!onBoard){host.innerHTML='';syncChangeUI();return}
  const n=chgDayCount(), review=daysNeedingReview();
  const money=chgMoneyKnown()?chgMoneyText(revDiffMoney()):'';
  if(chgDismissed()){
    // dismissed: one quiet line that still lets you get back in
    host.innerHTML=`<div class="chgbar dismissed">
      <span class="chgbar-lead">${icon('refresh')} Changes since ${esc(sd.prevLabel)} — hidden</span>
      <button type="button" class="chg-btn" data-chgrestore>Show them again</button>
      <button type="button" class="chg-btn" data-chgopen>Open the full list</button>
    </div>`;
  }else{
    host.innerHTML=`<div class="chgbar">
      <div class="chgbar-main">
        <span class="chgbar-lead">${icon('refresh')} <b>Changes since ${esc(sd.prevLabel)}</b></span>
        <span class="chgbar-sum">${esc(storedDiffHeadline(sd))}</span>
        ${money?`<span class="chg-chip money ${revDiffMoney()>0?'up':'down'}">${esc(money)} crowd cost</span>`
               :`<span class="chg-chip unknown" data-tip="The old schedule could not be costed here, so the money movement is not shown rather than shown as zero">Money impact not worked out</span>`}
        ${review?`<span class="chg-chip check">${icon('warn')} ${review} day${review===1?' needs':'s need'} checking</span>`:''}
      </div>
      ${(()=>{
        const cx=cancelCharge();
        if(!cx)return '';
        const nd=cx.days.length;
        return `<div class="chgbar-note">${icon('warn')} <span><b>${esc(gbpWhole(cx.charge))} of the dropped ${nd===1?'day is':'days are'} still payable.</b> ${nd===1?'It was':'They were'} cut inside your ${cx.noticeDays}-day cancellation notice period, so the agency still charges ${esc(String(cx.pct))}% of ${nd===1?'that day':'those days'}. The saving above is what you actually keep — ${esc(gbpWhole(cx.fullCost))} of work dropped, ${esc(gbpWhole(cx.charge))} still owed.${cx.undated?` ${cx.undated} dropped day${cx.undated===1?' has':'s have'} no date, so ${cx.undated===1?'it is':'they are'} not charged — check ${cx.undated===1?'it':'them'}.`:''}</span></div>`;
      })()}
      <div class="chgbar-acts">
        <label class="chk"><input type="checkbox" id="chgShow"${CHG_SHOW?' checked':''}> Highlight changes</label>
        <label class="chk${n?'':' off'}"><input type="checkbox" id="chgOnly"${CHG_ONLY?' checked':''}${n?'':' disabled'}> Changed days only<span class="chgbar-n">${n}</span></label>
        <button type="button" class="chg-btn primary" data-chgopen>What changed</button>
        <button type="button" class="chg-btn" data-chgdismiss data-tip="Hide this strip for this revision only — the next schedule brings it back">I've read this</button>
      </div>
      ${n?'':`<div class="chgbar-note">${icon('warn')} <span>${esc(emptyFilterWhy(sd))}</span></div>`}
      ${CHG_SHOW?`<div class="chgbar-key" aria-label="What the edge markers on the board mean">
        <span class="chgbar-keylead">Edges on the board:</span>
        <span><i class="chg-key new"></i>New, or moved here</span>
        <span><i class="chg-key changed"></i>Changed since ${esc(sd.prevLabel)}</span>
        <span><i class="chg-key down"></i>Scenes or money gone</span>
        <span><i class="chg-key check"></i>Saved settings need checking</span>
        <span class="chgbar-keynote">Every edge is also spelled out in words, so nothing depends on colour.</span>
      </div>`:''}
    </div>`;
  }
  syncChangeUI();
}
/** The strip's switches and the board's markers are rendered from the same
 *  state (chgOn / chgDismissed), so nothing extra has to be kept in step —
 *  this is the hook the switch handlers call after changing it. */
function syncChangeUI(){}
// switches and buttons on the strip
document.addEventListener('change',e=>{
  const t=/** @type {any} */(e.target);
  if(!t||!t.id)return;
  if(t.id==='chgShow'){CHG_SHOW=!!t.checked;if(!CHG_SHOW)CHG_ONLY=false;renderDays();return}
  if(t.id==='chgOnly'){CHG_ONLY=!!t.checked;applyFilters();return}
});
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(t.closest&&t.closest('[data-chgdismiss]')){chgDismiss();CHG_ONLY=false;CHG_SHOW=false;renderDays();return}
  if(t.closest&&t.closest('[data-chgrestore]')){chgUndismiss();CHG_SHOW=true;renderDays();return}
  if(t.closest&&t.closest('[data-chgopen]')){openChangeReport();return}
  const re=t.closest&&t.closest('[data-chgreseed]');
  if(re){
    const d=(MODEL.days||[]).find(x=>x.id===re.dataset.chgreseed);
    if(d){reseedDayConfig(d);refreshAll();setStatus(`D${d.num} now uses the new schedule's own crowd numbers, hours and travel band.`)}
    return;
  }
  const kp=t.closest&&t.closest('[data-chgkeep]');
  if(kp){
    const d=(MODEL.days||[]).find(x=>x.id===kp.dataset.chgkeep);
    if(d){confirmDayConfig(d);refreshAll();setStatus(`D${d.num} kept your saved settings — it is no longer flagged for checking.`)}
    return;
  }
});
// ---- "What changed" — the full account, uncapped ---------------------------
// This is the thing the crowd department prints and emails. It is therefore
// COMPLETE: every change, grouped by the day it happened on, with the money
// each day moved by. Nothing is capped, nothing is hidden behind "+22 more".
let CHG_REPORT_SRC=-1;
function chgReportSrc(){return SOURCES[CHG_REPORT_SRC]||SOURCES[ACTIVE]}
function chgReportDiff(){return srcRevDiff(chgReportSrc())}
/** The report as sheet rows — one row per change line, so CSV/Excel open flat
 *  and sortable. Reuses the app's own sheet exporters. */
function chgReportSheets(){
  const sd=/** @type {any} */(chgReportDiff());
  if(!sd)return [];
  const money=sd.moneyUnknown?'':'Crowd cost change (£)';
  const rows=[['Day','What changed'].concat(money?[money]:[])];
  for(const g of sd.groups||[]){
    let first=true;
    for(const l of g.lines||[]){
      rows.push([first?g.label:'',l].concat(money?[first&&g.money?String(g.money):'']:[]));
      first=false;
    }
    if(!(g.lines||[]).length)rows.push([g.label,'—'].concat(money?['']:[]));
  }
  if(!sd.moneyUnknown)rows.push(['Total','',String(storedDiffMoney(sd))]);
  return [{name:sheetName('What changed',[]),rows}];
}
function chgReportHTML(idx){
  const src=SOURCES[idx]||SOURCES[ACTIVE];
  const sd=/** @type {any} */(srcRevDiff(src));
  if(!sd)return `<div class="rev-sec"><div class="revline none">This revision has nothing to compare against — it is the first schedule for this unit.</div></div>`;
  const live=idx===ACTIVE;
  // The stored figure counts a dropped day as a full saving. Where cancellation
  // terms were in force it is not — add the charge back so this report says
  // what is genuinely saved, and print the charge as its own line beneath.
  const cx=(sd.cancel&&sd.cancel.charge)?sd.cancel:null;
  const total=storedDiffMoney(sd)+(cx?cx.charge:0);
  const c=sd.counts||{};
  const crowdMove=(Number(c.crowdAfter)||0)-(Number(c.crowdBefore)||0);
  let html=`<div class="chg-report">
    <div class="chg-hero">
      <div class="chg-hero-line">${esc(storedDiffHeadline(sd))}</div>
      <div class="chg-hero-figs">
        <span class="chg-fig"><b>${(Number(c.daysNew)||0)+(Number(c.daysChanged)||0)+(Number(c.daysCut)||0)}</b> days affected</span>
        <span class="chg-fig"><b>${(Number(c.scenesAdded)||0)+(Number(c.scenesMoved)||0)+(Number(c.scenesCut)||0)+(Number(c.scenesChanged)||0)}</b> scene changes</span>
        <span class="chg-fig">Crowd on the schedule <b>${(Number(c.crowdBefore)||0).toLocaleString()} → ${(Number(c.crowdAfter)||0).toLocaleString()}</b>${crowdMove?` <span class="chg-chip crowd ${crowdMove>0?'up':'down'}">${crowdMove>0?'+':'−'}${Math.abs(crowdMove).toLocaleString()}</span>`:''}</span>
        ${sd.moneyUnknown
          ?`<span class="chg-fig warnfig">${icon('warn')} Money impact could not be worked out — the previous schedule could not be costed, so no figure is shown rather than a misleading £0.</span>`
          :`<span class="chg-fig"><b class="chg-total ${total>0?'up':total<0?'down':''}">${total?(total>0?'+':'−')+gbp(Math.abs(total)):gbp(0)}</b> crowd cost</span>`}
        ${cx?`<span class="chg-fig warnfig">${icon('warn')} ${esc(gbpWhole(cx.charge))} of the dropped days is still payable: ${cx.days.length===1?'it was':'they were'} cut inside your ${cx.noticeDays}-day notice period, so the agency charges ${esc(String(cx.pct))}% of ${cx.days.length===1?'that day':'those days'}. ${esc(gbpWhole(cx.fullCost))} of work was dropped; ${esc(gbpWhole(cx.saved))} of it is actually saved.${cx.undated?` ${cx.undated} dropped day${cx.undated===1?' has':'s have'} no date, so no charge is assumed for ${cx.undated===1?'it':'them'} — check ${cx.undated===1?'it':'them'} yourself.`:''}</span>`:''}
      </div>
    </div>`;
  if(!(sd.groups||[]).length)html+=`<div class="rev-sec"><div class="revline none">Nothing changed between these two schedules.</div></div>`;
  for(const g of sd.groups||[]){
    const m=(!sd.moneyUnknown&&g.money)?`<span class="chg-chip money ${g.money>0?'up':'down'}">${g.money>0?'+':'−'}${gbp(Math.abs(g.money))}</span>`:'';
    html+=`<section class="chg-grp">
      <h4>${live&&g.dayId?`<button type="button" class="chg-jump" data-chgjump="${esc(g.dayId)}">${esc(g.label)}</button>`:`<span>${esc(g.label)}</span>`}${m}</h4>
      <ul>${(g.lines||[]).map(l=>`<li>${esc(l)}</li>`).join('')}</ul>
    </section>`;
  }
  html+=`<div class="chg-foot">Compared against <b>${esc(sd.prevLabel)}</b> · read on ${esc(fmtStamp(sd.at))} · every change is listed, nothing is truncated.</div></div>`;
  return html;
}
function openChangeReport(idx){
  CHG_REPORT_SRC=(idx==null||idx<0)?ACTIVE:idx;
  const src=chgReportSrc();
  const sd=chgReportDiff();
  const m=$('#chgModal');if(!m)return;
  $('#chgCrumbs').innerHTML=`<b>${esc((src&&(src.prod||src.title))||'')}</b> · ${esc((src&&src.unit)||'Main')} Unit`;
  const lbl=src?revLabel(src):'This schedule';
  $('#chgSub').textContent=sd
    ?`${lbl} compared with ${sd.prevLabel}`
    :`${lbl} — first schedule for this unit`;
  $('#chgReport').innerHTML=chgReportHTML(CHG_REPORT_SRC);
  m.classList.add('open');
}
document.addEventListener('click',e=>{
  const t=/** @type {any} */(e.target);
  if(!t.closest)return;
  if(t.closest('#chgClose')||(t.id==='chgModal')){const m=$('#chgModal');if(m)m.classList.remove('open');return}
  const j=t.closest('[data-chgjump]');
  if(j){const m=$('#chgModal');if(m)m.classList.remove('open');gotoDay(j.dataset.chgjump);return}
  const x=t.closest('[data-chgexp]');
  if(x){
    const fmt=x.dataset.chgexp;
    const src=chgReportSrc();
    const base=('What changed — '+((src&&(src.prod||src.title))||PRODUCT)+' — '+(src?revLabel(src):'')+' — '+exportStamp()).replace(/[\\/:*?"<>|]+/g,'-');
    if(fmt==='pdf'){printChangeReport();return}
    const sheets=chgReportSheets();
    if(!sheets.length){setStatus('There is nothing to export — this revision has no earlier schedule to compare against.');return}
    if(fmt==='xlsx'){exportSheetsXlsx(sheets,base);setStatus('Excel file downloaded — open it, or import it into Google Sheets (File → Import).')}
    else{downloadBlob(base+'.csv','﻿'+sheetsToCsv(sheets),'text/csv;charset=utf-8');setStatus('CSV downloaded — open it in Excel, or import it into Google Sheets (File → Import).')}
    const dt=x.closest('details');if(dt)dt.removeAttribute('open');
    return;
  }
});
document.addEventListener('keydown',e=>{if(e.key==='Escape'){const m=document.getElementById('chgModal');if(m)m.classList.remove('open')}});
/** Print the report through the browser's own Save-as-PDF, the same iframe
 *  path every other page uses — a full revision report runs to many pages, so
 *  it needs pagination that tracks the content. */
function printChangeReport(){
  const host=$('#chgReport');if(!host)return;
  const node=host.cloneNode(true);
  stripScreenOnly(node);
  printNodeInFrame(node,{
    title:'What changed — '+exportProdTitle(),
    page:'A4 portrait',bodyClass:'printing-chg',
    css:`.chg-day,.chg-sec{break-inside:avoid;page-break-inside:avoid}`,
  });
}
// What the person is told when work could not be moved. Plain sentences, one
// per piece of work, so nothing disappears without being named.
function carryCollisionLines(collisions){
  return (collisions||[]).map(c=>c.kind==='scene'
    ?`Scene ${c.scene} — your ${c.label} stayed on ${c.from}: ${c.to} already has different work saved for that scene.`
    :`${c.from} — your ${c.label} stayed where it was: ${c.to} already has its own saved settings.`);
}
// Say what actually happened, including anything that did not move.
function reportCarryResult(res,stitched,lead){
  res=res||{n:0,collisions:[]};
  const bits=[];
  if(res.n)bits.push(res.n+' of your edits carried over');
  if(stitched)bits.push(stitched+' already-shot day'+(stitched===1?'':'s')+' kept');
  const head=(lead||'Published')+(bits.length?' — '+bits.join(', '):'');
  const lines=carryCollisionLines(res.collisions);
  if(lines.length){
    setStatus(head+(bits.length?'. ':' — ')+lines.length+' thing'+(lines.length===1?'':'s')+' could not be moved: '+lines.slice(0,3).join(' ')+(lines.length>3?' Plus '+(lines.length-3)+' more.':''),{sticky:true});
    return;
  }
  if(bits.length)setStatus(head+'.');
}
...
// Advance ONE key of the "last known saved" baseline. Read-modify-write so
// two saves finishing at once can't undo each other's progress.
function shadowCommit(kind,k,val){
  let sh={};try{sh=JSON.parse(SHADOW[kind])}catch(e){sh={}}
  if(val===undefined)delete sh[k];else sh[k]=val;
  SHADOW[kind]=JSON.stringify(sh);
}
// work whose save to the account has failed and is still being retried:
// day edits (cday/adj) and whole schedules that never got a row of their own
const SYNC_FAIL={cday:new Set(),adj:new Set(),prod:new Set(),files:new Set()};
const SYNC_RETRY={cday:{timer:null,delay:0},adj:{timer:null,delay:0},prod:{timer:null,delay:0},files:{timer:null,delay:0}};
let SYNC_WARN_UP=false;
function unsavedEditCount(){return SYNC_FAIL.cday.size+SYNC_FAIL.adj.size+SYNC_FAIL.prod.size+SYNC_FAIL.files.size}
// Publishing must never report a success it hasn't got. A failed insert used
// to leave the freshly carried edits on this device only, marked as synced,
// with nothing retrying — the same problem the day-edit retry already solves,
// so it uses the same mechanism rather than a second one.
const PROD_SAVE_PENDING=new Map(); // schedule id → {src,files}
function saveSourceToCloud(src,opts){
  if(!CLOUD.session||!src)return;
  const uid=srcUid(src);
  const files=(opts&&opts.files)||[];
  PROD_SAVE_PENDING.set(uid,{src,files});
  const failed=e=>{
    src.cloudFailed=true;
    SYNC_FAIL.prod.add(uid);
    refreshUnsavedWarning();
    scheduleSyncRetry('prod');
  };
  cloud.insertProduction(src).then(({id,error})=>{
    if(error||!id){failed(error);return}
    src.cloudFailed=false;src.cloudId=id;
    // brand-new row: nothing of this schedule's is in the account yet, so an
    // empty subset really is empty (cloudSyncBlob's delete guard)
    src.blobsLoaded=true;
    PROD_SAVE_PENDING.delete(uid);SYNC_FAIL.prod.delete(uid);SYNC_RETRY.prod.delay=0;
    saveUserSources();
    ...
    resyncNsMaps(src.ns);pushAllBlobs();
    if(files.length)uploadSourceFilesToCloud(src,files);
    refreshUnsavedWarning();
  },failed);
}
function retryProdSaves(){
  for(const [uid,p] of [...PROD_SAVE_PENDING]){
    if(!p.src||!SOURCES.includes(p.src)||p.src.cloudId){PROD_SAVE_PENDING.delete(uid);SYNC_FAIL.prod.delete(uid);continue}
    saveSourceToCloud(p.src,{files:p.files});
  }
  refreshUnsavedWarning();
}
// A failed save must not fade away after five seconds — the person needs to
// know there is work on this device that isn't in their account yet.
function refreshUnsavedWarning(){
  const n=unsavedEditCount();
  if(n){
    SYNC_WARN_UP=true;
    const sched=SYNC_FAIL.prod.size,docs=SYNC_FAIL.files.size;
    const rest=n-sched-docs;
    const bits=[];
    if(sched)bits.push(sched+' schedule'+(sched===1?'':'s'));
    if(docs)bits.push(docs+' original document'+(docs===1?'':'s'));
    if(rest)bits.push(rest+' change'+(rest===1?'':'s'));
    const what=bits.length>1?bits.slice(0,-1).join(', ')+' and '+bits[bits.length-1]:bits[0];
    setStatus('Couldn’t save '+what+' to your account yet — still trying. Keep this tab open and stay on this computer until this message clears.',{sticky:true});
  }else if(SYNC_WARN_UP){
    SYNC_WARN_UP=false;
    setStatus('Everything is saved to your account.');
  }
}
// ---------- don't let a reload throw work away ----------
// There was no unload guard at all, so a stray ⌘R or a back-swipe during an
// import review — or while a failed save was still being retried — lost the
// lot silently. The prompt only appears when there really is something at
// risk; a browser that has everything safely in the account never sees it.
function unsavedWorkNote(){
  if(unsavedEditCount())return 'Some of your changes haven’t saved to your account yet.';
  if(TAB_STALE)return 'This page is out of date, so anything you have changed here since has not been saved to your account.';
  if($('#revPage')&&$('#revPage').classList.contains('open'))return 'You’re part-way through reviewing a schedule and haven’t published it yet.';
  if(typeof MEM_AUTHORITATIVE!=='undefined'&&MEM_AUTHORITATIVE)return 'This browser ran out of room to save, so your latest work is only in this tab.';
  return '';
}
// a debounced write must never be lost to a closing tab or a switched app
window.addEventListener('pagehide',()=>{try{flushCDAY();flushBlobTimers()}catch(e){}});
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden')try{flushCDAY();flushBlobTimers()}catch(e){}
});
window.addEventListener('beforeunload',e=>{
  try{flushCDAY();flushBlobTimers()}catch(e){}
  const note=unsavedWorkNote();
  if(!note)return;
  // Browsers show their own wording now, but returnValue is still what arms
  // the prompt — and older ones do show this sentence.
  e.preventDefault();
  e.returnValue=note+' Leave this page and it will be lost.';
  return e.returnValue;
});
// retry the whole map: anything still different from the baseline is re-sent
function scheduleSyncRetry(kind){
  const r=SYNC_RETRY[kind];
  if(r.timer)return;
  r.delay=Math.min(r.delay?r.delay*2:3000,60000);
  r.timer=setTimeout(()=>{
    r.timer=null;
    if(kind==='prod')retryProdSaves();
    else if(kind==='files')retryFileUploads();
    else cloudSyncMap(kind);
  },r.delay);
}
...
// The baseline only moves forward for writes that actually SUCCEEDED — moving
// it on a failed write told every later save the edit was already stored, and
// the next sign-in rebuilt from the account without it.
function cloudSyncMap(kind){
  // a tab that knows it is behind never writes: see markTabStale
  if(!CLOUD.session||TAB_STALE)return;
  ...
  for(const k of new Set([...Object.keys(prev),...Object.keys(cur)])){
    ...
    // demo edits and productions whose account copy hasn't been created yet
    // stay on this device by design (resyncNsMaps re-runs when the id lands)
    if(pid===undefined){shadowCommit(kind,k,cur[k]);continue}
    stillOutstanding.add(k);
    const val=cur[k];
    const op=val===undefined
      ?cloud.deleteDayEdit(pid,plain,kind)
      :cloud.upsertDayEdit(pid,plain,kind,val);
    const ok=()=>{shadowCommit(kind,k,val);SYNC_FAIL[kind].delete(k);SYNC_RETRY[kind].delay=0;refreshUnsavedWarning()};
    const bad=()=>{SYNC_FAIL[kind].add(k);failedNow=true;refreshUnsavedWarning();scheduleSyncRetry(kind)};
    op.then(r=>{if(r&&r.error)bad();else ok()},()=>bad());
  }
  // an edit that has since been undone, or whose production has been deleted,
  // is no longer outstanding — otherwise the warning would never clear
  for(const k of [...SYNC_FAIL[kind]])if(!stillOutstanding.has(k))SYNC_FAIL[kind].delete(k);
  if(!failedNow&&!SYNC_FAIL[kind].size)SYNC_RETRY[kind].delay=0;
  refreshUnsavedWarning();
}
...
function updateAccountUI(){
  ...
  // whose account this is — never the name of the agency that built the app
  const sw=$('#sideWho');if(sw)sw.textContent=s?(displayName(s)||'Signed in'):'Not signed in';
}
...
$('#auGoogle').addEventListener('click',async()=>{
  gateSay('Taking you to Google…');
  const {error}=await cloud.signInWithGoogle();
  if(error)gateSay(friendlyAuthError(error));
});

// ============================================================================
// The gate: sign in · create an account · reset a forgotten password.
//
// Three states in one card rather than three screens, because the fields are
// almost the same and the person is only ever one press from the one they
// actually wanted. Everything the user is told here is written for an AD or a
// producer — no Supabase strings reach the screen (see friendlyAuthError).
// ============================================================================
let GATE_MODE='signin'; // 'signin' | 'signup' | 'forgot'
/**
 * The one line under the sign-in fields.
 *
 * `ok` marks good news (an email has been sent), `busy` marks progress. Neither
 * is an error, and only an error is coloured as one — the whole thing used to
 * be olive-amber, which reads as a warning even when the news is good, and
 * never as the error red every other failure on the page uses.
 */
function gateSay(msg,ok,busy){
  const el=$('#auStatus');if(!el)return;
  el.textContent=msg||'';
  el.classList.toggle('ok',!!ok);
  el.classList.toggle('busy',!!busy);
  el.classList.toggle('bad',!!msg&&!ok&&!busy);
}
function setGateMode(m){
  GATE_MODE=m;
  const signup=m==='signup',forgot=m==='forgot',signin=m==='signin';
  $('#gateTitle').textContent=forgot?'Reset your password':signup?('Create your '+PRODUCT+' account'):('Welcome to '+PRODUCT);
  $('#gateSub').textContent=forgot
    ?'Type the email you signed up with and we will send you a link to set a new password.'
    :signup?'A few details and you are in. It takes about thirty seconds.'
    :'Sign in to your account to continue';
  $('#auNameRow').classList.toggle('hidden',!signup);
  $('#auPassField').classList.toggle('hidden',forgot);
  $('#auSignIn').classList.toggle('hidden',!signin);
  $('#auCreate').classList.toggle('hidden',!signup);
  $('#auSendReset').classList.toggle('hidden',!forgot);
  // Google is a way IN, not a way to reset a password you set here
  $('#auOr').classList.toggle('hidden',forgot);
  $('#auGoogle').classList.toggle('hidden',forgot);
  $('#auFootSignIn').classList.toggle('hidden',!signin);
  $('#auFootSwap').classList.toggle('hidden',!signin);
  $('#auFootBack').classList.toggle('hidden',signin);
  $('#auPass').setAttribute('autocomplete',signup?'new-password':'current-password');
  // C12: the "at least N characters" hint is advice for CHOOSING a password.
  // On the sign-in state the password already exists, so the hint was telling
  // people who were simply signing in that their own password was wrong.
  $('#auPassHint').textContent=signup?'At least 10 characters. Pick something you will remember — or save it in your password manager now.':'';
  $('#auPassHint').classList.toggle('hidden',!signup);
  gateSay('');
  (signup?$('#auFirst'):$('#auEmail')).focus();
}
/**
 * Turn whatever the sign-in service said into something an AD can act on.
 * Supabase's own wording ("Invalid login credentials", "Failed to fetch") is
 * either jargon or actively misleading about whose fault it is.
 */
function friendlyAuthError(err){
  const m=((err&&err.message)||String(err||'')).toLowerCase();
  if(!m)return 'Something went wrong. Try again, and email '+SUPPORT_EMAIL+' if it keeps happening.';
  if(/invalid login credentials|invalid grant/.test(m))
    return 'That email and password don’t match an account. Check for a typo — or use “Reset it” underneath if you’ve forgotten the password.';
  if(/email not confirmed/.test(m))
    return 'Your account exists but the email address hasn’t been confirmed yet. Open the email we sent when you signed up and click the link, then come back here.';
  if(/user already registered|already been registered/.test(m))
    return 'There is already an account with that email. Sign in instead — or use “Reset it” underneath if you’ve forgotten the password.';
  if(/password should be at least|password.*\d+ characters|weak password/.test(m))
    return 'That password is too short. It needs to be at least '+PW_MIN+' characters.';
  if(/same as the old password|different from the old/.test(m))
    return 'That’s the password you already had. Choose a different one.';
  if(/failed to fetch|network|load failed|fetch event|timeout|timed out/.test(m))
  ...
  if(/rate limit|too many requests|over_email_send_rate/.test(m))
    return 'That’s a few too many attempts in a row. Wait a minute, then try again.';
  ...
  if(/expired|invalid.*(token|link)|otp/.test(m))
    return 'That link has expired. Ask for a new reset email and use the newest one.';
  if(/disabled|not enabled|signups not allowed/.test(m))
    return 'Accounts can’t be created here at the moment. Email '+SUPPORT_EMAIL+' and we’ll sort it out.';
  return 'Something went wrong signing you in. Try again, and email '+SUPPORT_EMAIL+' if it keeps happening.';
}
const emailLooksReal=e=>/^[^@\s]+@[^@\s.]+\.[^@\s]+$/.test(e);

async function gateSignIn(){
  const email=$('#auEmail').value.trim(),pw=$('#auPass').value;
  if(!email){gateSay('Type the email address you signed up with.');$('#auEmail').focus();return}
  // Catch an obviously incomplete address here rather than making a round trip
  // and coming back with the service's own wording.
  if(!emailLooksReal(email)){gateSay('That doesn’t look like a complete email address — check it and try again.');$('#auEmail').focus();return}
  if(!pw){gateSay('Type your password.');$('#auPass').focus();return}
  gateSay('Signing you in…',false,true);
  const {error}=await cloud.signIn(email,pw);
  gateSay(error?friendlyAuthError(error):'');
}
// ONE press, one clear form. The old two-stage version revealed the name
// fields on the first press and asked for a second press with a button that
// still said "Create account" — which reads as a failure, not a step.
async function gateCreate(){
  const email=$('#auEmail').value.trim(),pw=$('#auPass').value;
  const first=$('#auFirst').value.trim(),last=$('#auLast').value.trim();
  const role=$('#auRole').value;
  if(!first||!last){gateSay('Add your first name and surname.');$('#auFirst').focus();return}
  if(!role){gateSay('Choose what you do — it decides which department the app opens on.');$('#auRole').focus();return}
  if(!emailLooksReal(email)){gateSay('Add the email address you want to sign in with.');$('#auEmail').focus();return}
  if(pw.length<PW_MIN){gateSay('Choose a password of at least '+PW_MIN+' characters.');$('#auPass').focus();return}
  gateSay('Creating your account…',false,true);
  const {data,error}=await cloud.signUp(email,pw,first,last,role);
  // Deliberately the SAME answer whether or not that email already has an
  // account — the same care the reset flow takes. Saying "there is already an
  // account with that email" let anyone type addresses in and find out who
  // has signed up.
  const inbox='Check your inbox — we’ve emailed '+email+'. Open it and click the link to confirm your account, then come back and sign in.';
  if(error){
    if(/user already registered|already been registered|already exists/i.test(error.message||'')){gateSay(inbox,true);return}
    gateSay(friendlyAuthError(error));return;
  }
  if(data&&data.session){gateSay('');return}
  gateSay(inbox,true);
}
async function gateSendReset(){
  const email=$('#auEmail').value.trim();
  if(!emailLooksReal(email)){gateSay('Type the email address you signed up with.');$('#auEmail').focus();return}
  gateSay('Sending…',false,true);
  const {error}=await cloud.resetPasswordForEmail(email);
  // Deliberately the same answer whether or not the address has an account:
  // saying "no such account" would let anyone test who has signed up.
  if(error&&/rate limit|too many/i.test(error.message||'')){gateSay(friendlyAuthError(error));return}
  gateSay('If there’s an account for '+email+', a reset link is on its way. Open it on this device — it takes you straight back here to choose a new password. It expires in an hour, and the email can take a minute or two to arrive. Check your spam folder if it doesn’t.',true);
}
$('#auSignIn').addEventListener('click',gateSignIn);
$('#auCreate').addEventListener('click',gateCreate);
$('#auSendReset').addEventListener('click',gateSendReset);
$('#auSignUp').addEventListener('click',()=>setGateMode('signup'));
$('#auForgot').addEventListener('click',()=>{setGateMode('forgot')});
$('#auBackToSignIn').addEventListener('click',()=>setGateMode('signin'));
// Enter submits, from any field on the card. Without this, typing a password
// and pressing Enter — which is what everyone does — did nothing at all.
/**
 * Keep the keyboard inside whichever full-page gate is up, and take everything
 * behind it out of the tab order entirely. `inert` does the real work (it also
 * hides the app from a screen reader); the Tab wrap is the belt-and-braces for
 * browsers that do not have it.
 */
function gateShown(){
  const g=/** @type {any} */(document.getElementById('pwReset'));
  if(g&&!g.classList.contains('hidden'))return g;
  const a=/** @type {any} */(document.getElementById('gate'));
  if(a&&!a.classList.contains('hidden'))return a;
  return null;
}
function syncGateInert(){
  const on=!!gateShown();
  for(const sel of ['.layout','#statusBar']){
    const el=/** @type {any} */(document.querySelector(sel));
    if(!el)continue;
    if(on){try{el.inert=true}catch(e){}el.setAttribute('aria-hidden','true');}
    else{try{el.inert=false}catch(e){}el.removeAttribute('aria-hidden');}
  }
  // The gate stacks OVER any open dialog, so a dialog's own trap must not fight
  // it — the gate wins while it is up.
  const g=gateShown();
  if(g&&!g.contains(document.activeElement)){
    const f=modalFocusables(g);
    if(f.length)try{f[0].focus({preventScroll:true})}catch(e){}
  }
}
{
  const mo=new MutationObserver(syncGateInert);
  for(const id of ['gate','pwReset']){
...
document.addEventListener('keydown',e=>{
  if(e.key!=='Tab')return;
  const g=gateShown();if(!g)return;
  const f=modalFocusables(g);
  if(!f.length)return;
  const first=f[0],last=f[f.length-1];
  const cur=/** @type {any} */(document.activeElement);
  if(!g.contains(cur)){e.preventDefault();(e.shiftKey?last:first).focus();return}
  if(e.shiftKey&&cur===first){e.preventDefault();last.focus()}
  else if(!e.shiftKey&&cur===last){e.preventDefault();first.focus()}
},true);
$('#gate').addEventListener('keydown',e=>{
  ...
  const t=/** @type {any} */(e.target);
  if(!t||!t.closest||!t.closest('.gate-field'))return;
  e.preventDefault();
  if(GATE_MODE==='signup')gateCreate();
  else if(GATE_MODE==='forgot')gateSendReset();
  else gateSignIn();
});

// ---------- back from the reset email ----------
// Supabase hands the user back with a short-lived recovery session. That looks
// exactly like a normal sign-in to the rest of the app, so it has to be caught
// before the board paints, and the ONE thing that session is good for offered.
// Held in sessionStorage, not just in memory: a RELOAD used to drop the flag
// and hand whoever holds the emailed link a fully signed-in board instead of
// the "choose a new password" panel. It clears only when a new password has
// actually been saved (or the person cancels, which ends the session).
const PW_RECOVERY_FLAG='crowdos-pw-recovery';
let PW_RECOVERY=false;
function pwRecoveryPending(){
  if(PW_RECOVERY)return true;
  try{return window.sessionStorage.getItem(PW_RECOVERY_FLAG)==='1'}catch(e){return false}
}
function openPasswordReset(){
  PW_RECOVERY=true;
  try{window.sessionStorage.setItem(PW_RECOVERY_FLAG,'1')}catch(e){}
...
function closePasswordReset(){
  PW_RECOVERY=false;
  try{window.sessionStorage.removeItem(PW_RECOVERY_FLAG)}catch(e){}
  $('#pwReset').classList.add('hidden');
}
async function savedNewPassword(){
  const a=$('#pwNew').value,b=$('#pwNew2').value;
  const say=(m,ok)=>{$('#pwStatus').textContent=m;$('#pwStatus').classList.toggle('ok',!!ok)};
  if(a.length<PW_MIN){say('Choose a password of at least '+PW_MIN+' characters.');$('#pwNew').focus();return}
  if(a!==b){say('Those two don’t match — type the same password in both boxes.');$('#pwNew2').focus();return}
  say('Saving…');
  const {error}=await cloud.updatePassword(a);
  if(error){say(friendlyAuthError(error));return}
  say('Saved. Signing you in…',true);
  closePasswordReset();
  setStatus('Your new password is saved — that’s the one to use from now on. Anyone who was signed in to your account anywhere else has been signed out.');
  // the board was deliberately held back while the reset panel was up
  if(CLOUD.session&&!CLOUD.hydrated){CLOUD.hydrated=true;updateAccountUI();cloudHydrate()}
}
$('#pwSave').addEventListener('click',savedNewPassword);
$('#pwCancel').addEventListener('click',async()=>{
  closePasswordReset();
  // the recovery session is no use for anything else — end it, so the gate
  // isn't left showing a half-signed-in state
  try{await cloud.signOut()}catch(e){}
  setGateMode('signin');
});
$('#pwReset').addEventListener('keydown',e=>{
  if(e.key==='Enter'&&/** @type {any} */(e.target).closest('.gate-field')){e.preventDefault();savedNewPassword();}
});
// Supabase puts `type=recovery` in the URL fragment before it swaps it for a
// session, so the panel can be up before the auth callback even lands.
try{
  const h=window.location.hash||'';
  const hashRec=/type=recovery/.test(h);
  const queryRec=/[?&]type=recovery/.test(window.location.search||'');
  // The hash form carries the one-time token itself, so the panel goes up
  // straight away — before the board can paint. A bare ?type=recovery in the
  // address bar carries nothing at all, and used to be enough for anyone to
  // put "Choose a new password" over someone else's board: it now only counts
  // when this browser really is holding a reset session.
  if(hashRec||pwRecoveryPending())openPasswordReset();
  else if(queryRec)cloud.getSession().then(s=>{if(s)openPasswordReset()}).catch(()=>{});
}catch(e){}
...
// Rate cards, the glossary, the risk assessments and the calculators used to
// be left behind here, on the grounds that they never migrate into another
// account and are sometimes the only copy. Both halves stopped being true:
// they all have account tables now, and the sign-in routine pushes a local
// copy up — so on a shared production-office computer one person's rate cards
// and risk assessments were written into the next person's account, and were
// on screen before that person had typed a password. Everything cloud-backed
// is cleared; clearAccountLocalData holds back anything whose account copy
// hasn't been confirmed.
// 'crowdos-files-idx' is in here with the raw uploaded documents it indexes
// (cleared from IndexedDB alongside it) — otherwise the next person on a
// shared machine could open the previous production's confidential PDFs.
const SIGNOUT_CLEAR=['crowdos-sources','crowdos-prods','crowdos-manualdays','crowdos-sced',
  ...
  'crowdos-files-idx','stuntos-cday','stuntos-adj','stuntos-notes',
  'crowdos-ratecards2','crowdos-glossary','stuntos-ra','stuntos-raedits',
  'stuntos-freecalc','stuntos-stuntcalc','crowdos-dancecalc',
  'crowdos-fcrows','crowdos-srows','crowdos-drows'];
// which account-backed store each of those keys belongs to, so a key whose
// save to the account failed (or never happened) is kept rather than wiped
const SIGNOUT_KINDS={
  'crowdos-sced':['sced'],'crowdos-stuntday':['stuntday'],'crowdos-stuntcfg':['stuntcfg'],
  'stuntos-notes':['notes'],'crowdos-dayloc':['dayloc'],'crowdos-briefs':['briefs','dbriefs'],
  'crowdos-ratecards2':['ratecards'],'crowdos-glossary':['glossary'],
  'stuntos-ra':['ra'],'stuntos-raedits':['raedits'],'stuntos-freecalc':['freecalc'],
  'stuntos-stuntcalc':['stuntcalc'],'crowdos-dancecalc':['dancecalc'],
  'crowdos-fcrows':['fcrows'],'crowdos-srows':['srows'],'crowdos-drows':['drows'],
};
// Returns the keys deliberately LEFT on this device: anything whose save to
// the account hasn't been confirmed, plus anything the browser refused to
// remove. The sign-out handler counts them so the user is told, rather than
// having the work quietly wiped.
function clearAccountLocalData(){
  const kept=[];
  // day-level edits with a save still outstanding — wiping them would lose
  // work that has never reached the account
  const hold=new Set();
  if(SYNC_FAIL.cday.size)hold.add('stuntos-cday');
  if(SYNC_FAIL.adj.size)hold.add('stuntos-adj');
  // a schedule that never got its account row — wiping it here would lose the
  // whole revision and everything carried onto it
  if(SYNC_FAIL.prod.size){hold.add('crowdos-sources');hold.add('crowdos-sced');hold.add('stuntos-notes');}
  // an original document that never reached account storage exists only here
  if(SYNC_FAIL.files.size){hold.add('crowdos-files-idx');hold.add('crowdos-sources');}
  // anything whose account copy isn't confirmed: a note, brief or rate card
  // whose write was refused is the ONLY copy, so it stays on this device
  for(const [key,kinds] of Object.entries(SIGNOUT_KINDS)){
    let has=false;try{has=window.localStorage.getItem(key)!=null}catch(e){has=true}
    if(!has)continue;
    if(kinds.some(k=>CLOUD_FAIL.has(k)||!CLOUD_OK.has(k)))hold.add(key);
  }
  for(const k of SIGNOUT_CLEAR){
    if(hold.has(k)){kept.push(k);continue}
    try{window.localStorage.removeItem(k)}
    catch(e){kept.push(k)}
  }
  return kept;
}
/** Resolve `p`, or give up after `ms` — never let one stalled request hold up
 *  a routine whose whole job is to leave nothing of this account behind. */
...
$('#auSignOut').addEventListener('click',async()=>{
  try{
    // A stalled network used to hang here forever, and everything below —
    // the IndexedDB wipe, the localStorage clear, even the reload in the
    // `finally` — simply never ran. On the shared machine this routine exists
    // for, that leaves the previous account's board on screen and its files on
    // disk. Three seconds, then carry on regardless.
    // anything still waiting to be written must not land AFTER the wipe below
    cancelPendingSaves();
    await withTimeout(cloud.signOut(),3000);
    // the uploaded originals themselves, not just the index of them
    try{FILEIDX.clear()}catch(e){}
    ...
    // Belt and braces for the reload: if the browser is slow to act on it (or
    // blocks it outright), nothing of the previous account should still be in
    // memory for a re-render to paint back onto the screen.
    try{
      SOURCES.length=0;MODEL=null;COST=null;CROWD=null;ACTIVE=0;
      for(const m of [CDAY,ADJ,SCED,NOTES,STUNTCFG,STUNTDAY,DAYLOC,BRIEFS,PRODS,EVENTS,
                      RASET,RAEDITS,FC,SC,DC,RATECARDS.sa,RATECARDS.spact,RATECARDS.stunts])
        if(m&&typeof m==='object')for(const k of Object.keys(m))delete m[k];
      GLOSSARY=[];FCROWS=[];SROWS=[];DROWS=[];
      NOTES_LEGACY=null;REVDIFF_CACHE={sd:null,byDay:null,cut:null};
    }catch(e){}
    // whatever went wrong above, the page MUST reload — leaving the previous
    // account's schedules on screen is the leak this whole routine exists for
    location.reload();
  }
});
...
/** Ask before adopting whatever is already saved on this computer. Names the
 *  productions, because on a shared machine the honest answer is often "those
 *  aren't mine". */
function confirmLocalMigration(){
  const local=SOURCES.filter(s=>s.kind);
  const names=[...new Set(local.map(s=>s.prod||s.title||'Untitled'))];
  const who=(CLOUD.session&&CLOUD.session.user&&CLOUD.session.user.email)||'this account';
  const list=names.slice(0,8).map(n=>' · '+n).join('\n')+(names.length>8?`\n · and ${names.length-8} more`:'');
  return askConfirm({
    title:'Are these yours?',
    body:
      `There ${local.length===1?'is 1 schedule':'are '+local.length+' schedules'} saved on this computer that ${names.length===1?'does not belong':'do not belong'} to any account yet:\n\n${list}\n\n`+
      `Move ${names.length===1?'it':'them'} into ${who}?\n\n`+
      `If this is a shared computer and ${names.length===1?'it belongs':'they belong'} to someone else, choose "Not mine" — ${names.length===1?'it stays':'they stay'} exactly where ${names.length===1?'it is':'they are'} and the person they belong to can sign in here and claim ${names.length===1?'it':'them'}.`,
    yes:`Yes, ${names.length===1?'it is':'they are'} mine`,
    no:'Not mine',
  });
}
async function migrateLocalToCloud(){
  for(const s of SOURCES.filter(x=>x.kind)){
    ...
    if(error){setStatus(friendlyCloudError(error,'your existing schedules'));return false}
    s.cloudId=id;
    for(const d of s.model.days.filter(x=>x.manual)){
      const r=await cloud.upsertManualDay(id,d);
      if(r.error){setStatus(friendlyCloudError(r.error,'your existing shoot days'));return false}
    }
  }
  // The result of these two used to be dropped, so a refused write still
  // returned "migrated" — and the caller then deleted every day config and
  // stunt adjustment on this device and rebuilt from what had actually landed.
  // Every crowd calculator and call/wrap override, gone from both sides.
  for(const [k,v] of Object.entries(CDAY)){
    const {ns,plain}=keyParts(k);const pid=prodIdForNs(ns);
    if(!pid)continue;
    const r=await cloud.upsertDayEdit(pid,plain,'cday',v);
    if(r&&r.error){setStatus(friendlyCloudError(r.error,'your saved day settings'));return false}
  }
  for(const [k,v] of Object.entries(ADJ)){
    const {ns,plain}=keyParts(k);const pid=prodIdForNs(ns);
    if(!pid)continue;
    const r=await cloud.upsertDayEdit(pid,plain,'adj',v);
    if(r&&r.error){setStatus(friendlyCloudError(r.error,'your saved stunt adjustments'));return false}
  }
  cloudSyncBlob('sced',SCED);
  cloudSyncBlob('stuntday',STUNTDAY);
  cloudSyncBlob('stuntcfg',STUNTCFG);
...
function hydrateUserBlobs(rows,adopted){
  const T=userBlobTargets();
  const seen=new Set();
  for(const r of rows||[]){
    const t=T[r.kind];if(!t||r.data==null)continue;
    seen.add(r.kind);
    // the account has a copy, so this device's copy is safe to clear at sign-out
    CLOUD_OK.add(r.kind);
    try{t.set(r.data);t.save()}catch(e){}
  }
  // nothing in the cloud for a kind this device HAS → push it up once. Never
  // when the person has just said the work on this computer isn't theirs.
...
// Nothing goes UP while the account is coming DOWN. Every write is deferred
// until the merge is finished and then run once — a save fired half-way
// through the merge saw sources whose rows had not arrived yet as empty, and
// deleted their account rows.
async function cloudHydrate(){
  BLOB_DEFER=BLOB_DEFER||new Set();
  try{await cloudHydrateInner()}
  finally{flushDeferredBlobs()}
}
async function cloudHydrateInner(){
  ...
  // First sign-in from a browser with local work and an empty account. This
  // used to migrate silently — and on a shared machine that hands one person's
  // productions to the next person who signs in, permanently, with the new
  // account as the owner. Ask, and name exactly what would move.
  // Whether this account has agreed to adopt what is already on this computer.
  // "Not mine" used to print its message and then FALL THROUGH to the three
  // push-up loops below, so the app said "they have not been added to your
  // account" while adding the rate cards, glossary, risk assessments and
  // calculators to it. Nothing local goes up unless the answer was yes.
  let adopted=true;
  if(!res.productions.length&&hasLocalUserData()){
    if(await confirmLocalMigration()){
      setStatus('Moving your locally-saved productions to the cloud…');
      if(!(await migrateLocalToCloud()))return;
      res=await cloud.loadAll();
      if(res.error){setStatus(friendlyCloudError(res.error,'your productions'));return}
    }else{
      adopted=false;
      // said once the rest of the hydrate has finished having its say
      setTimeout(()=>setStatus('The schedules already on this computer were left alone — they have not been added to your account. Whoever they belong to can sign in here and claim them.',{sticky:true}),0);
    }
  }
  ...
  if(res.glossary){
    CLOUD_OK.add('glossary');
    const cloudTerms=new Set(res.glossary.map(r=>(r.term||'').toLowerCase()+'|'+(r.production||'')));
    if(adopted)for(const g of GLOSSARY){
      const k=(g.term||'').toLowerCase()+'|'+(g.production||'');
      if(!cloudTerms.has(k)&&cloud.upsertGlossaryTerm)
        cloud.upsertGlossaryTerm(g.term,g.answer,g.production).then(r=>{if(r&&r.error)CLOUD_FAIL.add('glossary')},()=>CLOUD_FAIL.add('glossary'));
    }
  ...
  hydrateUserBlobs(res.userBlobs||[],adopted);
  ...
  for(const k of Object.keys(NOTES))if(/^[pm]:/.test(k))delete NOTES[k];NOTES_LEGACY=null;
  ...
  for(const rec of res.productions){
    ...
    try{
      if(rec.kind==='pdf'&&(rec.schedule_text||rec.ai_model)){
        ...
        if(addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.schedule_text,unit:rec.unit||'Main',cloudId:rec.id,colour:rec.colour||undefined,createdAt:rec.created_at,prod:rec.production,version:rec.version,schedDate:rec.sched_date,format:rec.format,rateCard:rec.rate_card,current:rec.is_current,aiModel:rec.ai_model||null,docKind:rec.doc_kind||null,pdfFiles:rec.pdf_files||null})){
          setSrcRevDiff(SOURCES[SOURCES.length-1],(rec.ai_model&&rec.ai_model._revDiff)||null);
        }
      }else{
        addSource({days:[],castMap:{},notes:[]},rec.title,rec.short,false,{kind:'manual',allowEmpty:true,colour:rec.colour||'white',cloudId:rec.id,createdAt:rec.created_at,prod:rec.production});
      }
  ...
  let scedNeedsMigrate=false;
  // sced/stuntday and friends are stored as one blob (keyed by whatever
  // namespace the writing device used — adoptBlobKeys re-files them onto this
  // schedule's); cday/adj are one row per day, keyed by the plain day key
  const BLOBS={sced:SCED,stuntday:STUNTDAY,stuntcfg:STUNTCFG,notes:NOTES,briefs:BRIEFS,dayloc:DAYLOC};
  for(const de of res.dayEdits){
    const src=SOURCES.find(s=>s.cloudId===de.production_id);
    if(!src||!src.ns)continue;
    if(BLOBS[de.kind]){
      Object.assign(BLOBS[de.kind],adoptBlobKeys(de.data,src));
      // remember where the account's copy stood, so a write from this tab can
      // be refused if another device has moved it on since
      blobStampSeen(src.cloudId,de.kind,de.updated_at||null);
      if(de.kind==='sced')scedNeedsMigrate=true;
      if(de.kind==='notes')NOTES_LEGACY=null;
      continue;
    }
    const localKey=src.ns+'|'+de.key;
    if(de.kind==='cday')CDAY[localKey]=de.data;else ADJ[localKey]=de.data;
  }
  // Write the merged blobs down ONCE, after every row is in. Saving inside the
  // loop was how a production lost its whole per-scene breakdown: the save
  // pushed every source back up, and a source whose rows had not been merged
  // yet looked empty, so its account row was deleted.
  store.set('stuntos-notes',JSON.stringify(NOTES));
  store.set('crowdos-briefs',JSON.stringify(BRIEFS));
  store.set('crowdos-dayloc',JSON.stringify(DAYLOC));
  store.set('crowdos-stuntcfg',JSON.stringify(STUNTCFG));
  store.set('crowdos-stuntday',JSON.stringify(STUNTDAY));
  store.set('crowdos-sced',JSON.stringify(SCED));
  // every source in this account has now had its rows merged, so an empty
  // subset from here on really is empty (see cloudSyncBlob)
  for(const s of SOURCES)s.blobsLoaded=true;
  if(scedNeedsMigrate)try{migrateScedKeys()}catch(e){console.error('scene key repair failed',e)}
  ...
  if(res.rateCards){
    ...
    if(adopted)for(const d of RATE_DEPTS)for(const [n,vals] of Object.entries(RATECARDS[d.kind]))
      if(!cloudNames.has(d.kind+'|'+n))
        cloud.upsertRateCard(d.kind,n,vals).then(r=>{if(r&&r.error)CLOUD_FAIL.add('ratecards')},()=>CLOUD_FAIL.add('ratecards'));
    saveRateCardsLocal();
  }
  ...
  // the account's copy may predate scene slot keys — upgrade it here too
  try{migrateSceneSlotKeys()}catch(e){console.error('slot key migration failed',e)}
  // every namespaced store now matches the account exactly, so sign-out is
  // free to clear them (clearAccountLocalData keeps back anything that fails
  // to save from here on)
  for(const k of ['sced','stuntday','stuntcfg','notes','briefs','dayloc'])if(!CLOUD_FAIL.has(k))CLOUD_OK.add(k);
  BRIEF_IDX=null;
  SHADOW.cday=JSON.stringify(CDAY);
  SHADOW.adj=JSON.stringify(ADJ);
  // the account copy has just been loaded, so nothing is outstanding any more
  SYNC_FAIL.cday.clear();SYNC_FAIL.adj.clear();
  SYNC_RETRY.cday.delay=0;SYNC_RETRY.adj.delay=0;
  refreshUnsavedWarning();
  ...
  // A brand-new account has no schedules at all — opening one here is what
  // used to leave the very first sign-in stuck on "Syncing your productions…"
  // Suppress nav-saving while we settle the default active schedule, so the
  // location saved from the previous session survives for restoreNav() below.
  NAV_BOOTING=true;
  const openable=clampActive(ACTIVE);
  if(openable>=0)setActive(openable);else ACTIVE=0;
  setStatus('');
  NAV_BOOTING=false;
  // The account's schedules have just (re)loaded from the cloud. On a plain
  // reload this runs after boot already placed the user, so re-apply their
  // saved location now that the real sources exist. Only fall back to the
  // dashboard when there's nowhere to return to (new account / first sign-in).
  if(!restoreNav())showDash();
}

cloud.onAuthChange((session,event)=>{
  CLOUD.session=session;
  // Arriving back from the "forgotten password" email. The session this hands
  // over is only good for setting a new password, so ask for one before the
  // board loads anything.
  if(event==='PASSWORD_RECOVERY'){openPasswordReset();return}
  // Same again after a RELOAD, which fires an ordinary sign-in event instead:
  // the recovery session is only good for setting a password, so the board
  // stays shut until one has been set.
  if(session&&pwRecoveryPending()){openPasswordReset();return}
  updateAccountUI();
  // signing out reloads the page, but if that reload is ever blocked the app
  // must NOT think it is still holding the previous account's data
  if(!session)CLOUD.hydrated=false;
  if(session)try{applyRoleAppMode(session)}catch(e){}
...
// The sample schedule.
//
// Seeing a real crowd budget before you have a schedule of your own is the
// single best way in — a page of empty tables tells a producer nothing about
// whether this is worth their afternoon. So the app ships a real-shaped
// schedule (Main + 2nd unit, ~90 shoot days) that can be loaded on demand.
//
// It is deliberately a "demo" source — `kind` is left unset — which is what
// the whole app already keys off: demo sources are never written to
// crowdos-sources, their day edits are never synced (prodIdForNs returns
// undefined for them), they have no Production Settings tab, and they sit
// under their own "Sample schedule" heading in the sidebar. So it can never be
// mistaken for the user's own work or turn up in their account.
//
// Only the fact that it's loaded is remembered, so it comes back on reload —
// and one press removes it again.
// ============================================================================
const SAMPLE_FLAG='crowdos-sample-loaded';
const SAMPLE_TITLE='Clown Town (sample)';
const hasSample=()=>SOURCES.some(s=>!s.kind);
let SAMPLE_LOADING=false;
/** Load the built-in sample. `open` also switches to it. */
async function loadSampleSchedule(opts){
  opts=opts||{};
  if(hasSample()){if(opts.open)setActive(SOURCES.findIndex(s=>!s.kind));return true}
  if(SAMPLE_LOADING)return false;
  SAMPLE_LOADING=true;
  if(opts.open)setStatus('Loading the sample schedule…');
  try{
    // pulled in on demand — it is a big string, and most sessions never want it
    const [{DEMO_FULLFAT},{DEMO_2NDUNIT}]=await Promise.all([
      import('../engine/demo/demo-fullfat'),
      import('../engine/demo/demo-2ndunit'),
    ]);
    const main=prepModel(parseAny(DEMO_FULLFAT),'Main');
    const second=prepModel(parseAny(DEMO_2NDUNIT),'2nd');
    second.castMap=Object.assign({},main.castMap,second.castMap);
    const model=mergeModels(main,second);
    const ok=addSource(model,SAMPLE_TITLE,'Sample schedule',!!opts.open,{ns:'p:sample'});
    if(!ok){SAMPLE_LOADING=false;return false}
    store.set(SAMPLE_FLAG,'1');
    renderSidebar();
    if(DASH)renderDash();
    if(opts.open)setStatus('This is the sample — a made-up production, not your work. Press anything you like; “Remove the sample” on the dashboard takes it away again.',{timeout:9000});
  }catch(err){
    console.error(err);
    setStatus('Couldn’t load the sample schedule just now. Refresh the page and try again.');
    SAMPLE_LOADING=false;return false;
  }
  SAMPLE_LOADING=false;
  return true;
}
function removeSampleSchedule(){
  const i=SOURCES.findIndex(s=>!s.kind);
  store.set(SAMPLE_FLAG,'');
  if(i<0){if(DASH)renderDash();return}
  const ns=SOURCES[i].ns;
  SOURCES.splice(i,1);
  // the sample's own day edits go with it — they were only ever on this device
  for(const k of Object.keys(CDAY))if(keyParts(k).ns===ns)delete CDAY[k];
  for(const k of Object.keys(ADJ))if(keyParts(k).ns===ns)delete ADJ[k];
  saveCDAY();saveAdj();
  if(i<ACTIVE)ACTIVE--;
  ACTIVE=Math.max(0,clampActive(ACTIVE));
  setStatus('Sample removed. You can load it again from the dashboard whenever you want it.');
  if(!SOURCES.length||DASH){showDash();renderSidebar();}
...
// ---------- remember where you were across a page reload ----------
// The app already restores your DATA on reload, but it always LANDED on a
// default view (the last schedule, or the dashboard). This keeps a tiny record
// of the current location — dashboard page / open production / open schedule /
// which tab — in this browser only, so a refresh returns you to the same spot.
const NAV_KEY='crowdos-nav';
// Suppressed while the app is first booting so the value saved from the LAST
// session survives long enough for restoreNav() to read it.
let NAV_BOOTING=true;
function saveNav(){
  if(NAV_BOOTING)return;
  try{
    let nav;
    if(DASH){
      nav={dash:true,dashPage:DASH_PAGE,prodHome:PROD_HOME||null,prodTab:PROD_TAB};
    }else{
      const s=SOURCES[ACTIVE];
      nav={dash:false,uid:s?srcUid(s):null,view:CUR_VIEW};
    }
    store.set(NAV_KEY,JSON.stringify(nav));
  }catch(e){/* Navigation persistence is best-effort. */}
}
// The single list of the board's own tabs, used by both the tab click handler
// and restoreNav so a restored tab lights up exactly like a clicked one.
const BOARD_VIEWS=['days','cal','stunts','crowd','cbdoc','briefs','doods','calc','cast'];
function applyBoardTab(view){
  const t=document.querySelector('.tabs [data-view="'+view+'"]');
  if(!t)return false;
  document.querySelectorAll('.tabs button').forEach(b=>b.classList.toggle('on',b===t));
  document.querySelectorAll('.tabs-list [role="tab"]').forEach(b=>{
    const on=b===t;
    b.setAttribute('aria-selected',on?'true':'false');
    b.setAttribute('tabindex',on?'0':'-1');
  });
  BOARD_VIEWS.forEach(v=>$('#view'+v[0].toUpperCase()+v.slice(1)).classList.toggle('hidden',v!==view));
  if(view==='briefs')renderBriefs();
  if(view==='doods')renderDoods();
  if(view==='cbdoc')renderCbDoc();
  CUR_VIEW=view;syncExpBar();syncToolRow();
  saveNav();
  return true;
}
// Send the user back to wherever they were before the reload. Returns true if
// it could; false (e.g. that schedule is gone now) leaves the default in place.
function restoreNav(){
  let nav=null;
  try{nav=JSON.parse(store.get(NAV_KEY)||'null')}catch(e){nav=null}
  if(!nav)return false;
  if(nav.dash){
    DASH=true;
    DASH_PAGE=nav.dashPage||'home';
    PROD_HOME=(nav.prodHome&&PRODS[nav.prodHome])?nav.prodHome:null;
    PROD_TAB=nav.prodTab==='settings'?'settings':'schedules';
    $('#boardView').classList.add('hidden');
    $('#dashView').classList.remove('hidden');
    $('#colourPill').style.display='none';
    renderDash();renderSidebar();updateCrumbs();window.scrollTo(0,0);
    return true;
  }
  // a board: match the schedule by its stable id, not its list position
  const i=nav.uid?SOURCES.findIndex(s=>s.kind&&srcUid(s)===nav.uid):-1;
  if(i<0)return false;
  hideDash();
  setActive(i);
  if(nav.view&&BOARD_VIEWS.includes(nav.view))applyBoardTab(nav.view);
  return true;
}
...
function renderSidebar(){
  ...
  $('#sideRatesNav').classList.toggle('on',DASH&&DASH_PAGE==='rates');
...
function renderDash(){
  updateCrumbs();
  saveNav(); // remember the dashboard page / open production across a reload
  ...
  // same rescue for the rate-card editor: it is the only copy of #rcaBody, and
  // the rates page re-parents it into #dashRatesSlot at the end — put it back
  // in its modal shell before any innerHTML wipe would destroy it.
  {const rb=$('#rcaBody');if(rb&&rb.parentElement&&rb.parentElement.id==='dashRatesSlot'){const sheet=$('#rateAdminModal .rp-sheet');if(sheet)sheet.appendChild(rb);}}
  ...
  if(DASH_PAGE==='rates'){
    // The rate-card library: every card, its rules and its rates, in one place.
    // The full editor already lives in #rcaBody (built by renderRateAdmin); we
    // re-parent that node into this page — exactly the trick the Calculator uses
    // with #viewCalc — so all of its edit / delete / save handlers keep working
    // without a second copy of them.
    $('#dashView').innerHTML=`
    <div class="dash-hero"><div><div class="dash-head">Rate cards</div><div class="dash-sub">Every rate card in one place — the baseline, the built-in standards, and any you create. Pick one as a production’s baseline in Production Settings.</div></div></div>
    <div id="dashRatesSlot"></div>`;
    const rb=$('#rcaBody');
    if(rb){$('#dashRatesSlot').appendChild(rb);}
    RCA_EDIT=null;renderRateAdmin();
    return;
  }
  const who=CLOUD.session?displayName(CLOUD.session):'';
  const mode=modeWord(), Mode=mode[0].toUpperCase()+mode.slice(1);
  const money=n=>n>0?`<b class="verfig">${gbpWhole(n)}</b>`:`<span class="noreq">No ${mode} requirement</span>`;
  const names=[...new Set([...prodNames(),...SOURCES.filter(s=>s.kind).map(s=>s.prod||s.title)])];
  const demoFull=SOURCES.findIndex(s=>!s.kind);
  ...
  // The sample is offered (or withdrawn) from the same strip on every
  // dashboard, not only the empty one — someone who loaded it to look round
  // needs an obvious way to be rid of it once they have their own schedule.
  const sampleStrip=demoFull>=0
    ?`<div class="samplebar"><b>Sample schedule loaded.</b> <span>“${esc(SOURCES[demoFull].title)}” is a made-up production for exploring the app. It is stored on this device only and is never part of your account.</span> <button class="ef-ghost" data-side="${demoFull}">Open the sample</button><button class="ef-ghost" id="efSampleRm">Remove the sample</button></div>`
    :'';
  ...
  const cards=sorted.map((name,i)=>{
    ...
    return `<div class="gcard" data-openprod="${esc(name)}" data-gname="${esc(name.toLowerCase())}">
      ${coverCssUrl(cover)?`<div class="gposter haspic" style="background-image:${coverCssUrl(cover)}"></div>`
  ...
  if(!sorted.length){
    $('#dashView').innerHTML=`
      ...
      <div class="empty-first">
        ...
        <p>Drag a shooting schedule onto this page — or press the button below — and ${PRODUCT} reads it for you — every shoot day, scene, location and cast number — then works out what the ${mode} costs across the whole shoot.</p>
        <div class="ef-acts">
          <button class="ef-primary" id="efAdd">Add your first production ${icon('arrow-right')}</button>
          ${demoFull>=0
            ?`<button class="ef-ghost" data-side="${demoFull}">Open the sample schedule</button>`
            :`<button class="ef-ghost" id="efSample">Load a sample schedule</button>`}
        </div>
        <div class="ef-note">${demoFull>=0
          ?'The sample is a made-up production. It lives on this device only, never in your account, and “Remove the sample” below takes it away.'
          :'Haven’t got a schedule to hand? Load a made-up one — 90 shoot days, Main and 2nd unit, fully costed — and have a proper look before you commit anything of your own. It stays on this device and you can remove it in one press.'}</div>
        <div class="ef-steps">
        ...
        <div class="ef-foot">Not sure where to start? The <b>?</b> at the bottom of the menu replays the tour. Stuck on something? Email <a href="${supportHref('Laural help — getting started')}">${SUPPORT_EMAIL}</a>.</div>
      </div>
      ${sampleStrip}`;
...
function renderProdHome(name,mode,Mode,money){
  ...
  for(const [uk,revs] of units){
    ...
    const verRows=revs.map(r=>{
      ...
      return `<div class="verrow ${isCur?'current':''}" data-openrev="${r.i}">
        ...
        ${srcRevDiff(r.s)?`<button type="button" class="verchg" data-chgreport="${r.i}" data-tip="The full list of what changed when this revision landed — printable and exportable">What changed</button>`:''}
  ...
  const schedulesPane=`
    <div class="summary" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));margin-top:14px">
      ...
      <div class="stat money costable"><div class="n">${gbpWhole(pFig)}</div><div class="l">${Mode} total</div></div>
  ...
  $('#dashView').innerHTML=`
    ...
    ${coverCssUrl(cover)?`<div class="prod-banner has" style="background-image:${coverCssUrl(cover)}"></div>`:''}
...
function renderDashTodayBar(slot){
  ...
  loadTodayWeather(anchor&&anchor.d.loc,slot.querySelector('#wxSlot'),(anchor&&anchor.prod)||null);
}
...
function hideDash(){
  ...
  // put the rate-card editor back in its modal shell so #rcaBody keeps one home
  const rb=$('#rcaBody');
  if(rb&&rb.parentElement&&rb.parentElement.id==='dashRatesSlot'){const sheet=$('#rateAdminModal .rp-sheet');if(sheet)sheet.appendChild(rb);}
...
// ---------- sidebar drawer ----------
// MUST match the drawer breakpoint in app/globals.css (the `@media
// (max-width:1024px)` block that turns the sidebar into a slide-over). While
// this said 820px, every width from 821 to 1024 — iPad Pro 11" portrait is
// 834px — got the CSS drawer with none of the JS: it opened over the board on
// load, with no scrim and no tap-to-close.
...
// Escape closes the drawer, like every other overlay on the page. Without this
// the drawer was the one thing on screen you could not dismiss from the
// keyboard: body.drawer-open persisted, the scrim stayed, and the page behind
// it stayed locked (overflow:hidden). Only when a dialog is not on top — a
// dialog's own Escape must still win.
document.addEventListener('keydown',e=>{
  if(e.key!=='Escape'||MODAL_STACK.length)return;
  if(!document.body.classList.contains('drawer-open'))return;
  e.preventDefault();
  setSidebarCollapsed(true);
  const t=$('#btnSidebar');if(t)t.focus({preventScroll:true});
});
...
window.addEventListener('resize',()=>{
  if(!isMobileNav()){document.body.classList.remove('drawer-open');return}
  // Crossing INTO the drawer band with the sidebar expanded (rotating an iPad
  // to portrait): it is a slide-over now, so it needs its scrim and its
  // tap-to-close, which only appear with this flag.
...
$('#sideRatesNav').addEventListener('click',()=>showDash('rates'));
...
document.addEventListener('click',e=>{
  ...
  if(/** @type {any} */(e.target).closest('#efSample')){loadSampleSchedule({open:true});return}
  if(/** @type {any} */(e.target).closest('#efSampleRm')){removeSampleSchedule();return}
  ...
  if(orv&&!e.target.closest('[data-delsrc]')&&!e.target.closest('[data-makecurrent]')&&!/** @type {any} */(e.target).closest('[data-chgreport]')){setActive(+orv.dataset.openrev);return}
  const crp=/** @type {any} */(e.target).closest('[data-chgreport]');
  if(crp){openChangeReport(+crp.dataset.chgreport);return}
...
});
{
  ...
  setActive=function(i){
    ...
    if(s&&s.kind){
      const pill=$('#colourPill');
      setPillLabel(s.version||revLabel(s));
      pill.style.display='';
      pill.title='Switch schedule — every revision in '+(s.prod||s.title);
      pill.setAttribute('aria-label','Switch schedule. Showing '+(s.version||revLabel(s))+' — every revision in '+(s.prod||s.title));
...
document.addEventListener('click',async e=>{
  ...
  if(!s)return;
  // the sample has its own removal — it is not the user's work and nothing
  // about it needs deleting from an account
  if(!s.kind){removeSampleSchedule();return}
  ...
  if(!await askConfirm({
    ...
    body:'This deletes '+label+' from “'+(s.prod||s.title)+'”, along with everything you have set on its days. It cannot be undone.',
    yes:'Delete the schedule',danger:true}))return;
  if(CLOUD.session&&s.cloudId)cloud.deleteProduction(s.cloudId).then(r=>{
    if(r.error)setStatus(friendlyCloudError(r.error,'that deletion'));
  });
  deleteSourceFiles(s); // drop the stored original (local cache + account storage)
  ...
  saveCDAY();saveAdj();saveUserSources();saveManualDays();
  if(i<ACTIVE)ACTIVE--;
  // deleting the schedule you were looking at (or the last one you had) lands
  // on whatever is left — and on the dashboard when nothing is left
  ACTIVE=Math.max(0,clampActive(ACTIVE));
  if(DASH){renderDash();renderSidebar();}
  else if(!SOURCES.length)showDash();
  else setActive(ACTIVE);
},true);
...
// Where the account's copy of each blob row stood when this tab last read it
// ("production id|kind" → updated_at). Every write carries the stamp it is
// based on, so a tab whose copy has been overtaken is refused instead of
// silently overwriting the newer work.
const BLOB_STAMP=new Map();
function blobStampSeen(pid,kind,stamp){if(pid)BLOB_STAMP.set(pid+'|'+kind,stamp||null)}
function blobMapOf(kind){
  return ({sced:SCED,stuntday:STUNTDAY,stuntcfg:STUNTCFG,notes:NOTES,briefs:BRIEFS,dayloc:DAYLOC})[kind]||null;
}
// These fire on every keystroke in a scene note, once PER SOURCE — twenty
// schedules open meant twenty full scans and twenty uploads per character,
// which is how an account gets rate-limited (and a rate-limited write is lost
// work). Debounced exactly like the account-level blobs.
const BLOB_T={};
let BLOB_DEFER=null; // a Set while a hydrate is merging: nothing goes up mid-merge
function cloudSyncBlob(kind,map){
  if(!CLOUD||!CLOUD.session||TAB_STALE)return;
  if(BLOB_DEFER){BLOB_DEFER.add(kind);return}
  clearTimeout(BLOB_T[kind]);
  BLOB_T[kind]=setTimeout(()=>{BLOB_T[kind]=null;pushBlob(kind,map)},900);
}
function flushDeferredBlobs(){
  const kinds=BLOB_DEFER;BLOB_DEFER=null;
  if(!kinds)return;
  for(const kind of kinds){const m=blobMapOf(kind);if(m)cloudSyncBlob(kind,m)}
}
// don't let a debounce still be waiting when the tab goes away
function flushBlobTimers(){
  for(const kind of Object.keys(BLOB_T)){
    if(!BLOB_T[kind])continue;
    clearTimeout(BLOB_T[kind]);BLOB_T[kind]=null;
    const m=blobMapOf(kind);if(m)pushBlob(kind,m);
  }
}
function pushBlob(kind,map){
  if(!CLOUD||!CLOUD.session||TAB_STALE)return;
  for(const s of SOURCES){
    if(!s.kind||!s.cloudId||!s.ns)continue;
    const pre=s.ns+'|';
    const subset={};
    for(const k of Object.keys(map))if(k.indexOf(pre)===0)subset[k]=map[k];
    ...
    // doesn't.
    const done=r=>{if(r&&r.error)noteCloudFail(kind,r.error);else{CLOUD_OK.add(kind);CLOUD_FAIL.delete(kind)}};
    const stampKey=s.cloudId+'|'+kind;
    if(Object.keys(subset).length){
      const expected=BLOB_STAMP.has(stampKey)?BLOB_STAMP.get(stampKey):null;
      const send=cloud.upsertDayEditGuarded
        ?cloud.upsertDayEditGuarded(s.cloudId,'__'+kind+'__',kind,subset,expected)
        :cloud.upsertDayEdit(s.cloudId,'__'+kind+'__',kind,subset);
      send.then(r=>{
        if(r&&r.stale){markTabStale(s);return}
        if(r&&r.stamp)blobStampSeen(s.cloudId,kind,r.stamp);
        else if(!(r&&r.error)&&cloud.getDayEditStamp)
          cloud.getDayEditStamp(s.cloudId,'__'+kind+'__',kind).then(x=>blobStampSeen(s.cloudId,kind,x&&x.stamp)).catch(()=>{});
        done(r);
      },e=>noteCloudFail(kind,e));
    }
    // A source whose account rows have not been read yet only LOOKS empty.
    // Deleting on that is how a production's entire per-scene breakdown
    // disappeared mid-sign-in, with nothing left to put it back.
    else if(s.blobsLoaded){
      cloud.deleteDayEdit(s.cloudId,'__'+kind+'__',kind).then(r=>{if(!(r&&r.error))blobStampSeen(s.cloudId,kind,null);done(r)},()=>{});
    }
  }
}
// runs once the sync machinery above exists, so a repaired key can go up
migrateScedKeys();
function scedKey(nk){return (NS?NS+'|':'')+nk}
// A scene's work is filed against the SCENE, not against where it happens to
// sit in the day's list. It used to end in the scene's array position, so a
// re-parse that inserted or dropped one scene silently re-attached every edit
// below it to the wrong scene — invisible, plausible and wrong in money. The
// last segment is now "#0" (or #1, #2… only where one day genuinely lists the
// same scene twice); see lib/engine/carry.ts.
function sceneNK(d,s,idx){
  const scenes=(d&&d.scenes)||[];
  let i=idx;
  if(s&&scenes[idx]!==s)i=scenes.indexOf(s);
  // the scene isn't on the day any more (it was just removed) — key it on the
  // scene's own identity, never on whatever now sits at that position
  if(i<0||i>=scenes.length)return [((d&&d.unit)||'Main'),d&&d.num,(s&&s.num)||'',(s&&s.part)||'','#0'].join('|');
  return sceneSlotKey(d,i);
}
function sceneFromKey(nk){
  const seg=nk.split('|'); if(seg.length!==5)return null;
  const unit=seg[0], num=seg[1];
  const d=(MODEL.days||[]).find(x=>(x.unit||'Main')===unit&&String(x.num)===num);
  if(!d)return null;
  // slotIndexOn also understands the old position-based last segment, so an
  // entry that hasn't been migrated yet still opens on the right scene
  const idx=slotIndexOn(d,seg[2],seg[3],seg[4]);
  if(idx<0||!d.scenes[idx])return null;
  return {d,s:d.scenes[idx],idx};
}
// One-off upgrade: move every position-keyed SCED / NOTES entry onto its
// scene's slot key. Runs per schedule (each has its own namespace and its own
// days), leaves anything it can't place exactly where it is, and is safe to
// run again — a key already in the new shape is skipped.
function slotKeysOf(map,ns){
  const pre=ns?ns+'|':'';
  return Object.keys(map).filter(k=>ns?k.startsWith(pre):!/^[pm]:/.test(k));
}
function migrateSceneSlotKeys(){
  let moved=0;
  // The one case where the upgrade can attach somebody's work to a DIFFERENT
  // scene: the scene the old key named is nowhere on the day any more, so the
  // scene now sitting at the stored position was used instead. That is a guess,
  // it is written to storage permanently, and the user is the only person who
  // can tell whether it is right — so it is never silent.
  const guessed=[];
  const stores=[
    {map:SCED,save:()=>{saveSced()}},
    {map:NOTES,save:()=>{(NOTES_LEGACY=null,store.set('stuntos-notes',JSON.stringify(NOTES)));cloudSyncBlob('notes',NOTES)}},
  ];
  for(const {map,save} of stores){
    let dirty=false;
    for(const s of SOURCES){
      const days=(s&&s.model&&s.model.days)||[];
      if(!days.length)continue;
      const ns=s.ns||'';
      const byDay=new Map();
      for(const d of days)byDay.set((d.unit||'Main')+'|'+String(d.num),d);
      const plan=planSlotMigration(slotKeysOf(map,ns),(u,n)=>byDay.get(u+'|'+n)||null,{prefix:ns?ns+'|':''});
      for(const mv of plan.moves){
        if(map[mv.from]===undefined||mv.from===mv.to)continue;
        // The delete used to run whether or not the copy above did, so when the
        // target was already occupied the source entry — a real edit somebody
        // made — was thrown away and nothing took its place. Move, or leave it
        // exactly where it is.
        if(map[mv.to]!==undefined)continue;
        map[mv.to]=map[mv.from];
        delete map[mv.from];
        moved++;dirty=true;
      }
      for(const g of plan.guessed||[])guessed.push({ns,...g});
    }
    if(dirty)save();
  }
  if(guessed.length)reportSlotGuesses(guessed);
  return moved;
}
/** Tell the user, in plain English, which saved crowd/stunt work was attached
 *  to a scene we had to guess at. */
function reportSlotGuesses(list){
  // the same scene shows up once per store (scene edits and notes) — say it once
  const seen=new Set();
  list=list.filter(g=>{const k=g.from+'>'+g.to;if(seen.has(k))return false;seen.add(k);return true});
  if(!list.length)return;
  const dayOf=k=>{const seg=String(k).split('|');return seg.length>=5?'D'+seg[seg.length-4]:''};
  const one=g=>`Scene ${g.wantedScene}'s saved crowd and stunt work could not be found on ${dayOf(g.from)||'its day'} any more, so it was attached to scene ${g.usedScene} — worth a check.`;
  const msg=list.length===1
    ?one(list[0])
    :`${list.length} pieces of saved crowd and stunt work were attached to a different scene, because the scene they were filed against is no longer on that day: `+
     list.slice(0,4).map(g=>`${g.wantedScene} → ${g.usedScene}${dayOf(g.from)?' on '+dayOf(g.from):''}`).join('; ')+
     (list.length>4?`; and ${list.length-4} more.`:'.')+' Worth a check.';
...
function applySced(model){
  for(const d of model.days)d.scenes.forEach((s,idx)=>{
    ...
    if(e.chars){
      ...
      const mk=c=>{const o={name:c.name||'',count:+c.count};if(c.note)o.note=c.note;if(+c.sup>0)o.sup=+c.sup;if(c.fromAbove)o.flags=['asAbove'];if((c.roleId||'').trim())o.roleId=String(c.roleId).trim();return o};
...
function reqEditorHTML(nk){
  ...
  if(APPMODE==='crowd'){
    ...
    const chars=e.chars||[
      ...
      ...(s.saChars||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:false,fromAbove:above(f),roleId:f.roleId||''})),
      ...(s.featured||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:true,fromAbove:above(f),roleId:f.roleId||''})),
      ...(s.spacts||[]).map(f=>({name:f.name,count:f.count,tier:'SPACT',featured:false,fromAbove:above(f),roleId:f.roleId||''})),
    ];
    // The production's named roles sit in the same picker as SA and SPACT,
    // because to the person filling this in that is exactly what they are.
    // With none defined the picker is the two options it always had.
    const roles=activeRoles();
    const tierPick=c=>`<select data-rq="ctier" aria-label="Paid as"><option value="SA"${!c.roleId&&c.tier!=='SPACT'?' selected':''}>SA</option><option value="SPACT"${!c.roleId&&c.tier==='SPACT'?' selected':''}>SPACT</option>${roles.map(r=>`<option value="role:${esc(r.id)}"${c.roleId===r.id?' selected':''}>${esc(roleLabel(r))}</option>`).join('')}</select>`;
    const row=(c,i)=>`<div class="reqrow" data-ri="${i}" data-btier="${c.tier==='SPACT'?'SPACT':'SA'}" data-bfeat="${c.featured?1:0}">
      ...
      <label class="reqfeat ${c.tier==='SPACT'||c.roleId?'off':''}"><input type="checkbox" data-rq="cfeat" ${c.featured?'checked':''}> Featured</label>
...
function commitReqEditor(area,keepOpen){
  ...
  if(APPMODE==='crowd'){
    ...
    ed.querySelectorAll('.reqrow').forEach(row=>{
      ...
      let name=((row.querySelector('[data-rq="cname"]')||{}).value||'').trim();
      const picked=(row.querySelector('[data-rq="ctier"]')||{}).value||'SA';
      // "role:xyz" = a named role. The row keeps the tier it had underneath,
      // because that is what prices it if the role is ever deleted.
      const roleId=picked.startsWith('role:')?picked.slice(5):'';
      const tier=roleId?(row.dataset.btier==='SPACT'?'SPACT':'SA'):picked;
      const featured=roleId?row.dataset.bfeat==='1':(tier!=='SPACT'&&!!(row.querySelector('[data-rq="cfeat"]')||{}).checked);
      const fromAbove=!!(row.querySelector('[data-rq="cabove"]')||{}).checked;
      // a role group is always a named group — plain background is stored as a
      // bare head count and has nowhere to carry one
      if(roleId&&!name){const r=roleById(roleId);if(r)name=roleLabel(r);}
      e.chars.push({name,count,tier,featured,fromAbove,...(roleId?{roleId}:{})});
    });
  ...
  renderFreeCalc=function(){
    ...
    const rows=FCROWS.map((r,i)=>{
      ...
      return `<tr>
        ...
        <td class="num money">${gbpWhole(p*(+r.count||0))}</td>
    ...
    card.innerHTML=`<h3>Rough day budget<span class="cnt">${heads} heads</span><span class="sum costable">${gbpWhole(total)}</span></h3>
...
// ---------------------------------------------------------------------------
// The crowd rate-card schema
// ---------------------------------------------------------------------------
// Every rule printed on a crowd rate card — SPACT, PACT/FAA, PACT/Equity, or
// whatever card turns up next — is one entry here. SA and SPACT share the SAME
// schema so a card can be duplicated from one talent type to the other and
// still make sense, and so a brand-new agreement is a new set of numbers rather
// than a new set of code.
//
// `key` is the schema key; the app's field ids are derived from it per talent
// type. The eleven ids that already existed before the card was opened up
// (cSA, cHol, cOTday, …) keep their names so every saved card, every
// production override and every hidden calculator input still resolves.
//
// `ref:true` marks a number the card carries but the budget does not yet
// charge automatically — it is shown in the editor with a "reference" tag so
// nobody assumes a total moved when they changed it.
const CROWD_SCHEMA=[
  // ---- what a day is worth ----
  {key:'day',group:'Day rates',label:'Day rate',unit:'£'},
  {key:'night',group:'Day rates',label:'Night rate',unit:'£'},
  {key:'phDay',group:'Day rates',label:'Public holiday — day',unit:'£'},
  {key:'phNight',group:'Day rates',label:'Public holiday — night',unit:'£'},
  {key:'halfDay',group:'Day rates',label:'Half day / rehearsal / travel day',unit:'£',ref:true},
  {key:'rehearsal',group:'Day rates',label:'Rehearsal day',unit:'£',ref:true},
  {key:'shiftCall',group:'Day rates',label:'Shift call',unit:'£',ref:true},
  {key:'nonPerf',group:'Day rates',label:'Non-performance call (up to 4 hrs)',unit:'£',ref:true},
  {key:'fitting4',group:'Day rates',label:'Fitting — up to 4 hrs',unit:'£',ref:true},
  {key:'fittingOver',group:'Day rates',label:'Fitting — over 4 hrs',unit:'£',ref:true},
  // ---- holiday ----
  {key:'holPct',group:'Holiday',label:'Holiday %',unit:'%',depts:['sa'],
    hint:'Set to 0 when the day rate already includes holiday pay.'},
  {key:'holFlat',group:'Holiday',label:'Payment in lieu of holiday, per day',unit:'£',depts:['spact']},
  // ---- the working day and overtime ----
  {key:'fwStd',group:'Working day & overtime',label:'Standard day — hours before overtime',unit:'hrs'},
  {key:'fwCwd',group:'Working day & overtime',label:'Continuous day — hours before overtime',unit:'hrs'},
  {key:'otDay',group:'Working day & overtime',label:'Overtime — day, per 30 min',unit:'£'},
  {key:'otNight',group:'Working day & overtime',label:'Overtime — night & early call, per 30 min',unit:'£'},
  {key:'otPhDay',group:'Working day & overtime',label:'Overtime — public holiday day, per 30 min',unit:'£'},
  {key:'otPhNight',group:'Working day & overtime',label:'Overtime — public holiday night, per 30 min',unit:'£'},
  {key:'brokenTurnHours',group:'Working day & overtime',label:'Minimum turnaround between calls',unit:'hrs',ref:true},
  {key:'brokenTurnRate',group:'Working day & overtime',label:'Broken turnaround, per hour',unit:'£',ref:true},
  // ---- travel ----
  {key:'travelA',group:'Travel allowances',label:'Cat A — calls within TfL Zones 1–3',unit:'£'},
  {key:'travelB',group:'Travel allowances',label:'Cat B — recognised studios / beyond Zone 3, within 40 miles',unit:'£'},
  {key:'earlyUplift',group:'Travel allowances',label:'Early call uplift (called at or before 06:00)',unit:'£'},
  {key:'travelDBase',group:'Travel allowances',label:'Beyond 40 miles — base allowance',unit:'£',ref:true},
  {key:'travelDMile',group:'Travel allowances',label:'Beyond 40 miles — per excess mile',unit:'£',ref:true},
  // ---- supplementary fees (these fill the fee picker on every crowd line) ----
  {key:'supA',group:'Supplementary fees',label:'Cat A — change of clothing, haircut / shaving',unit:'£'},
  {key:'supAOwn',group:'Supplementary fees',label:'Cat A — providing own clothing',unit:'£'},
  {key:'supAUnused',group:'Supplementary fees',label:'Cat A — unused change of clothes, per set',unit:'£'},
  {key:'supB',group:'Supplementary fees',label:'Cat B — doubling, special clothing, sports equipment, wet weather',unit:'£'},
  {key:'supC',group:'Supplementary fees',label:'Cat C — firearms, swimming, driving, scans, minimal dialogue',unit:'£'},
  {key:'supD',group:'Supplementary fees',label:'Cat D — creative reaction, uniforms, specialised driving, provision of car',unit:'£'},
  {key:'supE',group:'Supplementary fees',label:'Cat E — lookalike doubling, stand-in, dialogue (10+ words)',unit:'£'},
  // ---- meal break penalties ----
  {key:'mealShortDay',group:'Meal break penalties',label:'Short lunch — day',unit:'£'},
  {key:'mealShortNight',group:'Meal break penalties',label:'Short lunch — night',unit:'£'},
  {key:'mealShortPhDay',group:'Meal break penalties',label:'Short lunch — public holiday day',unit:'£'},
  {key:'mealShortPhNight',group:'Meal break penalties',label:'Short lunch — public holiday night',unit:'£'},
  {key:'mealLateDay',group:'Meal break penalties',label:'Late lunch — day',unit:'£'},
  {key:'mealLateNight',group:'Meal break penalties',label:'Late lunch — night',unit:'£'},
  {key:'mealLatePhDay',group:'Meal break penalties',label:'Late lunch — public holiday day',unit:'£'},
  {key:'mealLatePhNight',group:'Meal break penalties',label:'Late lunch — public holiday night',unit:'£'},
  {key:'mealSupperDay',group:'Meal break penalties',label:'Short supper — day',unit:'£'},
  {key:'mealSupperNight',group:'Meal break penalties',label:'Short supper — night',unit:'£'},
  {key:'mealSupperPhDay',group:'Meal break penalties',label:'Short supper — public holiday day',unit:'£'},
  {key:'mealSupperPhNight',group:'Meal break penalties',label:'Short supper — public holiday night',unit:'£'},
  // ---- meal allowances ----
  {key:'allowBreakfast',group:'Meal allowances',label:'Breakfast',unit:'£',ref:true},
  {key:'allowLunch',group:'Meal allowances',label:'Lunch',unit:'£',ref:true},
  {key:'allowDinner',group:'Meal allowances',label:'Dinner',unit:'£',ref:true},
  {key:'allowOvernight',group:'Meal allowances',label:'Overnight stay — additional',unit:'£',ref:true},
  // ---- cancellation ----
  {key:'cancelBefore12',group:'Cancellation',label:'Cancelled before 12 PM the day before',unit:'£',ref:true},
  {key:'cancelAfter12',group:'Cancellation',label:'Cancelled after 12 PM the day before',unit:'£',ref:true},
  {key:'cancelNight',group:'Cancellation',label:'Night shoot cancelled after 12 PM',unit:'£',ref:true},
  {key:'cancelFitting',group:'Cancellation',label:'Fitting cancelled within 24 hrs',unit:'£',ref:true},
  // ---- intimacy & nudity (these also fill the fee picker) ----
  {key:'intimacy',group:'Intimacy & nudity',label:'Sexual intimacy',unit:'£'},
  {key:'nudityTopless',group:'Intimacy & nudity',label:'Nudity (topless) — not featured',unit:'£'},
  {key:'nudityToplessFeat',group:'Intimacy & nudity',label:'Nudity (topless) — featured',unit:'£'},
  {key:'nudityFull',group:'Intimacy & nudity',label:'Full frontal nudity',unit:'£'},
];
// The field ids the eleven original crowd fields must keep, so every card,
// override and hidden input saved before this schema existed still resolves.
const CROWD_LEGACY_IDS={
  sa:{day:'cSA',holPct:'cHol',otDay:'cOTday',otNight:'cOTnight',earlyUplift:'cET',travelA:'cTravelA',travelB:'cTravelB'},
  spact:{day:'cSpact',night:'cSpactNight',holFlat:'cSpactHol',earlyUplift:'cSpactET'},
};
const CROWD_ID_PREFIX={sa:'sa_',spact:'sp_'};
/** The app field id for one schema key on one talent type. */
function crowdFieldId(dept,key){
  return (CROWD_LEGACY_IDS[dept]&&CROWD_LEGACY_IDS[dept][key])||CROWD_ID_PREFIX[dept]+key;
}
/** Every crowd field for one talent type, in schema order. */
function crowdFieldsFor(dept){
  return CROWD_SCHEMA
    .filter(s=>!s.depts||s.depts.includes(dept))
    .map(s=>({id:crowdFieldId(dept,s.key),key:s.key,label:s.label,unit:s.unit,group:s.group,hint:s.hint,ref:s.ref,dept}));
}
const RATE_FIELDS=/** @type {any[]} */([
  ...crowdFieldsFor('sa'),
  ...crowdFieldsFor('spact'),
  {id:'rPerf',label:'Performer day rate',unit:'£',dept:'stunts',group:'Day rates'},
  {id:'rCoord',label:'Coordinator day rate',unit:'£',dept:'stunts',group:'Day rates'},
  {id:'rHol',label:'Holiday pay / day',unit:'£',dept:'stunts',group:'Holiday'},
  {id:'rIns',label:'Insurance / day',unit:'£',dept:'stunts',group:'Insurance & usage'},
  {id:'rInsDays',label:'Insured days / week',unit:'×',dept:'stunts',group:'Insurance & usage'},
  {id:'rUse',label:'Usage % of day rate',unit:'%',dept:'stunts',group:'Insurance & usage'},
  {id:'rSDRate',label:'Stunt dept coordinator day rate',unit:'£',dept:'stunts',group:'Day rates'},
  {id:'rSDDays',label:'Stunt dept coordinator days/wk',unit:'×',dept:'stunts',group:'Day rates'},
  {id:'rTravelMode',label:'Travel allowance',dept:'stunts',group:'Travel allowances',options:[['none','None'],['mileage','Mileage'],['train','Train fare']]},
  {id:'rMileRate',label:'Mileage rate / mile',unit:'£',dept:'stunts',group:'Travel allowances'},
  ...
  {id:'rPerfWk',label:'Performer weekly (5 days)',unit:'£',dept:'stunts',group:'Weekly & premium days'},
  {id:'rCoordWk',label:'Coordinator weekly (5 days)',unit:'£',dept:'stunts',group:'Weekly & premium days'},
  {id:'rNightPct',label:'Night shoot uplift %',unit:'%',dept:'stunts',group:'Weekly & premium days'},
  {id:'rDay6Mult',label:'6th day in a week (× daily)',unit:'×',dept:'stunts',group:'Weekly & premium days'},
  {id:'rDay7Mult',label:'7th day in a week (× daily)',unit:'×',dept:'stunts',group:'Weekly & premium days'},
  // stored on the card now, consumed by the stunt calculator & Fittings builds
  {id:'rOTFrac',label:'OT per hour = daily ÷ (0 = none)',unit:'÷',dept:'stunts',group:'Working day & overtime'},
  {id:'rFitFlat',label:'Fitting fee (flat £)',unit:'£',dept:'stunts',group:'Fittings & travel days'},
  {id:'rFitPct',label:'Fitting fee (% of daily)',unit:'%',dept:'stunts',group:'Fittings & travel days'},
  {id:'rTravelDayPct',label:'Travel day (% of daily)',unit:'%',dept:'stunts',group:'Fittings & travel days'},
]);
const RC_INPUTS=RATE_FIELDS.map(f=>f.id);
// A crowd card written in schema keys, turned into app field ids for one
// talent type. Anything the card doesn't state stays blank/zero rather than
// silently inheriting the other agreement's money.
function crowdVals(dept,byKey){
  const out={};
  for(const [k,v] of Object.entries(byKey))out[crowdFieldId(dept,k)]=String(v);
  return out;
}
// ---- PACT/FAA 2026 (14 Jan – 31 Dec 2026) — the SA baseline ----------------
// Straight off the Extra People Pact/FAA card. Travel figures are the
// from-1-Mar-2026 set. Meal penalties, supplementary categories and allowances
// are the card's own printed tables.
const SA_DEFAULT_KEYS={
  day:'111.21',night:'166.82',phDay:'166.82',phNight:'250.22',
  halfDay:'0',rehearsal:'0',shiftCall:'69.51',nonPerf:'55.61',fitting4:'55.61',fittingOver:'111.21',
  holPct:'12.07',
  fwStd:'9',fwCwd:'7',
  otDay:'11.69',otNight:'17.54',otPhDay:'17.54',otPhNight:'26.29',
  brokenTurnHours:'11',brokenTurnRate:'0',
  travelA:'17.09',travelB:'23.89',earlyUplift:'20.91',travelDBase:'23.89',travelDMile:'0.45',
  supA:'23.00',supAOwn:'23.00',supAUnused:'11.50',supB:'23.00',supC:'30.51',supD:'37.22',supE:'61.63',
  mealShortDay:'23.38',mealShortNight:'35.08',mealShortPhDay:'35.08',mealShortPhNight:'52.58',
  mealLateDay:'23.38',mealLateNight:'35.08',mealLatePhDay:'35.08',mealLatePhNight:'52.58',
  mealSupperDay:'23.38',mealSupperNight:'35.08',mealSupperPhDay:'35.08',mealSupperPhNight:'52.58',
  allowBreakfast:'5.00',allowLunch:'10.00',allowDinner:'25.00',allowOvernight:'10.00',
  cancelBefore12:'0',cancelAfter12:'0',cancelNight:'0',cancelFitting:'0',
  intimacy:'0',nudityTopless:'0',nudityToplessFeat:'0',nudityFull:'0',
};
// ---- Take 3 SPACT 2026 (4 Mar – 31 Dec 2026) — the SPACT baseline ---------
// Straight off the Take 3 card, including its own travel, supplementary,
// meal, cancellation and intimacy tables. Holiday is a FLAT payment in lieu,
// not a percentage.
const SPACT_DEFAULT_KEYS={
  day:'255',night:'372',phDay:'387.50',phNight:'432.50',
  halfDay:'135.25',rehearsal:'255',shiftCall:'0',nonPerf:'0',fitting4:'62.32',fittingOver:'124.63',
  holFlat:'15.50',
  fwStd:'10',fwCwd:'8',
  otDay:'11.69',otNight:'17.54',otPhDay:'17.54',otPhNight:'26.29',
  brokenTurnHours:'11',brokenTurnRate:'23.38',
  travelA:'17.09',travelB:'23.89',earlyUplift:'20.91',travelDBase:'23.89',travelDMile:'0.45',
  supA:'23.00',supAOwn:'23.00',supAUnused:'0',supB:'23.00',supC:'30.51',supD:'37.22',supE:'61.62',
  mealShortDay:'23.38',mealShortNight:'35.08',mealShortPhDay:'23.38',mealShortPhNight:'35.08',
  mealLateDay:'23.38',mealLateNight:'35.08',mealLatePhDay:'23.38',mealLatePhNight:'35.08',
  mealSupperDay:'0',mealSupperNight:'0',mealSupperPhDay:'0',mealSupperPhNight:'0',
  allowBreakfast:'5.00',allowLunch:'10.00',allowDinner:'25.00',allowOvernight:'0',
  cancelBefore12:'135.25',cancelAfter12:'270.50',cancelNight:'270.50',cancelFitting:'62.32',
  intimacy:'100',nudityTopless:'150',nudityToplessFeat:'250',nudityFull:'300',
};
const RC_DEFAULTS={...crowdVals('sa',SA_DEFAULT_KEYS),...crowdVals('spact',SPACT_DEFAULT_KEYS),rPerf:'600',rCoord:'1000',rHol:'17.50',rIns:'17.50',rInsDays:'2',rUse:'55.5',rSDRate:'350',rSDDays:'4',rTravelMode:'none',rMileRate:'0.55',rPerfWk:'3000',rCoordWk:'5000',rNightPct:'0',rDay6Mult:'1',rDay7Mult:'1',rOTFrac:'0',rFitFlat:'0',rFitPct:'0',rTravelDayPct:'0'};
...
// PACT/Equity 2026 (1 Apr–31 Dec 2026, Extra People card): basic £128.20
// ALREADY INCLUDES holiday → holiday % is 0 (totals right; the £-holiday line
// in breakdowns reads £0 by design). Night and public holiday are the basic
// PLUS the £57.20 enhancement = £185.40. OT £9.53/30 min incl. holiday;
// night/PH OT £14.30. Early call bonus £9.53/30 min (calls 4–7am) — the app
// charges early call at the night OT figure, so that is what goes in.
// 10-hour day incl. 1 hour lunch. Travel is DISTANCE-based, not zone-based —
// mapped: Cat A slot = £11.00 (4–10.99 mi), Cat B slot = £16.00 (11–40 mi),
// beyond-40 base = the £20 cap. The 0–3.99 mi negotiable £4 cap has no slot.
// Supplementary categories are the card's SP1/SP2/SP3, mapped onto the
// nearest Cat letters: SP1 creative contribution £32.46 → Cat D, SP2
// performance skills £27.05 → Cat C, SP3 clothing/haircuts/weather £20.00 →
// Cat A/B. Broken meal £9.53 day / £14.30 night & public holiday.
const EQUITY_2026_KEYS={
  day:'128.20',night:'185.40',phDay:'185.40',phNight:'185.40',
  halfDay:'0',rehearsal:'0',shiftCall:'0',nonPerf:'63.34',fitting4:'63.34',fittingOver:'128.20',
  holPct:'0',
  fwStd:'10',fwCwd:'10',
  otDay:'9.53',otNight:'14.30',otPhDay:'14.30',otPhNight:'14.30',
  brokenTurnHours:'11',brokenTurnRate:'0',
  travelA:'11.00',travelB:'16.00',earlyUplift:'9.00',travelDBase:'20.00',travelDMile:'0',
  supA:'20.00',supAOwn:'20.00',supAUnused:'0',supB:'20.00',supC:'27.05',supD:'32.46',supE:'32.46',
  mealShortDay:'9.53',mealShortNight:'14.30',mealShortPhDay:'14.30',mealShortPhNight:'14.30',
  mealLateDay:'9.53',mealLateNight:'14.30',mealLatePhDay:'14.30',mealLatePhNight:'14.30',
  mealSupperDay:'9.53',mealSupperNight:'14.30',mealSupperPhDay:'14.30',mealSupperPhNight:'14.30',
  allowBreakfast:'5.00',allowLunch:'10.00',allowDinner:'25.00',allowOvernight:'0',
  cancelBefore12:'0',cancelAfter12:'0',cancelNight:'0',cancelFitting:'0',
  intimacy:'0',nudityTopless:'0',nudityToplessFeat:'0',nudityFull:'0',
};
const BUILTIN_CARDS={
  sa:{
    'PACT/Equity 2026':crowdVals('sa',EQUITY_2026_KEYS),
    'PACT/Equity 2025':crowdVals('sa',{...EQUITY_2026_KEYS,
      day:'123.15',night:'123.15',phDay:'123.15',phNight:'123.15',nonPerf:'0',fitting4:'0',fittingOver:'0',
      otDay:'9.16',otNight:'13.74',otPhDay:'13.74',otPhNight:'13.74',
      supA:'0',supAOwn:'0',supB:'0',supC:'0',supD:'0',supE:'0',
      mealShortDay:'0',mealShortNight:'0',mealShortPhDay:'0',mealShortPhNight:'0',
      mealLateDay:'0',mealLateNight:'0',mealLatePhDay:'0',mealLatePhNight:'0',
      mealSupperDay:'0',mealSupperNight:'0',mealSupperPhDay:'0',mealSupperPhNight:'0'}),
  },
...
// ---------------------------------------------------------------------------
// Card metadata — the paperwork around the numbers
// ---------------------------------------------------------------------------
// Stored in the same values bag as the rates, under ids beginning `_`.
// resolveRateVals() walks RATE_FIELDS only, so these never reach the engine —
// they exist so a card can say which agreement it is, when it runs, and which
// document it was typed from.
const RATE_META_FIELDS=[
  {id:'_from',label:'Effective from',type:'date'},
  {id:'_to',label:'Effective to',type:'date'},
  {id:'_ref',label:'Agreement / reference',type:'text',placeholder:'e.g. PACT/FAA 2026'},
  {id:'_notes',label:'Notes',type:'textarea',placeholder:'Anything a producer should know about this card'},
];
const RATE_META_IDS=RATE_META_FIELDS.map(f=>f.id).concat('_doc');
// The real rate card documents, shipped with the app, so the numbers on screen
// can always be checked against the page they came from.
const BUILTIN_DOCS={
  sa:{
    __default:{name:'Pact / FAA Rate Card 2026',url:'/rate-cards/pact-faa-2026.pdf'},
    'PACT/Equity 2026':{name:'PACT Equity Rate Card 2026',url:'/rate-cards/pact-equity-2026.pdf'},
  },
  spact:{
    __default:{name:'Take 3 SPACT Rate Card 2026',url:'/rate-cards/take3-spact-2026.pdf'},
  },
  stunts:{},
};
/** The source document for a card — a shipped PDF for a standard, or one the
 *  user uploaded onto their own card. */
function cardDoc(dept,name,vals){
  if(vals&&vals._doc){try{return JSON.parse(vals._doc)}catch(e){/* Ignore malformed saved document metadata. */}}
  return (BUILTIN_DOCS[dept]||{})[name||'__default']||null;
}
...
// the fee and meal-penalty pickers seeded earlier in the file with the 2026
// baseline can now be built properly from the card
refreshCardLists();
function applyRateVals(vals){
  ACTIVE_RATES={...RC_DEFAULTS,...vals};
  for(const id of RC_INPUTS){const el=/** @type {HTMLInputElement|null} */(document.getElementById(id));if(el&&vals[id]!=null)el.value=vals[id];}
  // the supplementary-fee and meal-penalty pickers are the card's own tables
  if(typeof refreshCardLists==='function')refreshCardLists();
  refreshAll();
}
...
let RCA_EDIT=null;
// Whether the source-document panel is open.
let RCA_DOC_OPEN=false;
// Rate cards now have their own sidebar page (Rate cards) — everything that
// used to open the modal navigates there instead, so there is one home.
function openRateAdmin(){showDash('rates');}
...
// A card's own values, whichever kind of card it is. '__default' is the
// baseline: it has no stored values of its own, it IS the defaults.
function rcaVals(kind,name){
  if(!name||name==='__default')return {};
  return RATECARDS[kind][name]||BUILTIN_CARDS[kind][name]||{};
}
function rcaIsCustom(kind,name){return !!(name&&name!=='__default'&&RATECARDS[kind][name])}
/** The full resolved value set for a card — what it actually prices with. */
function rcaResolved(kind,name){
  const v=rcaVals(kind,name),out={};
  for(const f of RATE_FIELDS.filter(f=>f.dept===kind))out[f.id]=v[f.id]!=null&&v[f.id]!==''?v[f.id]:RC_DEFAULTS[f.id];
  for(const id of RATE_META_IDS)if(v[id]!=null)out[id]=v[id];
  return out;
}
// one field input — a dropdown for mode-style fields, a number box otherwise
function rcaFieldHTML(f,v,dis){
  const tag=f.ref?`<span class="rca-ref" data-tip="On the card for reference — the budget does not charge this automatically yet">ref</span>`:'';
  const hint=f.hint?`<span class="rca-hint">${esc(f.hint)}</span>`:'';
  if(f.options)return `<div class="rfield"><label>${esc(f.label)}${tag}</label><div class="inwrap"><select data-rcaf="${f.id}" ${dis?'disabled':''} style="width:100%;border:none;background:var(--panel2);color:var(--ink);padding:9px 10px;font-family:var(--mono);font-size:13px">${f.options.map(([val,lab])=>`<option value="${val}"${v===val?' selected':''}>${lab}</option>`).join('')}</select></div>${hint}</div>`;
  return `<div class="rfield"><label>${esc(f.label)}${tag}</label><div class="inwrap"><span>${esc(f.unit||'')}</span><input data-rcaf="${f.id}" type="number" step="0.01" value="${esc(v)}" ${dis?'disabled':''}></div>${hint}</div>`;
}
/** The card's fields, split into the sections the printed card uses. */
function rcaGroupsHTML(kind,vals,dis){
  const fields=RATE_FIELDS.filter(f=>f.dept===kind);
  const order=[];
  for(const f of fields)if(!order.includes(f.group||'Rates'))order.push(f.group||'Rates');
  return order.map(g=>`<div class="rca-group">
      <div class="rca-grouphd">${esc(g)}</div>
      <div class="rca-fields">${fields.filter(f=>(f.group||'Rates')===g).map(f=>rcaFieldHTML(f,vals[f.id]!=null?vals[f.id]:RC_DEFAULTS[f.id],dis)).join('')}</div>
    </div>`).join('');
}
/** The paperwork block — dates, reference, notes. */
function rcaMetaHTML(vals,dis){
  return `<div class="rca-fields">${RATE_META_FIELDS.map(f=>{
    const v=vals[f.id]||'';
    if(f.type==='textarea')return `<div class="rfield" style="grid-column:1/-1"><label>${esc(f.label)}</label><div class="inwrap"><textarea data-rcam="${f.id}" rows="2" ${dis?'disabled':''} placeholder="${esc(f.placeholder||'')}" style="width:100%;border:none;background:var(--panel2);color:var(--ink);padding:9px 10px;font-family:var(--mono);font-size:12px;resize:vertical">${esc(v)}</textarea></div></div>`;
    return `<div class="rfield"><label>${esc(f.label)}</label><div class="inwrap"><input data-rcam="${f.id}" type="${f.type}" value="${esc(v)}" ${dis?'disabled':''} placeholder="${esc(f.placeholder||'')}"></div></div>`;
  }).join('')}</div>`;
}
/** The source-document panel — the real rate card, on screen, next to the numbers. */
function rcaDocHTML(kind,name,vals,dis){
  const doc=cardDoc(kind,name,vals);
  const open=RCA_DOC_OPEN;
  const head=`<div class="rca-dochd">
      <span class="rca-doctitle">${doc?esc(doc.name):'No document attached'}</span>
      ${doc?`<button class="tb-btn" data-rcadoctoggle="1">${open?'Hide':'Show'} the card</button>`:''}
      ${doc&&doc.url?`<a class="tb-btn" href="${esc(doc.url)}" target="_blank" rel="noopener">Open in a new tab</a>`:''}
      ${dis?'':`<button class="tb-btn" data-rcadocpick="1">${doc?'Replace':'Attach'} a PDF</button>`}
      ${dis||!doc||!vals._doc?'':`<button class="tb-btn" data-rcadocclear="1">Remove</button>`}
      <input type="file" id="rcaDocFile" accept="application/pdf,image/*" style="display:none">
    </div>`;
  // The pages are drawn into canvases rather than framed: the app sends
  // frame-ancestors 'none' on everything it serves, so even its own PDFs
  // cannot be put in an iframe. pdf.js is already bundled for reading
  // schedules, and it draws these just as well.
  const body=doc&&open
    ? `<div class="rca-docpages" id="rcaDocPages"><div class="rca-docnote">Loading the rate card…</div></div>`
    : `<div class="rca-docnote">${doc?'The rate card document is attached — show it to check any number on this page against the page it came from.':'Attach the agency’s own PDF so every number here can be checked against the document it came from.'}</div>`;
  return `<div class="rca-doc">${head}${body}</div>`;
}
function renderRateAdmin(){
  const deptList=(d,i)=>{
    const defName=d.defaults.replace(' (defaults)','');
    const cardRow=(name,label,meta,kind,standard)=>{
      const vals=rcaVals(kind,name);
      const doc=cardDoc(kind,name,vals);
      const dates=[vals._from,vals._to].filter(Boolean).join(' – ');
      return `<div class="prow" data-rcaopen="${esc(name)}" data-rcakind="${kind}">
        <span class="pname">${esc(label)}${doc?' <span class="rca-docdot" data-tip="Rate card document attached">PDF</span>':''}</span>
        <span class="pmeta">${esc(dates||meta)}</span>
        <span class="ptools">
          <span data-rcadup="${esc(name)}" data-rcakind="${kind}" data-tip="Duplicate this card">⧉</span>
          ${standard?`<span class="ps-src sched" style="cursor:default">STANDARD</span>`:`<span data-rcadel="${esc(name)}" data-rcakind="${kind}" data-tip="Delete card">✕</span>`}
        </span>
        <span class="prow-chev">›</span>
      </div>`;
    };
    const rows=cardRow('__default',defName,'the baseline — every production starts on these numbers',d.kind,true)
      +Object.keys(BUILTIN_CARDS[d.kind]).sort().map(n=>cardRow(n,n,'built-in standard — pick it in Production Settings',d.kind,true)).join('')
      +Object.keys(RATECARDS[d.kind]).sort().map(n=>cardRow(n,n,'your card',d.kind,false)).join('');
  ...
  let form='';
  if(RCA_EDIT){
    const kind=RCA_EDIT.kind;
    const dept=RATE_DEPTS.find(d=>d.kind===kind);
    const isNew=!RCA_EDIT.name;
    const standard=!isNew&&!rcaIsCustom(kind,RCA_EDIT.name);
    // a standard is read-only; duplicate it to get an editable copy
    const dis=standard;
    const vals=RCA_EDIT.seed||(isNew?{}:rcaVals(kind,RCA_EDIT.name));
    const title=isNew?('New '+dept.label+' card'):(RCA_EDIT.name==='__default'?dept.defaults.replace(' (defaults)',''):RCA_EDIT.name);
    const nameRow=standard?'':`<div class="rfield" style="max-width:340px"><label>Card name</label><div class="inwrap"><input id="rcaName" value="${esc(isNew?(RCA_EDIT.suggest||''):RCA_EDIT.name)}" placeholder="e.g. ${kind==='sa'?'PACT/Equity 2027':kind==='spact'?'Take 3 SPACT 2027':'Equity TV 2027'}" ${isNew?'':'disabled'}></div></div>`;
    const actions=standard
      ? `<button class="tb-btn" data-rcadup="${esc(RCA_EDIT.name)}" data-rcakind="${kind}" style="border-color:var(--hv-line);color:var(--hv)">Duplicate to edit</button>
         <button class="tb-btn" id="rcaCancel">Close</button>
         <span class="cdinfo">A standard card can’t be changed. Duplicate it and the copy is yours to change.</span>`
      : `<button class="tb-btn" id="rcaSave" style="border-color:var(--hv-line);color:var(--hv)">Save card</button>
         ...
         <span class="cdinfo">Every production using this card picks up the change.</span>`;
    form=`<div class="dash-sub" style="margin:22px 0 8px">${esc(title)}${standard?' — standard card, read-only':''}</div>
    <div class="prodcard"><div class="punits" style="padding:16px">
      ${nameRow}
      ${rcaDocHTML(kind,isNew?'':RCA_EDIT.name,vals,dis)}
      <div class="rca-group"><div class="rca-grouphd">The card itself</div>${rcaMetaHTML(vals,dis)}</div>
      ${rcaGroupsHTML(kind,vals,dis)}
      <div style="display:flex;gap:10px;margin-top:16px;align-items:center;flex-wrap:wrap">${actions}</div>
    </div></div>`;
  }
  // Named roles (a stand-in, a picture double) are one production's own deal,
  // not an account-wide card, so they are set up on the production. Say where.
  const rolesNote=APPMODE==='crowd'
    ? `<div class="dash-sub" style="margin:22px 0 8px">Named roles — a stand-in, a picture double</div>
       <div class="rates-note" style="font-family:var(--mono);font-size:11px;color:var(--faint);line-height:1.6">Someone paid on their own day rate rather than the crowd rate gets a <b>named role</b> — “Stand-in”, “Picture double” — with its own rates, and you put crowd groups on it. Roles belong to one production, so they are set up in <b>Production settings → Rate cards</b>, under the cards above.</div>`
    : '';
  $('#rcaBody').innerHTML=deptsForMode().map((d,i)=>deptList(d,i)).join('')+rolesNote+form;
  if(RCA_EDIT){const f=$('#rcaBody .prodcard');if(f)f.scrollIntoView({block:'nearest'})}
}
// Read what is on screen back off the form.
function rcaFormVals(){
  const vals={};
  $('#rcaBody').querySelectorAll('[data-rcaf]').forEach(el=>{vals[el.dataset.rcaf]=el.value});
  $('#rcaBody').querySelectorAll('[data-rcam]').forEach(el=>{if(el.value)vals[el.dataset.rcam]=el.value});
  if(RCA_EDIT&&RCA_EDIT.seed&&RCA_EDIT.seed._doc)vals._doc=RCA_EDIT.seed._doc;
  return vals;
}
/** Open a card in the editor, keeping whatever is typed but not yet saved. */
function rcaOpen(next){
  RCA_EDIT=next;
  RCA_DOC_OPEN=false;
  renderRateAdmin();
}
$('#rcaBody').addEventListener('click',async e=>{
  const nw=e.target.closest('[data-rcanew]');
  if(nw){rcaOpen({kind:nw.dataset.rcanew,name:''});return}
  if(e.target.closest('#rcaCancel')){rcaOpen(null);return}
  // ---- duplicate: the only way to get an editable copy of a standard, and
  // the fastest way to start next year's card from this year's ----
  const dup=e.target.closest('[data-rcadup]');
  if(dup){
    e.stopPropagation();
    const from=dup.dataset.rcadup,kind=dup.dataset.rcakind;
    const dept=RATE_DEPTS.find(d=>d.kind===kind);
    const base=from==='__default'?dept.defaults.replace(' (defaults)',''):from;
    const taken=cardsFor(kind);
    let suggest=base+' (copy)',n=2;
    while(taken[suggest]){suggest=base+' (copy '+(n++)+')'}
    rcaOpen({kind,name:'',suggest,seed:{...rcaResolved(kind,from)}});
    setStatus('Copied “'+base+'”. Change what you need and save it under the new name.');
    return;
  }
  ...
  if(del){
    ...
    if(!await askConfirm({
      title:'Delete the rate card “'+n+'”?',
      body:'Productions already using this card keep the numbers they have. They just stop picking up any future change you would have made here.',
      yes:'Delete the card',danger:true}))return;
  ...
  // ---- the document panel ----
  if(e.target.closest('[data-rcadoctoggle]')){
    RCA_DOC_OPEN=!RCA_DOC_OPEN;
    // paint the panel first — the pages are drawn into it, so a re-render
    // afterwards would wipe them
    renderRateAdmin();
    if(RCA_DOC_OPEN)rcaLoadDoc();
    return;
  }
  if(e.target.closest('[data-rcadocpick]')){const f=$('#rcaDocFile');if(f)f.click();return}
  if(e.target.closest('[data-rcadocclear]')){
    if(RCA_EDIT){RCA_EDIT.seed={...rcaFormVals()};delete RCA_EDIT.seed._doc;}
    RCA_DOC_OPEN=false;
    renderRateAdmin();
    return;
  }
  ...
  if(op){rcaOpen({kind:op.dataset.rcakind,name:op.dataset.rcaopen==='__default'?'__default':op.dataset.rcaopen});}
});
// ---- attaching the agency's own rate card document -------------------------
// The file goes to the same private bucket the schedule sources use, under the
// account's own folder, so nobody else can read it. Without an account there
// is nowhere private to put it, so we say so rather than losing the file.
$('#rcaBody').addEventListener('change',async e=>{
  const inp=e.target.closest('#rcaDocFile');
  if(!inp||!inp.files||!inp.files[0]||!RCA_EDIT)return;
  const file=inp.files[0];
  if(!CLOUD.session||!cloud.uploadScheduleFile){
    setStatus('Sign in to keep a rate card document with your card.');
    inp.value='';return;
  }
  if(file.size>15*1024*1024){setStatus('That document is too big — 15MB is the limit.');inp.value='';return}
  setStatus('Uploading “'+file.name+'”…');
  const owner=(CLOUD.session.user&&CLOUD.session.user.id)||'u';
  const safe=file.name.replace(/[^\w.\-]+/g,'_').slice(-80);
  const path=`${owner}/rate-cards/${RCA_EDIT.kind}/${Date.now()}_${safe}`;
  const r=await cloud.uploadScheduleFile(path,file,file.type||'application/pdf').catch(err=>({error:err}));
  if(r&&r.error){setStatus(friendlyCloudError(r.error,'that document'));inp.value='';return}
  const seed={...rcaFormVals()};
  seed._doc=JSON.stringify({name:file.name,path});
  RCA_EDIT.seed=seed;
  RCA_DOC_OPEN=true;
  renderRateAdmin();
  rcaLoadDoc();
  setStatus('Document attached. Save the card to keep it.');
});
/** The document's bytes — a shipped standard is fetched from /public, an
 *  uploaded one is pulled out of the account's private bucket. */
async function rcaDocBlob(doc){
  if(!doc)return null;
  if(doc.url){
  ...
  if(!doc.path||!CLOUD.session||!cloud.downloadScheduleFile)throw new Error('not available');
  const {data,error}=await cloud.downloadScheduleFile(doc.path);
  if(error||!data)throw (error||new Error('not available'));
  return data;
}
// bumped on every load so a slow fetch for a card you have navigated away from
// never paints over the one you are looking at
let RCA_DOC_GEN=0;
/** Draw the card document into the panel — every page, top to bottom. */
async function rcaLoadDoc(){
  if(!RCA_EDIT)return;
  const gen=++RCA_DOC_GEN;
  const vals=RCA_EDIT.seed||rcaVals(RCA_EDIT.kind,RCA_EDIT.name);
  const doc=cardDoc(RCA_EDIT.kind,RCA_EDIT.name,vals);
  if(!doc)return;
  const host=()=>document.getElementById('rcaDocPages');
  try{
    const blob=await rcaDocBlob(doc);
    if(gen!==RCA_DOC_GEN)return;
    const isPdf=/pdf/i.test(blob.type||'')||/\.pdf$/i.test(doc.name||'')||/\.pdf$/i.test(doc.url||'');
    let el=host();
    if(!el){await new Promise(r=>setTimeout(r,0));el=host()}
    if(!el||gen!==RCA_DOC_GEN)return;
    if(!isPdf){
      const url=URL.createObjectURL(blob);
      el.innerHTML=`<img class="rca-docpage" src="${url}" alt="${esc(doc.name)}">`;
      return;
    }
    const lib=await loadPdfjs();
    if(gen!==RCA_DOC_GEN)return;
    const pdf=await lib.getDocument({data:await blob.arrayBuffer(),...PDF_ASSET_OPTS}).promise;
    if(gen!==RCA_DOC_GEN){try{pdf.destroy()}catch(e){/* Best-effort cleanup. */}return}
    el=host();if(!el){try{pdf.destroy()}catch(e){/* Best-effort cleanup. */}return}
    el.innerHTML='';
    const width=Math.min(el.clientWidth||900,1100);
    for(let n=1;n<=pdf.numPages;n++){
      const page=await pdf.getPage(n);
      if(gen!==RCA_DOC_GEN){try{pdf.destroy()}catch(e){/* Best-effort cleanup. */}return}
      const base=page.getViewport({scale:1});
      // draw at device resolution so the small print on a rate card is legible
      const scale=(width/base.width)*Math.min(window.devicePixelRatio||1,2);
      const vp=page.getViewport({scale});
      const cv=document.createElement('canvas');
      cv.className='rca-docpage';
      cv.width=Math.ceil(vp.width);cv.height=Math.ceil(vp.height);
      cv.style.width='100%';
      await page.render({canvas:cv,canvasContext:cv.getContext('2d'),viewport:vp}).promise;
      if(gen!==RCA_DOC_GEN){try{pdf.destroy()}catch(e){/* Best-effort cleanup. */}return}
      el.appendChild(cv);
    }
    ...
    if(gen!==RCA_DOC_GEN)return;
    const el=host();
    if(el)el.innerHTML=`<div class="rca-docnote">That document couldn’t be opened${CLOUD.session?'':' — sign in to see a document you uploaded'}.</div>`;
  }
}
...
document.addEventListener('contextmenu',e=>{
  ...
  if(chipEl){
    ...
    if(ctx){
      e.preventDefault();
      // The keyboard Menu key raises this same event with no pointer position
      // (0,0) — anchor the menu on the chip instead so it opens where the
      // focus is rather than in the top-left corner.
      let x=e.clientX,y=e.clientY;
      if(!x&&!y){const r=chipEl.getBoundingClientRect();x=r.left;y=r.bottom;}
      openCrowdGroupMenu(x,y,ctx);
      return;
    }
  }
  ...
  if(stripEl){
    ...
    openCtxMenu(e.clientX,e.clientY,[
      ...
      {sep:true},
      {label:'Delete scene',icon:'trash',danger:true,onClick:async()=>{
        const s=d.scenes[idx];
        if(!await askConfirm({
          title:'Delete scene '+(s.num||'')+'?',
          body:'The crowd and stunt requirements on this scene go with it, and the day’s cost changes to match.',
          yes:'Delete the scene',danger:true}))return;
  ...
  if(prodEl){
    ...
    openCtxMenu(e.clientX,e.clientY,[
      ...
      {sep:true},
      {label:'Remove production',icon:'trash',danger:true,onClick:async()=>{
        if(!await confirmDeleteProduction(name))return;
        deleteProduction(name);
      }}
    ]);
...
$('#coverInput').addEventListener('change',async e=>{
  ...
  catch(err){setStatus(friendlyFileError(err))}
  COVER_TARGET=null;
});
...
$('#avatarInput').addEventListener('change',async e=>{
  ...
  try{
    ...
    if(error){setStatus(friendlyCloudError(error,'your profile photo'));return}
...
const PS_SECTIONS=[['general','General'],['branding','Branding'],['locations','Locations'],['shootlocs','Shooting locations'],['info','Production info'],['cast','Cast list'],['rates','Rate cards'],['oncosts','On-costs'],['columns','Columns'],['history','History']];
...
function psLocRow(l){
  ...
  return `<tr data-manual="${l.manual?1:0}">
    ...
    <td>${l.manual?'<button class="ps-rm" data-psrm="loc" aria-label="Remove this location">✕</button>':''}</td></tr>`;
}
...
function psRateRows(p,dept,liveOverrides){
  ...
  for(const f of RATE_FIELDS){
    ...
    // the card is long — carry its own section headings into the override
    // table so a producer can find the one number they mean to change
    const g=f.group||'Rates';
    if(g!==lastGroup){lastGroup=g;html+=`<tr class="ps-rategrp"><td colspan="3">${esc(g)}</td></tr>`}
    const cv=cardVals[f.id]!=null?cardVals[f.id]:RC_DEFAULTS[f.id];
    const ov=overrides[f.id];
    // £ reads before the number (£111.21); every other unit reads after it
    // (12.07%, 1.5×) — the old code always prefixed, so "%12.07" came out backwards.
    const pre=f.unit==='£';
    const shown=f.options?((f.options.find(o=>o[0]===cv)||f.options[0])[1]):(pre?esc(f.unit)+esc(cv):esc(cv)+esc(f.unit));
    const num=`<input class="ps-rateov" data-field="${f.id}" type="number" step="0.01" placeholder="auto" value="${ov!=null?esc(ov):''}">`;
    const ovCell=f.options
      ...
      :`<div class="inwrap ps-ovwrap">${pre?`<span>${esc(f.unit)}</span>${num}`:`${num}<span class="suffix">${esc(f.unit)}</span>`}</div>`;
    html+=`<tr><td>${esc(f.label)}</td><td style="font-family:var(--mono);color:var(--faint)">${shown}</td><td>${ovCell}</td></tr>`;
  }
  return html;
}
/** One free-form, user-named on-cost percentage line. */
// ---------------------------------------------------------------------------
// NAMED ROLES — the rate-card area, where you set up a stand-in or a picture
// double as its own kind of person with its own day rate.
//
// This sits with the rate cards because that is what it is: another card, this
// one named by the production. Every optional box shows the figure the role
// WOULD use if you leave it blank, so you can see what you are overriding
// rather than guessing. Saving is immediate (the same way a rate card saves),
// so nothing here rides on the Save button at the bottom of the page.
// ---------------------------------------------------------------------------
// null = list only; {id:''} = the new-role form; {id} = editing that role
let PS_ROLE_EDIT=null;
/** How much of the production is riding on a role — used before deleting one. */
function roleUsageCount(prodName,roleId){
  let groups=0;const days=new Set();
  for(const s of prodSourcesOf(prodName))for(const d of (s.model&&s.model.days)||[])
    for(const sc of d.scenes||[])
      for(const f of [...(sc.saChars||[]),...(sc.featured||[]),...(sc.spacts||[])])
        if(f&&f.roleId===roleId){groups++;days.add(s.ns+'|'+d.id)}
  for(const [k,cfg] of Object.entries(CDAY))
    if(cfg&&Array.isArray(cfg.chars))
      for(const ch of cfg.chars)if(ch&&ch.roleId===roleId){groups++;days.add('cd|'+k)}
  return {groups,days:days.size};
}
/** The optional money boxes, in the order the form discloses them. */
const PS_ROLE_FIELDS=[
  {id:'night',label:'Night rate',unit:'£',hint:'What one night pays. Leave blank and we scale your day rate the way the agreement itself scales a night — a role can never end up paying less for a night than for a day.'},
  {id:'phDay',label:'Public holiday — day',unit:'£',hint:'Leave blank and we scale it off your day rate the same way the agreement does.'},
  {id:'phNight',label:'Public holiday — night',unit:'£',hint:''},
  {id:'hol',label:'Holiday pay',unit:'%',hint:''},
  {id:'otDay',label:'Overtime — day, per 30 min',unit:'£',hint:'Holiday-inclusive, per half hour, as the card states it.'},
  {id:'otNight',label:'Overtime — night &amp; early call, per 30 min',unit:'£',hint:''},
  {id:'otPhDay',label:'Overtime on a public holiday — day',unit:'£',hint:''},
  {id:'otPhNight',label:'Overtime on a public holiday — night',unit:'£',hint:''},
  {id:'earlyTravel',label:'Early-call travel',unit:'£',hint:'Paid when the role is called at or before 06:00.'},
  {id:'travelA',label:'Travel allowance — Cat A',unit:'£',hint:'TfL zones 1–3.'},
  {id:'travelB',label:'Travel allowance — Cat B',unit:'£',hint:'Studios and anywhere beyond zone 3.'},
];
/** The inherited figure for one field, said the way its box says it. */
function psRoleInherited(draft,fid){
  // before a day rate is typed, show what the agreement's own card says —
  // scaling everything off £0 would print a form full of £0.00
  const res=roleResolved(draft.day>0?draft:{...draft,day:undefined});
  if(fid==='hol')return draft.base==='sa'?(( +res.hol||0)*100).toFixed(2):(+res.hol||0).toFixed(2);
  const v=+res[fid];
  return Number.isFinite(v)?v.toFixed(2):'';
}
/** One optional money box, showing what it would inherit. */
function psRoleFieldHTML(f,draft,stated){
  const unit=f.id==='hol'&&draft.base==='sa'?'%':'£';
  const inh=psRoleInherited(draft,f.id);
  const shown=stated!=null&&stated!==''?esc(stated):'';
  const holNote=f.id==='hol'
    ?(draft.base==='sa'
      ? ' Percentage of the day rate — that is how the SA card writes it.'
      : ' A flat amount per day in lieu of holiday — that is how the SPACT card writes it.')
    :'';
  return `<div class="ps-field ps-rolefield">
    <label for="psRoleF-${f.id}">${f.label}</label>
    <div class="inwrap"><span>${unit}</span><input id="psRoleF-${f.id}" class="ps-rolef" data-f="${f.id}" type="number" min="0" step="0.01" value="${shown}" placeholder="${esc(inh)}"></div>
    <span class="ps-hint">Blank = ${unit==='%'?esc(inh)+'%':unit+esc(inh)}, inherited from the ${draft.base==='spact'?'SPACT':'SA'} card.${holNote}${f.hint?' '+f.hint:''}</span>
  </div>`;
}
/** The whole roles block: the list, and the form when one is open. */
function psRolesBlockHTML(prodName){
  const roles=prodRoles(prodName);
  const rows=roles.length
    ? roles.map(r=>{
        const u=roleUsageCount(prodName,r.id);
        return `<tr>
          <td><b>${esc(roleLabel(r))}</b>${r.note?`<div style="color:var(--faint);font-size:10.5px">${esc(r.note)}</div>`:''}</td>
          <td style="font-family:var(--mono)">${gbp(r.day)}/day</td>
          <td>${esc(ROLE_BASE_SHORT[r.base])} — ${engineRoleFrameworkHours('std',/** @type {any} */(r.base))}h standard, ${engineRoleFrameworkHours('cwd',/** @type {any} */(r.base))}h continuous</td>
          <td style="color:var(--faint);font-size:10.5px">${u.groups?u.groups+' group'+(u.groups===1?'':'s')+' on '+u.days+' day'+(u.days===1?'':'s'):'not used yet'}</td>
          <td><button class="ps-add" type="button" data-psroleedit="${esc(r.id)}" style="margin:0;padding:4px 8px">Edit</button></td>
          <td><button class="ps-rm" type="button" data-psroledel="${esc(r.id)}" aria-label="Delete the role ${esc(roleLabel(r))}">✕</button></td>
        </tr>`;
      }).join('')
    : `<tr><td colspan="6" style="color:var(--faint);font-style:italic">No named roles yet. Everyone is costed as SA, Featured or SPACT, exactly as before.</td></tr>`;
  let form='';
  if(PS_ROLE_EDIT){
    const existing=roles.find(r=>r.id===PS_ROLE_EDIT.id);
    const draft=PS_ROLE_EDIT.draft||(existing?{...existing}:{id:'',label:'',base:'sa',day:0});
    const isNew=!existing;
    form=`<div class="prodcard ps-roleform" id="psRoleForm"><div class="punits" style="padding:16px">
      <div class="dash-sub" style="margin:0 0 12px">${isNew?'A new role':'Editing “'+esc(roleLabel(draft))+'”'}</div>
      <div class="ps-grid2">
        <div class="ps-field"><label for="psRoleName">What is this role called?</label>
          <input id="psRoleName" value="${esc(draft.label||'')}" placeholder="e.g. Stand-in, Picture double" maxlength="60">
          <span class="ps-hint">This is the name your budget prints.</span></div>
        <div class="ps-field"><label for="psRoleDay">Day rate</label>
          <div class="inwrap"><span>£</span><input id="psRoleDay" type="number" min="0" step="0.01" value="${draft.day?esc(draft.day):''}" placeholder="0.00"></div>
          <span class="ps-hint">What you have agreed to pay one of them for a normal shooting day.</span></div>
      </div>
      <div class="ps-field" style="margin-top:12px;max-width:520px"><label for="psRoleBase">Which agreement's working day does this role follow?</label>
        <select id="psRoleBase">
          <option value="sa"${draft.base!=='spact'?' selected':''}>${esc(ROLE_BASE_LABEL.sa)}</option>
          <option value="spact"${draft.base==='spact'?' selected':''}>${esc(ROLE_BASE_LABEL.spact)}</option>
        </select>
        <span class="ps-hint">This sets how long their working day is and when overtime starts — nothing else. The money is whatever you type here: a stand-in on £180 is still on £180 either way.</span></div>
      <div class="ps-field" style="margin-top:12px;max-width:520px"><label for="psRoleNote">Note (optional)</label>
        <input id="psRoleNote" value="${esc(draft.note||'')}" placeholder="e.g. agreed with the agency, 3 Feb" maxlength="200"></div>
      <details class="ps-roledetails"${PS_ROLE_EDIT.open?' open':''}>
        <summary>Nights, public holidays, holiday pay, overtime and travel — set any of these only if this role differs</summary>
        <div class="ps-grid2" style="margin-top:10px">
          ${PS_ROLE_FIELDS.map(f=>psRoleFieldHTML(f,draft,draft[f.id]!=null?(f.id==='hol'&&draft.base==='sa'?engineRound2(draft.hol*100):draft[f.id]):'')).join('')}
        </div>
      </details>
      <div style="display:flex;gap:10px;margin-top:16px;align-items:center">
        <button class="tb-btn" id="psRoleSave" type="button" style="border-color:var(--hv-line);color:var(--hv)">${isNew?'Create the role':'Save the role'}</button>
        <button class="tb-btn" id="psRoleCancel" type="button">Cancel</button>
      </div>
    </div></div>`;
  }
  return `<h5 class="ps-sub">Named roles with their own rates</h5>
    <div class="why">Some people on a crowd call are not on the crowd rate at all — a <b>stand-in</b>, a <b>picture double</b>, a driver. Give that kind of person a name and its own day rate here, then put crowd groups on it wherever you set their tier. Until you add one, nothing changes: everyone is still SA, Featured or SPACT.</div>
    <table class="ps-tbl"><thead><tr><th>Role</th><th style="width:110px">Day rate</th><th>Working day it follows</th><th style="width:150px">Used by</th><th style="width:60px"></th><th style="width:40px"></th></tr></thead>
    <tbody id="psRolesBody">${rows}</tbody></table>
    <button class="ps-add" id="psAddRole" type="button">+ Add a role</button>
    ${form}`;
}
/** Re-paint the roles block in place, keeping the rest of the page as it is. */
function psRenderRoles(){
  const host=document.getElementById('psRolesBlock');
  if(host)host.innerHTML=psRolesBlockHTML(PS_NAME);
}
/** Read the open form into a role object, or return null with a reason. */
function psRoleFromForm(){
  const label=(($('#psRoleName')||{}).value||'').trim();
  if(!label)return {err:'Give the role a name — “Stand-in”, “Picture double”.'};
  const base=(($('#psRoleBase')||{}).value)==='spact'?'spact':'sa';
  const day=Math.max(0,+(($('#psRoleDay')||{}).value)||0);
  if(!day)return {err:'Give the role a day rate — that is the one figure it cannot inherit.'};
  const role={id:PS_ROLE_EDIT.id||newRoleId(prodRoles(PS_NAME)),label:label.slice(0,60),base,day:engineRound2(day)};
  const note=(($('#psRoleNote')||{}).value||'').trim();
  if(note)role.note=note.slice(0,200);
  let bad='';
  document.querySelectorAll('#psRoleForm .ps-rolef').forEach(raw=>{
    const el=/** @type {any} */(raw);
    const v=(el.value||'').trim();
    if(v==='')return;
    let n=+v;
    if(!Number.isFinite(n)||n<0){bad=bad||'One of the rates is not a number we can use.';return}
    // holiday on an SA-shaped role is a percentage on screen and a fraction in
    // the engine; on a SPACT-shaped role it is a flat amount and stays as typed
    if(el.dataset.f==='hol'&&base==='sa'){
      if(n>100){bad=bad||'Holiday pay is a percentage of the day rate — 12.07, not 1207.';return}
      n=n/100;
    }
    // a fraction keeps its precision (0.1207); every money figure is pennies
    role[el.dataset.f]=(el.dataset.f==='hol'&&base==='sa')?Math.round(n*1e6)/1e6:engineRound2(n);
  });
  if(bad)return {err:bad};
  return {role};
}
/** Persist the production's roles: local, cloud, and every screen. */
function psSaveRoles(list,what){
  const P=PRODS[PS_NAME];
  if(!P)return;
  P.roles=list.length?list:undefined;
  saveProds();
  if(CLOUD.session&&cloud.upsertProd)cloud.upsertProd(PS_NAME,P).catch(()=>{});
  if(what)logProdEvent(PS_NAME,'settings','Roles — '+what);
  psRenderRoles();
  if(typeof refreshAll==='function')refreshAll();
}
function psUpliftRow(l){
  l=l||{};
  return `<tr>
    <td><input class="ps-uplab" value="${esc(l.label||'')}" placeholder="e.g. Employer NI" aria-label="What this on-cost is"></td>
    <td><div class="inwrap" style="max-width:130px"><span>%</span><input class="ps-uppct" type="number" min="0" max="100" step="0.1" value="${l.pct?esc(l.pct):''}" placeholder="0" aria-label="Percentage for ${esc(l.label||'this on-cost')}"></div></td>
    <td><button class="ps-rm" data-psrm="uplift" type="button" aria-label="Remove ${esc(l.label||'this on-cost line')}">✕</button></td></tr>`;
}
function psPersonRow(pr){
  return `<tr>
    ...
    <td><span class="ps-src" data-tip="This is your own note of who is on the production. Nobody here is emailed and nobody is given access — sharing an account with your team isn’t built yet.">Your note only</span></td>
    <td><button class="ps-rm" data-psrm="person" aria-label="Remove ${esc(pr.name||pr.role||'this person')}">✕</button></td></tr>`;
}
...
function openProdSettings(name,inline){
  ...
  $('#psContent').innerHTML=`
    ...
    <div class="ps-sec" id="ps-sec-branding"><h4>Branding</h4>
      ...
      <div class="ps-brand">
        <div class="ps-cover-prev${cover?'':' empty'}" id="psCoverPrev" style="${coverCssUrl(cover)?`background-image:${coverCssUrl(cover)}`:''}">${cover?'':'<span>No cover yet</span>'}</div>
      ...
      <div class="ps-brand" style="margin-top:16px">
        <div class="ps-cover-prev ps-banner-prev${banner?'':' empty'}" id="psBannerPrev" style="${coverCssUrl(banner)?`background-image:${coverCssUrl(banner)}`:''}">${banner?'':'<span>No banner yet</span>'}</div>
        <div class="ps-brand-side">
          <div class="ps-brand-actions">
    ...
    <div class="ps-sec" id="ps-sec-locations"><h4>Locations</h4>
      ...
      <div class="rfield" style="max-width:520px;margin-bottom:12px"><label>Locations we don't recognise are charged at</label>
        <div class="inwrap"><select id="psUnknownBand" style="width:100%;border:none;background:var(--panel2);color:var(--ink);padding:9px 10px;font-family:var(--mono);font-size:13px;font-weight:600">
          <option value="B"${(p.unknownBand||'B')==='B'?' selected':''}>Cat B — ${esc(gbp(gTB()))} a head (the safer guess)</option>
          <option value="A"${p.unknownBand==='A'?' selected':''}>Cat A — ${esc(gbp(gTA()))} a head</option>
        </select></div></div>
      <div class="why">Cat B is the default because a place nobody has listed is more likely to be out of town than in Zone 1, and a budget that can only come down is the safe one. Pick Cat A to go back to the older, cheaper assumption. Note that a day whose calculator has already been opened keeps whichever band was saved on it, so the same location can show two bands until you re-open those days.</div>
    ...
    <div class="ps-sec" id="ps-sec-rates"><h4>Rate cards</h4>
      ...
      ${APPMODE==='crowd'?`<div id="psRolesBlock">${psRolesBlockHTML(name)}</div>`:''}</div>
    <div class="ps-sec" id="ps-sec-oncosts"><h4>On-costs</h4>
      <div class="why">The rate cards above price what the <b>artist</b> is paid. That is not what the production pays. Everything on this page is <b>off until you switch it on</b> — nothing here changes a single number until you type a percentage.</div>

      <h5 class="ps-sub">What you pay on top of the artists' fees</h5>
      <div class="why">These are charged on the artist cost — the rate card, supplementary fees and meal penalties added together.</div>
      <div class="ps-grid2">
        <div class="ps-field"><label for="psAgencyPct">Agency commission (%)</label>
          <input id="psAgencyPct" type="number" min="0" max="100" step="0.1" value="${oc.uplift.agencyPct||''}" placeholder="0"></div>
        <div class="ps-field"><label for="psContingencyPct">Contingency (%)</label>
          <input id="psContingencyPct" type="number" min="0" max="100" step="0.1" value="${oc.uplift.contingencyPct||''}" placeholder="0">
          <span class="ps-hint">Your buffer against the crowd going over. Charged on the subtotal — the artist cost <i>plus</i> the commission and any lines below. Crowd budgets commonly carry 5–10%.</span></div>
      </div>

      <h5 class="ps-sub">Your own extra on-cost lines</h5>
      <div class="why">Anything else your production carries on top of the artists' fees — employer NI, payroll administration, costume handling. Name each one yourself and give it a percentage; each is charged on the artist cost, never on top of the commission or on each other, so the order you add them in cannot change the total. We deliberately do not guess UK employment law for you.</div>
      <table class="ps-tbl"><thead><tr><th>What it is</th><th style="width:150px">Percentage</th><th style="width:40px"></th></tr></thead>
      <tbody id="psUpliftBody">${(oc.uplift.additional.length?oc.uplift.additional:[]).map(psUpliftRow).join('')}</tbody></table>
      <button class="ps-add" id="psAddUplift" type="button">+ Add an on-cost line</button>

      <h5 class="ps-sub">UK bank holidays</h5>
      <div class="ps-toggle"><span class="tn">Flag bank holidays automatically</span><span class="td">A public holiday prices roughly 50% above a normal day. <b>The dates we hold are England &amp; Wales.</b> Scotland differs (2 January; the Summer holiday is the first Monday in August, not the last; no Easter Monday) and Northern Ireland adds St Patrick's Day and the Twelfth — so if you are shooting there, set your public holidays by hand on each day. Whatever you set on a day always wins over this.</span><button class="ps-tgl${oc.autoPublicHolidays?' on':''}" id="psAutoPH" type="button" role="switch" aria-checked="${oc.autoPublicHolidays?'true':'false'}" aria-label="Flag UK bank holidays automatically"></button></div>

      <h5 class="ps-sub">If you cancel a shoot day</h5>
      <div class="why">When a new schedule drops a shoot day we tell you what cutting it saves. Inside your agency's notice period you still owe most of it, so without these terms that saving is overstated — we would tell you that you saved £38,000 when you saved nothing. Terms vary by agency and by production, so they are yours to state. Leave at zero and a cut day still reads as a full saving, exactly as it did before.</div>
      <div class="ps-grid2">
        <div class="ps-field"><label for="psCancelDays">Notice period (days)</label>
          <input id="psCancelDays" type="number" min="0" max="60" step="1" value="${oc.cancellation.noticeDays||''}" placeholder="0">
          <span class="ps-hint">Cancel this many days before the shoot date, or fewer, and the charge applies. A day already in the past always counts — you cannot give notice backwards.</span></div>
        <div class="ps-field"><label for="psCancelPct">Charged at (% of the day)</label>
          <input id="psCancelPct" type="number" min="0" max="100" step="1" value="${oc.cancellation.pct||''}" placeholder="0">
          <span class="ps-hint">How much of the day's full cost is still payable inside that window. 100 = the whole day.</span></div>
      </div>
      <div class="why">A day we cannot date is never charged — we will not invent a charge we cannot work out.</div>
    </div>
...
$('#psContent').addEventListener('click',e=>{
  ...
  if(t.id==='psAddUplift'){$('#psUpliftBody').insertAdjacentHTML('beforeend',psUpliftRow({}));const i=$('#psUpliftBody').lastElementChild.querySelector('input');if(i)i.focus();return}
  // ---- named roles (they save immediately, like a rate card) ----
  if(t.id==='psAddRole'){
    PS_ROLE_EDIT={id:'',draft:{id:'',label:'',base:'sa',day:0}};
    psRenderRoles();
    const n=document.getElementById('psRoleName');if(n)n.focus();
    return;
  }
  {
    const ed=t.closest&&t.closest('[data-psroleedit]');
    if(ed){PS_ROLE_EDIT={id:ed.dataset.psroleedit};psRenderRoles();const n=document.getElementById('psRoleName');if(n)n.focus();return}
  }
  if(t.id==='psRoleCancel'){PS_ROLE_EDIT=null;psRenderRoles();return}
  if(t.id==='psRoleSave'){
    const out=psRoleFromForm();
    if(out.err){setStatus(out.err);return}
    const list=prodRoles(PS_NAME);
    const i=list.findIndex(r=>r.id===out.role.id);
    if(i>=0)list[i]=out.role;else list.push(out.role);
    PS_ROLE_EDIT=null;
    psSaveRoles(list,(i>=0?'“'+out.role.label+'” updated':'“'+out.role.label+'” added')+' at '+gbp(out.role.day)+'/day');
    setStatus('Role “'+out.role.label+'” saved — '+gbp(out.role.day)+' a day on the '+ROLE_BASE_SHORT[out.role.base].toLowerCase()+'.');
    return;
  }
  {
    const del=t.closest&&t.closest('[data-psroledel]');
    if(del){
      const id=del.dataset.psroledel;
      const role=prodRoles(PS_NAME).find(r=>r.id===id);
      if(!role)return;
      const u=roleUsageCount(PS_NAME,id);
      (async()=>{
        if(!await askConfirm({
          title:'Delete the role “'+roleLabel(role)+'”?',
          body:u.groups
            ? u.groups+' crowd '+(u.groups===1?'group':'groups')+' across '+u.days+' day'+(u.days===1?'':'s')+' '+(u.groups===1?'is':'are')+' being paid as this role. '+(u.groups===1?'It':'They')+' will go back to being costed on their own tier — SA, Featured or SPACT — so the budget will change. We will flag each one on the day so you can point it somewhere else.'
            : 'Nothing is using it, so no number changes.',
          yes:'Delete the role',danger:true}))return;
        psSaveRoles(prodRoles(PS_NAME).filter(r=>r.id!==id),'“'+roleLabel(role)+'” deleted');
        setStatus('Role “'+roleLabel(role)+'” deleted.'+(u.groups?' '+u.groups+' group'+(u.groups===1?'':'s')+' went back to their own tier.':''));
      })();
      return;
    }
  }
...
$('#psContent').addEventListener('change',e=>{
  ...
  // The role form's inherited figures depend on the day rate and on which
  // agreement's day it follows, so changing either re-states every "blank =
  // £x" hint. Done on change (not on every keystroke) so the box you are
  // typing in keeps the cursor.
  if(PS_ROLE_EDIT&&(e.target.id==='psRoleBase'||e.target.id==='psRoleDay')){
    const base=(($('#psRoleBase')||{}).value)==='spact'?'spact':'sa';
    const draft={
      id:PS_ROLE_EDIT.id||'',
      label:(($('#psRoleName')||{}).value||'').trim(),
      note:(($('#psRoleNote')||{}).value||'').trim(),
      base,
      day:Math.max(0,+(($('#psRoleDay')||{}).value)||0),
    };
    document.querySelectorAll('#psRoleForm .ps-rolef').forEach(raw=>{
      const el=/** @type {any} */(raw);
      const v=(el.value||'').trim();
      ...
      draft[el.dataset.f]=(el.dataset.f==='hol'&&base==='sa')?n/100:n;
    });
    const wasOpen=!!(/** @type {any} */(document.querySelector('#psRoleForm .ps-roledetails')||{})).open;
    PS_ROLE_EDIT={id:PS_ROLE_EDIT.id,draft,open:wasOpen};
    psRenderRoles();
    const back=document.getElementById(e.target.id);if(back)back.focus();
    return;
  }
...
$('#psSave').addEventListener('click',()=>{
  ...
  const ub=$('#psUnknownBand')&&$('#psUnknownBand').value==='A'?'A':'B';
  if(ub!==(P.unknownBand||'B')){P.unknownBand=ub;changed.push('unrecognised locations → Cat '+ub);}
  ...
  // ON-COSTS. All of this is zero/off by default, so a production nobody has
  // opened this section on costs exactly what it always did.
  {
    const num=id=>{const el=/** @type {any} */(document.getElementById(id));return el?Math.max(0,+el.value||0):0};
    const lines=[];
    const body=document.getElementById('psUpliftBody');
    if(body)body.querySelectorAll('tr').forEach(tr=>{
      const label=((tr.querySelector('.ps-uplab')||{}).value||'').trim();
      const pct=+((tr.querySelector('.ps-uppct')||{}).value)||0;
      if(label||pct)lines.push({label:label||'Additional',pct});
    });
    const phTgl=document.getElementById('psAutoPH');
    const oc={
      agencyPct:num('psAgencyPct'),
      contingencyPct:num('psContingencyPct'),
      additional:lines,
      autoPublicHolidays:!!(phTgl&&phTgl.classList.contains('on')),
      cancelNoticeDays:Math.round(num('psCancelDays')),
      cancelPct:num('psCancelPct'),
    };
    if(JSON.stringify(oc)!==JSON.stringify(P.oncosts||{})){
      const was=P.oncosts||{};
      P.oncosts=oc;
      const bits=[];
      if(oc.agencyPct!==(+was.agencyPct||0))bits.push('agency commission '+oc.agencyPct+'%');
      if(oc.contingencyPct!==(+was.contingencyPct||0))bits.push('contingency '+oc.contingencyPct+'%');
      if(JSON.stringify(lines)!==JSON.stringify(was.additional||[]))bits.push(lines.length?lines.length+' additional on-cost line'+(lines.length===1?'':'s'):'additional on-cost lines cleared');
      if(oc.autoPublicHolidays!==!!was.autoPublicHolidays)bits.push('auto UK bank holidays '+(oc.autoPublicHolidays?'on':'off'));
      if(oc.cancelNoticeDays!==(+was.cancelNoticeDays||0)||oc.cancelPct!==(+was.cancelPct||0))
        bits.push(oc.cancelNoticeDays&&oc.cancelPct?('cancellation '+oc.cancelPct+'% within '+oc.cancelNoticeDays+' days'):'cancellation terms cleared');
      changed.push('on-costs ('+(bits.join(', ')||'updated')+')');
    }
  }
...
$('#psDelete').addEventListener('click',async()=>{
...
// ---------- boot ----------
{
  ...
  // Schedules whose saved work has to move: [where it was filed, where it
  // lives now]. A schedule keeps the namespace it was saved under — until
  // another schedule already holds it, which is the case this whole change is
  // about (two revisions exported from Movie Magic under one filename). The
  // second one gets its own namespace and its own COPY of the shared work,
  // so from then on neither can overwrite or delete the other's.
  const nsMoves=[];
  const noteNsMove=rec=>{
    const src=SOURCES[SOURCES.length-1];
    if(!src)return;
    const old=rec.ns||legacyNsOf(rec);
    if(old&&old!==src.ns)nsMoves.push([old,src.ns]);
  };
  for(const rec of saved){
    try{
      if(rec.kind==='pdf'&&(rec.text||rec.aiModel)){
        const m=modelFrom(rec,rec.unit||'Main');
        if(addSource(m,rec.title,rec.short,false,{uid:rec.uid||undefined,ns:rec.ns||undefined,kind:'pdf',text:rec.text,unit:rec.unit||'Main',cloudId:rec.cloudId||null,pdfFiles:rec.pdfFiles||null,prod:rec.prod,version:rec.version,schedDate:rec.schedDate,colour:rec.colour||undefined,format:rec.format,rateCard:rec.rateCard,current:rec.current,createdAt:rec.createdAt||undefined,aiModel:rec.aiModel||null,docKind:rec.docKind||null})){
          // the account of what changed came back with the model — put it back
          setSrcRevDiff(SOURCES[SOURCES.length-1],rec.revDiff||(rec.aiModel&&rec.aiModel._revDiff)||null);
          noteNsMove(rec);
        }
      }else if(rec.kind==='manual'){
        if(addSource({days:[],castMap:{},notes:[]},rec.title,rec.short,false,{uid:rec.uid||undefined,ns:rec.ns||undefined,kind:'manual',allowEmpty:true,colour:'white',prod:rec.prod}))noteNsMove(rec);
      }
    }catch(e){console.error('restore source failed',e)}
  }
  // move saved work off the old title-keyed namespace before anything reads it
  let nsMoved=0;
  try{nsMoved=migrateNsKeys(nsMoves)}catch(e){console.error('namespace upgrade failed',e)}
  for(const s of SOURCES)restoreManualDays(s);
  // move any scene edits still filed under a scene's position onto the scene
  // itself, now that every schedule's days are loaded
  try{migrateSceneSlotKeys()}catch(e){console.error('slot key migration failed',e)}
  // schedules saved before schedules had their own id got one just now — write
  // it down straight away so it doesn't change again on the next reload
  if(nsMoves.length||nsMoved||saved.some(r=>r&&!r.uid))saveUserSources();
  ...
  // Crowd is the default. This is a crowd product: a new user invited to it
  // used to land in Stunt mode — titled "Laural — Stunt", with the three crowd
  // tabs hidden — and had no way of knowing the thing they were sent here for
  // was one click away behind a toggle they hadn't noticed.
  setAppMode(store.get('stuntos-appmode')||roleAppMode(CLOUD.session)||'crowd');
  ...
  // Everything above picks a DEFAULT landing spot. Now, if this browser saved
  // where the user was before their last reload, return them there instead.
  // saveNav() was suppressed through all the boot rendering above (NAV_BOOTING)
  // so the value from the previous session is still intact for this read.
  try{restoreNav();}catch(e){console.error('nav restore failed',e)}
  NAV_BOOTING=false; // from here on, navigation is recorded live
  // bring the sample back if it was loaded last time (it is never stored — only
  // the fact that it was wanted, so it can never age or conflict)
  if(store.get(SAMPLE_FLAG))loadSampleSchedule({open:false});
  maybeWelcomeTour();
  syncToggleA11y(); // state that is otherwise only a colour, stated in ARIA
