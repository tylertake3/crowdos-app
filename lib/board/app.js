...
import {
  ...
  parseAny, prepModel, mergeModels,
} from "../engine";
...
<div class="modal" id="addModal">
  <div class="box" style="max-width:480px">
    <div class="mhead"><h3>Add a production</h3><button class="x" id="amClose">Close</button></div>
    <div style="padding:16px;display:flex;flex-direction:column;gap:10px">
      <button class="tb-btn" id="amPdf" style="padding:13px 16px;text-align:left;line-height:1.5"><b>Upload a schedule PDF</b><br><span style="color:var(--sub);font-size:11.5px;font-weight:500">One-Liner or Expanded / Full Fat — days, scenes and crowd parsed automatically</span></button>
      <button class="tb-btn" id="amBlank" style="padding:13px 16px;text-align:left;line-height:1.5"><b>Start a blank production</b><br><span style="color:var(--sub);font-size:11.5px;font-weight:500">No schedule needed — add shoot days and crowd requirements by hand</span></button>
      <div id="amNameRow" class="hidden" style="display:flex;gap:10px;align-items:stretch">
        <div class="rfield" style="flex:1"><div class="inwrap"><input id="amName" type="text" placeholder="Production name…"></div></div>
        <button class="tb-btn" id="amCreate" style="border-color:var(--hv-line);color:var(--hv)">Create</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal" id="impModal">
  <div class="box" style="max-width:560px">
    ...
    <div style="padding:16px">
      <div class="rates-grid" style="padding:0;grid-template-columns:repeat(2,1fr)">
        <div class="rfield" style="grid-column:1/-1"><label>Production</label><div class="inwrap"><input id="impProd" type="text" placeholder="e.g. Victura"></div></div>
        ...
        <div class="rfield"><label>Version / colour</label><div class="inwrap"><input id="impVer" type="text" placeholder="Blue, B&amp;W…"></div></div>
        <div class="rfield" style="grid-column:1/-1"><label>Schedule date</label><div class="inwrap"><input id="impDate" type="text" placeholder="11 May 2026"></div></div>
      </div>
      <div style="display:flex;gap:10px;margin-top:16px;align-items:center">
...
function computeCosts(){
  ...
  COST=engineComputeStuntCosts(MODEL,ADJ,{...R,sdOn,sdRate,sdDays:sdDaysPerWk});
...
function stripHTML(d,s,idx){
  ...
  return `<div class="strip ${todClass(s)} ${sceneHasStunts(s)?'stunt-row':''}" data-stunt="${sceneHasStunts(s)?1:0}">
    ...
    <div class="ccol"><div class="codes">${(stunts.length||(s.extras||[]).length)?stunts.map(codeChip).join('')+(s.extras||[]).map(extraChip).join(''):'<span class="dash">—</span>'}</div></div>
    <div class="ccol"><div class="codes">${crowdChips||'<span class="dash">—</span>'}</div></div>
...
function addSource(model,title,short,activate=true,opts={}){
  ...
  SOURCES.push({model,title,short:short||title,colour,kind:opts.kind,text:opts.text,unit:opts.unit,ns:opts.ns,cloudId:opts.cloudId,createdAt:opts.createdAt,prod:opts.prod,version:opts.version,schedDate:opts.schedDate});
...
function saveUserSources(){
  store.set('crowdos-sources',JSON.stringify(SOURCES.filter(s=>s.kind).map(s=>({kind:s.kind,title:s.title,short:s.short,unit:s.unit||'Main',text:s.text||null,prod:s.prod||null,version:s.version||null,schedDate:s.schedDate||null}))));
}
...
function guessImportMeta(text,title){
  ...
  let prod='';
  for(const l of head){
    if(/^[A-Z][A-Z0-9 &']{2,26}$/.test(l)&&!/SCHEDULE|SHOOTING|FULL FAT|BLOCK|INTERIM|CONFIDENTIAL|SHOOT|^DAY\b|PAGE|UNIT/i.test(l)){prod=l;break}
...
async function handlePDF(file){
  try{
    ...
    $('#impProd').value=guess.prod;
    $('#impUnit').value=/2nd|second/i.test(title)?'2nd':'Main';
...
function openEditMeta(i){
  ...
  $('#impSub').textContent='Edit details';
  $('#impProd').value=s.prod||s.title;
...
$('#impGo').addEventListener('click',()=>{
  if(IMP_EDIT!=null){
    ...
    if(s){
      s.prod=($('#impProd').value||'').trim()||s.title;
      s.version=($('#impVer').value||'').trim();
      s.schedDate=($('#impDate').value||'').trim();
      const newUnit=$('#impUnit').value;
      if(s.kind==='pdf'&&s.text&&newUnit!==s.unit){
        s.unit=newUnit;
        s.model=prepModel(parseAny(s.text),newUnit);
  ...
  const P=PENDING_IMPORT;if(!P)return;
  const unit=$('#impUnit').value;
  const prod=($('#impProd').value||'').trim()||P.title;
  const version=($('#impVer').value||'').trim();
  const schedDate=($('#impDate').value||'').trim();
  const m=prepModel(parseAny(P.text),unit);
  m._raw=P.text.slice(0,1000)+' '+P.title;
  if(addSource(m,P.title,P.title.slice(0,18),true,{kind:'pdf',text:P.text,unit,ns:'p:'+P.title,prod,version,schedDate})){
...
// ---------- door 1: manual — start from nothing ----------
$('#btnAdd').addEventListener('click',()=>{$('#amNameRow').classList.add('hidden');$('#addModal').classList.add('open')});
$('#amClose').addEventListener('click',()=>$('#addModal').classList.remove('open'));
$('#addModal').addEventListener('click',e=>{if(e.target.id==='addModal')$('#addModal').classList.remove('open')});
$('#amPdf').addEventListener('click',()=>{$('#addModal').classList.remove('open');$('#fileInput').click()});
$('#amBlank').addEventListener('click',()=>{$('#amNameRow').classList.remove('hidden');$('#amName').focus()});
function createBlankProduction(){
  const name=($('#amName').value||'').trim()||'Untitled production';
  addSource({days:[],castMap:{},notes:[]},name,name.slice(0,16),true,{kind:'manual',ns:'m:'+name,allowEmpty:true,colour:'white'});
  saveUserSources();
  const src=SOURCES[SOURCES.length-1];
  src.createdAt=new Date().toISOString();
  if(CLOUD.session)cloud.insertProduction(src).then(({id,error})=>{
    if(error)setStatus('Cloud save failed: '+error.message);else src.cloudId=id;
  });
  $('#amName').value='';
  $('#addModal').classList.remove('open');
  setStatus('Blank production created — add your first shoot day on the day board.');
}
$('#amCreate').addEventListener('click',createBlankProduction);
$('#amName').addEventListener('keydown',e=>{if(e.key==='Enter')createBlankProduction()});
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('#addModal').classList.remove('open');$('#dayModal').classList.remove('open')}});
document.addEventListener('click',e=>{
...
// "+ Add shoot day" is a first-class, always-available action on the day board
{
  ...
  renderDays=function(){
    ...
    for(const d of (MODEL?MODEL.days:[])){
      ...
      if(!top)continue;
      // scene-less days have no crowd cell to click, so give manual days a
      // permanent way into the day calculator
      const pen=document.createElement('button');
      pen.className='dnum';pen.setAttribute('data-costday',d.id);
      pen.style.cssText='cursor:pointer;background:none;color:var(--hv)';
      pen.setAttribute('data-tip','Open the crowd day calculator');
      pen.textContent='✎ crowd day';
      top.appendChild(pen);
...
async function cloudHydrate(){
  ...
  // user sources and their edits, then rebuild from the cloud
  for(let i=SOURCES.length-1;i>=0;i--)if(SOURCES[i].kind)SOURCES.splice(i,1);
  ...
  for(const rec of res.productions){
    try{
      if(rec.kind==='pdf'&&rec.schedule_text){
        const m=prepModel(parseAny(rec.schedule_text),rec.unit||'Main');
        m._raw=rec.schedule_text.slice(0,1000)+' '+rec.title;
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.schedule_text,unit:rec.unit||'Main',ns:'p:'+rec.title,cloudId:rec.id,colour:rec.colour||undefined,createdAt:rec.created_at,prod:rec.production,version:rec.version,schedDate:rec.sched_date});
      }else{
...
function renderSidebar(){
  const mk=(s,i,label)=>`<button class="side-item ${!DASH&&i===ACTIVE?'on':''}" data-side="${i}" title="${esc(s.title)}"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(label||s.short)}</span><span class="k">${s.model.days.length}d</span>${s.kind?`<span class="del" data-delsrc="${i}" data-tip="Delete this production">✕</span>`:''}</button>`;
  $('#sideDash').classList.toggle('on',DASH);
  // group the user's schedules by production
  const groups=new Map();
  SOURCES.forEach((s,i)=>{
    if(!s.kind)return;
    const key=s.prod||s.title;
    if(!groups.has(key))groups.set(key,[]);
    groups.get(key).push({s,i});
  });
  let html='';
  for(const [name,items] of groups){
    html+=`<div class="side-label" style="color:var(--sub)">${esc(name)}</div>`;
    html+=items.map(({s,i})=>mk(s,i,s.kind==='manual'?'Manual':unitVersionLabel(s))).join('');
  }
...
function renderDash(){
  const user=SOURCES.map((s,i)=>({s,i})).filter(x=>x.s.kind);
  ...
  // portfolio totals across the user's productions
  const costs=new Map();
  let tDays=0,tCrowd=0,tStunt=0;
  for(const {s,i} of user){
    const c=costsFor(s);costs.set(i,c);
    tDays+=s.model.days.length;tCrowd+=c.crowd;tStunt+=c.stunt;
  }
  const card=({s,i})=>{const c=costs.get(i)||costsFor(s);return `
    <button class="dash-card" data-side="${i}">
      ${s.kind?`<span class="dash-edit" role="button" data-editsrc="${i}" data-tip="Edit name, unit, version, date">✎</span>`:''}
      <div style="display:flex;align-items:baseline;gap:8px;justify-content:space-between;${s.kind?'padding-right:16px':''}"><h3>${esc(s.kind?(s.prod||s.title):s.title)}</h3><span class="dash-kind">${s.kind==='pdf'?'PDF schedule':s.kind?'Manual':'Sample'}</span></div>
      ${s.kind==='pdf'?`<div style="color:var(--sub);font-size:11px;margin-top:2px">${esc(unitVersionLabel(s))}</div>`:''}
      <div class="dash-stats">
        <div><div class="n">${s.model.days.length}</div><div class="l">Shoot days</div></div>
        <div><div class="n costable">${gbp(Math.round(c.crowd))}</div><div class="l">Crowd</div></div>
        <div><div class="n costable">${gbp(Math.round(c.stunt))}</div><div class="l">Stunts</div></div>
      </div>
      <div class="dash-edited">${s.kind?('Last edited '+fmtStamp(s.lastEdited||s.createdAt)):'Built-in sample — explore the app with real schedule data'}</div>
    </button>`};
  // schedules grouped under their production, like the sidebar
  const groups=new Map();
  for(const u of user){
    const k=u.s.prod||u.s.title;
    if(!groups.has(k))groups.set(k,[]);
    groups.get(k).push(u);
  }
  let cards='';
  for(const [name,items] of groups){
    if(items.length>1)cards+=`<div class="dash-group">${esc(name)}</div>`;
    cards+=items.map(card).join('');
  }
  $('#dashView').innerHTML=`
    <div class="dash-hero">
      <div>
        <div class="dash-head">Your productions</div>
        <div class="dash-sub">${who?esc(who):''}</div>
      </div>
    </div>
    <div class="summary" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr))">
      <div class="stat"><div class="n">${user.length}</div><div class="l">Productions</div></div>
      <div class="stat"><div class="n">${tDays.toLocaleString()}</div><div class="l">Shoot days</div></div>
      <div class="stat money costable"><div class="n">${gbp(Math.round(tCrowd))}</div><div class="l">Crowd total</div></div>
      <div class="stat money costable"><div class="n">${gbp(Math.round(tStunt))}</div><div class="l">Stunt total</div></div>
    </div>
    <div class="dash-grid">
      ${cards}
      <button class="dash-card dash-new" id="dashNew">+ New production</button>
      ${demoFull>=0?card({s:SOURCES[demoFull],i:demoFull}):''}
    </div>`;
}
...
$('#sideNew').addEventListener('click',()=>{$('#amNameRow').classList.add('hidden');$('#addModal').classList.add('open')});
document.addEventListener('click',e=>{
  if(e.target.closest('#dashNew')){$('#amNameRow').classList.add('hidden');$('#addModal').classList.add('open');return}
...
document.addEventListener('click',e=>{
  ...
  if(CLOUD.session&&!s.cloudId&&!s.cloudFailed){setStatus('Still syncing this production — try deleting again in a moment.');return}
  if(!window.confirm('Delete “'+s.title+'” and all its days and edits? This cannot be undone.'))return;
...
// between cards — for productions negotiating their own rates.
{
  ...
  $('#rcSelect').addEventListener('change',()=>{
    ...
    applyVals(n&&RATECARDS[n]?RATECARDS[n]:RC_DEFAULTS);
  });
  ...
  $('#rcDelete').addEventListener('click',()=>{
    ...
    delete RATECARDS[n];
    store.set('crowdos-ratecards',JSON.stringify(RATECARDS));
    refreshSel();applyVals(RC_DEFAULTS);
  });
}
  ...
  for(const rec of saved){
    try{
      if(rec.kind==='pdf'&&rec.text){
        const m=prepModel(parseAny(rec.text),rec.unit||'Main');
        m._raw=rec.text.slice(0,1000)+' '+rec.title;
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.text,unit:rec.unit||'Main',ns:'p:'+rec.title,prod:rec.prod,version:rec.version,schedDate:rec.schedDate});
