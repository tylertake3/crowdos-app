...
<div class="modal" id="authModal">
  <div class="box" style="max-width:420px">
    <div class="mhead"><h3 id="auTitle">Sign in</h3><button class="x" id="auClose">Close</button></div>
    <div style="padding:16px">
      <div id="auForm">
        ...
        <div style="color:var(--faint);font-size:11px;margin-top:12px">Signing in stores your productions, shoot days and day-calculator edits privately in the cloud, synced across devices. Signed out, everything stays in this browser only.</div>
      </div>
      <div id="auSignedIn" class="hidden">
...
function addSource(model,title,short,activate=true,opts={}){
  ...
  SOURCES.push({model,title,short:short||title,colour,kind:opts.kind,text:opts.text,unit:opts.unit,ns:opts.ns,cloudId:opts.cloudId});
...
function updateAccountUI(){
  ...
  $('#auForm').classList.toggle('hidden',!!s);
  $('#auSignedIn').classList.toggle('hidden',!s);
  ...
  for(const rec of res.productions){
    try{
      if(rec.kind==='pdf'&&rec.schedule_text){
        ...
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.schedule_text,unit:rec.unit||'Main',ns:'p:'+rec.title,cloudId:rec.id,colour:rec.colour||undefined});
      }else{
        addSource({days:[],castMap:{},notes:[]},rec.title,rec.short,false,{kind:'manual',ns:'m:'+rec.title,allowEmpty:true,colour:rec.colour||'white',cloudId:rec.id});
      }
...
// user productions get a delete control on their source pill
{
  const origRenderSrcBar=renderSrcBar;
  renderSrcBar=function(){
    origRenderSrcBar();
    document.querySelectorAll('#srcBar button[data-src]').forEach(b=>{
      const i=+b.dataset.src;
      if(!SOURCES[i]||!SOURCES[i].kind)return;
      const x=document.createElement('span');
      x.dataset.delsrc=i;x.textContent='✕';
      x.style.cssText='margin-left:7px;opacity:.55;font-size:10px;cursor:pointer';
      x.setAttribute('data-tip','Delete this production');
      b.appendChild(x);
    });
  };
}
document.addEventListener('click',e=>{
  ...
  setActive(i<=ACTIVE?Math.max(0,Math.min(2,ACTIVE-1)):Math.min(ACTIVE,SOURCES.length-1));
},true);
