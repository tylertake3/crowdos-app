...
async function exportCbdXlsx(fileBase){
  ...
  sheet.rows.forEach((r,i)=>{
    ...
    if(r.kind==='title'){
      ...
      if(col1.sceneNum)row.getCell(col1.sceneNum).font={name:CBX_FONT,size:8,bold:true};
...
function stripHTML(d,s,idx){
  ...
  const crChip=(g,cls,tip)=>{
    ...
    const fromAbove=!!g.fromAbove||((g.count||0)>0&&(d.scenes||[]).slice(0,idx).some(prev=>
      scGroupsOf(prev).some(pg=>pg.tier===g.tier&&!!pg.featured===!!g.featured&&
        (pg.name||'').trim().toLowerCase()===(g.name||'').trim().toLowerCase()&&pg.count===g.count)));
