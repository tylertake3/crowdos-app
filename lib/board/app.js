...
function stripHTML(d,s,idx){
  ...
  const crowdChips=[
    ...
    featN?`<span class="code feat${APPMODE==='crowd'?' click':''}" ${APPMODE==='crowd'?cAttr:`data-tip="${esc(featTip)}" tabindex="0"`}>Feat ${featN}</span>`:'',
    spactN?`<span class="code spact${APPMODE==='crowd'?' click':''}" ${APPMODE==='crowd'?cAttr:`data-tip="${esc(spactTip)}" tabindex="0"`}>SPACT ${spactN}</span>`:'',
...
function addSource(model,title,short,activate=true,opts={}){
  ...
  SOURCES.push({model,title,short:short||title,colour,kind:opts.kind,text:opts.text,unit:opts.unit,ns:opts.ns,cloudId:opts.cloudId,createdAt:opts.createdAt});
...
function saveUserSources(){
  store.set('crowdos-sources',JSON.stringify(SOURCES.filter(s=>s.kind).map(s=>({kind:s.kind,title:s.title,short:s.short,unit:s.unit||'Main',text:s.text||null}))));
}
...
async function pdfToText(buf){
  ...
  for(let p=1;p<=doc.numPages;p++){
    ...
    for(const y of ys){
      const line=rows[y].sort((a,b)=>a.transform[4]-b.transform[4]).map(it=>it.str).join(' ').replace(/\s{2,}/g,' ').trim();
...
async function handlePDF(file){
  try{
    ...
    const is2U=/2nd|second/i.test(title);
    const short=is2U?'2nd Unit':/expanded|full\s*fat/i.test(title)?'Main Unit':/one.?lin/i.test(title)?'One-Liner':title.slice(0,16);
    const unit=is2U?'2nd':'Main';
    const m=prepModel(parseAny(text),unit);
    m._raw=text.slice(0,1000)+' '+title;
    if(addSource(m,title,short,true,{kind:'pdf',text,unit,ns:'p:'+title})){
  ...
  for(const rec of res.productions){
    try{
      if(rec.kind==='pdf'&&rec.schedule_text){
        ...
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.schedule_text,unit:rec.unit||'Main',ns:'p:'+rec.title,cloudId:rec.id,colour:rec.colour||undefined,createdAt:rec.created_at});
      }else{
        addSource({days:[],castMap:{},notes:[]},rec.title,rec.short,false,{kind:'manual',ns:'m:'+rec.title,allowEmpty:true,colour:rec.colour||'white',cloudId:rec.id,createdAt:rec.created_at});
      }
...
function renderSidebar(){
  const mk=(s,i)=>`<button class="side-item ${!DASH&&i===ACTIVE?'on':''}" data-side="${i}" title="${esc(s.title)}"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(s.short)}</span><span class="k">${s.model.days.length}d</span>${s.kind?`<span class="del" data-delsrc="${i}" data-tip="Delete this production">✕</span>`:''}</button>`;
  $('#sideDash').classList.toggle('on',DASH);
  $('#sideList').innerHTML=SOURCES.map((s,i)=>s.kind?mk(s,i):'').join('')||'<div style="color:var(--faint);font-size:11px;padding:2px 10px 6px">None yet — start one below.</div>';
...
function renderDash(){
  ...
  const card=({s,i})=>{const c=costsFor(s);return `
    <button class="dash-card" data-side="${i}">
      <div style="display:flex;align-items:baseline;gap:8px;justify-content:space-between"><h3>${esc(s.title)}</h3><span class="dash-kind">${s.kind==='pdf'?'PDF schedule':s.kind?'Manual':'Sample'}</span></div>
...
document.addEventListener('click',e=>{
  ...
  if(CLOUD.session&&!s.cloudId){setStatus('Still syncing this production — try deleting again in a moment.');return}
  ...
  for(const rec of saved){
    try{
      if(rec.kind==='pdf'&&rec.text){
        ...
        addSource(m,rec.title,rec.short,false,{kind:'pdf',text:rec.text,unit:rec.unit||'Main'});
      }else if(rec.kind==='manual'){
        addSource({days:[],castMap:{},notes:[]},rec.title,rec.short,false,{kind:'manual',ns:'m:'+rec.title,allowEmpty:true,colour:'white'});
      }
