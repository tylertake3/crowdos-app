...
import { projectCrowdDoc, cbToSheet, cbToStyledSheet } from "../engine/breakdown-doc";
...
async function exportCbdXlsx(fileBase){
  ...
  sheet.rows.forEach((r,i)=>{
    ...
    if(r.kind==='title'){
      ...
      [4,7].forEach(c=>row.getCell(c).alignment={horizontal:'center',vertical:'middle'});
    }else if(r.kind==='scene'){
      row.getCell(1).alignment={horizontal:'center',vertical:'top',wrapText:true};
      row.getCell(1).font={name:'Arial',size:8,bold:true};
      row.getCell(3).alignment={horizontal:'center',vertical:'top',wrapText:true};
      [4,7].forEach(c=>row.getCell(c).alignment={horizontal:'center',vertical:'top'});
      row.getCell(5).font={name:'Arial',size:8,bold:!r.pending,
        italic:!!r.fromAbove,color:{argb:r.pending?'FFA35D19':r.fromAbove?'FF5B3FA8':CBX_INK}};
      if(r.fromAbove)row.getCell(6).font={name:'Arial',size:8,italic:true,color:{argb:'FF5B3FA8'}};
      row.getCell(8).font={name:'Arial',size:8,bold:true};
    }
    // full-width bands and the title block read as one cell
    if(r.full)ws.mergeCells(i+1,1,i+1,sheet.columns.length);
  });
...
function seedCday(d){
  ...
  if(pd&&(pd.saChars||pd.feats||pd.spacts)){
    for(const [n,c] of Object.entries(pd.saChars||{}))chars.push({name:n,count:c,tier:'SA',scene:sc(n)});
    for(const [n,c] of Object.entries(pd.feats||{}))chars.push({name:n,count:c,tier:'Featured',scene:sc(n)});
    for(const [n,c] of Object.entries(pd.spacts||{}))chars.push({name:n,count:c,tier:'SPACT',scene:sc(n)});
  }else{
    ...
    for(const [n,c] of Object.entries(grp('saChars')))chars.push({name:n,count:c,tier:'SA',scene:sc(n)});
    for(const [n,c] of Object.entries(grp('featured')))chars.push({name:n,count:c,tier:'Featured',scene:sc(n)});
    for(const [n,c] of Object.entries(grp('spacts')))chars.push({name:n,count:c,tier:'SPACT',scene:sc(n)});
  }
...
function stripHTML(d,s,idx){
  ...
  const crChip=(g,cls,tip)=>{
    ...
    return `<span class="code ${cls} named click crgrp" draggable="true" data-crgrp="1" data-gtier="${g.tier}" data-gfeat="${g.featured?1:0}" data-gname="${esc(g.name||'')}" data-gcount="${g.count}" data-tip="${esc(tip)}" tabindex="0">${label}<button class="crmenu" data-crmenu="1" tabindex="-1" aria-label="Move, copy or delete this crowd group">⋯</button></span>`;
  };
  let crowdChips;
  if(APPMODE==='crowd'){
    const parts=[];
...
// ============================================================================
const CBD={date:'',sched:'',hideEmpty:false,other:true,weeks:true};
...
function cbdLineCell(l,side,sc,i,ph){
  if(side!=='crowd'){
    if(!l)return `<td class="cbno"></td><td class="cbname"></td>`;
    const c=['cbrow-'+String(l.tier||'SA').toLowerCase(),l.fromAbove?'fromabove':'',l.tbc?'tbc':''].filter(Boolean).join(' ');
    return `<td class="cbno ${c}">${esc(l.fromAbove?'':(l.no||''))}</td><td class="cbname ${c}">${esc(l.name)}</td>`;
  }
  const cls=l?['cbrow-'+String(l.tier||'SA').toLowerCase(),l.fromAbove?'fromabove':'',l.tbc?'tbc':''].filter(Boolean).join(' '):'';
  const tier=l?String(l.tier||'SA'):'SA';
  const label=(CBD_TIERS.find(t=>t[0]===tier)||CBD_TIERS[0])[1];
  const chip=l&&!l.fromAbove
    ...
    : '';
  return cbdEditCell(l,sc,i,'no',cls)
    +`<td class="cbname ${cls} cbnamecell">${chip}<span class="cbedit cbnametext${ph?' cbph':''}" contenteditable="true" spellcheck="false" data-cbaddr="${esc(cbdAddr(sc,i))}" data-cbf="name"${ph?` data-ph="${esc(ph)}"`:''} data-tip="Character or group name — leave blank for plain background">${esc(l?l.name:'')}</span>${l&&!l.fromAbove?`<button class="cbdel" data-cbdel="${esc(cbdAddr(sc,i))}" data-tip="Remove this line" aria-label="Remove line">✕</button>`:''}</td>`
    +cbdEditCell(l,sc,i,'note',cls);
}
function cbdSceneHTML(sc,cols){
  ...
  const desc=`<div class="cbslug"><b>${esc(sc.ie)}</b>${sc.ie?'&nbsp;&nbsp;&nbsp;':''}<b>${esc(sc.slug)}</b></div>`
  ...
  const day=`<div>${esc(dayLine)}</div>${sc.pages?`<div class="cbsub">${esc(sc.pages)}</div>`:''}`;
  let out='';
  for(let i=0;i<n;i++){
    const first=i===0;
    ...
    if(first){
      out+=`<td class="cbscnum" rowspan="${n}"><div class="cbnum">${esc(sc.num)}</div>${sc.loc?`<div class="cbloc">${esc(sc.loc)}</div>`:''}</td>`
        +`<td class="cbdescell" rowspan="${n}">${desc}</td>`
        +`<td class="cbdaycell" rowspan="${n}">${day}</td>`;
    }
    ...
    // rather than a dead label, so the fix is one click from the complaint.
    out+=cbdLineCell(c,'crowd',sc,i,emptyName);
    if(cols>6)out+=cbdLineCell(o,'other',sc,i);
    out+='</tr>';
  }
  return out;
}
function renderCbDoc(){
  ...
  for(const r of doc.rows){
    if(r.kind==='scene'){body+=cbdSceneHTML(r,cols);continue}
    ...
    body+=`<tr class="${cls}"><td colspan="3" class="cbtlabel">${esc(r.label)}</td><td class="cbno"><b>${r.no}</b></td>`
      +(cols>6?`<td></td><td class="cbtlabel">${esc(r.otherLabel)}</td><td class="cbno"><b>${r.otherNo}</b></td><td></td>`:`<td></td><td></td>`)
      +'</tr>';
  }
  host.innerHTML=bar+`<div class="cbdoc">
    ...
    <div class="tscroll"><table class="cbtable"><thead><tr>${doc.columns.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div>
  </div>`;
}
...
function renderDoods(){
  ...
  const tiers=crowdCharData();
  const days=MODEL.days.filter(d=>d.scenes.some(sceneHasCrowd));
  const groups=[];
  for(const tier of ['SA','Featured','SPACT'])for(const p of Object.values(tiers[tier]))groups.push({tier,...p});
  groups.sort((a,b)=>b.heads-a.heads||a.code.localeCompare(b.code));
  if(!groups.length){host.innerHTML='<div class="tablecard"><div class="note">No crowd in this schedule yet — add crowd to scenes on the day board and the day-out-of-days grid builds itself.</div></div>';return}
  const cell=(g,d)=>{const c=g.dayCounts.get(d.id)||0;return `<td class="dood-cell${c?' w':''}">${c||''}</td>`};
  const head=`<thead><tr><th class="dood-name">Character / group</th><th class="num">Days</th><th class="num">P-days</th>
  ...
  const body=groups.map(g=>{
    const key=g.tier+'|'+g.code;
    const open=DOOD_EXP.has(key);
    const expandable=g.max>1&&!g.code.includes('(unnamed)');
    let html=`<tr class="dood-row${open?' open':''}" ${expandable?`data-doodtoggle="${esc(key)}"`:''}>
      <td class="dood-name">${expandable?`<span class="dood-chev">${open?'▾':'▸'}</span>`:'<span class="dood-chev"></span>'}${esc(g.code)}${g.tier!=='SA'?` <span class="ctype">${g.tier}</span>`:''}</td>
...
document.addEventListener('click',e=>{
  ...
  if(sset){
    ...
    else if(DASH&&PROD_HOME)openProdSettings(PROD_HOME);
...
document.addEventListener('click',e=>{
  ...
  else if(c.dataset.crumb==='prod'){PROD_HOME=c.dataset.prod;showDash();}
});
...
function scGroupsOf(s){
  const out=[];
  if((s.sa||0)>0)out.push({tier:'SA',featured:false,name:'',count:s.sa,note:''});
  for(const g of s.saChars||[])if((g.count||0)>0)out.push({tier:'SA',featured:false,name:g.name||'',count:g.count,note:g.note||''});
  for(const g of s.featured||[])if((g.count||0)>0)out.push({tier:'SA',featured:true,name:g.name||'',count:g.count,note:g.note||''});
  for(const g of s.spacts||[])if((g.count||0)>0)out.push({tier:'SPACT',featured:false,name:g.name||'',count:g.count,note:g.note||''});
  return out;
}
// the editable char-rows for a scene, in the shape SCED / the inline editor use
function sceneCrowdRows(s){
  return scGroupsOf(s).map(g=>({name:g.name,count:g.count,tier:g.tier,featured:g.featured,note:g.note||''}));
}
...
document.addEventListener('click',e=>{
  ...
  if(op&&!e.target.closest('[data-prodedit]')){PROD_HOME=op.dataset.openprod;renderDash();return}
...
function applySced(model){
  for(const d of model.days)d.scenes.forEach((s,idx)=>{
    ...
    if(e.chars){
      ...
      const mk=c=>{const o={name:c.name||'',count:+c.count};if(c.note)o.note=c.note;return o};
...
const PS_SECTIONS=[['general','General'],['locations','Locations'],['shootlocs','Shooting locations'],['info','Production info'],['cast','Cast list'],['rates','Rate cards'],['columns','Columns'],['history','History']];
