...
function renderSidebar(){
  ...
  // group the user's schedules under their production entity — and list
  // productions that have no schedules yet too
  const byProd=new Map();
  for(const name of prodNames())byProd.set(name,[]);
  SOURCES.forEach((s,i)=>{ if(s.kind){const key=s.prod||s.title;if(!byProd.has(key))byProd.set(key,[]);byProd.get(key).push({s,i});} });
  let html='';
  for(const [name,items] of byProd){
    ...
    html+=items.map(({s,i})=>mk(s,i,s.kind==='manual'?'Manual entry':unitVersionLabel(s))).join('')
      ||`<div style="color:var(--faint);font-size:10.5px;padding:1px 10px 4px">No schedules yet</div>`;
  }
...
function renderDash(){
  ...
  // group schedules under their production entity; list empty productions too
  const groups=new Map();
  for(const name of prodNames())groups.set(name,[]);
  for(const u of user){
  ...
  for(const [name,items] of groups){
    cards+=`<div class="dash-group">${esc(name)}<span class="dash-group-tools"><span data-prodimport="${esc(name)}" data-tip="Import a schedule">＋ Schedule</span><span data-prodedit="${esc(name)}" data-tip="Production settings">✎ Settings</span></span></div>`;
    cards+=items.length?items.map(card).join('')
      :`<button class="dash-card dash-new" data-prodimport="${esc(name)}">Import or add days →</button>`;
  }
  $('#dashView').innerHTML=`
    <div class="dash-hero">
      <div>
    ...
    <div class="summary" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr))">
      <div class="stat"><div class="n">${groups.size}</div><div class="l">Productions</div></div>
...
document.addEventListener('click',e=>{
  ...
  if(pi){e.stopPropagation();e.preventDefault();CURPROD=pi.dataset.prodimport;$('#fileInput').click();return}
