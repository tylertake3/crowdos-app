...
function seedCday(d){
  ...
  return {shift:night?'Night':'Day',fw:(d.type||'').toUpperCase().includes('CWD')?'cwd':'std',ph:false,
    call,wrap,travel:bandFor(d.loc,{bands:activeBands(),unknownBand:activeUnknownBand()}).band,chars,
...
/** Does this day's schedule say it is a night? Same test the day calculator seeds from. */
function dayIsNight(d){
  const m=/(\d{1,2})[:.]?(\d{2})\s*[-–]\s*(\d{1,2})[:.]?(\d{2})/.exec((d&&d.hours)||'');
  const crossesMidnight=!!m&&(+m[3]*60+ +m[4])<(+m[1]*60+ +m[2]);
  return /CWN|SWN|night/i.test((d&&d.type)||'')||crossesMidnight;
}
/**
...
function renderProvBar(){
  ...
  const nights=APPMODE==='crowd'?nightDaysPricedAsDay():[];
  ...
  const warn=nights.length
    ? `<div class="prov-warn">
        ...
        <div class="prov-warn-a"><button class="tb-btn" id="provFixNights">Price ${nights.length===1?'this night':'these '+nights.length+' nights'} off their hours</button><button class="tb-btn" id="provShowNights" aria-expanded="${PROV_NIGHTS_OPEN?'true':'false'}">${PROV_NIGHTS_OPEN?'Hide the list':'Show me which days'}</button></div>
...
document.addEventListener('click',e=>{
  ...
  if(t){
    ...
    // roving tabindex + aria-selected: only the active tab is a tab stop, so
    // Tab moves past the whole strip and Left/Right walks it (see below)
