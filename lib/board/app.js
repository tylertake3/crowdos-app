...
function daySplitSeed(d){
  ...
  for(const s of realScenes(d)){
    for(const g of s.saChars||[])if(g&&g.name)bump(bySA,'n|'+g.name.toLowerCase(),{name:g.name,count:+g.count||0,tier:'SA',featured:false});
    for(const g of s.featured||[])if(g)bump(bySA,'f|'+(g.name||'').toLowerCase(),{name:g.name||'',count:+g.count||0,tier:'SA',featured:true});
    for(const g of s.spacts||[])bump(bySP,(g.name||'').toLowerCase(),{name:g.name||'',count:+g.count||0,tier:'SPACT',featured:false});
  }
...
function renderSplitModal(){
  ...
  $('#splitBody').innerHTML=`
    <div class="splitpool">
      <span class="splitpool-n">${pool}</span>
      <span class="splitpool-l">supporting artist${pool===1?'':'s'} on this day<small>Give them characters below — anyone left over stays as unnamed background.</small></span>
    </div>
    <div class="reqchars splitchars">${SPLIT_ROWS.length?SPLIT_ROWS.map(rowHTML).join(''):'<div class="splitempty">No characters yet — add one below.</div>'}</div>
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('#costModal').classList.remove('open');$('#raOverlay').classList.remove('open');$('#calModal').classList.remove('open');$('#splitModal').classList.remove('open');if($('#cdayModal').classList.contains('open')){$('#cdayModal').classList.remove('open');cdRecalcApp()}}});
