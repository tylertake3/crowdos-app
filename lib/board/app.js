...
async function exportCbdXlsx(fileBase){
  ...
  sheet.rows.forEach((r,i)=>{
    ...
    r.cells.forEach((v,c)=>{if(v!==null&&v!==undefined)row.getCell(c+1).value=v});
    const band=CBX_FILL[r.kind];
    const dark=r.kind==='day'||r.kind==='unit';
    row.eachCell({includeEmpty:true},cell=>{
      cell.border=cbxBorder();
      cell.alignment={vertical:'top',wrapText:true};
      cell.font={name:'Arial',size:8,color:{argb:dark?CBX_PAPER:CBX_INK}};
    ...
    if(r.kind==='title'){
      ...
      row.getCell(1).font={name:'Arial',size:8,bold:true,color:{argb:dark?CBX_PAPER:CBX_INK}};
      ...
      row.eachCell({includeEmpty:true},cell=>{cell.font={name:'Arial',size:8,bold:true}});
      ...
      if(col1.sceneNum)row.getCell(col1.sceneNum).font={name:'Arial',size:8,bold:true};
      ...
      if(crowdNameCol)row.getCell(crowdNameCol).font={name:'Arial',size:8,bold:!r.pending,
        italic:!!r.fromAbove,color:{argb:r.pending?'FFA35D19':r.fromAbove?'FF5B3FA8':CBX_INK}};
      if(r.fromAbove&&col1.crowdNotes)row.getCell(col1.crowdNotes).font={name:'Arial',size:8,italic:true,color:{argb:'FF5B3FA8'}};
      if(col1.otherName)row.getCell(col1.otherName).font={name:'Arial',size:8,bold:true};
    }
...
function exportCbdPDF(){
  ...
  node.querySelectorAll('.cbtier,.cbdel,.noprint').forEach(n=>n.remove());
  ...
  doc.write(`<!doctype html><html><head><meta charset="utf-8">
    ...
    </style></head><body class="light"></body></html>`);
  doc.close();
...
function stripHTML(d,s,idx){
  ...
  const crChip=(g,cls,tip)=>{
    ...
    return `<span class="code ${cls}${needsName?' needsname':''} named click crgrp" draggable="true" data-crgrp="1" data-gtier="${g.tier}" data-gfeat="${g.featured?1:0}" data-gname="${esc(g.name||'')}" data-gcount="${g.count}" data-tip="${esc(tip)}" tabindex="0">${label}<button class="crmenu" data-crmenu="1" tabindex="-1" aria-label="Move, copy or delete this crowd group">⋯</button></span>`;
  };
  let crowdChips;
  if(APPMODE==='crowd'){
    const parts=[];
    if(s.sa>0)parts.push(crChip({tier:'SA',featured:false,name:'',count:s.sa},'cr anon',`Unnamed SA ×${s.sa} — drag or use ⋯ to move / copy / delete; click to edit`));
    for(const g of s.saChars||[])if((g.count||0)>0)parts.push(crChip({tier:'SA',featured:false,name:g.name||'',count:g.count},'cr',saCharTip));
    for(const g of s.featured||[])if((g.count||0)>0)parts.push(crChip({tier:'SA',featured:true,name:g.name||'',count:g.count},'feat',featTip));
    for(const g of s.spacts||[])if((g.count||0)>0)parts.push(crChip({tier:'SPACT',featured:false,name:g.name||'',count:g.count},'spact',spactTip));
...
function dayLocHTML(d){
  ...
  return `<span class="dlocs">`+blocks.map(b=>{
    const scene=b.loc||'', real=b.real||'';
    const differs=real&&normLoc(real)!==normLoc(scene);
    const mapTarget=real||scene;
    const linkTip=differs?('Scene location — actually shot at “'+esc(real)+'”'):(real?'Location confirmed':'Open in Google Maps');
    return `<span class="dlocblk">`
      +`<a class="dloc loclink" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapTarget)}" target="_blank" rel="noopener" data-tip="${linkTip}">${esc(scene||'—')}</a>`
      +(differs?`<span class="dloc-real" data-tip="Actually shot here — the schedule’s scene location is “${esc(scene)}”">→ ${esc(real)}</span>`:'')
      +`<button class="dloc-edit${real?' on':''}" data-locedit="${esc(d.id)}" data-locblk="${esc(scene)}" data-tip="${real?('Real shooting location set: “'+esc(real)+'”. Click to change.'):'Set where this is actually shot (the schedule only names the scene’s location)'}">${icon('pin')}</button>`
      +`</span>`;
  }).join('')+`</span>`;
}
function renderDays(){
    ...
    return `<div class="daycard ${work?'has-stunts':''}${dayIsToday(d)?' today':''}" id="day-${d.id}" data-stunt="${work?1:0}">
      <div class="dayhead">
        <div class="dh-top">
          ...
          ${APPMODE==='crowd'&&(cd&&cd.sa||peak)?`<button class="dpill sa click" data-costday="${esc(d.id)}" data-tip="${esc(crowdCharTip(d))}">SA ${cd&&cd.sa?cd.sa:peak}</button>`:''}
...
const CBD={date:'',sched:'',hideEmpty:false,other:true,weeks:true,costs:false,notes:true,mergeCrowd:false,order:CB_SEG_ORDER.slice()};
...
function cbdEditCell(l,sc,i,field,cls){
  // a carried line is a pointer to other people — editing it here would
  // silently fork the group, so it stays read-only
  if(l&&l.fromAbove)return `<td class="cbnote ${cls}">${esc(field==='note'?(l.notes||''):l.name)}</td>`;
  ...
  return `<td class="${base} ${cls} cbedit" contenteditable="true" spellcheck="false"
    data-cbaddr="${esc(cbdAddr(sc,i))}" data-cbf="${field}"
    data-tip="${field==='no'?'Number of people — set to 0 to remove the line':field==='name'?'Character or group name':'Notes / continuity'}">${esc(v)}</td>`;
}
// One editable NO. cell for a crowd line.
function cbCrowdNoCell(c,sc,i,cls){return cbdEditCell(c,sc,i,'no',cls)}
// The CROWD CHARACTER cell — tier chip, editable name (with placeholder when
// blank) and the delete affordance.
function cbCrowdNameCell(c,sc,i,cls,ph){
  const tier=c?String(c.tier||'SA'):'SA';
  const label=(CBD_TIERS.find(t=>t[0]===tier)||CBD_TIERS[0])[1];
  const chip=c&&!c.fromAbove
    ? `<button class="cbtier t-${tier.toLowerCase()}" data-cbtier="${esc(cbdAddr(sc,i))}" data-tip="Click to change tier — SA, Featured or Spact">${esc(label)}</button>`
    : '';
  return `<td class="cbname ${cls} cbnamecell">${chip}<span class="cbedit cbnametext${ph?' cbph':''}" contenteditable="true" spellcheck="false" data-cbaddr="${esc(cbdAddr(sc,i))}" data-cbf="name"${ph?` data-ph="${esc(ph)}"`:''} data-tip="Character or group name — leave blank for plain background">${esc(c?c.name:'')}</span>${c&&!c.fromAbove?`<button class="cbdel" data-cbdel="${esc(cbdAddr(sc,i))}" data-tip="Remove this line" aria-label="Remove line">✕</button>`:''}</td>`;
}
...
function cbCrowdComboCell(c,sc,i,cls,ph){
  const addr=esc(cbdAddr(sc,i));
  // a carried line is a pointer to other people — read-only, name only, no count
  if(c&&c.fromAbove)return `<td class="cbname ${cls} cbcombo">${esc(c.name)}</td>`;
  const tier=c?String(c.tier||'SA'):'SA';
  const label=(CBD_TIERS.find(t=>t[0]===tier)||CBD_TIERS[0])[1];
  const chip=c&&!c.fromAbove
  ...
  return `<td class="cbname ${cls} cbnamecell cbcombo">${chip}<span class="cbedit cbcombono" contenteditable="true" spellcheck="false" data-cbaddr="${addr}" data-cbf="no" data-tip="Number of people — set to 0 to remove the line">${esc(no)}</span><span class="cbedit cbnametext${ph?' cbph':''}" contenteditable="true" spellcheck="false" data-cbaddr="${addr}" data-cbf="name"${ph?` data-ph="${esc(ph)}"`:''} data-tip="Character or group name — leave blank for plain background">${esc(c?c.name:'')}</span>${c&&!c.fromAbove?`<button class="cbdel" data-cbdel="${addr}" data-tip="Remove this line" aria-label="Remove line">✕</button>`:''}</td>`;
}
...
function cbOtherNoCell(o){
  ...
  return `<td class="cbno ${c}">${esc(o.fromAbove?'':(o.no||''))}</td>`;
}
...
function cbdSceneHTML(sc,layout){
  ...
  for(let i=0;i<n;i++){
    ...
    const ccls=c?['cbrow-'+String(c.tier||'SA').toLowerCase(),c.fromAbove?'fromabove':'',c.tbc?'tbc':''].filter(Boolean).join(' '):'';
    out+=`<tr class="cbscene ${first?'cbfirst':''}" data-cbscene="${esc(sc.dayId)}|${esc(sc.sceneNum)}">`;
    for(const def of layout){
      switch(def.role){
        // Block cells print once, on the first line, and span the block.
        case 'sceneNum': if(first)out+=`<td class="cbscnum" rowspan="${n}">${sceneHtml}</td>`; break;
        case 'desc': if(first)out+=`<td class="cbdescell" rowspan="${n}">${descHtml}</td>`; break;
        case 'day': if(first)out+=`<td class="cbdaycell" rowspan="${n}">${dayHtml}</td>`; break;
        ...
        case 'crowdNo': out+=cbCrowdNoCell(c,sc,i,ccls); break;
        case 'crowdName': out+=cbCrowdNameCell(c,sc,i,ccls,emptyName); break;
        case 'crowdCombo': out+=cbCrowdComboCell(c,sc,i,ccls,emptyName); break;
        case 'crowdNotes': out+=cbCrowdNoteCell(c,sc,i,ccls); break;
        case 'otherNo': out+=cbOtherNoCell(o); break;
        case 'otherName': out+=cbOtherNameCell(o); break;
        case 'fees': out+=cbFeeCell(c,sc,i); break;
        case 'cost': out+=cbCostCell(c); break;
      }
    }
    out+='</tr>';
  }
...
// The Columns builder: reorder the six column blocks and show/hide the notes
// column, all in one place so an AD never has to drag a merged cell by hand.
// Order changes flow straight through to the screen, the PDF and the exports.
let cbColPanelOpen=false;
function cbColBuilderHTML(){
  ...
  const rows=order.map((seg,i)=>{
    ...
    return `<li class="cbcolrow${hidden?' off':''}" data-seg="${seg}">
      <span class="cbcolname">${esc(CB_SEG_LABELS[seg]||seg)}${hidden?' <em class="cbcolhint">hidden</em>':''}</span>
      <span class="cbcolbtns">
        <button class="cbcolmv" data-cbmv="${seg}" data-dir="up" ${i===0?'disabled':''} aria-label="Move up" data-tip="Move up">↑</button>
        <button class="cbcolmv" data-cbmv="${seg}" data-dir="down" ${i===order.length-1?'disabled':''} aria-label="Move down" data-tip="Move down">↓</button>
        ${optional?`<label class="cbcolvis" data-tip="Show or hide this column"><input type="checkbox" data-cbvis="${seg}" ${hidden?'':'checked'}></label>`:''}
      </span>
    </li>`;
  }).join('');
  return `<div class="cbcolwrap">
    <button class="tb-btn" id="cbdCols" aria-expanded="${cbColPanelOpen}" data-tip="Reorder columns and show or hide the notes column — applies to the screen, the PDF and the exports">Columns ▾</button>
    <div class="cbcolpanel" id="cbdColPanel" ${cbColPanelOpen?'':'hidden'}>
      <div class="cbcolhd">Column order</div>
      <ul class="cbcollist">${rows}</ul>
      <label class="chk cbcolnotes"><input type="checkbox" id="cbdNotes" ${CBD.notes!==false?'checked':''}> Notes / continuity column</label>
      <label class="chk cbcolnotes" data-tip="Show the count in front of the name in one column, e.g. &quot;38 SA&quot; — totals still show the number"><input type="checkbox" id="cbdMerge" ${CBD.mergeCrowd?'checked':''}> Combine No. + name</label>
      <div class="cbcolfoot">
        <span class="cbcoltip">Number and name always stay together. Fees &amp; cost move as a pair.</span>
        <button class="tb-btn" id="cbdColsReset" data-tip="Put the columns back to the standard reference layout">Standard layout</button>
      </div>
    </div>
  </div>`;
}
function renderCbDoc(){
  ...
  if(!MODEL||!MODEL.days.length){host.innerHTML='<div class="tablecard"><div class="note">Load a schedule and this breakdown builds itself.</div></div>';return}
  const doc=cbdDoc(),cols=doc.columns.length;
  const t=doc.totals;
  const bar=`<div class="cbdbar noprint">
    <label class="cbdf">Breakdown date <input id="cbdDate" type="text" value="${esc(CBD.date)}" placeholder="20/6/26" size="9"></label>
    <label class="cbdf">Based on schedule <input id="cbdSched" type="text" value="${esc(CBD.sched)}" placeholder="18/6/26" size="9"></label>
    ...
    ${cbColBuilderHTML()}
    <span class="grow"></span>
    ${doc.costs?`<span class="cbdstat money" data-tip="Supplementary fees · whole-breakdown cost, from the crowd cost engine">${gbp(Math.round(t.fees))} fees · ${gbp(Math.round(t.cost))} total</span>`:''}
    ...
    <button class="tb-btn" id="cbdReset" hidden data-tip="Restore the original schedule order">Reset order</button>
  </div>`;
  ...
  for(const r of doc.rows){
    ...
    if(leadCount>0)row+=`<td colspan="${leadCount}" class="cbtlabel">${esc(r.label)}</td>`;
    for(let idx=leadCount;idx<layout.length;idx++){
      const def=layout[idx];
      if(def.role==='crowdNo'||def.role==='crowdCombo')row+=`<td class="cbno"><b>${r.no}</b></td>`;
      else if(def.role==='otherNo')row+=`<td class="cbno"><b>${r.otherNo}</b></td>`;
      else if(def.role==='fees')row+=`<td class="cbfee">${r.fees?gbp(Math.round(r.fees)):''}</td>`;
      else if(def.role==='cost')row+=`<td class="cbcost"><b>${r.cost?gbp(Math.round(r.cost)):'—'}</b></td>`;
      else if(idx===otherLabelIdx&&otherIdx>=0&&r.otherLabel&&!def.count&&!def.money)row+=`<td class="cbtlabel">${esc(r.otherLabel)}</td>`;
      else row+=`<td></td>`;
    }
    body+=row+'</tr>';
  }
  host.innerHTML=bar+`<div class="cbdoc">
    <div class="cbhead"><div class="cbtitle">${esc(doc.title)}</div><div class="cbsubtitle">${esc(doc.subtitle)}</div></div>
    <div class="tscroll"><table class="cbtable${doc.costs?' cbmoney':''}${hasOther?' cbother':''}">${cbColgroup(layout)}<thead><tr>${doc.columns.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div>
  </div>`;
}
...
function cbdCommit(addr,field,raw){
  ...
  if(line>=rows.length){
    ...
    if(field==='no'){
      const n=Math.max(0,Math.round(+val.replace(/[^0-9.-]/g,'')||0));
      if(!n)return false;
    ...
    const r=rows[line];
    if(field==='no')r.count=Math.max(0,Math.round(+val.replace(/[^0-9.-]/g,'')||0));
...
document.addEventListener('change',e=>{
  ...
  if(id==='cbdNotes'){CBD.notes=e.target.checked;cbColPanelOpen=true;saveCbd();renderCbDoc();return}
  if(id==='cbdMerge'){CBD.mergeCrowd=e.target.checked;cbColPanelOpen=true;saveCbd();renderCbDoc();return}
  // show/hide toggles inside the Columns builder mirror the toolbar toggles
  if(e.target.matches('[data-cbvis]')){
    ...
    cbColPanelOpen=true;saveCbd();renderCbDoc();return;
  }
...
document.addEventListener('click',e=>{
  ...
  if(fh){
    ...
    if(hn){
      ...
      setStatus(`D${d.num}: named ${hn.count} SA as “${hn.name}” across ${n} scene${n===1?'':'s'} — the day now counts ${hn.count}, not ${hn.counted}. Change any scene back in its own crowd editor if they really are different people.`);
    }
    return;
  }
  ...
  if(t){
    ...
    CUR_VIEW=t.dataset.view;syncExpBar();
    return;
  }
...
function scGroupsOf(s){
  const out=[];
  if((s.sa||0)>0)out.push({tier:'SA',featured:false,name:'',count:s.sa,note:'',sup:0});
  for(const g of s.saChars||[])if((g.count||0)>0)out.push({tier:'SA',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0});
  for(const g of s.featured||[])if((g.count||0)>0)out.push({tier:'SA',featured:true,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0});
  for(const g of s.spacts||[])if((g.count||0)>0)out.push({tier:'SPACT',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0});
  return out;
}
// the editable char-rows for a scene, in the shape SCED / the inline editor use
function sceneCrowdRows(s){
  return scGroupsOf(s).map(g=>({name:g.name,count:g.count,tier:g.tier,featured:g.featured,note:g.note||'',sup:+g.sup||0}));
}
...
function writeSceneCrowd(d,idx,rows){
  ...
  if(!e.chars.length&&!(+e.perf||+e.coord||+e.dbl))delete SCED[key];
  else SCED[key]=e;
}
...
function mergeSameInto(rows,g){
  ...
  else rows.push({name:g.name,count:g.count,tier:g.tier,featured:g.featured});
}
...
function crowdGroupFromEl(el){
  ...
  return {d,idx,g:{tier:chip.dataset.gtier==='SPACT'?'SPACT':'SA',featured:chip.dataset.gfeat==='1',
    name:chip.dataset.gname||'',count:+chip.dataset.gcount||0}};
}
  ...
  if(targets.length){
    items.push({label:'Move to scene…',icon:'arrow-right',onClick:()=>{
      openCtxMenu(x,y,targets.map(t=>({label:t.label,onClick:()=>{
        moveCrowdGroup(d,idx,g,t.i);setStatus(crowdGroupLabel(g)+' moved to '+t.label.toLowerCase()+'.');
      }})));
    }});
    items.push({label:'Copy to scene…',icon:'copy',onClick:()=>{
      openCtxMenu(x,y,targets.map(t=>({label:t.label,onClick:async()=>{
        ...
        setStatus(crowdGroupLabel(g)+(mode==='new'?' added as new people to ':' copied (same people) to ')+t.label.toLowerCase()+'.');
      }})));
    }});
    items.push({sep:true});
  }
  items.push({label:'Delete from this scene',icon:'trash',danger:true,onClick:()=>{
    deleteCrowdGroup(d,idx,g);setStatus(crowdGroupLabel(g)+' removed from this scene.');
  }});
...
function applySced(model){
  for(const d of model.days)d.scenes.forEach((s,idx)=>{
    ...
    if(e.chars){
      ...
      const mk=c=>{const o={name:c.name||'',count:+c.count};if(c.note)o.note=c.note;if(+c.sup>0)o.sup=+c.sup;return o};
...
document.addEventListener('drop',e=>{
  ...
  if(!t)return;
  e.preventDefault();
  const ctx=DRAG_CROWD;DRAG_CROWD=null;clearDropHints();
  const label='scene '+(ctx.d.scenes[t.idx].num||(t.idx+1));
  if(moveCrowdGroup(ctx.d,ctx.idx,ctx.g,t.idx))setStatus(crowdGroupLabel(ctx.g)+' moved to '+label+'.');
});
document.addEventListener('contextmenu',e=>{
  ...
  if(prodEl){
    ...
    openCtxMenu(e.clientX,e.clientY,[
      {label:'Production settings',icon:'pencil',onClick:()=>openProdSettings(name)},
...
document.addEventListener('click',e=>{
  ...
  // ---- Columns builder ----
  if(e.target.closest('#cbdCols')){
    cbColPanelOpen=!cbColPanelOpen;
    const p=$('#cbdColPanel'),b=$('#cbdCols');
    if(p)p.hidden=!cbColPanelOpen;
    if(b)b.setAttribute('aria-expanded',String(cbColPanelOpen));
    return;
  }
  const mv=e.target.closest('.cbcolmv');
  if(mv){
    const seg=mv.dataset.cbmv,dir=mv.dataset.dir==='up'?-1:1;
    const order=(Array.isArray(CBD.order)&&CBD.order.length?CBD.order.slice():CB_SEG_ORDER.slice())
      .filter(s=>CB_SEG_ORDER.includes(s));
    for(const s of CB_SEG_ORDER)if(!order.includes(s))order.push(s);
    const i=order.indexOf(seg),j=i+dir;
    if(i>=0&&j>=0&&j<order.length){[order[i],order[j]]=[order[j],order[i]];CBD.order=order;cbColPanelOpen=true;saveCbd();renderCbDoc();}
    return;
  }
  if(e.target.closest('#cbdColsReset')){
    CBD.order=CB_SEG_ORDER.slice();CBD.notes=true;CBD.mergeCrowd=false;cbColPanelOpen=true;saveCbd();renderCbDoc();return;
  }
  // click outside an open Columns panel closes it
  if(cbColPanelOpen&&!e.target.closest('.cbcolwrap')){
    cbColPanelOpen=false;const p=$('#cbdColPanel'),b=$('#cbdCols');
