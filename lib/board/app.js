...
async function getSourceFiles(s){
  ...
  if(man&&man.length&&man[0]&&man[0].path&&CLOUD.session&&cloud.downloadScheduleFile){
    ...
    if(files.length){try{await idbPut(key,files.map(f=>({name:f.name,type:f.type,blob:f.blob})));FILEIDX.add(key);saveFileIdx();}catch(e){}}
...
function openReview({prev,nextModel,mergeStats,onAccept,ctx}){
  ...
  function renderTable(){
    ...
    $('#rpTable').innerHTML=`<div class="rp-sec-title">Day by day<span class="cnt">${nextModel.days.length} days · ${follow?'click a row to find it in the original · ':''}click a date or location to edit · ✕ removes a day</span></div>
