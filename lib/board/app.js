...
function cbAboveToggle(c,addr){
  // A whole-scene pointer is the only line with nothing to tick — it already
  // says what it points at. The anonymous bucket DOES get the tickbox: its flag
  // lives on the scene rather than on a row, but it is the same promise, and
  // the day board has offered it since recurring groups landed.
