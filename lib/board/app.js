...
function renderBriefs(){
  ...
  const rows=list.map(x=>{
    ...
    return `<tr class="briefrow" data-openbrief="${esc(x.id)}">
      ...
      <td class="rowlabel">${esc(x.b.character)}${c?'':` <span class="briefwarn" data-tip="No scenes carry this character name any more — check the schedule">${icon('warn')}</span>`}</td>
      ...
      <td class="datescol">${c?`<div class="daylist">${c.days.slice(0,8).map(x2=>`<button class="dchip" data-calpop="${esc(x2.d.id)}">${esc(chipDate(x2.d))}</button>`).join('')}${c.days.length>8?`<span class="dchip more">+${c.days.length-8}</span>`:''}</div>`:'—'}</td>
      ...
      <div class="tablecard"><h3>Casting requirements</h3>
        <div class="briefform">
          ...
          <label class="bf-block">Measurements &amp; sizing <span class="cdinfo">(goes to the agency with the brief)</span>
            <textarea data-brieffld="measurements" data-bid="${esc(id)}" placeholder="e.g. Height 5'8”–6'1”, chest 38–42, waist 30–34, collar 15–16, shoe 8–11 — costumes already made / to be fitted…">${esc(b.measurements||'')}</textarea></label>
...
function briefText(id){
  ...
  const lines=[
    ...
    b.measurements?`Measurements / sizing: ${b.measurements}`:'',
