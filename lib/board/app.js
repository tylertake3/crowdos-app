...
function exportViewPDF(view){
  ...
  const clean=()=>{host.classList.remove('print-target');document.body.classList.remove('printing-board');window.removeEventListener('afterprint',clean)};
...
document.addEventListener('click',e=>{
  ...
  if(t){
    ...
    ['days','cal','stunts','crowd','briefs','doods','calc','cast'].forEach(v=>$('#view'+v[0].toUpperCase()+v.slice(1)).classList.toggle('hidden',v!==t.dataset.view));
...
function setAppMode(m){
  ...
  if(m!=='crowd'&&(!$('#viewBriefs').classList.contains('hidden')||!$('#viewDoods').classList.contains('hidden')))document.querySelector('.tabs button[data-view="days"]').click();
  /* Calculator tab lives in both modes — content branches on APPMODE */
  if(MODEL){computeCosts();computeCrowdCosts();renderSummary();renderDays();renderStunts();renderCrowd();if(!$('#viewDoods').classList.contains('hidden'))renderDoods();renderCalendar();renderCast();renderFreeCalc();}
...
function setActive(i){
  ...
  computeCosts();computeCrowdCosts();renderSummary();renderDays();renderStunts();renderCrowd();if(!$('#viewDoods').classList.contains('hidden'))renderDoods();renderCalendar();renderCast();renderFreeCalc();
...
function scGroupsOf(s){
  const out=[];
  if((s.sa||0)>0)out.push({tier:'SA',featured:false,name:'',count:s.sa});
  for(const g of s.saChars||[])if((g.count||0)>0)out.push({tier:'SA',featured:false,name:g.name||'',count:g.count});
  for(const g of s.featured||[])if((g.count||0)>0)out.push({tier:'SA',featured:true,name:g.name||'',count:g.count});
  for(const g of s.spacts||[])if((g.count||0)>0)out.push({tier:'SPACT',featured:false,name:g.name||'',count:g.count});
  return out;
}
// the editable char-rows for a scene, in the shape SCED / the inline editor use
function sceneCrowdRows(s){
  return scGroupsOf(s).map(g=>({name:g.name,count:g.count,tier:g.tier,featured:g.featured}));
}
...
function writeSceneCrowd(d,idx,rows){
  ...
  e.chars=rows.filter(r=>(+r.count||0)>0)
    .map(r=>({name:(r.name||'').trim(),count:+r.count,tier:r.tier==='SPACT'?'SPACT':'SA',featured:!!r.featured}));
