...
function renderBriefs(){
  ...
  host.innerHTML=`
  <div class="tablecard"><h3>${solo?'Scratchpad briefs':'Casting briefs'}<span class="cnt">${list.length}${unbriefed.length?' of '+(list.length+unbriefed.length)+' characters':''}</span>
...
function renderDash(){
  ...
  $('#dashView').innerHTML=`
    ...
    <div class="dash-head dash-calchead" data-briefstoggle style="margin:30px 0 4px;cursor:pointer;user-select:none">Casting briefs <span class="chev"${store.get('crowdos-dashbriefs-open')==='1'?' style="transform:rotate(90deg)"':''}>▶</span><span class="dash-sub" style="display:block;font-weight:400">Write and email a brief without opening a production${briefsForDash()?' · '+briefsForDash()+' saved here':''}</span></div>
    <div id="dashBriefsSlot" class="${store.get('crowdos-dashbriefs-open')==='1'?'':'hidden'}"><div id="dashBriefs"></div></div>`;
  ...
  // …and below it, the standalone briefs scratchpad (its own 'd:' namespace,
  // no schedule behind it). Rendered even when collapsed so the count is real.
...
document.addEventListener('click',e=>{
  ...
  const bt=e.target.closest('[data-briefstoggle]');
  if(bt){
    const slot=$('#dashBriefsSlot');if(!slot)return;
    const nowHidden=slot.classList.toggle('hidden');
    store.set('crowdos-dashbriefs-open',nowHidden?'0':'1');
    const chev=bt.querySelector('.chev');if(chev)chev.style.transform=nowHidden?'':'rotate(90deg)';
    if(!nowHidden){setBriefScope(BRIEF_SANDBOX);renderBriefs();}
  }
});
