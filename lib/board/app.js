...
function stripHTML(d,s,idx){
  ...
  const crChip=(g,cls,tip)=>{
    ...
    const rg=g.groupId?rGroupById(g.groupId):null;
    ...
    const rgTip=rg?` · recurring group “${rg.name}” (pool ${rg.poolSize}) — same people across every day`:'';
    return `<span class="code ${cls}${needsName?' needsname':''}${fromAbove?' fromabove':''}${rg?' rgroup':''} named click crgrp" draggable="true" data-crgrp="1" data-gtier="${g.tier}" data-gfeat="${g.featured?1:0}" data-gname="${esc(g.name||'')}" data-gcount="${g.count}" data-gabove="${fromAbove?1:0}" data-ggid="${esc(g.groupId||'')}" data-tip="${esc(tip)}${rgTip}${APPMODE==='crowd'?' · right-click or drag to move, copy or delete':''}" tabindex="0">${rg?'⛓ ':''}${label}${fromAbove?' (FROM ABOVE)':''}</span>`;
  };
  let crowdChips;
  if(APPMODE==='crowd'){
    const parts=[];
    if(s.sa>0)parts.push(crChip({tier:'SA',featured:false,name:'',count:s.sa,fromAbove:!!s.saAbove},'cr anon',`Unnamed SA ×${s.sa} — click to edit · ⋯ or right-click for move / copy / delete · drag to another scene to move, or drop outside to remove`));
...
function nameAllAnonSA(d,name,count){
  ...
  d.scenes.forEach((s,idx)=>{
    ...
    for(const f of s.saChars||[])chars.push({name:f.name,count:f.count,tier:'SA',featured:false,fromAbove:above(f),groupId:f.groupId});
    for(const f of s.featured||[])chars.push({name:f.name,count:f.count,tier:'SA',featured:true,fromAbove:above(f),groupId:f.groupId});
    for(const f of s.spacts||[])chars.push({name:f.name||'',count:f.count,tier:'SPACT',featured:false,fromAbove:above(f),groupId:f.groupId});
...
function dayLocHTML(d){
  ...
  return `<span class="dlocs">`+blocks.map(b=>{
    // The schedule's location column IS the day's real filming location (the
    // physical place the unit travels to — "West Wycombe", "Four Seasons Hotel
    // Morocco"). The scene's story location is the slugline set name, shown per
    // scene in the rows below — it is NOT what we show here. A schedule imported
    // before the sequence-title guard may carry a narrative label ("Hotel
    // opening") in this column; never show that as a place — fall back to the
    // block's scene slug so the banner still reads sensibly.
    const rawLoc=b.loc||'';
    const scheduled=looksLikeSequenceTitle(rawLoc)
      ? (((b.scenes||[]).find(s=>(s.slug||'').trim())||{}).slug||'')
      : rawLoc;
    const real=b.real||'';              // a user-confirmed / corrected exact address
    const hasReal=!!real, differs=hasReal&&!!scheduled&&normLoc(real)!==normLoc(scheduled);
    const mapsHref=q=>`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`;
    const editBtn=`<button class="dloc-edit${hasReal?' on':''}" data-locedit="${esc(d.id)}" data-locblk="${esc(scheduled)}" data-tip="${hasReal?('Confirmed filming address: “'+esc(real)+'”. Click to change.'):'Confirm or set the exact filming address for this day'}">${icon('pin')}</button>`;
    // Nothing in the location column and no confirmed address yet → TBC.
    if(!scheduled&&!real){
      return `<span class="dlocblk"><span class="dloc-tbc" data-tip="Filming location not set yet — click the pin to add it">→ TBC</span>${editBtn}</span>`;
    }
    // Primary = the filming location from the schedule (or the confirmed address
    // if the schedule left it blank). Always mappable.
    const primaryText=scheduled||real;
    const primaryMap=scheduled||real;
    const primaryEl=`<a class="dloc loclink" href="${mapsHref(primaryMap)}" target="_blank" rel="noopener" data-tip="Filming location${differs?' (as scheduled)':''} — open in Google Maps">${esc(primaryText)}</a>`;
    // If a confirmed address differs from the schedule (e.g. studio-for-location),
    // show it too so the real call reads at a glance.
    const actualEl=differs
      ? `<a class="dloc-real loclink on" href="${mapsHref(real)}" target="_blank" rel="noopener" data-tip="Confirmed filming address — actually shot here. Open in Google Maps.">→ ${esc(real)}</a>`
      : '';
    return `<span class="dlocblk">${primaryEl}${actualEl}${editBtn}</span>`;
  }).join('')+`</span>`;
}
function renderDays(){
  const notesByDay={};
...
function renderCrowdBreakdown(){
  ...
  // Recurring crowd groups — the same people carried across days. Only shown
  // when the production defines groups. Unique heads (the pool peak) is the
  // honest booking count; person-days is what's paid; once-run fees are charged
  // a single time on the pool.
  {const groups=Object.values(CROWD.groups||{}).filter(g=>g.dayCounts.size>0)
     .sort((a,b)=>b.uniqueHeads-a.uniqueHeads||a.name.localeCompare(b.name));
  if(groups.length){
    const onceSum=groups.reduce((a,g)=>a+g.onceRunTotal,0);
    const dayChipsFor=g=>[...g.dayCounts.entries()].map(([id,qty])=>{
      const d=(MODEL.days||[]).find(x=>x.id===id);
      return `<span class="rgdaychip" data-goto="${esc(id)}" data-tip="${d?esc(chipDate(d)):''}">D${d?d.num:'?'}<small>×${qty}</small></span>`;
    }).join('');
    const feeText=g=>{
      const parts=[];
      const once=(g.onceRunTotal>0)?`${gbp(Math.round(g.onceRunTotal))} once`:'';
      const per=(g.perDayFeeTotal>0)?`${gbp(Math.round(g.perDayFeeTotal))} across working days`:'';
      if(once)parts.push(once); if(per)parts.push(per);
      return parts.length?parts.join(' + '):'—';
    };
    html+=`<div class="tablecard"><h3>Recurring crowd groups<span class="cnt">${groups.length}</span>${onceSum>0?`<span class="sum costable">${gbp(Math.round(onceSum))}</span>`:''}</h3>
    <div class="tscroll"><table><thead><tr><th>Group</th><th>Tier</th><th class="num">Pool</th><th class="num">Unique heads</th><th class="num">Person-days</th><th class="num">Fees</th><th class="datescol">Days used</th></tr></thead><tbody>
    ${groups.map(g=>`<tr><td class="rowlabel">⛓ ${esc(g.name)}</td><td>${esc(g.tier)}</td><td class="num">${g.poolSize||'—'}</td><td class="num"><b data-tip="The same people across the run — the pool peak, never the days added up">${g.uniqueHeads}</b></td><td class="num" data-tip="Every day's quantity added up — what you pay">${g.personDays}</td><td class="num ${g.feeTotal?'':'dim'}">${feeText(g)}</td><td class="datescol"><div class="daylist rgdaylist">${dayChipsFor(g)}</div></td></tr>`).join('')}
    </tbody></table></div>
    <div class="note">A recurring group is the same people booked once and carried across every day it appears. <b>Unique heads</b> is the pool (the peak) — who you booked; <b>person-days</b> is every day's quantity added up — what you pay (unchanged from before). One-off fees (fittings, wardrobe) are charged once on the pool${onceSum>0?` and are the ${gbp(Math.round(onceSum))} above`:''}; per-day fees recur on each working day.</div></div>`;
  }}
...
function renameCrowdCharacter(oldName,tier,newName){
  ...
  for(const d of MODEL.days)d.scenes.forEach((s,idx)=>{
    ...
    const chars=[];
    const above=f=>!!(f.flags||[]).includes('asAbove');
    if(s.sa)chars.push({name:'',count:s.sa,tier:'SA',featured:false,fromAbove:!!s.saAbove});
    for(const f of s.saChars||[])chars.push({name:f.name,count:f.count,tier:'SA',featured:false,fromAbove:above(f),groupId:f.groupId});
    for(const f of s.featured||[])chars.push({name:f.name,count:f.count,tier:'SA',featured:true,fromAbove:above(f),groupId:f.groupId});
    for(const f of s.spacts||[])chars.push({name:f.name||'',count:f.count,tier:'SPACT',featured:false,fromAbove:above(f),groupId:f.groupId});
...
document.addEventListener('click',e=>{
  ...
  if(addc){
    ...
    // group picker starts disabled — a group must be named first; it fills with
    // the production's groups once the row is re-rendered after a name is typed
    row.innerHTML=`<input data-rq="ccount" type="number" min="0" value="1"><select data-rq="ctier"><option selected>SA</option><option>SPACT</option></select><input data-rq="cname" value="" placeholder="Character / group (optional)"><label class="reqfeat"><input type="checkbox" data-rq="cfeat"> Featured</label><label class="reqfeat"><input type="checkbox" data-rq="cabove"> From above</label><select data-rq="cgroup" class="reqgroup" disabled title="Name the row first"><option value="" selected>One-off (not recurring)</option><option value="__new__">＋ New recurring group…</option></select><button data-rqdel="1">✕</button>`;
    // reuse-dropdown: point the new row's name field at this editor's day roster
    const dl=addc.closest('.reqedit').querySelector('datalist[id^="rqroster-"]');
    if(dl)row.querySelector('[data-rq="cname"]').setAttribute('list',dl.id);
...
function scGroupsOf(s){
  ...
  if((s.sa||0)>0)out.push({tier:'SA',featured:false,name:'',count:s.sa,note:'',sup:0,fromAbove:!!s.saAbove});
  for(const g of s.saChars||[])if((g.count||0)>0)out.push({tier:'SA',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),groupId:g.groupId||''});
  for(const g of s.featured||[])if((g.count||0)>0)out.push({tier:'SA',featured:true,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),groupId:g.groupId||''});
  for(const g of s.spacts||[])if((g.count||0)>0)out.push({tier:'SPACT',featured:false,name:g.name||'',count:g.count,note:g.note||'',sup:+g.sup||0,fromAbove:carried(g),groupId:g.groupId||''});
  return out;
}
// the editable char-rows for a scene, in the shape SCED / the inline editor use
function sceneCrowdRows(s){
  return scGroupsOf(s).map(g=>({name:g.name,count:g.count,tier:g.tier,featured:g.featured,note:g.note||'',sup:+g.sup||0,fromAbove:!!g.fromAbove,groupId:g.groupId||''}));
}
...
function writeSceneCrowd(d,idx,rows){
  ...
  e.chars=rows.filter(r=>(+r.count||0)>0)
    .map(r=>{
      ...
      // keep the recurring-group link with the row so it survives a recompute
      // and travels with the group when it is moved to another scene
      if(r.groupId&&rGroupById(r.groupId))o.groupId=r.groupId;
      return o;
    });
...
function mergeSameInto(rows,g){
  ...
  if(j>=0){rows[j].count=Math.max(rows[j].count,g.count);if(g.groupId&&!rows[j].groupId)rows[j].groupId=g.groupId;}
  else rows.push({name:g.name,count:g.count,tier:g.tier,featured:g.featured,fromAbove:!!g.fromAbove,groupId:g.groupId||''});
}
...
function crowdGroupFromEl(el){
  ...
  return {d,idx,g:{tier:chip.dataset.gtier==='SPACT'?'SPACT':'SA',featured:chip.dataset.gfeat==='1',
    name:chip.dataset.gname||'',count:+chip.dataset.gcount||0,fromAbove:chip.dataset.gabove==='1',groupId:chip.dataset.ggid||''}};
}
...
// LINK: point this scene's row at a recurring group (or clear the link with
// groupId=''). Same people/count — only the cross-day identity changes.
function linkCrowdGroupToRecurring(d,idx,g,groupId){
  const rows=sceneCrowdRows(d.scenes[idx]);
  const i=findRowIdx(rows,g); if(i<0)return false;
  rows[i].groupId=groupId||'';
  registerCrowdUndo(JSON.stringify(SCED),crowdGroupLabel(g)+(groupId?' linked':' unlinked'));
...
// "Are these a recurring group?" — the yes/no prompt shown when a crowd line is
// first named. Yes → collect group details; No → stays a one-off. Resolves
// 'yes' | 'no' | null (cancelled/escape).
function askIsRecurring(name){
  return new Promise(resolve=>{
    const wrap=document.createElement('div');wrap.className='samenew-wrap';
    wrap.innerHTML=`<div class="samenew" role="dialog" aria-modal="true">
      <div class="samenew-h">Is <b>${esc(name)}</b> a recurring group?</div>
      <div class="samenew-q">A recurring group is the same people across several days (e.g. the hotel guests, a security detail). They're counted once across the run and never re-booked as new faces.</div>
      <div class="samenew-btns">
        <button class="samenew-same" data-v="yes">Yes, recurring<small>the same people appear on other days</small></button>
        <button class="samenew-new" data-v="no">No, one-off<small>just this scene — nothing changes</small></button>
      </div>
      <button class="samenew-cancel" data-v="cancel">Cancel</button>
    </div>`;
    document.body.appendChild(wrap);
    const done=v=>{wrap.remove();document.removeEventListener('keydown',onKey,true);resolve(v==='cancel'?null:v);};
    const onKey=e=>{if(e.key==='Escape'){e.preventDefault();done('cancel');}};
    wrap.addEventListener('click',e=>{const b=e.target.closest('[data-v]');if(b){done(b.dataset.v);return;}if(e.target===wrap)done('cancel');});
    document.addEventListener('keydown',onKey,true);
    const f=wrap.querySelector('.samenew-same');if(f)f.focus();
  });
}
// Group details dialog — create a new recurring group or edit an existing one.
// Captures name, tier, pool size, and the supplementary fees split by whether
// they recur once across the run (fittings) or per day (night premium).
// Resolves the saved group object, or null if cancelled.
function askRecurringGroup(init){
  const g=Object.assign({name:'',tier:'SA',poolSize:0,fees:[]},init||{});
  return new Promise(resolve=>{
    const wrap=document.createElement('div');wrap.className='samenew-wrap';
    const feeRow=(f,i)=>`<div class="rgfee" data-fi="${i}">
      <input data-rgf="label" value="${esc(f.label||'')}" placeholder="Fee (e.g. Costume fitting)">
      <input data-rgf="amount" type="number" min="0" step="0.01" value="${+f.amount||0}" title="£ per head">
      <select data-rgf="kind"><option value="onceRun"${f.kind!=='perDay'?' selected':''}>once per run</option><option value="perDay"${f.kind==='perDay'?' selected':''}>per day</option></select>
      <button data-rgfdel="1" type="button">✕</button></div>`;
    wrap.innerHTML=`<div class="samenew rgdlg" role="dialog" aria-modal="true">
      <div class="samenew-h">${init&&init.id?'Edit recurring group':'New recurring group'}</div>
      <div class="rgfield"><label>Name</label><input data-rg="name" value="${esc(g.name)}" placeholder="e.g. Hotel Guests"></div>
      <div class="rgfield"><label>Tier</label><select data-rg="tier">
        <option value="SA"${g.tier==='SA'?' selected':''}>SA</option>
        <option value="Featured"${g.tier==='Featured'?' selected':''}>Featured</option>
        <option value="SPACT"${g.tier==='SPACT'?' selected':''}>SPACT</option></select></div>
      <div class="rgfield"><label>Pool size</label><input data-rg="pool" type="number" min="0" value="${+g.poolSize||0}"></div>
      <div class="rgfield"><label>Pool size</label><small class="rghelp">The people you booked. Each day draws a quantity from this pool; unique heads for the run is the pool, never the daily totals added up.</small></div>
      <div class="rgfeeshead">Supplementary fees <small>(optional)</small></div>
      <div class="rgfees">${(g.fees||[]).map(feeRow).join('')}</div>
      <button class="rgfeeadd" data-rgfeeadd="1" type="button">+ Add fee</button>
      <div class="samenew-btns rgbtns">
        <button class="samenew-same" data-v="save">Save group</button>
        <button class="samenew-cancel" data-v="cancel">Cancel</button>
      </div>
    </div>`;
    document.body.appendChild(wrap);
    const feesHost=wrap.querySelector('.rgfees');
    const readFees=()=>[...feesHost.querySelectorAll('.rgfee')].map(r=>({
      label:(r.querySelector('[data-rgf="label"]').value||'').trim(),
      amount:Math.max(0,+r.querySelector('[data-rgf="amount"]').value||0),
      kind:r.querySelector('[data-rgf="kind"]').value==='perDay'?'perDay':'onceRun',
    })).filter(f=>f.label&&f.amount>0);
    const close=()=>{wrap.remove();document.removeEventListener('keydown',onKey,true);};
    const onKey=e=>{if(e.key==='Escape'){e.preventDefault();close();resolve(null);}};
    wrap.addEventListener('click',e=>{
      if(e.target.closest('[data-rgfeeadd]')){const d=document.createElement('div');d.className='rgfee';d.innerHTML=feeRow({},feesHost.children.length).replace(/data-fi="\d+"/,'');feesHost.appendChild(d);d.querySelector('[data-rgf="label"]').focus();return;}
      const del=e.target.closest('[data-rgfdel]');if(del){del.closest('.rgfee').remove();return;}
      const b=e.target.closest('[data-v]');if(!b)return;
      if(b.dataset.v==='cancel'){close();resolve(null);return;}
      // save
      const name=(wrap.querySelector('[data-rg="name"]').value||'').trim();
      const tier=wrap.querySelector('[data-rg="tier"]').value||'SA';
      const pool=Math.max(0,+wrap.querySelector('[data-rg="pool"]').value||0);
      if(!name){wrap.querySelector('[data-rg="name"]').focus();return;}
      const saved=upsertRGroup({id:g.id,name,tier,poolSize:pool,fees:readFees()});
      close();resolve(rGroupById(saved));
    });
    document.addEventListener('keydown',onKey,true);
    const f=wrap.querySelector('[data-rg="name"]');if(f){f.focus();f.select&&f.select();}
  });
}
...
function openCrowdGroupMenu(x,y,ctx){
  ...
  // Recurring group — only for a named row (a group must have a name). Lets you
  // link this crowd to a production-level group so the same people carry across
  // every day it appears, unlink it, or spin up a new group from it.
  if(g.name){
    const effTier=g.tier==='SPACT'?'SPACT':(g.featured?'Featured':'SA');
    const linked=g.groupId&&rGroupById(g.groupId);
    items.push({label:linked?('Recurring group: '+linked.name):'Part of recurring group…',icon:'link',onClick:()=>{
      const sub=[];
      for(const grp of rGroupsForNS().filter(x=>x.tier===effTier)){
        sub.push({label:grp.name+' ('+grp.poolSize+')'+(linked&&linked.id===grp.id?' ✓':''),onClick:()=>{
          linkCrowdGroupToRecurring(d,idx,g,grp.id);setStatus(crowdGroupLabel(g)+' linked to recurring group “'+grp.name+'”.',{undo:crowdUndo});
        }});
      }
      sub.push({label:'＋ New recurring group…',onClick:async()=>{
        const grp=await askRecurringGroup({name:g.name,tier:effTier,poolSize:g.count});
        if(grp){linkCrowdGroupToRecurring(d,idx,g,grp.id);setStatus(crowdGroupLabel(g)+' is now the recurring group “'+grp.name+'”.',{undo:crowdUndo});}
      }});
      if(linked){sub.push({sep:true});sub.push({label:'Not recurring (unlink)',onClick:()=>{
        linkCrowdGroupToRecurring(d,idx,g,'');setStatus(crowdGroupLabel(g)+' is no longer a recurring group.',{undo:crowdUndo});
      }});}
      openCtxMenu(x,y,sub);
    }});
    items.push({sep:true});
...
async function cloudHydrate(){
  ...
  for(const de of res.dayEdits){
    ...
    if(de.kind==='rgroups'){Object.assign(RGROUPS,de.data||{});store.set('crowdos-rgroups',JSON.stringify(RGROUPS));continue;}
...
// ---------- recurring crowd groups (production-level, cross-day) ----------
// A named cohort ("Hotel Guests", "Bob's Security") attached to scene rows by
// groupId. Same shape as SCED: one namespaced flat map, synced as a single blob
// row per production. Key is "<ns>|<groupId>", value is a RecurringGroup.
let RGROUPS={};
try{RGROUPS=JSON.parse(store.get('crowdos-rgroups')||'{}')}catch(e){RGROUPS={}}
function saveRGroups(){store.set('crowdos-rgroups',JSON.stringify(RGROUPS));cloudSyncBlob('rgroups',RGROUPS)}
function rgKey(id){return (NS?NS+'|':'')+id}
// Every group belonging to the active production, as RecurringGroup objects.
function rGroupsForNS(){
  const pre=(NS?NS+'|':'');
  return Object.keys(RGROUPS).filter(k=>k.startsWith(pre)).map(k=>RGROUPS[k]).filter(Boolean);
}
function rGroupById(id){return id?RGROUPS[rgKey(id)]||null:null}
function rGroupByName(name,tier){
  const n=(name||'').trim().toLowerCase();
  if(!n)return null;
  return rGroupsForNS().find(g=>g.name.trim().toLowerCase()===n&&(!tier||g.tier===tier))||null;
}
function newGroupId(){return 'rg_'+Math.random().toString(36).slice(2,9)+Date.now().toString(36).slice(-4)}
// Create or update a group and persist. Returns its id.
function upsertRGroup(g){
  if(!g.id)g.id=newGroupId();
  RGROUPS[rgKey(g.id)]={id:g.id,name:(g.name||'').trim(),tier:g.tier||'SA',poolSize:+g.poolSize||0,fees:Array.isArray(g.fees)?g.fees:[]};
  saveRGroups();
  return g.id;
}
...
function applySced(model){
  for(const d of model.days)d.scenes.forEach((s,idx)=>{
    ...
    s.sa=b.sa;
    // continuity flag for the anonymous background SA bucket (the s.sa integer
    // has no per-row flag slot, so its "from above" state lives here). Reset to
    // the parsed baseline (never carried) and only re-set from an SCED override.
    s.saAbove=false;
    ...
    if(e.chars){
      ...
      const mk=c=>{const o={name:c.name||'',count:+c.count};if(c.note)o.note=c.note;if(+c.sup>0)o.sup=+c.sup;if(c.fromAbove)o.flags=['asAbove'];if(c.groupId&&rGroupById(c.groupId))o.groupId=c.groupId;return o};
      const anon=rows.filter(c=>c.tier!=='SPACT'&&!c.featured&&!c.name);
      s.sa=anon.reduce((a,c)=>a+(+c.count),0);
      // the anonymous bucket is "from above" when every unnamed SA row feeding
      // it is marked from above (one anon row is the norm)
      s.saAbove=anon.length>0&&anon.every(c=>!!c.fromAbove);
  ...
  // expose the production's recurring-group registry on the model so the cost
  // engine can pool group appearances across the whole run (unique heads = pool
  // peak, not the per-day sum) and charge once-per-run fees a single time.
  model.recurringGroups=rGroupsForNS();
}
// Every distinct NAMED crowd line already used elsewhere in this scene's DAY,
// so an AD can reuse a group across scenes without retyping it. Keyed by
// name+tier+featured; carries a representative head-count and any recurring-
// group link, so picking one re-books the same people (e.g. the four Guards).
function dayRosterFor(nk){
  const ref=sceneFromKey(nk); if(!ref)return [];
  const seen=new Map();
  (ref.d.scenes||[]).forEach(s=>{
    const rows=[
      ...(s.saChars||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:false,groupId:f.groupId})),
      ...(s.featured||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:true,groupId:f.groupId})),
      ...(s.spacts||[]).map(f=>({name:f.name,count:f.count,tier:'SPACT',featured:false,groupId:f.groupId})),
    ];
    for(const r of rows){
      const name=(r.name||'').trim();
      if(!name)continue;
      const key=name.toLowerCase()+'|'+r.tier+'|'+(r.featured?'1':'0');
      const prev=seen.get(key);
      if(!prev)seen.set(key,{name,tier:r.tier,featured:r.featured,count:+r.count||0,groupId:r.groupId||''});
      else{
        if(!prev.groupId&&r.groupId)prev.groupId=r.groupId;      // prefer a group link
        if((+r.count||0)>prev.count)prev.count=+r.count||0;      // and the peak head-count
      }
    }
  });
  return [...seen.values()].sort((a,b)=>a.name.localeCompare(b.name));
}
// DOM id for a scene's day-roster <datalist>, sanitised so it is a valid id
function rosterListId(nk){return ('rqroster-'+nk).replace(/[^a-zA-Z0-9_-]/g,'_')}
...
function reqEditorHTML(nk){
  ...
  if(APPMODE==='crowd'){
    ...
    const above=f=>!!(f.flags||[]).includes('asAbove');
    const chars=e.chars||[
      ...(s.sa?[{name:'',count:s.sa,tier:'SA',featured:false,fromAbove:!!s.saAbove}]:[]),
      ...(s.saChars||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:false,fromAbove:above(f),groupId:f.groupId})),
      ...(s.featured||[]).map(f=>({name:f.name,count:f.count,tier:'SA',featured:true,fromAbove:above(f),groupId:f.groupId})),
      ...(s.spacts||[]).map(f=>({name:f.name,count:f.count,tier:'SPACT',featured:false,fromAbove:above(f),groupId:f.groupId})),
    ];
    // Recurring-group picker for a row: existing groups whose tier matches the
    // row, plus "one-off" (not recurring) and "new group". A row with no name
    // can't be a group (a group must be named), so the picker is disabled there.
    const groupSel=c=>{
      const effTier=c.tier==='SPACT'?'SPACT':(c.featured?'Featured':'SA');
      const cur=c.groupId&&rGroupById(c.groupId);
      const opts=rGroupsForNS().filter(g=>g.tier===effTier)
        .map(g=>`<option value="${esc(g.id)}"${cur&&cur.id===g.id?' selected':''}>${esc(g.name)} (${g.poolSize})</option>`).join('');
      return `<select data-rq="cgroup" class="reqgroup"${c.name?'':' disabled title="Name the row first"'}>
        <option value=""${!cur?' selected':''}>One-off (not recurring)</option>
        ${opts}
        <option value="__new__">＋ New recurring group…</option>
      </select>`;
    };
    // Names already used elsewhere in this DAY — offered as a dropdown on the
    // name field so a group can be reused across scenes in one pick.
    const roster=dayRosterFor(nk);
    const rlid=rosterListId(nk);
    const rosterHTML=roster.length?`<datalist id="${rlid}">${roster.map(r=>{
      const tl=r.tier==='SPACT'?'SPACT':(r.featured?'Featured SA':'SA');
      return `<option value="${esc(r.name)}">×${r.count} · ${tl}${r.groupId?' · recurring':''}</option>`;
    }).join('')}</datalist>`:'';
    const listAttr=roster.length?` list="${rlid}"`:'';
    const row=(c,i)=>`<div class="reqrow" data-ri="${i}">
      ...
      <input data-rq="cname"${listAttr} value="${esc(c.name||'')}" placeholder="Character / group — reuse or type new">
    ...
    return `<div class="reqedit" data-rk="${esc(nk)}">
      ...
      <div class="reqhint">Tip: mark a crowd line as a recurring group and the same people carry across every day it appears — counted once, not re-booked.</div>
...
function commitReqEditor(area,keepOpen){
  ...
  if(APPMODE==='crowd'){
    ...
    ed.querySelectorAll('.reqrow').forEach(row=>{
      ...
      // Recurring-group link. The select holds a resolved group id (the "__new__"
      // sentinel is turned into a real id by the change handler before commit).
      // A link only sticks if the row is named and the group still exists.
      let gsel=((row.querySelector('[data-rq="cgroup"]')||{}).value||'');
      if(gsel==='__new__')gsel=''; // unresolved sentinel — never persist it
      const groupId=(name&&gsel&&rGroupById(gsel))?gsel:'';
      const rec={name,count,tier,featured,fromAbove};
...
// Two crowd-row fields intercept first because they open a dialog:
//   • the recurring-group select (create a new group, or link to one)
//   • naming a previously-unnamed row (offer "is this a recurring group?")
let RG_DIALOG=false; // re-entrancy guard while a group dialog is open
document.addEventListener('change',async e=>{
  const rq=e.target.closest('[data-rq]');
  if(!rq)return;
  const area=rq.closest('.reqarea');if(!area)return;
  if(RG_DIALOG)return;
  const which=rq.dataset.rq;
  const row=rq.closest('.reqrow');

  // (1) group select changed
  if(which==='cgroup'&&row){
    const nameEl=row.querySelector('[data-rq="cname"]');
    const name=((nameEl&&nameEl.value)||'').trim();
    if(rq.value==='__new__'){
      RG_DIALOG=true;
      const tierSel=row.querySelector('[data-rq="ctier"]').value;
      const feat=!!(row.querySelector('[data-rq="cfeat"]')||{}).checked;
      const tier=tierSel==='SPACT'?'SPACT':(feat?'Featured':'SA');
      const cnt=Math.max(0,+((row.querySelector('[data-rq="ccount"]')||{}).value)||0);
      const g=await askRecurringGroup({name,tier,poolSize:cnt});
      RG_DIALOG=false;
      if(g){rq.value=g.id;if(!name&&nameEl){nameEl.value=g.name;}}
      else{rq.value='';}
    }
    commitReqEditor(area,true);
    return;
  }

  // (2) a row just got named (was blank, now has text) and isn't linked yet →
  // offer to make it a recurring group. Only fire on the name field.
  if(which==='cname'&&row){
    const name=(rq.value||'').trim();
    const gsel=row.querySelector('[data-rq="cgroup"]');
    const wasBlank=rq.dataset.hadName!=='1';
    rq.dataset.hadName=name?'1':'';
    // Reuse: if this name is already used elsewhere in the day, re-book the same
    // people — copy its tier/featured/head-count and link its recurring group.
    // Doing this here also suppresses the "is this recurring?" prompt below.
    let reused=false;
    if(name){
      const nk=area.dataset.reqkey||(area.querySelector('[data-rk]')&&area.querySelector('[data-rk]').dataset.rk);
      const lc=name.toLowerCase();
      const hits=dayRosterFor(nk).filter(r=>r.name.toLowerCase()===lc);
      const match=hits.find(r=>r.groupId)||hits[0];
      if(match){
        const tierSel=row.querySelector('[data-rq="ctier"]');
        if(tierSel)tierSel.value=match.tier;
        const featEl=row.querySelector('[data-rq="cfeat"]');
        if(featEl)featEl.checked=!!match.featured;
        const cntEl=row.querySelector('[data-rq="ccount"]');
        // only fill a count the AD hasn't deliberately set (blank / default 1)
        if(cntEl&&match.count>0&&(!(+cntEl.value>0)||+cntEl.value===1))cntEl.value=match.count;
        if(gsel&&match.groupId&&rGroupById(match.groupId)){
          if(![...gsel.options].some(o=>o.value===match.groupId)){
            const g=rGroupById(match.groupId);
            const opt=document.createElement('option');
            opt.value=match.groupId;opt.textContent=`${g.name} (${g.poolSize})`;
            gsel.appendChild(opt);
          }
          gsel.disabled=false;gsel.value=match.groupId;
        }
        reused=true;
      }
    }
    if(!reused&&name&&wasBlank&&gsel&&!gsel.value){
      RG_DIALOG=true;
      const ans=await askIsRecurring(name);
      if(ans==='yes'){
        const tierSel=row.querySelector('[data-rq="ctier"]').value;
        const feat=!!(row.querySelector('[data-rq="cfeat"]')||{}).checked;
        const tier=tierSel==='SPACT'?'SPACT':(feat?'Featured':'SA');
        const cnt=Math.max(0,+((row.querySelector('[data-rq="ccount"]')||{}).value)||0);
        // if a group of this name+tier already exists, link to it; else create
        const existing=rGroupByName(name,tier);
        const g=existing||await askRecurringGroup({name,tier,poolSize:cnt});
        if(g&&gsel)gsel.value=g.id;
      }
      RG_DIALOG=false;
    }
    commitReqEditor(area,true);
